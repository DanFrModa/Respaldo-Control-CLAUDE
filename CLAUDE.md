...
4. **ECONOMÍA DE TOKENS (innegociable — el costo se dispara fácil; toda sesión la cumple):**
   ...
   - **⚠️ Y la máquina también se comparte: `npm run lint` del backend puede morir por memoria** (`Killed`/`EXIT=137`, o `heap out of memory`/`EXIT=134`) cuando hay otra sesión compitiendo — eslint pide ~6.7 GB aquí. ⚠️ **OJO CON ESE «~6.7 GB»: es la TERCERA cifra para la misma memoria y la única SIN ORIGEN CONOCIDO** (barrido de la v0.173, 18-sep-2026). Las dos que sí están medidas viven en `HOJA-DE-RUTA.md` §4: **pico 6011 MiB** (murió contra 6144) y **pico 7034 MiB** (pasó con 8192) — 6.7 GB ≈ 6861 MiB **no es ninguna de las dos** y ningún sitio dice de dónde sale. No se re-midió (pide instrumentar el pico en una máquina comparable a la del runner); queda **escrito, no resuelto**. **No es un defecto del código**: súbele `NODE_OPTIONS=--max-old-space-size=10240` y dilo en el reporte. El CI, que corre solo, sigue siendo el juez.
...
- **⏱️ CUÁNTO TARDA EL CI, Y QUE SON CUATRO TRABAJOS — medido el 7-sep-2026, para no volver a equivocarse en las dos direcciones.** El CI son **CUATRO** jobs: `backend` · `frontend` · **`e2e`** · **`imagenes-docker`**, y **los dos últimos ARRANCAN SÓLO CUANDO LOS DOS PRIMEROS PASAN** ⇒ mientras backend/frontend corren, `get_check_runs` **devuelve sólo 2** y parece que el CI ya casi está. **Contar CUATRO antes de decidir mergear**; con dos en verde el CI va por la mitad. **Duración TOTAL medida** (las 6 corridas de `prueba` v0.117→v0.122): **39, 40, 45, 45, 46 y 48 min** — o sea **~40-50 min**, con `backend` llevándose casi todo (~38 min él solo) y `e2e` ~8. ⇒ **no sospechar de atasco antes de ~1h15**, y aun entonces comparar contra este baseline antes de relanzar nada. ⚠️ **Cicatriz doble de esa noche:** primero se puso un umbral de sospecha de 45 min **inventado, sin medir** —**por debajo de la mediana**: aplicarlo habría matado una corrida sana y costado otros 45 min—; y después se estuvo a punto de mergear con sólo 2 jobs en verde porque los otros dos aún no existían en la lista. **Las dos son la misma falta: dar por buena una expectativa que nunca se comprobó.** 🔄 **RE-MEDIDO EL 18-SEP-2026, DOS VECES, y la segunda corrigió a la primera (fila 0.208):** lo de arriba es del 7-sep y **se conserva con su fecha**. **Medición vigente — 20 corridas SANAS del 16-18 sep-2026** (API de GitHub; corrida = `created_at`→`updated_at`, job = `started_at`→`completed_at`): **corrida completa 41.1-61.2 min** (mediana 51.6) · **job `backend` 32.3-53.2 min** (mediana 43.3). Las **dos corridas `cancelled` a 45.3** del 17-sep quedan **fuera**: son truncamientos del reloj, no duraciones. ⚠️ **La primera re-medición del 18-sep publicó «45.6-53.2» de job y era falsa por abajo** —salía de **dos** corridas, y **la corrida de esa misma entrega la desmintió** con 40.55 min (run `35370866527`)—. 🔑 **De ahí la forma obligatoria: la duración se escribe como MEDICIÓN FECHADA con su N** (*«medido en N corridas del día D: X»*), nunca como *«tarda X»* — lo segundo envejece y se vuelve falso solo; lo primero sigue siendo cierto para siempre, y quien cronometre 70 min el mes que viene sabe que tiene **una medición nueva**, no una contradicción. ⚠️ **Y lo que cambia el diagnóstico: la causa que este repo daba por «medida» —el arranque repetido de testcontainers— ES FALSA** (el contenedor arranca UNA vez). El reparto real, los tres caminos y lo que NO está medido viven en la **fila 0.208** de `HOJA-DE-RUTA.md`. ⏱️ **EL TECHO DEL JOB `backend` ESTÁ EN 90 min desde el 18-sep-2026** (antes 82; decisión de Gabriel), y es **la primera vez que ese número se calcula sobre una MUESTRA**: regla de la casa *tope ≈ 1.7× la duración sana* aplicada a la peor sana de las 20 medidas (`53.15 × 1.7 = 90.4`). ⭐ **La regla operativa que sale de ahí, y vale para cualquier sesión: antes de recalcular ese techo se mide la MUESTRA COMPLETA con la API de GitHub, no la última corrida que se tenga a mano** — las tres subidas anteriores (45 → 70 → 82) se fijaron todas sobre «la más lenta conocida a esa hora» y las tres nacieron cortas. 🔑 **Y el corolario operativo:** una corrida cuesta **41-61 min** (el rango medido de arriba), así que **NO comitear en una rama con CI en vuelo** salvo que el commit sea el arreglo de un fallo — un push cancela la corrida y la reinicia desde cero, incluidos los jobs que ya habían pasado.
...
- **⚠️ MUTAR EL CAMINO FELIZ NO PRUEBA LA CONDICIÓN QUE ACABAS DE AÑADIR (cicatriz del 17-sep-2026, fila 0.155).** Un arreglo metió condiciones para **NO** hacer algo: no pisar lo que la persona tecleó encima de un valor precargado (`actual === anterior.X`, **dos** comparaciones, una por campo) y no limpiar nada al volver a «proyecto nuevo» (`if (idProyectoElegido === null) return;`). El coder validó con **cinco mutaciones y las cinco murieron**… pero todas rompían el arreglo **en la dirección del defecto original** (dejar el valor pegado): **a las tres ramas nuevas nadie les quitó la condición**. El reviewer mutó **a la inversa** —`return anterior !== null ? '' : actual;`, o sea *barrer siempre*— y salió **`EXIT=0` con las 13 pruebas del archivo en verde: la mutación SOBREVIVE**. El código acertaba; la red habría dejado volver el defecto sin que nadie se enterara. ⭐ **Y el segundo acto, que es la parte fina:** con la prueba que el reviewer escribió ya puesta, esa misma mutación **seguía sobreviviendo (15/15 en verde)** — porque su prueba medía el **año** y la rama mutada era la del **género**: son dos `setX` con dos condiciones distintas, y **una prueba por condición** es literal. Sólo cayó al añadir la gemela del género (y para escribirla hubo que meter un segundo género en el mock: con uno solo el caso no se puede expresar). 🔑 **La regla: cuando un arreglo mete una condición para NO hacer algo, la mutación que hay que probar es QUITARLE la condición, no romper el camino feliz.** Y su corolario, que es donde estaba el agujero: si las pruebas cubren **los dos extremos** de una comparación (el valor ausente y el valor intacto), **falta el de en medio** —el que alguien cambió a mano—, que es justo el caso que el arreglo existe para proteger. ⛔ **Y una decisión escrita en un comentario que ninguna prueba verifica es una decisión que el siguiente cambio borra en silencio** (aquí el comentario prometía que «proyecto nuevo» no toca nada, y nada lo comprobaba).
