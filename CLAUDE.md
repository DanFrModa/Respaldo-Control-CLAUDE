...
| **F2 · Pedidos + Órdenes** (E1–E5) | ✅ **COMPLETA** (17-jun-2026, verificada por Gabriel) — pedidos internos/reales, órdenes con matriz color×talla (D4) + referencias del cliente (D7), consultas/tableros/buscador global, **impreso de orden PDF (R9)**, y **ETL de pedidos/órdenes** (cierre de fase). En la misma etapa: **retiro total de los códigos de barra** (columnas `upc` + generador de F1-E5). Pendiente operativo: commit → PR a `prueba` → correr el ETL en Railway | CIERRE por etapa en `docs/hoja-de-ruta/F2-etapas.md` |
