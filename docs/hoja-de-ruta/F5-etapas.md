...
> **Estado:** 🔄 EN CURSO (6/7) — **F5-E1 ✅, F5-E2 ✅, F5-E3 ✅, F5-E4 ✅, F5-E5 ✅ (22-jun-2026) y F5-E6 ✅ (23-jun-2026)** — el motor completo (datos + duraciones + generación + CPM + captura + semáforo), las **pantallas de operación** (Programar RC, Bandeja con semáforo PC+móvil, RC por orden, badge de alertas) y el **auto-avance por eventos de F3/F4** (las fechas se llenan solas) están construidos; decisiones de fase (a–h) cerradas con Daniel (`Documentacion_MJD/DECISIONES.md` §"Decisiones de negocio de F5"). Resto E7 ⬜.
...
> - **Estabilización del CI de `prueba` (parte de F5-E4, 22/23-jun — antes sin documentar):** además del motor, esta etapa dejó el job **e2e VERDE** en `prueba` tras varios merges en rojo crónico. `dd0d62c` arregló los **6 e2e crónicos** (helper `crearColorYTalla` para la matriz color×talla que el seed no siembra → ordenes/entrega/inventario-pt; `login.spec` con piso ≥18 + módulos clave en vez de `toHaveCount` exacto; `bordados.spec` navega a `/catalogos` antes de la galería; `DialogContent` con `max-h`+overflow para alcanzar el footer con mucho contenido); `6e2c307` destrabó el selector de color en Movimientos/Traspasos PT; el **rate-limit de login** quedó resuelto EN CÓDIGO (`AUTH_LOGIN_RATE_MAX` en `backend/src/auth/config.ts` default 20 —prod intacta— y `1000` en `docker-compose.yml`); y se hicieron deterministas los flaky de backend (`2411c57` ETL comentarios, `ad51921`+#72 ETL telas). Verificado: la corrida de `prueba` del #73 quedó con `e2e/backend/frontend/imagenes-docker` en verde. **Cierra el pendiente diferido del rate-limit** que CLAUDE.md §8 traía como TODO.
...
> - **BUG DE PRODUCTO del motor (E3/E4) que destapó el e2e de E5 — corregido (reviewer independiente APROBÓ):** `generarRutaOrden` dejaba TODO renglón en `'pendiente'`, el CPM no toca el estado, y solo `activarSucesoresListos` (al COMPLETAR un antecesor) ponía `'activo'` → el/los proceso(s) RAÍZ nunca se activaban → la **bandeja (`estado='activo'`) jamás se poblaba tras programar** (la RC no se podía operar). El e2e `ruta-critica-motor.spec.ts` (flujo real programar→bandeja) fue el PRIMERO en ejercerlo y lo cazó; hasta `rutaOrden.int.test.ts` tenía "horneada" la conducta incompleta (esperaba `'pendiente'` en un sucesor cuyo antecesor ya estaba completado). **Fix:** helper `activarProcesosListos(tx, idOrden)` en `cumplimiento.ts` (promueve `'pendiente'→'activo'` todo renglón cuyos antecesores estén TODOS `'completado'` —raíz sin antecesores califica vacuamente—; una pasada; nunca toca `'completado'`/`'activo'`), llamado al final de `generarRutaOrden` Y `ajustarRutaOrden` (tras crear/recomponer filas+aristas, antes del DTO). Tests de E3 corregidos (el `corte` del caso RESURTIDO ahora `'activo'` porque su antecesor `ficha` se auto-completó por duración 0) + 2 tests de regresión (ARRANQUE al generar / al ajustar). **Lección:** el e2e de extremo a extremo es el que cazó un bug del motor que 2 reviewers de E3 + 2 de E4 + sus suites unit/int no vieron (los tests pre-seteaban `estado='activo'`).
> - **El e2e del flujo `ruta-critica-motor.spec.ts`** (escrito en E5) ajustado: la plantilla fija duración>0 a los procesos (sin tiempo → duración 0 → el motor los auto-completaría → bandeja vacía); ambos procesos son raíces independientes → arrancan activos, capturar uno lo saca y el otro queda, capturar el segundo vacía la bandeja (el encadenamiento real "la pelota pasa" lo cubre el test de integración del backend).
> - **Gates locales en verde (comandos exactos del CI):** backend `format:check`/`typecheck`/`lint`/`test`/`build` (**1381** unit/int, +2 regresiones del arranque); frontend `format:check`/`typecheck` (`tsc -b`, cubre `e2e/`)/`lint`/`test` **400**/`build`. Tests de integración (testcontainers: bandeja con 2 antecesores uno incompleto ⇒ no activa, rol secundario SÍ ve la tarea, `todas`, conteo, `capturadoPorNombre`, arranque del conjunto listo) y e2e Playwright (flujo programar→bandeja→capturar Hoy→cierre; + viewport móvil 390×844) corren en CI.
...
## F5-E6 · Auto-avance: eventos de dominio en F3/F4 y suscriptor de la RC — ✅ COMPLETA (23-jun-2026)

> **✅ Nota de cierre (23-jun-2026; 1 coder + 1 reviewer independiente — APROBADO; pendiente verificación de Gabriel en `prueba`).** "Que las fechas se llenen solas donde aplica": los servicios de F3/F4 emiten eventos al confirmar su transacción y un consumidor NUEVO de la RC los consume.
> - **Decisiones de Daniel aplicadas (`DECISIONES.md` §F5, cerradas 22-jun ANTES de codear — ⚠️ (e) y (f) INVIRTIERON los defaults que el *Alcance* de abajo proponía; mandan las de Daniel):** **(d)** el proceso se auto-completa SOLO al cubrirse la cantidad COMPLETA (color×talla, D4); desde el 1er evento parcial lleva la marca visible `parcialEnCurso`. **(e)** el evento **PISA** la fecha capturada a mano (`origenCaptura='evento'`, rastro `pisoCapturaManual` en `Bitacora`). **(f)** cancelar el corte/recibo origen **DES-COMPLETA** el proceso, revisa los sucesores ya activados (`activo`→`pendiente` los que perdieron antecesores; nunca toca `completado`), reabre la RC si era terminal y encola el recálculo del CPM.
> - **Emisión por OUTBOX DURABLE (ADR-0011), NO el emitter in-process:** se extendió `comun/eventos-dominio.ts` con los eventos de etapa de producción (corte/envío/recibo/entrega) + sus cancelaciones + la cancelación de material recibido. Ganchos MÍNIMOS en `produccion/{etapas,recibos,entregas-cliente}.ts` y `compras/recepciones.ts` (`reversarRecepcion`): `registrarEventoOutbox(tx,…)` DENTRO de la tx del hecho (atómico: si el hecho hace rollback, el evento no existe) + `dispararPublicacion()` tras el commit. El `emitir()` in-process de F3-E1 se conservó intacto (inofensivo). `material-recibido` (F4-E3) ahora SÍ tiene consumidor (antes nadie escuchaba). `autorizarOC` NO emite (no hay `tipoEvento` de autorización de OC en el catálogo; solo un comentario que lo justifica).
> - **Consumidor `dominio/ruta-critica/autoAvance.ts` (A1/A2/A7):** worker de la cola `eventos-dominio` (registrado en `servidor.ts` DESPUÉS de `iniciarMotorJobs` para que el `encolarJob` del CPM no caiga en una ventana no-op), SIN sesión de usuario (el evento es AUTORITATIVO → no valida rol; bitácora de sistema `registrarBitacora(tx, null, …)`, patrón `cpm-job.ts`). **IDEMPOTENTE por RE-EVALUACIÓN del estado físico** (Σ `EtapaMovimientoDet` vivos vs `OrdenLineaTalla`, por celda color×talla con `≥`; ignora celdas pedidas en 0; no completa si no hay nada pedido) — NO trata el evento como delta, así que un duplicado/reintento de pg-boss produce UN solo efecto (sin persistir el id del evento origen). `fechaReal` = la **fecha FÍSICA** del hecho (la `fecha` de la etapa/recepción que llevó la cantidad al tope), NO el reloj del servidor (KPI D11 "cuándo" + principio rector "el dato físico manda"); `capturadoEn` sí es el sello de registro del sistema. Serializa por orden con el MISMO `pg_advisory_xact_lock(empresa,orden)` (familia `0x4F='O'`) de `cumplimiento.ts`. Tras cada cambio encola el recálculo del CPM (fire-and-forget, §11). **`recepcionTela`:** el denominador NO es color×talla (la tela no se mide por la matriz de la prenda) — es el **saldo de las líneas de TELA de las OCs ligadas** a la orden (Σ recibido en recepciones activas ≥ pedido por línea, R7); sin OC de tela ligada → no se auto-completa.
> - **Migración aditiva `20260623120000_f5_e6_autoavance`:** UNA columna `RutaOrden.parcialEnCurso BOOLEAN NOT NULL DEFAULT false`. **SIN permisos, SIN seed, SIN tablas nuevas** (reusa el outbox + el motor de jobs ya existentes) → el deploy a `prueba` necesita la migración pero **NO `SEED_ON_START`**. Validada con `prisma migrate diff` (≡ `migration.sql`).
> - **Contrato/Frontend (cambio menor, A1):** `parcialEnCurso` declarado en los Zod de salida y poblado en la proyección del `GET …/ruta` y la bandeja; `openapi.json` (back+front) + `esquema.gen.ts` regenerados. Badge **"Parcial en curso"** en `BandejaTareasPagina` y `RutaPorOrdenPagina` (solo muestra lo que viene del backend).
> - **Regresión obligatoria:** las suites COMPLETAS de F3 y F4 quedan VERDES y SIN debilitar (el reviewer verificó que ningún `*.test.ts` de `produccion/` ni `compras/` se tocó). **Tests nuevos:** `autoAvance.test.ts` (unit: mapeo evento→`tipoEvento`, completitud color×talla con sobre-cantidad/celdas-0, completitud de tela) + `autoAvance.int.test.ts` (testcontainers: el recibo cuadra WIP+IPT+EsMa+RC en UNA tx y auto-completa; parcial no completa + marca `parcialEnCurso`; la remesa que completa marca completado + activa sucesor; **duplicado = 1 efecto**; el evento PISA una fecha manual con rastro en Bitácora; cancelación des-completa + regresa el sucesor a `pendiente`).
> - **Gates locales verdes (comandos exactos del CI):** backend `format:check`/`typecheck`/`lint`/`test:unit` (650)/`build`; frontend `format:check`/`typecheck`/`lint`/`test` (402)/`build`. Integración con testcontainers + e2e corren en CI.
> - **Equipo:** 1 coder + 1 reviewer independiente (cadena emisión→consumo no paralelizable; el reviewer corrió la regresión íntegra de F3/F4 y validó la migración con `prisma migrate diff`). 2 observaciones NO bloqueantes del reviewer atendidas: el orden de arranque del worker (corregido) y el borde documentado "completado-manual + `parcialEnCurso=true`" (el auto-avance nunca revierte una completación manual; solo enciende la marca visible).
> - **Pendiente de Gabriel (deploy):** correr la migración en `prueba` (auto-aplica en el deploy); el auto-avance real necesita la cola viva (`EVENTOS_COLA_ACTIVA != 'false'`) y el motor de jobs (`JOBS_ACTIVOS != 'false'`) — ya es el caso en Railway.
...
- Regla de conflicto evento vs captura manual: **[Daniel decidió (e): el evento PISA lo manual]** si ya hay fechaReal manual y llega el evento que cubre la cantidad, el evento la sobrescribe (`origenCaptura='evento'`) y se registra en `Bitacora` que estaba manual (`pisoCapturaManual`). *(El default propuesto era "el evento NO la pisa"; Daniel lo invirtió — principio rector: el dato físico manda.)*
- Regla de cancelación del movimiento origen: **[Daniel decidió (f): el proceso SÍ se des-completa]** cancelar el corte/recibo origen des-completa el proceso (si re-evaluado ya no está cubierto), revisa los sucesores ya activados, reabre la RC si era terminal, recalcula el CPM y deja rastro en `Bitacora`. *(El default propuesto era "no se descompleta solo"; Daniel lo invirtió.)*
