...
>   "contado" de "N días" y así el cálculo de vencimientos de las CxP (F9) no necesita un mapeo extra.
...
> - Permisos `maquileros.*` y `cortadores.*` retirados del catálogo (cubiertos por `proveedores.*`). Los accesos LEGADO 15/37 de `Accesos.csv` (programar maquileros / alta de asegurados) pertenecen a flujos de producción/EsMa y se remapean en el ETL de `UsuAccesos` (F10). Migración `fusion_terceros` (DROP de las 3 tablas — solo traían datos de seed en `prueba`).
...
- Tabla de mapeo texto→idColor persistida + mapeo tela vieja→Tela unificada + mapeo maquilero/estampador→Maquilero (los reutilizan E7/F2/F4/F10)
