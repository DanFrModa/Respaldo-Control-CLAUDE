...
3. **`ETL_DESDE=2025` se exporta ANTES del primer comando** y vale para toda la sesión. Si se olvida en uno solo, ese ETL desalinea a todos los demás. Cada ETL imprime la ventana que aplicó: "Ventana temporal: DESACTIVADA" en un reporte = ese ETL corrió mal.
...
**Red de seguridad (construida el 11-ago-2026):** los cinco loaders con folio propio ya no dan por "el mismo documento" a cualquiera que ocupe el folio. `backend/migracion/comun/colision-folio.ts` distingue la recuperación de una corrida cortada (lo creó el ETL y nadie más lo reclama) de una **COLISIÓN**, que **no se mapea, no se crea y se REPORTA**. Antes, la colisión escribía el mapeo `IdOrdenes(Access) → orden de v2` y todos los hijos del volcado (cortes, envíos, recibos, cargos EsMa, costos, RC, auditorías) se pegaban a la orden equivocada, en silencio.
