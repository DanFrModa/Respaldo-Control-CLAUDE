...
## F8-E4 · Factores del cliente + lista de precios por Cliente+Departamento + aprobación del dueño (R20a) — ✅ (pend. verif. de Gabriel en `prueba`)
...
**✅ Nota de cierre (2026-07-06 — pend. verif. de Gabriel en `prueba`):**
- **1 coder + 1 reviewer independiente.** Veredicto inicial **CAMBIOS REQUERIDOS** (1 bloqueante B1 + observaciones N1–N7) → ronda de corrección con TODO atendido en la misma vuelta (regla *"un defecto conocido no es menor"*) → **APROBADO**. Gates locales verdes: backend `typecheck`/`lint`/`format:check`/`test:unit` (**761**) + `openapi` sin drift; frontend `typecheck`/`lint`/`format:check`/`test` (**504**) + `build` + `gen:api` sin drift. Integración (~20 casos testcontainers) y e2e (`listas-precios.spec.ts`) corren en CI.
- **SIN migración, SIN permisos, SIN seed nuevos:** las tablas `ClienteFactores`/`ListaPrecios`/`ListaPreciosLinea`/`EstadoLista` y los permisos `listas.*` ya nacieron en E1. La clave de secuencia `lista-precios` se **auto-crea** al primer folio (no necesita seed). (El deploy de la fase a `prueba` sigue requiriendo `SEED_ON_START=true` por E1.)
- **Fórmula de lista** (`dominio/costos/precio-lista.ts`, helper PURO con tests): **cascada** (decisión (b)) `costo/(1−margen%)` → `/(1−(descuentos+regalías+costoVentas)%)` → **redondeo AL ALZA** (D2 #4); guardas de rango (margen y suma en [0,100)), `costo≤0⇒0`. Aislado a propósito: si Daniel entrega su Excel con otra composición, es cambio de una línea + su test.
> ⏳ **HISTÓRICO — tres cosas de esta ficha ya NO son ciertas** (las cambió **V1-E8b**, §Post-F9.125,
> 26-ago-2026). Se marca en vez de reescribirse, porque una ficha de fase cuenta **qué se hizo
> entonces**, no cuál es el estado de hoy:
> 1. `editar-factores` ya **NO** es `listas.administrar`: exige **`listas.aprobar`** — el precio de
>    venta es sólo del dueño.
> 2. Los cuatro factores ya **NO** se ocultan por `consultas.ver-importes`, sino por **`listas.aprobar`**.
> 3. `editarFactoresLista` ya **NO** deja intacto el `precioAprobado`: **lo tumba**, con nota, y la
>    firma vieja va al evento inmutable (D3).
>
> *El estado de hoy manda desde `DECISIONES.md` §Post-F9.125 y la ficha `V1-E8b`.*

- **Dominio** (`dominio/desarrollo/listas-precios.ts` + `cliente-factores.ts`): `crearLista` (A2/A3) valida cliente+depto+empresa (A9), no-apagado, **precosto CONGELADO** y **desarrollo no-ya-en-otra-lista**, **fusionando TODOS los faltantes en un solo error**; **snapshot** de factores y `costoUnit` copiados (D3, no vivos); folio `lista-precios` dentro de la tx. `editarFactoresLista` recalcula todos los `precioCalculado` bajo lock, **nunca** toca `precioAprobado`. `aprobarLinea` (UPDATE atómico columna→columna; rechaza calculado ≤0) / `ajustarPrecioLinea` (precio>0) sellan quién/cuándo (A7). Factores por default del cliente + **override por departamento** (`resolverFactores`). Estado del desarrollo → **en-lista** derivado por el helper de E2 (no seteado a mano).
- **Invariante "un desarrollo → a lo más UNA lista"** enforcada en el dominio y **serializada con `pg_advisory_xact_lock` por empresa** (cierra el TOCTOU B1 que halló el reviewer). **TODO(E5):** blindarla a nivel BD con `@@unique([idDesarrollo])` en `lista_precios_linea` cuando E5 toque migración.
- **API**: RBAC **AND** (mutar⇒leer): lecturas `listas.ver`; crear/editar-factores `listas.administrar`+`listas.ver`; aprobar/ajustar `listas.aprobar`+`listas.ver`. Importes (`costoUnit`/`precioCalculado`/`precioAprobado`/factores %) en `null` sin `consultas.ver-importes` (**ocultación server-side, en el dominio**). **Impreso PDF (R9, `@react-pdf/renderer`) + export Excel (`exceljs`)** exigen `listas.ver`+`consultas.ver-importes`; columnas modelo/nº cliente/precio (`aprobado ?? calculado`). **Cierra el pendiente del impreso "Lista de precios" de `HOJA-DE-RUTA.md` §4.**
- **Frontend**: módulo nuevo **"Listas de precios"** en el menú (sub-vista de Desarrollo, `listas.ver`) con lista+detalle, diálogo crear (candidatos = cotizados sin lista), **vista de aprobación del dueño móvil-friendly** (aprobar/teclear solo con `listas.aprobar`; importes derivados del permiso real), editar factores del snapshot, botones PDF/Excel; **factores en el detalle del Cliente**. `login.spec` (+"Listas de precios") y `catalogo.test` (90→91) ajustados en la misma etapa (lección de CI de E2).
- **Desviaciones/decisiones documentadas:** (1) salida de detalle renombrada a `esquemaListaPreciosDetalle` para no chocar con el `esquemaListaPreciosSalida` de F7 (lista por MODELO, que sigue viva). (2) N7b: el loop de UPDATEs de `editarFactoresLista` se deja como está (correcto; N acotado por renglones del cliente+depto). (3) Punto abierto no bloqueante: la composición exacta de la fórmula (b) queda a reconciliar contra el Excel de Daniel si lo entrega (helper aislado para ello).
- **Pendiente:** verificación de Gabriel en `prueba` (checklist de "Verificación de Gabriel" de arriba) tras el deploy con `SEED_ON_START=true`.

---

## F8-E5 · Negociación por versiones: re-costeo interactivo + acuerdos + estados (R20b) — ✅ (pend. verif. de Gabriel en `prueba`)
...
- Dominio (`listas-precios.ts` + `negociacion.ts`):
  ...
  - **⚠️ Heredado de E4 (marcado como `TODO(E5)` en el código de `dominio/desarrollo/listas-precios.ts`):** al habilitar aquí las transiciones de estado, **agregar el guard de `esCierre`** en `editarFactoresLista`, `aprobarLinea` y `ajustarPrecioLinea` (hoy inalcanzable porque E4 deja la lista en `abierta`, pero obligatorio en cuanto una lista pueda cerrarse); y **blindar la invariante "un desarrollo → a lo más una lista"** a nivel BD con `@@unique([idDesarrollo])` en `lista_precios_linea` en la migración de E5 (E4 ya la enforza en dominio bajo `pg_advisory_xact_lock`, esto es defensa en profundidad).
...
**✅ Nota de cierre (2026-07-06 — pend. verif. de Gabriel en `prueba`):**
- **1 coder + 1 reviewer independiente que APROBÓ.** El reviewer levantó **1 solo hallazgo** (no bloqueante): un comentario `TODO(E5)` obsoleto en `listas-precios.ts` que decía "pendiente blindar la invariante a nivel BD" cuando E5 ya lo hizo → **corregido en la misma ronda** (regla *"un defecto conocido no es menor"*), reescrito para reflejar que el `@@unique` ya está en la BD y que el lock de creación es ahora optimización de UX. Los otros 3 puntos del reviewer eran informativos con razón de diseño (índice compuesto redundante que se deja a propósito; clave del comparador `concepto||descripción` con delta autoritativo aparte; `migration.sql` sin `IF EXISTS` porque casa exacto con `migrate diff`). Gates locales verdes: backend `typecheck`/`lint`/`format:check`/`test:unit` (**761**) + `openapi` sin drift; frontend `typecheck`/`lint`/`format:check`/`test` (**520**) + `build` + `gen:api` sin drift. Integración (`negociacion.int.test.ts`, testcontainers) y e2e (`listas-precios.spec.ts` extendido) corren en CI.
- **Migración ÚNICA de E5** `20260706120000_f8_e5_negociacion`: `ListaPreciosLinea` pasa `@@index([idDesarrollo])` → `@@unique([idDesarrollo])` (drop del índice de E1 + create del unique; nombres cotejados contra la migración de E1). **Blinda a nivel BD la invariante "un desarrollo vive en A LO MÁS UNA lista"** que E4 solo enforzaba en dominio bajo advisory lock — defensa en profundidad que vuelve ese lock una optimización de UX (error claro con TODOS los conflictos, en vez de un 500 por el unique). Las tablas `NegociacionEvento`/`EstadoLista.esCierre` ya nacieron en E1. **SIN permisos ni seed nuevos** (`listas.negociar` ya sembrado y repartido en E1: lo tienen Administrador/AdministracionDireccion/Directivo/Gerencial; Ventas hacia abajo no — decisión (h)). El deploy de la fase a `prueba` sigue requiriendo `SEED_ON_START=true` por E1.
- **Dominio** (`backend/src/dominio/desarrollo/negociacion.ts`): `registrarRonda` (re-apunta el renglón a un precosto **congelado** del MISMO desarrollo y empresa, distinto del actual; recalcula `precioCalculado` con los factores **snapshot** de la lista; **resetea `precioAprobado`/`aprobadoPorId`/`aprobadoEn`**; inserta `NegociacionEvento` con `precioAnterior = aprobado ?? calculado` previo y `precioNuevo = acordado ?? calculado` nuevo), `registrarAcuerdo` (evento SIN re-costeo, no toca precosto ni aprobado), `cambiarEstadoLista` (SIN guard `esCierre` → permite **reabrir** una lista de cierre, con la reapertura en bitácora), `listarEventosDeLinea` (importes `null` sin `consultas.ver-importes`; versiones y acuerdo siempre visibles). En `listas-precios.ts` se resolvieron los **3 `TODO(E5)`**: guard `esCierre` en `editarFactoresLista`/`aprobarLinea`/`ajustarPrecioLinea`, y `aprobarLinea`/`ajustarPrecioLinea` ahora **toman el `pg_advisory_xact_lock` por lista** (mismo namespace `NAMESPACE_LOCK_LISTA` que exporta ese archivo) antes de leer el estado → el guard es **race-free** contra `cambiarEstadoLista` (cierra el TOCTOU; el `idLista` de un renglón es inmutable, verificado). D3 respetada: los eventos y los congelados solo se AGREGAN; el precosto anterior queda con FK `Restrict`, recuperable.
- **Decisiones de diseño (documentadas para confirmar con Daniel, NO bloquean):** (1) una **ronda resetea `precioAprobado`** — el precio re-costeado se vuelve a aprobar con `listas.aprobar`, separando al **negociador** (gerente comercial, `listas.negociar`) del **aprobador** (dueño, `listas.aprobar`), tal como reparte el seed; el precio **acordado** de la ronda vive en el `NegociacionEvento` (`precioNuevo`), nunca fija el aprobado. (2) El **comparador de versiones** es front-diff reusando `GET /precostos/:id` de E3 (que ya oculta importes server-side → sin fuga); el delta autoritativo de costo total viene del backend. (3) El **archivo del departamento** reusa los filtros ya existentes de `ListasPreciosPagina` (cliente/departamento/estado), sin endpoint ni módulo nuevos.
- **API/Frontend:** 4 rutas nuevas en `listas-precios.rutas.ts` (RBAC **AND** mutar⇒`listas.ver`: rondas/acuerdos/estado con `listas.negociar`, eventos con `listas.ver`); contrato en `contrato/esquemas/negociacion.ts`. Pantallas: `DialogoNegociacionRenglon` (historial + "nueva ronda" que reusa `DialogoPrecosto` de E3 → generar/editar/congelar → elegir versión + acuerdo), `ComparadorVersiones`, `SelectorEstadoLista`, integradas en `ListasPreciosPagina`. Controles de negociar/estado gateados por `listas.negociar` (y el backend re-decide, A1); importes derivados del permiso real. Sin módulo nuevo al menú → `login.spec`/`catalogo.test` intactos por menú.
- **Nota de proceso (rebase a mitad de sesión):** durante la construcción, Gabriel mergeó el **PR #98 (administración de roles y permisos)** a `prueba`. La rama de E5, basada en el `prueba` previo, se **rebasó sobre el `prueba` con #98** (con respaldo por stash, sin pérdida) y se **regeneraron** OpenAPI + cliente para que quedaran con roles **+** negociación sin drift; verificado que el diff vs `prueba` es solo de E5 y que los archivos de #98 siguen intactos. Se corrigieron 3 usos de `selectOption({ label: /regex/ })` en el e2e (Playwright exige `label: string`).
- **Pendiente:** verificación de Gabriel en `prueba` (checklist de "Verificación de Gabriel" de arriba) tras el deploy con `SEED_ON_START=true`.

---

## F8-E6 · Enganche con pedidos/órdenes y compras (MRP/OC) + tablero + docs + cierre de fase — ✅ (pend. verif. de Gabriel en `prueba`)
...
**✅ Nota de cierre (E6a backend + E6b frontend/adjuntos/cierre, 2026-07-06 — pend. verif. de Gabriel en `prueba`):**
- **Construida en 2 sub-piezas**, cada una con **coder + reviewer independiente que APROBÓ**. Gates locales verdes (corridos por el lead tras caídas de infraestructura de los agentes): backend `typecheck`/`lint`/`format:check`/`test:unit` (**775**) + `openapi` sin drift; frontend `typecheck`/`lint`/`format:check`/`test` (**532**) + `build` + `gen:api` sin drift. Integración (adjuntos + liga/MRP, testcontainers) y e2e corren en CI.
- **E6a — MRP enganchado + liga (SIN migración, SIN permisos nuevos):** el MRP (`dominio/compras/mrp.ts`) **hereda el amarre**: telas resuelven proveedor/precio con `resolverPrecioTela` (cascada de E1; sin amarre → `idProveedorSugerido` NULL como antes, no-regresión); avíos anteponen `ModeloAvio.idAvioProveedor` vía `resolverPrecioAvio` con **fallback al "más barato" F4 intacto**; avíos `consumoPorTalla` compran por **Σ(medida×piezas) por talla** (R18) leyendo `OrdenLineaTalla`, con fallback a consumo-por-prenda + AVISO; nuevo `avisos: string[]` en la explosión (tela multi-color, talla sin medida, **proveedor amarrado inactivo**). **No-regresión F4 demostrada por test** (modelo sin amarres/talla explota idéntico). **Liga desarrollo↔orden** (`dominio/desarrollo/liga-orden.ts` sobre `DesarrolloOrden`, ya creada en E1): `ligarOrden`/`quitarLiga` (`desarrollo.administrar`; A2/A7/A9; guard de orden cancelada + coherencia modelo+cliente + unicidad `idOrden`); `sugerenciaLigaOrden` + **vista 360** (`expedienteOrden`) + **tablero** (`tableroDesarrollos`, agregación en servidor) con `desarrollo.ver`; el estado `ligado-produccion` se activa solo por el helper de E2. Importes ocultos server-side sin `consultas.ver-importes`. Reviewer: **CAMBIOS REQUERIDOS** (6 hallazgos, núcleo sólido, no-regresión probada) → **los 6 corregidos en la misma ronda** (filtro de empresa en la sugerencia A9; AVISO de proveedor amarrado inactivo en tela y avío; normalización del factor R1 alineada amarre=más-barato; guard de orden cancelada; tests cross-empresa/genérico-por-talla/inactivo; diff de la explosión por proveedor/precio). Verificado por el lead cuando la infra tumbó al reviewer en la confirmación.
- **E6b — frontend + adjuntos R6 + cierre (1 migración aditiva):** sección **"Desarrollo"** en el detalle de orden (sugerencia de liga con precio propuesto / vista 360 con precosto+lista+acuerdos / quitar liga); **avisos** de la explosión en pantalla; **tablero** de desarrollos por estado como **pestaña** dentro de `/desarrollo` (Proyectos | Tablero) → **sin entrada nueva al menú** (`catalogo.test`/`login.spec` intactos). **Adjuntos R6** full-stack: migración `20260706140000_f8_e6_adjuntos_orden` (tabla `orden_archivo`, espejo de `proveedor_archivo`, **aditiva pura**), dominio `dominio/produccion/adjuntos-orden.ts` (subir presigned/listar/eliminar; `ordenes.ver`/`ordenes.administrar` — **sin permiso nuevo**; A9/A7; servicio de archivos inyectable), y el **borrado físico en R2** (`eliminarObjeto`/`DeleteObjectCommand` en `comun/archivos.ts`, best-effort DESPUÉS del commit) → **salda la deuda técnica §8 para los adjuntos de orden**. Docs de módulo: `docs/modulos/desarrollo-cotizacion.md` (nuevo, cierre del módulo 15) + `compras-mrp.md` actualizado. Reviewer: **APROBADO** (N1 —docstring que blinda el best-effort R2 contra uso anidado— aplicado en la misma ronda; N2/N3 informativas, consistentes con patrones existentes).
- **Decisiones (en `DECISIONES.md` F8):** (e-confirmada, 6-jul) **precio al pedido = MOSTRAR el precio acordado editable, NO pre-llenar `PedidoLinea.precio`** (la orden cuelga de un renglón de pedido ya capturado en F2 → escribirlo hacia atrás sería raro; el pre-llenado en F2 queda como **mejora futura en el backlog** de `HOJA-DE-RUTA.md` §4). Permiso de adjuntos = reusa `ordenes.ver`/`ordenes.administrar` (patrón fotos-modelo). Borrado físico R2 solo para adjuntos de orden; extenderlo a modelos/bordados/proveedores = backlog.
- **Nota de proceso:** caídas de infraestructura (watchdog/API error) de los agentes; el trabajo aterrizó completo en disco y el lead verificó cada arreglo + corrió los gates (precedente E3). En un punto convivieron 2 agentes de E6b por un relanzamiento sin verificar → detectado, ambos detenidos, el parcial revertido a la base limpia de E6a y relanzado **un solo** constructor.
- **Pendiente:** verificación de Gabriel en `prueba` (checklist de "Verificación de Gabriel" de arriba) tras el deploy con `SEED_ON_START=true`; luego PR `prueba` → `main` = **cierre de F8**.
