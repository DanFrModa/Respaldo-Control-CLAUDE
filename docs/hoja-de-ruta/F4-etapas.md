...
## F4-E5 · Notas de salida estructuradas (R4): captura, consumo de avíos, envío documentado de telas, consultas e impreso — ✅ (21-jun-2026)

> **Cierre (21-jun-2026) — 2 reviewers independientes APROBARON (backend y frontend, 0 bloqueantes; todos los hallazgos cerrados; pendiente verificación de Gabriel en `prueba`).** 1 coder + 1 reviewer, en varias pasadas (backend → correcciones → almacén → presentación → re-revisiones). **Dominio `backend/src/dominio/notas/notas-salida.ts` (A1):** `crearNotaSalida`/`actualizarNotaSalida`/`confirmarNotaSalida`/`cancelarNotaSalida`/`obtenerNotaSalida`/`listarNotasSalida`. **`confirmarNotaSalida`** en UNA transacción (A2): toma un **advisory lock por nota** (`bloquearNotaSalida`, `pg_advisory_xact_lock(bigint)` namespace propio `0x4e53` — disjunto del de OC `0x4f43` y del espacio `(int,int)` del kardex → sin doble-confirmación ni deadlock; el lock se toma ANTES de leer el estatus) y descuenta el kardex **solo de los renglones de AVÍO** (movimiento `salida-por-nota`, ya sembrado en E1) con no-negativo por suma directa bajo lock (D3). **Los renglones de TELA solo REFERENCIAN una salida-a-orden YA registrada (`idMovimientoSalidaTela`) y NUNCA generan un segundo movimiento de kardex (decisión (e))** — la garantía es **estructural** (confirmar itera solo `lineas.filter(idAvio != null)`), reforzada por `validarRenglones`: el movimiento referenciado debe ser de la empresa, `origenTipo=salida-tela-orden`, de la MISMA orden del renglón, con la tela/lote en su detalle, **no reversado** (`anuladoPor` vacío) y **único entre renglones** de la nota. `cancelarNotaSalida` = movimiento(s) **inverso(s) auditado(s)** (D3, nada se borra) que reversa SOLO los avíos; la tela no se toca. **Almacén origen en el ENCABEZADO (decisión (g) de Daniel, espejo de la recepción de compra):** `NotaSalida.idAlmacen` NOT NULL, validado con el helper compartido `comun/almacenes.ts` (`exigirAlmacen`: existe + activo + global-o-de-la-empresa A9) al crear, editar **y re-validado al confirmar** (por si se desactiva entre borrador y confirmación). Folio `NumNota` por secuencia atómica por empresa (A3), cancelación suave, Bitácora A7, A9. **Migración aditiva `20260621130000_f4_e5_notas_salida`** (`NotaSalida` con `idAlmacen` + `NotaSalidaLinea`; revalidada con `prisma migrate diff --from-empty --to-schema` = match exacto). **3 permisos nuevos `notas.ver`/`notas.administrar`/`notas.cancelar`** → **el deploy a `prueba` requiere `SEED_ON_START=true`** (el tipo de movimiento `salida-por-nota` ya estaba de E1). **Impreso PDF de nota (R9):** `dominio/notas/impresos/impreso-nota-salida.ts` + ruta binaria `GET /notas-salida/:id/impreso` (permiso `notas.ver`), espejo de `impreso-orden-compra`. **Frontend:** módulo `frontend/src/modulos/notas-salida/` — captura (encabezado maquilero + almacén + fechas + observaciones; renglones avío del catálogo con cantidad, o tela elegida de una salida-a-orden existente **sin cantidad libre** —decisión (e) impuesta en la UI—), consulta por nota y notas por orden (responsive regla 10), diálogos de editar/cancelar (motivo obligatorio), editor de renglones y descarga del PDF; cliente del API regenerado, router y 3 entradas de menú con su permiso. **En el MISMO entregable: fix del CI** — `prettier --write` sobre 10 archivos de E4 (`mrp.*`, los 2 impresos de E4, las 2 páginas de explosión/estatus y el spec e2e) que dejaban roja la `prueba` (PR #65) por formato; cero cambio de lógica. **SIN tocar F3.** CI verde local: backend **524** unit (incl. notas + 7 del PDF de nota + 38 de impresos), frontend +21 de notas; integración con testcontainers en CI (anti-doble-descuento de tela, doble-confirmación concurrente → un solo juego de movimientos, almacén inexistente/desactivado, atomicidad/rollback, folio concurrente, reverso). OpenAPI + cliente del frontend regenerados (idempotentes). Decisión (g) registrada en `DECISIONES.md §F4`.
