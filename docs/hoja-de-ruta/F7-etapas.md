...
> **Estado:** 🟡 EN CURSO — decisiones de negocio CERRADAS con Daniel (2026-07-02, ver abajo). E1 arrancando.

## ✅ Decisiones de negocio cerradas con Daniel (2026-07-02) — LEER ANTES DE CODEAR
...
5. **Margen por pedido** = `1 − ( Costo ÷ ( PrecioVenta − bonificacionesCliente ) )`. Las bonificaciones del cliente (logística, publicidad) **restan de la venta**, no del costo.
6. **EDR nuevo desde la FACTURACIÓN real** (modelo/precio/cantidad/cliente → **totales por cliente** + corte por empresa). El **precio manda de lo facturado, NO del pedido**; varios precios del mismo modelo → **precio ponderado**. Como F8/CFDI aún no existe, la conciliación **captura/importa la facturación** del mes, pre-propuesta como comodín desde las **entregas a cliente** de F3, y el usuario la ajusta a lo realmente facturado.
7. **Gastos del EDR: globales por mes** (encabezado global, como hoy). Ventas/Costo por empresa **y por cliente** desde las líneas.
8. **"Entrega a tiempo"** = `fechaReal ≤ fechaPlaneada` del **último proceso de la RC** (E3).
9. **5S FUERA de F7** (0 filas; se retoma en Calidad). E5 construye **solo el inventario cíclico**.
10. **Histórico de EDR NO se migra** (se arranca 2026). El histórico de **costeos** (`CostoOrd` ~2,513) SÍ se migra en E6; los 44 encabezados/1,431 líneas del EDR viejo **no**.
11. Verificado en código: `Ordenes.noCostear` **YA existe** (F2) → E1 **no lo agrega**. `ConfiguracionEmpresa.utilidadSugerida`/`.regaliasBase` y `EtiquetaMarca.regalias` ya existen → **consumir, no recrear**.

## F7-E1 · Motor de costeo: pre-costo, costo de orden y márgenes por pedido — ✅ 3-jul-2026

> **Nota de cierre (3-jul-2026; reviewer independiente APROBÓ; pend. verif. de Gabriel en `prueba`).** Cadena esquema→dominio→API→UI completa: tabla `CostoOrden` 1:1 (tela/procesos/avíos + otros — **regalía FUERA del costo**, D2) + enum `BaseProrrateo` (default `cortado`); dominio `backend/src/dominio/costos/` (`pre-costo`, `costo-orden` con `guardarCostoOrden` transaccional A2 + Bitácora A7 que rechaza `noCostear`, `cantidades` por suma D3/D4, `margenes` en SQL crudo con la fórmula D2 `1 − costo/(precio−bonif)`, `precio-sugerido` = `ceil(costo/(1−util/100)/(1−reg/100))` → con 50/10 reproduce el `(costo×2)/0.9` viejo pero **redondeando al alza**); 8 rutas `/api/costos/**` con RBAC (`costos.ver`/`costos.capturar`/`precostos.consultar`; ocultamiento de importes **reusa** `consultas.ver-importes`, no crea permiso); 5 pantallas teal + 2 impresos PDF (lista de precios, márgenes) + Excel de márgenes. CI verde (backend 707 unit, frontend 453; int/e2e en CI). Reviewer aplicó 1 fix menor (denominador del margen ponderado excluye órdenes de margen NULL). **Sin migración de histórico ni EDR (E2/E6).** **Pendiente:** cuadre numérico contra la hoja manual de Daniel (cuando la entregue) — criterio de cierre de fase. **Supuestos a validar en el cuadre:** bonificaciones del cliente = 0 en F7 (llegan con F8), grano márgenes 1 orden = 1 renglón (resurtidos cuentan cantidadPedida por orden).
...
- Tabla CostoOrden (← CostoOrd): 1:1 con la orden; componentes en doble juego teórico/guardado — **DECISIÓN D2: la regalía sale del costo** → telaCalc/telaCost, procesosCalc/procesosCost (maquila+estampado+otros procesos), aviosCalc/aviosCost (habilitación de costura y empaque), otros+descOtros (flexibilidad); **YA NO** regaliasCalc/regaliasCost (la regalía va sobre la venta, ver lista de precios). costoTotal = Σ de los GUARDADOS (doc 06 §3); baseProrrateo explícita (enum cortado|recibido|vendido, **default cortado = CantCorte** — confirmado por Daniel: tela ÷ prendas cortadas); observaciones; idEmpresa (A9); auditoría A7 + Bitacora (módulo financiero). *(Nota: bordado ya no es componente propio salvo que se necesite — entra dentro de procesos/avíos según doc 06 §3; el coder decide al mapear.)*
- ✅ RESUELTO — Ordenes.noCostear: **YA existe** en el schema v2 (`Ordenes.noCostear` @map `no_costear`, lo trajo F2; verificado). E1 **NO** agrega el campo; solo lo **consume** para rechazar costear una orden marcada. Sin backfill pendiente.
...
## F7-E2 · EDR automatizado: generación desde entregas, conciliación y consultas mes/año — ✅ 3-jul-2026

> **Nota de cierre (3-jul-2026; reviewer independiente APROBÓ sin cambios; CI verde; pend. verif. de Gabriel en `prueba`).** Cadena esquema→dominio→API→UI completa: tablas `Edr` (encabezado **GLOBAL** por mes, `@@unique([anio,mes])`, gastos/intereses/bonificaciones/otros globales — D2 #6, sin `idEmpresa`) + `EdrLinea` (`idOrden?`/`idEmpresa`/`idCliente?`/`idModelo?`, `cantVendida`, `precioVenta` **editable = lo facturado**, `origen` = automatica|ajustada|manual, `costoHistorico` NULL que solo llenará el ETL de E6) + enum `EdrOrigenLinea`. Dominio en `backend/src/dominio/edr/edr.ts`: **`generarEdrMes`** transaccional e **IDEMPOTENTE** (reconcilia SOLO las `automatica` —crea/actualiza/borra huérfanas por `deleteMany`— y **nunca** toca `ajustada`/`manual`; ancla `@@unique([idEdr,idOrden])`, los NULL de manuales conviven); **`calcularEdr`** (Ventas − Costo − Gastos − Intereses + Bonificaciones ± Otros, fórmula legacy `EdoResultTotales`; **costo ACTUAL vivo** D1 = `CostoOrden.costoTotal ÷ base de prorrateo`, reusa `dominio/costos/cantidades.ts`, NUNCA se congela en la línea; cortes por empresa Y por cliente derivados de las líneas; líneas sin `CostoOrden` entran con costo 0 marcadas `sinCosto`); `listarLineasEdr`, `edrPorMes`, `edrPorAnio`, `ajustarLineaEdr` (automatica→ajustada), `agregarLineaManual` (exige empresa `paraEdr` + cliente), `eliminarLineaManual` (solo `manual`), `actualizarEncabezado`. Todo A2/A7 + Bitácora (patrón financiero de `costo-orden.ts`). **EDR CONSOLIDADO** (NO por empresa activa): abarca todas las empresas `paraEdr=true` y excluye órdenes `noCostear` (derivado de D2 #6). **Ventas desde la FACTURACIÓN** (D2 #5): las líneas se pre-proponen desde las etapas `entrega_cliente` de F3 del mes con el precio del pedido como comodín, editable a lo facturado. 12 endpoints `/api/edr/**` con RBAC (`edr.ver`/`edr.capturar`, sembrados a los mismos roles que `costos.*` — Admin/AdministracionDireccion/Directivo, NO Ventas); OpenAPI + cliente del frontend regenerados. Frontend: módulo propio "Estado de resultados" (no sub-vista de Costos) con 4 pantallas teal (Gestión del mes, Conciliación auto-propuesta, EDR por mes, EDR por año) + PDF mensual/anual + Excel. CI verde (backend 712 unit + build; frontend 457 + build; los int con testcontainers en CI prueban idempotencia, exclusión paraEdr=false/noCostear, revaluación a costo actual y `sinCosto`). **SIN histórico** (el EDR arranca 2026, D2 #8; el ETL de costeos es E6). **El deploy a `prueba` requiere `SEED_ON_START=true`** por `edr.ver`/`edr.capturar`. **Decisiones de diseño del coder:** módulo de menú propio; conciliación filtra por origen/empresa/cliente/modelo (no por fecha intra-mes: la línea es agregada por orden del mes); `eliminarLineaManual` = borrado duro (las manuales no tienen efectos derivados). **Deuda menor aceptada:** `generarEdrMes` no toma advisory lock del mes → dos regeneraciones concurrentes del MISMO mes harían fallar una por el unique (sin corrupción; op mensual de directivo). **Pendiente (criterio de cierre de fase):** cuadre numérico de un mes contra el EDR manual de Daniel — hay que **costear las órdenes del mes** (E1) primero, si no las líneas salen `sinCosto`.
...
## F7-E3 · Motor de KPIs en segundo plano + tableros directivos (RC, calidad, WIP) — ✅ 3-jul-2026

> **Nota de cierre (3-jul-2026; reviewer independiente pidió cambios → corregidos y re-verificados; CI verde; pend. verif. de Gabriel en `prueba`).** Corte semi-horizontal completo. **Motor de cálculo pesado en 2º plano:** reutiliza el pg-boss ya cableado (F5) — cola nueva `kpi-refrescar` + `backend/src/comun/jobs/refrescar-kpis.ts` (espejo de `riesgo-rc.ts`: cuerpo testeable `refrescarKpis(bd)` + `registrarRefrescoKpis` con `schedule` cron `KPIS_CRON` default 20 min; registrado en `servidor.ts`; NO-OP si `JOBS_ACTIVOS=false`). **7 vistas materializadas** (SQL crudo en la migración, cada una con índice ÚNICO para `REFRESH … CONCURRENTLY`, `WITH DATA` en el primer poblado + fallback no-concurrente; refresco FUERA de transacción): `kpi_entregas_a_tiempo`, `kpi_lead_time_proceso`, `kpi_cuellos_botella`, `kpi_desempeno_responsable`, `kpi_calidad_maquilero`, `kpi_defecto_maquilero`, `kpi_wip` + tabla `KpiRefresco` (sello "datos al:"). **La captura NUNCA espera el recálculo (plan §11):** las consultas leen la vista ya materializada; `POST /api/indicadores/refrescar` solo ENCOLA el job y regresa. **KPIs:** "entrega a tiempo" = `fecha_real ≤ fecha_planeada_vigente` de la fila `ruta_orden` con `ultimo_proceso=TRUE` (D2/#7); denominador del % = **medibles** (últimos procesos cumplidos CON plan; las `completadasSinPlan` quedan fuera y se muestran como dato informativo — fix M1 del reviewer). Lead time real por proceso = `fecha_real − COALESCE(max(fecha_real de antecesores del DAG), arranque RC)`, promediado y comparado con `duracion_dias`; cuellos = atraso medio por proceso; desempeño por `capturado_por_id` (el "por rol" depende de F9). Calidad por maquilero desde `auditorias` vivas (% aprobación, defectos top, tendencia); WIP reusa la agregación de `dominio/produccion/wip.ts` (F3-E5). **Dominio** `backend/src/dominio/indicadores/kpis.ts` (A1, SQL agregado, nunca pivote en cliente) + impresos PDF/Excel. **4 endpoints** `/api/indicadores/**` (rc, calidad-maquileros, wip, refrescar) con RBAC `indicadores.ver` (directivo/gerencial; no revela costos/precios); OpenAPI + cliente regenerados. **Frontend:** módulo `indicadores` con 3 tableros teal responsive (KPI de RC, calidad por maquilero, WIP analítico), sello "datos al:" y botón Refrescar (encola y regresa). **ADR-0015** (vistas materializadas + job pg-boss). **LIMITACIÓN v1 documentada (ADR-0015 §4 + TSDoc + rótulo `BadgeHistorico` en la UI — fix B1 del reviewer):** lead time / cuellos / desempeño / defectos-top son **histórico acumulado** (vistas pre-agregadas por proceso/maquilero) → NO filtran por periodo/cliente/maquilero; las tarjetas lo rotulan para no engañar al directivo. Solo % entregas a tiempo y las tendencias honran el periodo. Si se requiere cortar esos KPIs por periodo, se re-modela la vista al grano por instancia. **Deploy a `prueba`:** requiere `SEED_ON_START=true` (permiso `indicadores.ver`) y `JOBS_ACTIVOS≠false` para el cron; el primer poblado también se dispara con el botón Refrescar. **Pendiente:** verificación en vivo de Gabriel; el desempeño "por responsable/rol" se completa cuando F9 migre usuarios (`UsuarioRol`).
...
## F7-E4 · Productividad unificada IP/Almacén + fichas confiables + muestrarios — ✅ 3-jul-2026

> **Nota de cierre (3-jul-2026; reviewer independiente APROBÓ —completitud + correctitud—; CI verde; pend. verif. de Gabriel en `prueba`).** Cadena esquema→dominio→API→UI completa para las **3 sub-features**. **Productividad unificada IP/Almacén:** tablas `PersonalArea` (nombre/horasBase/puesto/activo/**area** ip|almacen), `ActividadProductividad` (actividad/area/`porcentajeD`/`pzPersDia`/`porcenPzas`), `RegistroProductividad` (fecha/persona/actividad/cantidad/horasTrabajadas/**personas** cuadrilla/idCliente) + nueva columna **`ConfiguracionEmpresa.jornadaBaseAlmacen`** (default 9, ya no hardcodeada). Índice IP = `(horasBase/horasTrabajadas)×porcentajeD×cantidad`; índice Almacén = `((((J/pzPersDia)/J)×piezas)/personas)×(J/horasTrabajadas)` con **J = `jornadaBaseAlmacen`** de la config; rechazo por división por cero (horas/personas/pzPersDia ≤ 0, Zod + guardas). **Vistas semanal/mensual = AGREGACIÓN real** de los registros diarios (Σ + promedio en SQL, periodos ISO), NO las heurísticas /5 y /30 del viejo — documentado en TSDoc como default a confirmar con Daniel. **Fichas confiables:** `ChecklistFichaDef` (reactivos por FILAS, A6; seed de los 8: InfGeneral/InfTela/InfHab/Medidas/Dibujo/InfEtiqueta/EspCostura/MedidasPrendas) + `FichaVerificacion` por orden; % confiables = Σ OK ÷ Σ activos; hook TSDoc para R5 (ficha estructurada, módulo 12). **Muestrarios:** tabla `Muestrario` (fechas solicitado/requerida/entregado, boardsOK/muestrasOK, cancelación suave) + KPI de cumplimiento (fechaEntregado ≤ fechaRequerida). **API:** submódulos `productividad`/`fichas`/`muestrarios` bajo `/api/indicadores` (23 endpoints) con RBAC por submódulo (`indicadores.ip-productividad`/`.almacen-productividad`/`.ip-confiabilidad`/`.ip-muestrarios`, ya en el catálogo → sin permisos nuevos; el atajo de fecha libre exige `indicadores.fecha-libre`). Guard nuevo **`conAlgunPermiso`** en `auth/plugin.ts` (deny-by-default, exige ≥1 permiso) para la lectura de productividad visible con IP **o** almacén; el dominio reaplica el permiso fino por área (A1). OpenAPI + cliente regenerados. **Frontend:** 5 pantallas teal en el módulo `indicadores` (Captura unificada con atajos Hoy/Ayer/Sábado, Tablero vs estándar, Catálogos de personal+actividades, Fichas confiables, Muestrarios). Auditoría A7 + cancelación suave; A9 por empresa activa. **Migración aditiva SIN permisos/roles nuevos**; el seed solo agrega los 8 reactivos (idempotente) → el deploy a `prueba` **igual conviene con `SEED_ON_START=true`** para sembrarlos. **Alcance negativo (registrado en `MEJORAS.md` módulo 05):** la auto-alimentación de la productividad desde movimientos/recibos existentes (doc 05 Obs. 4) se DESCARTA para F7 (captura manual con atajos); queda como mejora futura. CI verde (backend 723 unit + build; frontend 463 + build; int con testcontainers valida ambas fórmulas, fichas, muestrarios y A9). **Observaciones menores del reviewer (no bloquean, a criterio de Daniel):** (1) el índice IP con horasBase/porcentajeD nulos aporta 0 al promedio del tablero (COALESCE) en vez de excluirse; (2) `actualizarMuestrario` permite editar seguimiento tras la entrega. **Nota de proceso (Gabriel, 3-jul):** el coder de E4 fue interrumpido antes del cleanup final; el lead corrió lint/format él mismo — corregido el criterio: en adelante también los arreglos mecánicos van a un coder.
...
## F7-E5 · Inventario cíclico contra kardex propio + auditoría 5S condicionada — ✅ 3-jul-2026

> **Nota de cierre (3-jul-2026; reviewer independiente APROBÓ tras 1 ciclo de corrección; CI verde; pend. verif. de Gabriel en `prueba`).** Cadena esquema→dominio→API→UI completa del inventario cíclico contra el **kardex propio de v2** (D6/D3/D4). **5S DESCARTADO** (decisión de Daniel #8, `DECISIONES.md` D2: `Alm_5s` tiene 0 filas reales; se retoma en Calidad) → E5 = **solo inventario cíclico**, sin consultar nada más a Daniel. **Modelo de datos:** enum `EstadoInventarioCiclico{abierto,contado,cerrado,cancelado}` + `InventarioCiclico` (encabezado, folio A3 por secuencia `inventario-ciclico`, A9 por empresa, cancelación suave) + `InventarioCiclicoDet` a la **granularidad REAL del artículo de kardex incluyendo `idOrden` (nullable)** — **decisión de arquitectura del lead**: la ficha decía D4 (modelo×color×talla×almacén) pero la existencia PT de v2 se llavea también por `idOrden` (F6-E2 "PT por orden", ADR-0014); congelar y ajustar por artículo real evita inventar un reparto entre órdenes y mantiene el ajuste como movimiento de kardex 1:1 no-negativo (documentado en el TSDoc del modelo y del dominio para que Gabriel/Daniel lo confirmen). Migración aditiva `20260703170000_f7_e5_inventario_ciclico` copiada 1:1 de `prisma migrate diff --from-empty` (cero drift). **Dominio** `backend/src/dominio/indicadores/inventario-ciclico.ts` (A1): **alta CONGELA el teórico** con `bloquearArticuloPt`+`existenciaPtBloqueada(...,idOrden)` (suma directa bajo lock — NUNCA la vista para congelar; la vista `existencia_pt` solo ENUMERA candidatos con existencia≠0; siembra el detalle por lotes con `createMany`); **conteo CIEGO en 3 capas** (el `select` de conteo no trae `cantTeorica`, el esquema de salida no lo incluye, el PDF lo deja en blanco); exactitud = `cantReal − cantTeorica` (solo en la consulta); **generar ajuste = MOVIMIENTO de kardex** (D3) con tipos dedicados `ajuste-ciclico-entrada/-salida` + `ORIGEN.ajusteCiclico`, salidas validan no-negativo bajo lock contra la existencia ACTUAL, agrupado en ≤2 movimientos (entradas/salidas juntas por almacén) enlazados por renglón vía `idMovimientoAjuste`, **protegido contra doble-generación con `SELECT … FOR UPDATE` sobre el encabezado + re-verificación del estado bajo el lock** (fix B1 del reviewer: el delta se precalcula sobre la teórica congelada → NO idempotente, así que el lock del encabezado es LA garantía de ejecución única); cancelación suave sin ajuste. Auditoría A7 + Bitácora en cada mutación. **API** `backend/src/api/indicadores/ciclicos.rutas.ts` (montada en `app.ts`): CRUD + conteo + exactitud + generar-ajuste + cancelar + **hoja de conteo PDF (backend, sin teórico)**; RBAC server-side con los 3 permisos `indicadores.ciclicos-*` **YA existentes** (heredados de los forms `Alm_IC_*` → SIN permisos nuevos). OpenAPI + cliente del frontend regenerados (diff puramente aditivo, no editado a mano). **Frontend:** 3 pantallas teal en el módulo `indicadores` — lista/alta (`InventariosCiclicosPagina`), conteo móvil ciego (`ConteoCiclicoPagina`), exactitud+generar-ajuste (`ExactitudCiclicoPagina`) — + ruta en `App.tsx` y entrada de menú en `catalogo.ts`. CI verde (backend 734 unit + build; frontend 469 + build; el `inventario-ciclico.int.test.ts` cubre congela→mov-posterior-no-cambia-teórico→conteo-ciego→ajuste-solo-por-movimiento→re-generar-rechazado→salida-negativa-rechazada→**concurrencia B1** (2 ajustes paralelos → 1 éxito + 1 conflicto + 1 solo movimiento)→A9, corre en CI con testcontainers). **Migración aditiva SIN permisos nuevos**; el seed agrega 2 tipos de movimiento (`ajuste-ciclico-entrada/-salida`, idempotentes con `update:{}`) → **el deploy a `prueba` requiere `SEED_ON_START=true`**. **Decisiones de diseño confirmadas por el reviewer:** (1) generar ajuste exige `estado=contado` (sin ajuste parcial: evita omitir renglones sin contar); (2) delta sobre la teórica congelada validando las salidas contra la existencia ACTUAL = correcto (preserva los movimientos legítimos ocurridos entre alta y ajuste; no reconcilia a `cantReal`, que los borraría); (3) agrupación en ≤2 movimientos con enlace por renglón (conjuntos entrada/salida disjuntos, trazabilidad intacta). **Pendiente:** verificación en vivo de Gabriel.
