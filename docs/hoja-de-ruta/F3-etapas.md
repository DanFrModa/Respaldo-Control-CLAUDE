...
> **Estado:** 🔄 **EN CURSO (3/6)** — `F3-E1` ✅ (17-jun-2026), `F3-E2` ✅ (18-jun-2026) y `F3-E3` ✅ (19-jun-2026; reviewer independiente APROBADO; pendiente verificación de Gabriel en `prueba`). Siguen E4→E6. El desglose se confirmó al arrancar (supuestos verificados: `TipoProceso` ya existía de F1, el motor kardex NO existía — se construyó en E1).
>
...
## F3-E3 · Inventario PT operable: movimientos manuales, traspasos, existencias y kardex — ✅ 19-jun-2026 (reviewer independiente APROBADO; pendiente verificación de Gabriel en `prueba`)

> **Cierre F3-E3 (19-jun-2026; reviewer independiente APROBADO; pendiente verificación de Gabriel en `prueba`).** Primer uso real del motor kardex de E1 con todo el inventario PT manual y sus consultas. **Dominio** `backend/src/dominio/inventarios/movimientos-pt.ts`: `registrarMovimientoPt` (entrada/salida manual con tipo del catálogo `TipoMovimientoInventario`; las **salidas validan no-negativo** con `bloquearArticuloPt`+`existenciaPtBloqueada` DENTRO de la tx —suma directa del detalle, NUNCA la vista, ADR-0010 §3—; `origenTipo` = `movimientoManual`); `registrarTraspasoPt` (abre la tx en dominio, toma el lock y valida existencia del ORIGEN por artículo, y delega al `registrarTraspasoPt` del MOTOR en la MISMA tx — **cierra el TODO de `comun/kardex.ts`** sin tocar el núcleo); `cancelarMovimientoPt` (= movimiento INVERSO auditado: tipo inverso por la dirección del original, entrada→`error-entrada`, salida→`error-salida`; motivo en Bitácora A7; reemplaza la práctica vieja 'Error de Entrada/Salida', NUNCA edita/borra — D3); `consultarExistenciasPt` (lee la vista `existencia_pt` por `$queryRaw` con filtros modelo/color/talla/almacén — aquí SÍ la vista, es CONSULTA); `kardexPt` (por modelo con saldo corrido por artículo, y por folio). **6 endpoints** `/api/inventarios/pt/*` con RBAC (`inventario-pt.ver` consultas / `inventario-pt.mover` escrituras); **CERO endpoints de edición/borrado** (D3). **Frontend** `frontend/src/modulos/inventarios/`: 4 pantallas teal lista+detalle — Movimientos manuales, Traspaso entre almacenes, **Existencias responsive PC+MÓVIL** (la consulta móvil del módulo), y Kardex (por modelo/por folio, con cancelación con confirmación); reutiliza `MatrizColorTalla` y un `SelectorModelo`. **SIN migración** (las tablas y la vista `existencia_pt` ya nacieron en F3-E1) y **SIN permisos nuevos** (`inventario-pt.*` ya sembrados en E1); lo ÚNICO que toca el seed son **2 tipos de movimiento** nuevos (`transferencia-salida`/`transferencia-entrada`, idempotentes, para las dos patas del traspaso) → el deploy a `prueba` necesita `SEED_ON_START=true`. **`IPT_Revision` NO se construye** (con kardex puro no hay saldo materializado que recuadrar — 04-Inventarios Obs.1; se documenta su desaparición). OpenAPI + cliente regenerados (regla 7). **Decisiones de diseño del lead (reversibles, dentro del patrón):** (a) el traspaso se materializa como dos patas con tipos `transferencia-salida`/`transferencia-entrada`; (b) **salidas y la pata-origen del traspaso NO pueden dejar existencia negativa** (validado por suma directa bajo lock); entradas e inversos de cancelación NO llevan ese tope. **Patrón consistente (clave):** la consulta de existencias/kardex re-valida en el dominio con esquemas LOCALES `z.boolean()` (no el `stringbool` del contrato) — evita de raíz el bug de re-validación de banderas que afectó a las consultas de F2 (corregido el 19-jun en el hotfix `fix(ordenes)` PR #56). **Equipo:** 1 coder + 1 reviewer independiente (cadena dominio→API→UI del módulo inventarios). **CI/checks (corridos por el lead sin Docker):** backend `tsc`/eslint/prettier + **375 unit** verde; frontend `tsc`/eslint/prettier + **293** verde; integración (existencia = suma de movimientos; no-negativo en salida y traspaso; traspaso atómico; inverso de cancelación neutraliza el saldo; dos salidas concurrentes del mismo artículo no dejan negativo; rechazo de dirección `traspaso` como movimiento simple; kardex con saldo corrido y por folio) + e2e Playwright (entrada manual → traspaso → existencias → kardex) corren en CI con testcontainers. Nits del reviewer aplicados (query de existencias deshabilitada hasta tener modelo/origen; orden determinista de locks anti-deadlock; renombre menor).
