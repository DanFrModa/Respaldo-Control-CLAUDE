...
>
> **Pendiente (no bloquea):** el ETL se corre a mano post-deploy (`npx tsx --env-file=.env migracion/etl-calidad.ts` y `.../etl-esma.ts`, luego `.../cuadre-f6.ts`); Gabriel valida el cuadre EN VIVO en `prueba` (criterio de salida "EsMa cuadra contra los recibos del periodo"). Refinamiento F9: un maquilero `Proceso=1` puede quedar en v2 solo con rol `maquila-costura` (sin `estampado`) → afecta filtros de UI por tipo, no la validez del cargo. Ubicación final del módulo de Calidad = D8 (sigue pendiente).
