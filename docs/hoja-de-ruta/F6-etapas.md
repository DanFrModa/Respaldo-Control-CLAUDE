...
## F6-E4 · EsMa — movimientos, validación de cargos, saldos, conciliación y recibo de pago (R9) — ✅ HECHA (1-jul-2026)

> **NOTA DE CIERRE (F6-E4, 1-jul-2026 — 1 coder backend + 1 coder frontend + 2 reviewers independientes APROBARON; pendiente verificación de Gabriel en `prueba`).** El corazón contable de EsMa: se completó el cargo nacido en F3, se agregaron los 3 conceptos (abonos/descuentos/pagos), el saldo SIEMPRE derivado (D3) y la conciliación contra recibos, + el recibo de pago PDF (R9). Decisiones (e)–(h) de Daniel YA aplicadas. **SIN permisos, seed ni tipos-movimiento nuevos** (reusa `esma.cargo-validar` / `esma.modificar` / `esma.ver-pagos` / `consultas.ver-importes`, ya en el contrato/seed de F0) → el deploy a `prueba` **NO** requiere `SEED_ON_START` (sí trae migración aditiva).
>
> **Qué entregó:**
> - **Migración aditiva `20260701120000_f6_e4_esma_movimientos`** (2 enums + columnas + 4 tablas): en `esma_cargo` agrega `cantidadPagada` (cache Decimal @default 0), `sinCosto` (@default false), `conFactura` (nullable); en `proveedores` `modalidadFacturacion` (enum `ModalidadFacturacion` solo_con/solo_sin/ambos, nullable); en `ordenes` `pagadaForzada` (nullable, override auditado); 3 tablas **planas** `AbonoMaquilero`/`DescuentoMaquilero`/`PagoMaquilero` (idEmpresa A9, idMaquilero→Proveedor, monto Decimal(14,2), fecha, `conFactura`, `estadoRevision` @default capturado, obs, A7) + puente **`PagoAplicacion`** (pago↔cargo, PK `(idPago,idCargo)`, cantidad+importe). Los DEFAULTS backfillean las filas viejas → **sin UPDATE manual** (evita la trampa de F6-E2). SQL a mano == DDL canónico de Prisma (validado con `migrate diff` schema-a-schema, sin BD local).
> - **Dominio (A1)** `dominio/esma/`: `cargos.ts` extendido (precio propuesto de referencia por proceso —costura→`maquilaOrd`, estampado/aplicación→`aplicacionOrd`, fallback `precioPactado`—, decisión (e) que **corrige el bug v1**; `sinCosto`; fija `conFactura`); `facturacion.ts` (`resolverConFactura`: solo_con/solo_sin fuerzan, ambos obliga a elegir, decisión (h)); `movimientos.ts` (captura de abonos/descuentos, tx + bitácora); `pagos.ts` (**prendas por pagar**, decisión (g): `porPagar = cantidadReal − Σ PagoAplicacion.cantidad` por suma directa bajo `pg_advisory_xact_lock` por (empresa,maquilero); re-pagar/pagar de más → `ErrorConflicto` DURO; `cantidadPagada` es solo cache sellado en la misma tx; `monto` derivado de Σ cantidad×precioReal); `saldos.ts` (saldo = Σcargos(validado,no-sinCosto) + Σabonos − Σpagos − Σdescuentos, nulos=0, **agregación derivada NUNCA columna, D3**, segmentable por `conFactura`); `conciliacion.ts` (recibido vs cargado por orden+maquilero+proceso → `faltantePorCargar` + `cargosSinRecibo`, lógica CuantasFaltan unificada sin el par /Est); `orden-pagada.ts` (`Orden.pagada` **derivada** —ignora `sinCosto`/`cancelado`— + override `pagadaForzada` auditado; lectura autoritativa recomputa en vivo); `impresos/impreso-recibo-pago.ts` (recibo R9, pagador = `Empresa.razonSocial ?? nombre`, NUNCA hardcodeado).
> - **Rutas REST** `/api/esma/…`: `POST abonos`/`descuentos`/`pagos`, `GET pagos/:id` (+ `/impreso` → application/pdf), `GET maquileros/:id/{abonos,descuentos,pagos,saldo}`, `GET conciliacion`, `GET|POST ordenes/:id/pagada`. RBAC deny-by-default por acción (A4): abonos/descuentos + forzar pagada → `esma.modificar`; pagos + lectura de cuenta (saldo/conciliación/listas/impreso) → `esma.ver-pagos`; validar cargo → `esma.cargo-validar`; **importes ocultos sin `consultas.ver-importes`** en las vistas de cuenta (la cola de validación de cargos SÍ los muestra —quien valida un precio necesita verlo—, política ratificable con Daniel).
> - **Frontend (teal lista+detalle, responsive):** `EsMaPagina` (hub con tarjetas por permiso), `ValidacionCargosPagina` extendida (marcar `sinCosto` + elegir `conFactura` tri-estado), `ConciliacionCargosPagina` (recibido/cargado/faltante + toggle "solo con faltante"), `CapturaMovimientoPagina` (abonos/descuentos, prop `concepto`), `CapturaPagosPagina` (pago ligado a cargos con "prendas por pagar" + imprimir recibo PDF), `SaldoMaquilero` (tarjeta de saldo segmentable). Rutas en `App.tsx` + 4 sub-vistas en `catalogo.ts` (testids únicos, sin strict-mode); `catalogo.test.ts` 61→65.
> - **Contrato** OpenAPI regenerado + cliente del frontend sincronizado. **Tests:** dominio (saldo con nulos, prendas-por-pagar re-pago=error bajo lock, estampado con aplicacionOrd, segundas sin costo excluidas, Orden.pagada derivada+forzada, conciliación con faltantes/cargos-sin-recibo), API (deny-by-default 403: solo `esma.ver-pagos` NO crea abonos/descuentos; PDF 200), unitario del PDF (pagador de empresa), y componente + e2e del frontend (gate por permiso, importe oculto "—", envío de pago con aplicaciones, imprimir recibo). CI: **684 unit backend + 442 frontend**, typecheck/lint/format/build verdes, OpenAPI en sync.
>
> **Decisiones de modelado (lead) — para el reviewer:** los estados capturado/revisado/pagado se **proyectan** de `estado` (propuesto=capturado, validado=revisado) + `cantidadPagada` (pagado), sin columna `estadoConciliacion` redundante. El anti-doble-pago es estructural (aplicaciones a cargos + lock), no un mero aviso por monto+ventana (decisión g). El saldo extiende D3 (nunca editable). El esquema **permite pagos LIBRES sin aplicaciones a cargos** (monto persistido, `aplicaciones` 0..N) → el ETL de E6 podrá migrar los 5,935 pagos históricos sin liga; la exigencia de ≥1 aplicación es solo de la captura interactiva.
>
> **Aceptado por los reviewers (no bloquea):** (A) `conFactura` tri-estado en el frontend (el contrato del Proveedor no expone `modalidadFacturacion`; el backend es autoridad y obliga para "ambos" con toast) — **recomendación E5: exponer `modalidadFacturacion` en el contrato del proveedor**. (B) conciliación más ligera que la ficha (cortado/entregado y filtro "pagadas" → pantalla central de **E5**; el criterio de cierre de E4 —faltantes + cargos sin recibo— SÍ se cumple).
>
> **⏳ Pendiente de ratificar con Daniel (no bloquea):** (1) la **política de visibilidad** —precios visibles en la cola de validación de cargos vs ocultos en el estado de cuenta—; (2) una orden con **solo cargos `sinCosto`** (o ninguno validado) deriva `pagada=false` permanente salvo override (probablemente intencional: no hay nada que pagar).
>
> **Notas para E5:** validar que la factura de un pago cuadre con la de los cargos que abona (proveedor "ambos", `saldos.ts`); si E5 agrega cancelar abonos/descuentos/pagos debe **revertir** `cantidadPagada` y recomputar `Orden.pagada` (hoy los movimientos son append-only); filtrar el selector de maquilero a proveedores con rol de maquila (hoy trae todos, tope 100); validar en cliente la cantidad del pago contra `porPagar` (hoy solo el backend la bloquea).
>
> **NO toca / deja para después:** la pantalla CENTRAL del estado de cuenta completo (saldos-de-todos, semanales, desglosado, duplicar partida, vista móvil) = **F6-E5**; el **ETL del histórico** (11,369 EsMa → 7,401 cargos / 554 abonos / 743 descuentos / 5,935 pagos) + el reporte de cuadre v1↔v2 = **F6-E6** (cierre de fase). Sin `docs/modulos/esma.md` todavía (se documenta al cerrar el módulo en E6).
...
## F6-E5 · EsMa — estado de cuenta, consultas semanales, desglosado e impreso del estado de cuenta (R9) + vista móvil — ✅ HECHA (1-jul-2026)

> **NOTA DE CIERRE (F6-E5, 1-jul-2026 — 1 coder + 1 reviewer independiente APROBÓ; pendiente verificación de Gabriel en `prueba`).** La experiencia de usuario completa de EsMa sobre el motor contable de E4: la pantalla central del estado de cuenta, saldos de todos, consultas semanales, desglosado exportable e impreso R9, + vista móvil. **SIN migración, SIN permisos nuevos, SIN re-seed** (reusa `esma.ver-pagos` / `esma.modificar` / `consultas.ver-importes`, ya en el contrato/seed de F0) → el deploy a `prueba` **NO** requiere `SEED_ON_START`.
>
> **Qué entregó:**
> - **Dominio (A1)** nuevo en `dominio/esma/`: `estado-cuenta.ts` (`estadoCuentaMaquilero` unifica los 4 conceptos por fecha con marca `pendienteRevision` + saldo derivado reusando `saldoDeMaquilero` + totales; `estadoCuentaDesglosado` por orden/modelo/cantidad/precio/importe) · `saldos-todos.ts` (`saldosDeTodosMaquileros` con **SQL crudo agregado** —NO N+1—, fórmula IDÉNTICA a `saldoDeMaquilero`: cargos solo `validado` y no `sinCosto`, +abonos −pagos −descuentos, nulos=0; A9; excluye saldo 0; segmenta por `conFactura`) · `semanales.ts` (`pagosSemanales` + `recibosSemanalesMaquilaEsMa`) · `maquileros.ts` (`listarMaquilerosEsMa` = activos con rol de maquila `maquila-costura`/`estampado`, **cierra la deuda tope-100 de E4**) · `revisarMovimiento` en `movimientos.ts` (transición `estadoRevision` capturado→revisado, tx A2 + bitácora A7 + 409 si ya revisado). Impresos `impresos/impreso-estado-cuenta.ts` (PDF R9, pagador = razón social de `Empresa`, patrón `impreso-recibo-pago`) y `impresos/excel-estado-cuenta.ts` (exceljs, 3 hojas — **V1-E8k le sumó una cuarta, «Prendas incompletas»**).
> - **Rutas REST** `backend/src/api/esma/estado-cuenta.rutas.ts` (9 endpoints, estáticas antes de `/:id`, registrado en `app.ts`): `GET maquileros/:id/estado-cuenta`, `…/desglosado` (+ `/impreso` PDF y `/excel` xlsx), `GET saldos`, `GET pagos-semanales`, `GET recibos-semanales`, `GET maquileros?tipo=`, `POST movimientos/:concepto/:id/revisar`. RBAC deny-by-default (A4): consultas `esma.ver-pagos`, revisar `esma.modificar`; importes ocultos sin `consultas.ver-importes` (server-side, también en los impresos). 403 real verificado por test.
> - **Frontend (teal, responsive):** `EstadoCuentaPagina` (central: selector tipo+maquilero, grid de 4 conceptos con marcas de revisión, saldo, existencias del maquilero —reusa F3, gated por `produccion.wip-ver`—, **Duplicar partida** que pre-llena la captura de E4, autorizar partida en móvil, acceso a desglosado/PDF/Excel), `SaldosMaquilerosPagina` (activos con saldo≠0, drill-down), `PagosSemanalesPagina`, `RecibosSemanalesEsMaPagina`, `DesglosadoPagina` (Excel + PDF), `SelectorMaquilero` reusable. Hooks en `api/esma.ts`; rutas en `App.tsx`; hub `EsMaPagina.tsx` + menú `catalogo.ts` (`catalogo.test` 65→70).
> - **Decisiones (Gabriel, 1-jul):** (1) exponer `modalidadFacturacion` del Proveedor en el contrato (salida + crear/editar) — cierra la recomendación de E4; (2) alcance = **consulta + "duplicar partida"**, SIN cancelar/editar movimientos (append-only de E4 intacto). Además se cerró el item **diferido de E4**: la conciliación (`conciliarEsMa` + `ConciliacionCargosPagina`) ahora trae columnas **cortado/entregado** (F3/wip) + badge **pagada** y filtro todas/pagadas/no-pagadas. El selector de maquilero de las capturas de E4 se migró a `useMaquilerosEsMa` (mata el workaround `porPagina:100`).
> - **Contrato** OpenAPI regenerado + cliente del frontend sincronizado (idempotente). **Tests:** dominio int (paridad del saldo E4↔saldos-todos, orden determinista, ocultamiento de importes, `revisar` transición+bitácora+409, desglosado), API (deny-by-default 403; PDF/xlsx 200 con content-type), impreso/excel unit, componente (Estado de cuenta, Saldos, "duplicar partida") + e2e shallow. CI local: **686 unit backend + 450 frontend**, typecheck/lint/format/build verdes, OpenAPI en sync.
>
> **Reviewer independiente APROBÓ** sin bloqueantes (verificó a fondo el SQL crudo de `saldos-todos.ts` —`@@map` correctos + fórmula matemáticamente idéntica a E4—, el RBAC 403 real de las 9 rutas, el ocultamiento de importes, A2/A7/A9 de `revisar`, el pagador del impreso desde `Empresa`, y que el toque a E4 no regresiona). Nota menor NO bloqueante: `format:check` del backend marca `migracion/_progreso.ts` (gitignored, artefacto local de un ETL) → falso positivo, no existe en el checkout limpio de CI.
>
> **NO toca / deja para después:** el **ETL del histórico** (11,369 EsMa → 7,401 cargos / 554 abonos / 743 descuentos / 5,935 pagos; 40 defectos / 488 auditorías / 15,296 detalles) + el reporte de cuadre v1↔v2 + `docs/modulos/{esma,calidad}.md` = **F6-E6** (cierre de fase). Pendientes de ratificar con Daniel heredados de E4 (no bloquean): política de visibilidad de precios y orden con solo cargos `sinCosto`.
...
## F6-E6 · ETL Calidad + EsMa, reporte de cuadre v1 vs v2, documentación y cierre de fase — ✅ HECHA (2-jul-2026)

> **NOTA DE CIERRE (F6-E6, 2-jul-2026 — 2 coders en paralelo + 1 reviewer independiente APROBÓ; pendiente verificación de Gabriel en `prueba`).** Cierre de la fase F6: se migró el histórico real de Calidad y EsMa **vía los servicios de dominio** (modo migración), se construyó el **reporte de cuadre v1↔v2 obligatorio** y se documentaron ambos módulos. **SIN migración de esquema, SIN permisos, SIN seed nuevos** → el deploy a `prueba` NO requiere `SEED_ON_START`; el ETL se corre a mano post-deploy (como los demás cierres de fase).
>
> **Qué entregó (Calidad — pieza A):**
> - `migracion/loaders/calidad-defectos.ts`: `CC_Catalogo` (40) → `DefectoCatalogo` vía `crearDefecto`; **severidad INFERIDA del AQL** (1→crítico, 2.5→mayor, 10→menor; metadato "para revisión", NO veredicto —decisión (a)—), `aplicaGeneral=true` para todos (v1 no tiene tipo de producto —decisión (d)—; el tipo se etiqueta a mano después). Idempotente por mapeo + clave insensible a mayúsculas.
> - `migracion/loaders/calidad-auditorias.ts`: `CC_Auditorias` (488) + `CC_AuditoriasDet` (15,296) → `Auditoria` + `AuditoriaDefecto` vía el nuevo **modo migración**. Mapeos `Resultado 1/2/0→aprobado/reprobado/no_calificado` y `TipoAuditoria 1/2/0→en_piso/final/no_definida` (verificados contra `QueResultado`/`QueTipoAudit` del viejo). Pares `(auditoría,defecto)` duplicados del viejo → **fusionados sumando fallas** (defensa del `@@unique`). Auditoría con orden sin mapeo → OMITIDA (idOrden FK obligatoria); todo LISTADO.
> - Dominio (A1): `crearAuditoriaMigrada` en `dominio/calidad/auditorias.ts` — **preserva el folio histórico `numAuditoria`** (no lo saca de la secuencia) y al final **recalibra la secuencia A3** por empresa (GREATEST, idempotente); fija resultado/tipo/cancelación explícitos, `resultadoManual=true`; **NO publica el evento de RC** (nada de 488 auto-avances). El maquilero de la auditoría se resuelve SOLO por `Proveedor:IdMaquileros` (verificado: `CC_Auditorias.IdMaquilero` → tabla `Maquileros` de costura); usuarios elaboró/auditor se preservan como TEXTO del id viejo (sin FK, ADR-0005; remapeables en F10).
> - `migracion/etl-calidad.ts` (orquestador + `main()`) + `docs/modulos/calidad.md`.
>
> **Qué entregó (EsMa — pieza B):**
> - **FIX del gap de estampadores** (`loaders/esma-cargos.ts`): los cargos `EsMa_Recibos.EsEstampado=1` resolvían el maquilero contra el mapa de Estampadores (44 del catálogo aparte), pero en realidad apuntan a **maquileros con `Proceso=1`** (que hacen estampado). Se corrigió para resolver por `mapaMaquilero` (fallback estampador) → **recupera 1,251 de 1,252 cargos** antes omitidos en F3-E6; 1 huérfano real (`IdEsMa_Recibos=5811`, sin IdMaquileros) listado. **Cierra la deuda de `DECISIONES §F3-E6 (a)` con la causa raíz correcta (bug de loader, NO proveedores faltantes) — no hizo falta extender la migración de proveedores.** El `idTipoProceso` del cargo sigue siendo estampado.
> - Dominio (A1): `crearAbonoMigrado`/`crearDescuentoMigrado`/`crearPagoMigrado` en `dominio/esma/migracion.ts` — tx (A2) + bitácora `operacion:'migracion'` (A7), `conFactura=null` (v1 no tiene el flag), SIN efectos derivados; **los 5,935 pagos históricos son LIBRES, sin aplicaciones a cargos** (sin `pg_advisory_xact_lock`, sin recomputar `Orden.pagada` —el esquema de E4 lo dejó permitido a propósito—).
> - `loaders/esma-{abonos,descuentos,pagos}.ts` (554 / 743 / 5,935) aplanando la cabecera `EsMa.csv` (FechaEsMa/IdMaquileros/ObsEsMa) a cada movimiento; `estadoRevision` = revisado (abonos/desc, histórico conciliado) / por `RevisionPendienteP` (pagos). `migracion/etl-esma.ts` (orquestador idempotente; los cargos también los carga F3 vía el mismo `MapeoMigracion` → no duplica) + `docs/modulos/esma.md`.
> - **`migracion/cuadre-f6.ts` (criterio de salida):** conteos v1 (parser real CP850) vs v2 (Prisma) de las 7 tablas; **saldo por maquilero v1 vs v2 con la MISMA fórmula derivada** de `dominio/esma/saldos.ts` (Σcargos validado no-sinCosto + Σabonos − Σpagos − Σdescuentos, ceronulo, D3 — SQL crudo con `@@map` verificados uno por uno por el reviewer, sin repetir el bug singular→plural de F5-E7); conciliación recibido-vs-cargado; inconsistencias LISTADAS (nunca corregidas): cargos con `IdEsMa=0`, refs de maquilero de empresas viejas sin mapeo (pendiente F10).
>
> **Ciclo coder+reviewer:** 2 coders en paralelo (Calidad ∥ EsMa, piezas sin archivos compartidos salvo el registro `comun/mapeo.ts`, que coexistió sin colisión en el mismo working tree). El reviewer independiente APROBÓ **sin bloqueantes** tras verificar el fix de estampado (sin regresión de costura), el modo-migración sin efectos derivados, la idempotencia, y que el SQL del cuadre calca la fórmula de dominio; 2 notas menores (texto del reporte v2≤v1 + `.prettierignore`) corregidas. CI verde (typecheck/lint/format/test:unit 692/build).
>
> **Cifras esperadas del ETL (± exclusiones listadas):** 40 defectos · 488 auditorías · 15,296 detalles · 7,401 cargos (1,251 estampado recuperados, 1 huérfano) · 554 abonos · 743 descuentos · 5,935 pagos.
>
> **Pendiente (no bloquea):** el ETL se corre a mano post-deploy (`npx tsx --env-file=.env migracion/etl-calidad.ts` y `.../etl-esma.ts`, luego `.../cuadre-f6.ts`); Gabriel valida el cuadre EN VIVO en `prueba` (criterio de salida "EsMa cuadra contra los recibos del periodo"). Refinamiento F10: un maquilero `Proceso=1` puede quedar en v2 solo con rol `maquila-costura` (sin `estampado`) → afecta filtros de UI por tipo, no la validez del cargo. Ubicación final del módulo de Calidad = D8 (sigue pendiente).

**Objetivo:** Migrar los datos históricos reales de los dos módulos, probar el cuadre obligatorio de saldos v1 vs v2 (la herramienta que F10 usará en el paralelo), documentar cómo quedaron los módulos y verificar el criterio de salida de la fase en el ambiente de prueba. Va al final porque necesita todo lo construido en E1–E5 (los ETL cargan VÍA los servicios de dominio).
