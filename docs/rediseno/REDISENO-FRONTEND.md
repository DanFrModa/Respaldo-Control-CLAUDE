...
- **Trazabilidad de punta a punta (visión Daniel, 7-jul) — cadena completa:** `Desarrollo (pre-costeo) → Cotización/Lista → [Pedido del cliente en su formato] → Pedido interno → OP por modelo (color×talla) → Producción (RC) → **Inventario PT** → Factura → Entrega`, todo ligado hacia atrás. Se construye por etapas:
  ...
  - **Etapa 3 (la visión):** **importar el pedido del cliente** — una **plantilla de mapeo por cliente** (se le enseña UNA vez qué columna es modelo/color/talla/cantidad/precio); al subir su archivo genera el pedido interno + las OPs automáticamente. NUEVO (no está en el plan actual); a futuro, IA para detectar formatos nuevos.
