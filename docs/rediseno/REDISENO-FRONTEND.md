...
- **Constructor de pedido interno + generación de OP (Etapa 1, petición Daniel 7-jul):** el alta simple de "Nueva orden" era un atajo del proto; se reemplazó por el **flujo real** (que el backend F2 ya modela: `Pedido`/`PedidoLinea` → `Orden` colgada del renglón con FK `idPedidoLinea`, matriz color×talla D4 + refs cliente D7). **Panel deslizante "Nuevo pedido interno":** encabezado (cliente · empresa · fecha) + N **modelos**, cada uno con su **nº de modelo (nuestro)**, precio, y su **matriz color×talla** (agregar/quitar color; la **cantidad del modelo = suma de la matriz**). Botón **"Crear pedido y generar OPs"**: folio `-F` automático, y **cada modelo genera su propia OP** (con su matriz) + su **Ruta Crítica** (auto). Los botones "Nueva orden" de Órdenes/Resumen ahora abren este constructor — **la OP no se crea suelta, nace del pedido**.
