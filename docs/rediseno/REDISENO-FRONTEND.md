...
- **Dificultad de la prenda (1-6) (petición Daniel, 6-jul):** se define en el **desarrollo del modelo** (campo nuevo en el editor, sección Identificación, escala Sencillo 1/6 … Difícil+ 6/6). La **Ruta Crítica la toma de aquí** para calcular los tiempos de cada proceso — impacta **sobre todo el tiempo de costura (maquila)** (ej. Maquila 6→20 d según dificultad). Ver §4.9. En el real: campo del modelo/precosto que alimenta la duración por dificultad del CPM.
