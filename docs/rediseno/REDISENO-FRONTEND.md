...
- **Editor de modelo** (panel deslizante): números **nuestro + del cliente**, **telas** (un modelo puede llevar **varias** — D5, ej. felpa + acompañante) cada una con su **consumo por prenda**, **corte** y **maquila (costura)** (dos costos fijos por prenda, **separados** — petición Daniel, 6-jul), **procesos** (estampado/bordado/lavado/otros — lista N), **avíos** + consumo, con **costeo en vivo** (tela + avíos + procesos + **corte** + maquila = costo; **precio sugerido** por margen). 
- **Corte como costo fijo (petición Daniel, 6-jul):** el **corte** es un costo por prenda **aparte de la maquila (costura)** — antes faltaba en el precosteo. Va en la sección "Corte, maquila y margen" y aparece como su propio renglón en el "Costo del modelo" (Tela · Avíos · Procesos · **Corte** · Maquila). Ya existe como columna en el módulo de **Costos** del sistema, así que es una línea de costo consistente. También se puede editar en la **negociación** (§4.8). En el real: campo de costo del modelo/precosto (F8), separado de la maquila.
