...
- **Dificultad DERIVADA del # de operaciones (idea Daniel, 6-jul):** la dificultad **NO se teclea a mano** — sería subjetiva. En el desarrollo del modelo se captura el **# de operaciones de costura** de la prenda (un **dato real/objetivo**), y de ahí el sistema **deriva la dificultad (1-6)** por umbrales (configurables; ejemplo en el proto: ≤8→1/6 · ≤14→2/6 · ≤20→3/6 · ≤28→4/6 · ≤38→5/6 · >38→6/6) y con ella el **tiempo de costura**. El editor lo muestra **en vivo**: ej. **34 ops → Difícil 5/6 → costura ≈ 16 d**. La Ruta Crítica toma esto del modelo; impacta **sobre todo la costura** (Maquila 6→20 d). Ver §4.9. En el real: `# operaciones` en el modelo/precosto → tabla de umbrales → duración por dificultad del CPM (o, si se quiere más fino, tiempo de costura calculado directo por operación).
