...
- **Constructor de pedido interno + generación de OP (Etapa 1, petición Daniel 7-jul):** el alta simple de "Nueva orden" era un atajo del proto; se reemplazó por el **flujo real** (que el backend F2 ya modela: `Pedido`/`PedidoLinea` → `Orden` colgada del renglón con FK `idPedidoLinea`, matriz color×talla D4 + refs cliente D7). Dos pasos:
  - **Panel "Nuevo pedido interno":** encabezado (cliente · empresa · fecha) + N **modelos**, cada uno con **nº de modelo (nuestro) · cantidad · precio** (importe en vivo, folio `-F` automático). **El pedido NO lleva color×talla** (aclaración Daniel 7-jul).
  - **"Generar OP" por modelo:** en la lista de Pedidos, cada modelo sin OP trae el botón **"Generar OP"** → panel donde **NACE la matriz color×talla** (distribuir la cantidad del pedido por color y talla, con validación cuadra/faltan/sobran) → crea la OP (nº consecutivo), y **su Ruta Crítica se programa sola**. Es el flujo del CONTROL actual (meto el pedido y de ahí genero la OP de cada modelo).
...
- **Trazabilidad de punta a punta (visión Daniel, 7-jul) — cadena completa:** `Desarrollo (pre-costeo) → Cotización/Lista → [Pedido del cliente en su formato] → Pedido interno → OP por modelo (color×talla) → Producción (RC) → **Inventario PT** → Factura → Entrega`, todo ligado hacia atrás. Se construye por etapas:
  ...
  - **Etapa 2 (pendiente):** ligar cada modelo del pedido a su **desarrollo/pre-costeo** por el **nº de modelo interno** (la liga; el nº nace en Desarrollo, ya con fotos/telas/avíos/BOM cargados — decisión Daniel: en v2 ya no se genera el nº en el pedido ni desarrollo sube cosas después, solo se referencia y hereda). Mostrar la cadena Desarrollo→Lista→Pedido→OP en la ficha.
