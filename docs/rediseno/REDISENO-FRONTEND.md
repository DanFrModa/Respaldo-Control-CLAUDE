...
- **Constructor de pedido interno + generación de OP (Etapa 1, petición Daniel 7-jul):** el alta simple de "Nueva orden" era un atajo del proto; se reemplazó por el **flujo real** (que el backend F2 ya modela: `Pedido`/`PedidoLinea` → `Orden` colgada del renglón con FK `idPedidoLinea`, matriz color×talla D4 + refs cliente D7). Dos pasos:
  - **Panel "Nuevo pedido interno":** encabezado (cliente · empresa · fecha) + N **modelos**, cada uno con **modelo de desarrollo · cantidad · precio** (importe en vivo, folio `-F` automático). El modelo se **elige de un selector de modelos de desarrollo** (ya no texto libre; muestra su nombre + proyecto/cliente). **El pedido NO lleva color×talla** (aclaración Daniel 7-jul). **Desarrollo y Producción son bases distintas** (Etapa 2): aquí el nº de producción **aún no existe**.
  - **"Generar OP" por modelo = salida a producción:** en la lista de Pedidos, cada modelo sin OP trae el botón **"Generar OP"** → panel donde **NACE la matriz color×talla** (distribuir la cantidad del pedido por color y talla, con validación cuadra/faltan/sobran); al confirmar se **mintea el nº interno de producción** y el modelo **se liga a su ficha de desarrollo** (hereda BOM/telas/avíos/dificultad), se crea la OP (nº consecutivo) y **su Ruta Crítica se programa sola**. Trae una **cadena de trazabilidad navegable** (Desarrollo → Lista → Pedido → OP) y botón **"Ver desarrollo"**. Es el flujo del CONTROL actual (meto el pedido y de ahí genero la OP de cada modelo).
  - Los botones "Nueva orden" de Órdenes/Resumen abren este constructor — **la OP no se crea suelta, nace del pedido**.
  - **OC original del cliente como referencia en cada OP (petición Daniel 7-jul):** el pedido captura la **OC del cliente** (su nº de orden de compra) — en el alta manual (campo del encabezado), en el importador (paso 1) y en el pedido demo — y **se guarda en CADA OP interna** que nace de él. Se muestra en el encabezado y en los datos de la orden (drawer), como **chip** en la fila del pedido, y como **nodo inicial de la cadena** de trazabilidad (`OC cliente → Desarrollo → Lista → Pedido → OP`). Queda **amarrada a la orden aunque el pedido interno se reorganice**. Encaja con **D7** (referencias del cliente por orden) del backend F2 — la OC es una de esas referencias; el back debería además poder **guardar el documento/archivo original** como adjunto.
...
- **Ruta hacia atrás por orden (CPM) — se genera SOLA al dar de alta la OP (decisión Daniel, 7-jul):** **NO hay botón manual "Programar RC"** (se quitó) — al **crear la orden de producción** el sistema arma su ruta automáticamente (en el proto, el alta de OP llama a `generarRutaRC()` y el toast confirma "su Ruta Crítica se programó sola"). Cada **proceso** (Compra de tela · Corte · Envío a maquila · Maquila · Recibo · Estampado/Bordado · Auditoría de calidad · Entrega) con su **fecha compromiso** (calculada hacia atrás desde la fecha de entrega) y su **responsable**. Se ve en el **panel deslizante "Ruta de la orden"** (línea de tiempo con semáforo por proceso: Hecho/Vencido/Hoy/Programado; resalta los pasos de la persona con badge "tú"). Se abre al hacer clic en cualquier pendiente.
...
- **Trazabilidad de punta a punta (visión Daniel, 7-jul) — cadena completa:** `Desarrollo (pre-costeo) → Cotización/Lista → [Pedido del cliente en su formato] → Pedido interno → OP por modelo (color×talla) → Producción (RC) → **Inventario PT** → Factura → Entrega`, todo ligado hacia atrás. Se construye por etapas:
  - **Etapa 1 ✅ (7-jul):** constructor de pedido interno → genera OP por modelo con matriz color×talla (§4.1).
  - **Etapa 2 ✅ (7-jul):** liga **Desarrollo ↔ Producción**. **Aclaración clave de Daniel (7-jul):** *Desarrollo y Producción NO son la misma base de datos.* En **Desarrollo** el modelo es un **modelo de desarrollo** (su nº de desarrollo, con fotos/telas/avíos/BOM/dificultad). El **nº de modelo interno de PRODUCCIÓN se asigna hasta que el modelo SALE A PRODUCCIÓN** (al generar su 1ª OP), momento en que **se liga** a su ficha de desarrollo y **hereda** su BOM/telas/avíos/dificultad. Lo construido en el proto:
    - El **pedido interno** referencia el **modelo de desarrollo** (selector de modelos de desarrollo, ya no texto libre; muestra nombre + proyecto/cliente). El nº de producción **NO existe todavía** aquí.
    - **"Generar OP" = salida a producción:** ahí se **mintea el nº interno de producción** (secuencia propia, distinta de la de desarrollo — en el demo arranca en 81391), se guarda la **liga bidireccional** (dev↔producción) y la OP **hereda** el desarrollo. Banner explícito + botón **"Ver desarrollo"**.
    - **Cadena de trazabilidad** visible y navegable (`Desarrollo → Lista de precios → Pedido interno → OP · producción`): nodos clicables que saltan a cada eslabón; en la lista de Pedidos, el código de modelo abre su **ficha de desarrollo**. ⚠️ **Corregido en la fila 0.151:** los renglones sin expediente avisan **en qué catálogo vive el modelo y que ese renglón no está ligado a un expediente de Desarrollo** — NUNCA su edad. La frase que decía aquí *"modelo anterior al módulo de Desarrollo"* se RETIRÓ del código: afirmaba algo que la pantalla no puede comprobar y era **falsa** justo en el caso más común de hoy (los renglones que nacen del importador de OC por PDF se crean sin `idDesarrollo` aunque su modelo sea de DESARROLLO). No reimplantarla.
    - **Sub-decisión abierta (default propuesto):** el **nº interno de producción es a nivel MODELO** (uno por modelo, reusable entre varias OPs del mismo modelo), **distinto del folio de OP** y **distinto del nº de desarrollo**. Se asigna la 1ª vez que el modelo sale a producción. *(Confirmar con Daniel; así quedó modelado en el proto.)*
  - **Etapa 3 ✅ (7-jul) — MAQUETA de flujo (el motor es backend):** **importar el pedido del cliente** con **plantilla de mapeo por cliente**. **Aclaración de reparto (Daniel preguntó "¿es trabajo tuyo o del back?"):** el **diseño/flujo** es del prototipo (esto); el **motor real** (parsear el archivo, guardar la plantilla, amarrar modelo-del-cliente↔desarrollo, la transacción de alta) es **BACKEND** — **función NUEVA que NO está en el PLANMAESTRO** y hay que meterla al plan (probablemente colgada de F8/F10). Lo maquetado (asistente de 3 pasos, botón "Importar de cliente" en Pedidos):
    - **Paso 1 · Origen:** elegir cliente + "cargar" su archivo (Excel/PDF). Muestra si el cliente **ya tiene formato guardado** (se mapea solo) o no (hay que enseñárselo).
    - **Paso 2 · Formato (enseñar UNA vez):** por cada columna del archivo, decir qué es (Modelo del cliente / Color / Talla / Cantidad / Precio / ignorar) → se guarda como **plantilla de ese cliente**; la próxima vez se pre-mapea solo.
    - **Paso 3 · Vista previa:** aplica el formato y **amarra cada modelo del cliente con nuestro desarrollo por su nº de cliente** (`clienteNum`); marca reconocido / **sin reconocer** (con selector para ligarlo a mano). Al confirmar, como el archivo trae **color y talla**, genera de una vez el **pedido interno + las OPs con su matriz** (y su Ruta Crítica) — reusa `salidaAProduccion` (Etapa 2).
    - **Demo:** archivo de C&A (CA-KM-114→#62410, CA-KM-115→#62411, CA-KM-999 sin reconocer). La "IA que detecta formatos nuevos sola" es aún posterior (servicio). **Pendiente:** que el back guarde/versionar las plantillas y el amarre; el prototipo solo valida la experiencia.
