...
- **Datos:** `RUTAS` (orden → `proc[]` con `{proceso, responsable, fecha, estado, díasHolgura}`); `misPendientes(persona)`. Responsables tomados del catálogo de usuarios (Producción/Compras/Calidad/Ventas/Desarrollo). En el real: `ProcesoDef` + `ProcesoDefRol` (roles N:M) + CPM backward-pass + `UsuarioRol` (F5, ya construido); "Mis pendientes" = la **bandeja** filtrada por los roles del usuario logueado (RBAC). El auto-avance (F3→F5) ya marca los procesos al registrar corte/envío/recibo.
