...
- **Catálogos GRANDES por USO** — ⭐ **criterio del DUEÑO: solo lo USADO en la ventana.** Una entidad SIN actividad dentro de la ventana **NO entra aunque tenga existencia o saldo viejo** (el dueño acepta perder ese inventario: *"ya no me sirve"*). Prescan en `comun/prescan-uso.ts`:
  - **Clientes**: referenciados por pedidos/órdenes dentro de la ventana.
  - **Modelos**: con pedidos/órdenes en ventana ∪ movimiento de kardex PT **de fecha ≥ corte** ∪ cíclico en ventana. *(Los prongs de "existencia / neto pre-corte" quedaron RETIRADOS.)*
  - **Telas**: en el BOM de un modelo usado ∪ referenciadas por órdenes en ventana ∪ movimiento (`Entradas`/`Salidas`) **≥ corte** (por cabecera y por renglones). Las OC/notas legacy NO pueden aportar telas: sus renglones son texto libre sin FK a catálogo (verificado en sus loaders).
  - **Avíos y bordados**: en el BOM de un modelo usado — se encogen solos con los modelos.
  - **Proveedores/terceros**: OC en ventana ∪ proveedor (texto) de telas/avíos usados ∪ terceros con **actividad en ventana**: maquileros de órdenes migradas (órdenes/entregas/recibos/auditorías) o de notas en ventana, o con **movimiento EsMa de fecha ≥ corte**; estampadores y cortadores de órdenes migradas. *(RETIRADO el criterio grueso de "cualquiera con cuenta EsMa", que retenía 334/496 por saldo viejo.)*
