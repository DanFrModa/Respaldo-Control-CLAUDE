...
**2. Re-sembrar:** reinicia el backend en Railway con **`SEED_ON_START=true`** (el TRUNCATE también borró catálogos base, permisos, roles y el usuario `admin`; sin el seed no hay login ni menús). ⚠️ El `admin` vuelve al password del seed — **cámbialo** al entrar. Nota: las fotos ya subidas a **R2 quedan huérfanas** (el registro en BD se borró, el objeto físico sigue en el bucket; limitación conocida — deuda aparcada en `HOJA-DE-RUTA.md` §4). Otra nota: la limpieza solo toca el esquema `public` — el esquema **`pgboss` NO se trunca**, así que pueden quedar **jobs de RC encolados** apuntando a filas ya borradas; los handlers son resilientes y los absorben (fallan/expiran sin tumbar nada), pero conviene saberlo — el reinicio del backend de este mismo paso los deja drenarse en limpio.
