...
- **Catálogos GRANDES por USO** (pedido del dueño: "hay muchísimos modelos y ya no me sirven"; prescan en `comun/prescan-uso.ts`):
  - **Clientes**: referenciados por pedidos/órdenes dentro de la ventana.
  - **Modelos**: con pedidos/órdenes en ventana ∪ movimiento de kardex PT ≥ corte ∪ **con existencia** (neto pre-corte ≠ 0 o snapshot `IPT_Mod_Alm` ≠ 0 — su saldo inicial los necesita) ∪ cíclico en ventana. Los que migran **solo por existencia** (sin actividad en la ventana) salen en una sección propia del reporte: _"candidatos a depurar"_, para que Daniel decida.
  - **Telas**: en el BOM de un modelo usado ∪ referenciadas por órdenes en ventana ∪ movimiento (`Entradas`/`Salidas`) ≥ corte ∪ existencia en `TelasColAlm`.
  - **Avíos y bordados**: en el BOM de un modelo usado. (Las OC/notas legacy NO referencian telas/avíos por id — sus renglones son texto libre.)
  - **Proveedores/terceros**: OC en ventana ∪ proveedor (texto) de telas/avíos usados ∪ maquileros con actividad de órdenes migradas (órdenes/entregas/recibos/notas/auditorías) **o con cuenta EsMa** (criterio grueso: su saldo inicial de EsMa los necesita) ∪ estampadores/cortadores de órdenes migradas.
  - El BOM y las **fotos** siguen a su modelo/bordado (cascada); el grid `TelasColores` sigue a su tela.
- **Migra COMPLETO (sin filtro):** **colores** (chicos, y referenciados por texto libre desde muchas fuentes — filtrarlos arriesga romper un saldo inicial; decisión declarada), empresas, almacenes, géneros, temporadas, etiquetas de marca, tela-categorías, tallas/curvas, y la configuración de Ruta Crítica (procesos, plantillas, roles, calendario).
