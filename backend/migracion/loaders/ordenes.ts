 ...
 * IdOrdenes→Orden.id.
 *
 * VENTANA temporal (recarga limitada, p. ej. `ETL_DESDE=2025-01-01`): la orden entra si su
 * `Fecha` propia cae dentro (fecha nula = dentro). CASCADA: si su `IdPedidosDet` cuelga de un
 * pedido que quedó fuera de la ventana, la orden se excluye también (`padreFueraVentana`,
 * bucket propio — NUNCA migra un hijo cuyo padre quedó fuera). La pre-pasada de colores/tallas
 * SOLO considera las órdenes que sí migran (no ensucia el catálogo con colores/tallas de
 * órdenes excluidas). Todo lo excluido se CUENTA (§7). Ventana inactiva → migra todo, como hoy.
 */
import {
...
export interface ResultadoOrdenes {
  ...
  /** # de órdenes FUERA de la ventana por su `Fecha` propia; 0 con ventana inactiva. */
  fueraVentana: number;
  /** # de órdenes excluidas por CASCADA (su pedido quedó fuera de la ventana). */
...
export async function cargarOrdenes(
  ...
  // Prescan de la ventana (cascada pedido→orden): lo pasa el orquestador; suelto se calcula aquí.
  ...
  // Con la VENTANA activa, la pre-pasada solo mira las órdenes que SÍ migran (no crea colores/
  // tallas al vuelo para órdenes excluidas); el bucle principal sí recorre todas (para contarlas).
  const filasOrden = leerCsv('Ordenes.csv');
  const filasEnVentana =
    pre === null
      ? filasOrden
      : filasOrden.filter((f) => !pre.ordenesFuera.has((f.IdOrdenes ?? '').trim()));
  const detEnVentana =
    pre === null
      ? detPorOrden
      : new Map([...detPorOrden].filter(([idOrd]) => !pre.ordenesFuera.has(idOrd)));
  await precrearColores(sesion, bd, cliente, reporte, resultado, {
    detPorOrden: detEnVentana,
    idPorColorNorm,
    mapaColorF1,
  });
  await precrearTallas(sesion, bd, cliente, reporte, resultado, {
    filasOrden: filasEnVentana,
    idPorTallaNorm,
  });
  ...
  for (const res of contribs) {
    ...
    else if (c.orden === 'fueraVentana') resultado.fueraVentana += 1;
    else if (c.orden === 'padreFueraVentana') resultado.padreFueraVentana += 1;
  ...
  if (resultado.fueraVentana > 0 || resultado.padreFueraVentana > 0) {
    reporte.nota(
      `${describirVentana(ventana)} Ordenes fuera de ventana: ${String(resultado.fueraVentana)} por fecha propia + ` +
        `${String(resultado.padreFueraVentana)} por cascada (pedido fuera) — NO migradas (su matriz/Monarch/comentarios tampoco).`,
    );
  }
...
/** Ventana + prescan de cascada que aplica cada orden. */
...
async function procesarOrden(
  ...
  const fechaOrden = parsearFechaSoloDia(f.Fecha);

  // Ventana ANTES de los mapeos: exclusión a propósito, con su propio bucket. Se clasifica con
  // el MISMO conjunto del prescan (`ordenesFuera` = fecha propia fuera O cascada del pedido)
  // para que el bucle y la pre-pasada de colores/tallas nunca diverjan.
  if (ventanaOrden.prescan !== null && ventanaOrden.prescan.ordenesFuera.has(idViejo)) {
    return sinContrib(
      dentroVentana(fechaOrden, ventanaOrden.ventana) ? 'padreFueraVentana' : 'fueraVentana',
    );
  }
