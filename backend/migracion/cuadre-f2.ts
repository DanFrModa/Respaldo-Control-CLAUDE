...
export async function calcularCuadreF2(cliente: PrismaClient): Promise<CuadreF2> {
  ...
  const filas: RenglonCuadreF2[] = [
    { entidad: 'Pedidos', v1: v1Pedidos, v2: v2Pedidos, nota: '≈1:1 (omite sin folio/cliente/empresa mapeable).' },
    { entidad: 'PedidoLinea (renglones)', v1: v1PedidosDet, v2: v2PedidoLinea, nota: 'v2 ≤ v1 por renglones con modelo sin mapeo (al reporte).' },
    { entidad: 'PedidosReales', v1: v1Reales, v2: v2Reales, nota: '≈1:1 (omite sin pedido mapeable).' },
    { entidad: 'PedidoRealLinea (renglones)', v1: v1RealesDet, v2: v2RealLinea, nota: 'v2 ≤ v1 por renglones con IdPedidosDet sin mapeo.' },
    { entidad: 'Ordenes', v1: v1Ordenes, v2: v2Ordenes, nota: '≈1:1 (omite sin folio/empresa/modelo/cliente mapeable).' },
    { entidad: 'OrdenesDet (renglones color)', v1: v1OrdenesDet, v2: v2OrdenLinea, nota: 'v2 = un OrdenLinea por color (un OrdenesDet con color vacío no crea renglón).' },
    { entidad: 'OrdenLineaTalla (despivotadas)', v1: 0, v2: v2OrdenLineaTalla, nota: 'v1 N/A: despivote de T1..T8>0 contra la cadena Tallas.' },
    { entidad: 'OrdenReferencia (Monarch real)', v1: 0, v2: v2Referencias, nota: 'v1 N/A: solo Monarch ≠ código de modelo (default descartado).' },
    { entidad: 'ComentaOrd', v1: v1Comenta, v2: v2Comenta, nota: '≈1:1 (omite sin orden mapeable o texto vacío).' },
    { entidad: 'SUMA cantidades matriz', v1: v1SumaMatriz, v2: v2SumaMatriz, nota: 'Σ T1..T8 (v1) vs Σ OrdenLineaTalla.cantidad (v2): deben CUADRAR (toda cantidad>0 migra).' },
  ];

  // ── Nivel 2: columnas (v1 → v2) ──
  const colPedidos = contarColumnasCsv('Pedidos.csv', [
    'IdPedidos', 'IdClientes', 'NumeroPed', 'FechaPedido', 'FechaDe', 'FechaHasta',
    'EntregadoTienda', 'FechaTela', 'IdOrdCompra', 'PedCancelado', 'NoProducir',
    'FechaElaboracion', 'IdEmpresas',
  ]);
  const colPedidosDet = contarColumnasCsv('PedidosDet.csv', [
    'IdPedidosDet', 'IdPedidos', 'IdModelos', 'CantPed', 'Precio', 'EntregadoParcial', 'CantFalt',
  ]);
  const colReales = contarColumnasCsv('PedidosReales.csv', [
    'IdPedidosReales', 'IdPedidos', 'FechaPedPR', 'FechaInicioPR', 'FechaFinPR', 'NumPedReal',
    'Cedis', 'FechaEntregadaReal', 'IdUsuarios', 'FechaUsuario', 'Apertura',
  ]);
  const colRealesDet = contarColumnasCsv('PedidosRealesDet.csv', [
    'IdPedidosRealesDet', 'IdPedidosReales', 'IdPedidosDet', 'CantidadPR', 'CantidadEnviada',
    'CantidadEntregadaReal', 'Empaques',
  ]);
  const colOrdenes = contarColumnasCsv('Ordenes.csv', [
    'IdOrdenes', 'Numero', 'IdPedidosDet', 'IdModelos', 'IdMaquileros', 'IdEtiquetasM',
    'IdClientes', 'IdTelasDis', 'Fecha', 'FechaEntrega', 'Observaciones', 'Tallas', 'MaquilaOrd',
    'NoCost', 'Monarch', 'OrdCancelada', 'MotivoCancelada', 'IdEmpresas', 'UPC', 'IdCP_Articulos',
    'IdRC_Aplicaciones', 'IdRC_TipoTelas', 'FechaInicioRC', 'FechaEntregaRC', 'FechaProg',
    'EnRiesgo', 'SI_RC', 'FechaDet', 'Composicion', 'CompForzada', 'Pagada', 'ObsMaquila',
    'AplicacionOrd', 'RC_Viva',
  ]);
  const colOrdenesDet = contarColumnasCsv('OrdenesDet.csv', [
    'IdOrdenesDet', 'IdOrdenes', 'Color', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8',
  ]);
  const colComenta = contarColumnasCsv('ComentaOrd.csv', [
    'IdComentaOrd', 'IdOrdenes', 'IdUsuarios', 'FechaComen', 'Comentario',
  ]);
  ...
  const columnas: RenglonColumna[] = [
    ...
    C('PedidosReales', 'FechaUsuario', '(auditoría v1; creadoEn=now ETL)', colReales, 'fecha de captura v1, informativa'),
    C('PedidosReales', 'Apertura', 'apertura', colReales),
    // PedidosRealesDet (7)
    C('PedidosRealesDet', 'IdPedidosRealesDet', 'mapeo→PedidoRealLinea.id', colRealesDet, 'PK (mapeo)'),
    ...
    C('Ordenes', 'UPC', 'EXCLUIDA POR DECISIÓN', colOrdenes, 'Gabriel 16-jun-2026: códigos de barra en retiro; la columna destino Orden.upc fue eliminada del modelo'),
    ...
    // OrdenesDet (11)
    C('OrdenesDet', 'IdOrdenesDet', '(no se mapea; OrdenLinea.id propio)', colOrdenesDet, 'la matriz se reagrupa por color'),
