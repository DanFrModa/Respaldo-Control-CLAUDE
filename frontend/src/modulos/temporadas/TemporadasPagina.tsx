...
import { type AccionesTemporada, columnasTemporadas } from './columnas';
import { DialogoTemporada } from './DialogoTemporada';

/** Columnas por las que el backend sabe ordenar. */
type ColumnaOrden = NonNullable<TemporadasQuery['ordenarPor']>;
...
/**
 * Pantalla de Temporadas — CRUD del catalogo (replica del patron de Almacenes).
 * Lista con busqueda (debounce), orden y paginacion de servidor; alta/edicion en
 * dialogo; borrado suave reversible; toasts; estados de carga/vacio/error;
 * consciente de permisos. `temporadas.ver` gobierna el acceso;
 * `temporadas.administrar` decide las acciones de escritura. El backend decide (A1).
 */
export function TemporadasPagina(): React.JSX.Element {
  ...
  const tabla = useReactTable<Temporada>({
    data: datos,
    columns: columnasTemporadas,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Catálogo de temporadas (ciclos comerciales del año).
          </p>
        </div>
        {puedeAdministrar ? (
          <Button onClick={abrirAlta} data-testid="nueva-temporada">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 sm:max-w-xs">
          ...
          <Input
            ...
            aria-label="Buscar temporadas por nombre"
            data-testid="buscar-temporada"
          />
        </div>
        <Button
          type="button"
          ...
          aria-label="Mostrar también las temporadas desactivadas"
      ...
      {datosPagina && datosPagina.total > 0 ? (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm">
          <p className="text-muted-foreground" data-testid="resumen-paginacion">
            {datosPagina.total} temporada{datosPagina.total === 1 ? '' : 's'} · página{' '}
...
function BotonOrden({
  ...
  cabecera: ReturnType<
    ReturnType<typeof useReactTable<Temporada>>['getHeaderGroups']
