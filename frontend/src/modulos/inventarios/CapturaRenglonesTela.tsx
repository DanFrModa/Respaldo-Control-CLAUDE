...
export function CapturaRenglonesTela({
  ...
  function agregar(): void {
    if (tela === undefined || idLote === '' || cantidad === '') return;
    const idLoteNum = Number(idLote);
    const cantidadNum = Number(cantidad);
    if (!Number.isFinite(cantidadNum) || cantidadNum <= 0) return;
    const fila = lotesDeTela.find((f) => f.idLote === idLoteNum);
    ...
    <div className="space-y-4" data-testid="captura-renglones-tela">
      <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
        <div className="space-y-3 rounded-md border p-3">
          ...
          <SelectorTela idSeleccionado={tela?.id} alSeleccionar={setTela} testid="captura-tela" />
          {tela !== undefined ? (
          ...
          <Button
            ...
            variant="secondary"
            onClick={agregar}
            disabled={soloLectura || tela === undefined || idLote === '' || cantidad === ''}
