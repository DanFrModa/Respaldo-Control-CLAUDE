...
/**
 * E2E del CRUD de Colores contra el stack real (patron de Almacenes + ALTA RAPIDA
 * ENCADENADA). Cubre crear VARIOS colores seguidos sin cerrar el dialogo, luego el
 * ciclo completo sobre uno de ellos: editar -> desactivar (con confirmacion) ->
 * mostrar desactivados -> reactivar -> buscar. Usa un sufijo unico por corrida.
 */
test.describe('CRUD de Colores', () => {
  ...
  test('crear, editar, desactivar, reactivar y buscar un color', async ({ page }) => {
    ...
    // ── Editar (en edicion el dialogo SI se cierra al guardar) ──────────────────
    await filaNueva.getByTestId('acciones-color').click();
    ...
    await filaEditada.getByTestId('acciones-color').click();
    ...
    // ── Mostrar desactivados → reaparece como Inactivo ─────────────────────────
    ...
    // ── Reactivar ──────────────────────────────────────────────────────────────
    await filaInactiva.getByTestId('acciones-color').click();
    await page.getByTestId('activar-color').click();
    await expect(page.getByText(`Color "${nombreEditado}" activado.`)).toBeVisible();
    const filaReactivada = page.getByTestId('fila-color').filter({ hasText: nombreEditado });
