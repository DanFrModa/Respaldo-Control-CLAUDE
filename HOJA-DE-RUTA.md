...
> | **0.110** ⬜ | 🔴 **LA FACTURACIÓN NO PUEDE QUEDAR SIN DEFINIR** (Daniel 3-sep: *«es un campo **obligatorio** de llenar. A fuerzas hay que definir si es con, sin o ambas»*) | Daniel 3-sep ✅ **MEDIDO — sus tres valores YA EXISTEN con esos nombres**: `Proveedor.modalidadFacturacion` = `solo_con` · `solo_sin` · `ambos`. 🔴 **Pero es NULLABLE**, y `conFactura` en `AbonoMaquilero`/`DescuentoMaquilero`/`PagoMaquilero` **también** ⇒ un movimiento se puede guardar **sin decir** si lleva factura, justo donde él dijo *«no deben mezclarse»*. El *«tercero»* que no entendía es el **filtro de la vista**, legítimo **sólo si el dato de abajo siempre está clasificado**. ⇒ obligatorio en el proveedor **y** por movimiento; **derivado** cuando el proveedor es de un solo tipo. ⚠️ **REGLA 0-B**: obligatorio para lo NUEVO; lo migrado en null **se tolera** |
...
> **📊 Dónde va el programa — al 3-sep-2026, versión en `prueba` `0.095`.** De las **65 filas** numeradas: **34 ✅ cerradas** · **1 🔶 a medias** (0.082) · **1 ⏸️ aparcada** (0.087) · **29 ⬜ por hacer** (0.061, 0.072, 0.086, 0.096, 0.097, 0.099–0.123 salvo la 0.098 y la 0.105). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
