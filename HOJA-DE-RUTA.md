...
> | **0.105** ✅ *(entregada como **0.094**)* | 🔴 **LOS PERFILES DEJAN DE HEREDAR POR CASCADA** (Daniel 3-sep: *«los permisos por cascada no son funcionales. Así lo hice en la primera versión de Access y luego lo modifiqué por permisos concretos… puede haber alguien que tenga el permiso A pero no el B, y otra persona el B pero no el A»*) | Daniel 3-sep ✅ **MEDIDO:** el **motor NO es cascada** —`RolPermiso` N:M explícito, `UsuarioRol` N:M con UNIÓN, guardia = `permisos.has(clave)`, **cero** comparaciones de nivel— ⇒ **su escenario A-sin-B ya funciona hoy y no se toca el motor**. 🔴 **La cascada vive SÓLO en el seed** (`prisma/seed.ts:111`, `sin(base,…)`): corriendo `definirRoles()`, **76 de 122 permisos no se le restan a NADIE** ⇒ los tiene todo perfil salvo `Basico`. **Secretarial puede HOY**: `compras.autorizar`, `esma.cargo-validar`, `pedidos.modificar`, `notas.cancelar`, `inventario-pt.mover`. **De los 76, 22 pueden hacer daño; 54 son de sólo ver.** 🔑 **El mecanismo es un guardián al revés: el silencio OTORGA.** ⇒ voltear el seed de **RESTA a SUMA** + prueba que truene si un permiso queda **sin atribuir**. 📌 Daniel (3-sep): **los perfiles concretos se arman AL FINAL**, con los puestos reales; esta fila construye **el mecanismo**, no el contenido. ⭐ **Y su petición de «un permiso suelto además de su perfil» YA SE PUEDE**: un usuario lleva **varios perfiles** y sus permisos son la **unión**. **Habilita 0.104 y 0.109**  ✅ **CERRADA (3-sep, v0.094).** El seed pasó de **RESTA a SUMA**: los 9 perfiles declaran **lo que tienen**, no lo que les falta ⇒ **un permiso nuevo nace en NADIE** hasta que se le nombre dueño **con su razón escrita** (`SOLO_ADMINISTRADOR` y `NUEVOS_DESDE_LA_FOTO`, ambos con `razon` obligatoria **por el tipo**). **El reparto NO cambia**: los 9 conjuntos quedan idénticos —`122·122·100·94·85·76·76·76·0`, verificado por dos fuentes independientes— porque esto es un cambio de **FORMA**. ⭐ **La prueba de atribución muere en los DOS sentidos** (clave nueva sin dueño · clave fantasma) y **`Basico` en cero es incondicional**, sin depender de ninguna foto. ⭐⭐ **Y el coder desmintió el encargo dos veces, con razón**: la prueba de huérfanas que pedí **no podía fallar nunca** (Administrador lleva las 122 ⇒ el conjunto siempre era vacío), y **`SEED_ON_START` NO hace falta** (si el reparto no cambia, re-sembrar no escribe). 🔴 **CUATRO rondas de reviewer, y el hallazgo mayor no fue el código sino la PROSA**: 13 comentarios describían la cascada muerta y **TRES eran falsos de dato** — uno en el catálogo de permisos llevaba **ocho días** mintiendo (§Post-F9.123, fechado en `862cdb41`). *La misma enfermedad de la fila, en la documentación.* 📌 Deja viva la **0.120**. **Sin migración, sin permisos nuevos, sin `SEED_ON_START`** |
