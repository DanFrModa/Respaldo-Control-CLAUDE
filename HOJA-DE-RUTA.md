...
> | **0.150** ⬜ | ⭐ **LA GUARDA: hoy la cascada cancela OP YA PRODUCIDAS** (Daniel 7-sep) | §Post-F9.211 + §Post-F9.212 🔴 **ES UN DEFECTO, no una mejora.** `cancelarPedido` (`dominio/pedidos/pedidos.ts:759`) tiene **sólo dos** guardas —ya cancelada (`:782`) y orden **cerrada** (`:825-831`)— así que **marcando la casilla de cascada se cancela una OP AUNQUE YA ESTÉ PRODUCIDA**, justo lo que Daniel dijo que no quiere: *«No quiero que se borren las OP en ese caso»*. Y `cancelarOrden` (`produccion/ordenes.ts:1224`) tiene **las mismas dos y ninguna más**: el criterio **no existe en NINGÚN camino de cancelación**. **Desenlace (b), elegido:** se cancela el pedido y las OP **sin** actividad, y **las que sí tienen se quedan VIVAS y NOMBRADAS** (el estilo ya está en `pedidos.ts:803`, que nombra por folio y ofrece las salidas). ⚠️ **`tieneActividadProduccion` (`requisitos-orden.ts:194`) NO alcanza sola**: sólo mira `EtapaMovimiento` — no ve compras, kardex de tela, notas de salida, cierres de maquila ni EsMa. **16 señales medidas**, y **3 son gratis** (ya vienen en la fila que `pedidos.ts:794` lee: `cerradaEn`, `recetaLiberadaEn`, `recetaAbiertaEn`). 🔴 **DOS TRAMPAS MEDIDAS:** al kardex se llega por **tres caminos** (PT por `MovimientoDetPt.idOrden`; **tela** por `origenTipo='salida-tela-orden'`+`origenId` de TEXTO; **avíos** por `NotaSalidaLinea.idOrden`) — mirar sólo el obvio deja hueco; y `cancelarMovimientoPt` **copia `idOrden` al movimiento inverso** (`comun/kardex.ts:535`) ⇒ contar **nunca da cero** y la guarda bloquearía para siempre una orden cuya actividad se deshizo entera: hay que descartar por `Movimiento.idMovimientoInverso`. 📌 **«Comprado» = `ESTATUS_OC_COMPROMETIDA`** (excluye borrador), no `ESTATUS_OC_QUE_CUBREN` — *un borrador no compromete a nadie* (`comprometido-en-oc.ts:71-78`); default del lead, **pendiente de que Daniel lo confirme**. 📌 **DEUDA A PAGAR AQUÍ:** «¿tiene actividad?» está escrita **a mano 3 veces** (`requisitos-orden.ts:194` sin exportar · `ordenes.ts:572` inline idéntica · `etapas.ts:1214/:1240`); escribirla a pelo sería **la cuarta**. ⚠️ **A2:** reusar `bloquearEtapasDeOrden` (`produccion/recibos.ts:391`) o un corte capturado entre el conteo y el `update` se pierde. 🔴 **HALLAZGO LATERAL, defecto vivo:** un pedido con una OP **CERRADA** **no se puede cancelar por ninguna vía** (los dos `throw` se tapan, `:803` y `:828`) y el mensaje de `:828` ofrece *«o cancela el pedido sin arrastrar las OPs»*, **puerta que NO existe**. **V1** |
...
> esa misma noche**).** De las **28 ⬜ pendientes**:
> **12 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.121, 0.123, 0.131, 0.151, 0.153, **0.156**, **0.158**, **0.159**)
> — de ésas **dos son repasos de Daniel** y **una la ejecuta Gabriel**, así que **son 9 de construcción** ·
...
> **📊 Dónde va el programa — al 7-sep-2026, versión en `prueba` `0.123`.** De las **104 filas** numeradas: **61 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **30 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.121, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.150, 0.151, 0.152, 0.153, 0.154, 0.155, 0.156, 0.157, 0.158, 0.159, 0.160, 0.161, 0.162). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
