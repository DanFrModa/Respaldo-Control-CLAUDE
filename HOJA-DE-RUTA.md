...
> | **0.091** ⬜ | **EL HUÉRFANO INVISIBLE: borrar un PADRE deja su archivo en la nube.** Nace de la 0.081(a); **LATENTE, no vivo** — medido y confirmado por el reviewer. Los **siete** puentes a `Archivo` (`ProveedorArchivo`, `ModeloFoto`, `ModeloArteFoto`, `PedidoArchivo`, `OrdenArchivo`, `DesarrolloArchivo`, `EntradaTelaArchivo`) llevan `onDelete: Cascade` **en los dos lados** ⇒ borrar el **padre** se lleva el puente y **deja viva la fila `Archivo` y su objeto en R2**, y el embudo de la 0.081(a) **no lo atraparía** porque cuelga de `archivo.delete`, no de la cascada. ✅ **Hoy NO ocurre**: verificado que ningún código de producción hace hard-delete de `modelo`/`orden`/`pedido`/`desarrollo`/`proveedor`/`entradaTela`. ⚠️ **Se vuelve real el día que alguien construya un «eliminar modelo/pedido/orden»** ⇒ **quien lo construya tiene que leer esta fila primero**. *(Los `scripts/demo-*.ts` sí borran en duro, pero son generadores de datos de demo sobre `prueba`, cubiertos por REGLA 0-B.)* | — |
> | **0.092** ⬜ | 🧹 **LA VENDA DE LA MEMORIA — separada de la 0.081 porque MEDIRLA la desmintió.** La 0.081(b) decía que el OOM del CI es *«señal de que algo crece sin control»*. 🔴 **Medido el 1-sep: en CI los 6 GB SÍ alcanzan.** `tsc --extendedDiagnostics` sobre `prueba`: **5,058 MiB** contra el techo de **6,144** ⇒ **~18 % de margen**. Y el dato que desarma el relato: desde agosto los tipos pasaron de 3.5 M a **4.2 M** y las instanciaciones de 19.7 M a **23.3 M** (+20 % de trabajo) **con la memoria casi igual**. ⚠️ **Lo que SÍ muere por OOM es el SANDBOX local** (otro techo): pasó **cuatro veces en una noche**, a dos coders y a dos reviewers, y un `Killed` **se puede leer como pase** — ése es el daño real, y es de **PROCESO**, no de CI. 📌 La cura ya está escrita en `ci.yml:118-140` desde agosto: *«cuando 6 GB no alcancen hay que ATACAR LA CAUSA (proyectos de TS separados / `--build` incremental / adelgazar los tipos generados), **no volver a subir el número**»*. **No es urgente**; se numera para que el día que el margen se acabe nadie ponga otra venda | — |
...
>
>
> **📊 Dónde va el programa — al 2-sep-2026, versión en `prueba` `0.088`.** De las **37 filas** numeradas: **24 ✅ cerradas** · **4 🔶 a medias** (0.068, 0.079, 0.082, 0.084) · **9 ⬜ por hacer** (0.061, 0.072, 0.086, 0.087, 0.088, 0.091, 0.092, 0.094, 0.095). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
