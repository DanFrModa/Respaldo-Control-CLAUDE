...
> | **0.159** ✅ | 🔴 **BLOQUEA V1** (Daniel 7-sep) · ⭐⭐ **LOS COLORES DUPLICADOS: se pueden fusionar aunque ya estén en órdenes, y el MRP respeta la fusión** | §Post-F9.222 🔑 **Nace de la pantalla de Daniel:** sus dos renglones de etiqueta decían «Blanco Hueso Pantone 14-0002 Tcx Pumice Stone» y «Blanco Hueso» — `Color.nombre` es `@unique` ⇒ **necesariamente dos filas distintas para el mismo color real**, y las dos metidas en las OP 5565/5566/5567. ✅ **CERRADA — v0.130.** **(1) LA FUSIÓN DEJÓ DE BLOQUEAR Y PASÓ A CLASIFICAR.** La negativa de §Post-F9.129 protegía algo real —mover `OrdenLinea` reescribiría lo que el cliente pidió (D7), y una misma orden puede tener los DOS duplicados, lo que chocaría con `@@unique([idOrden, idColor, pack])`— así que **no se levantó la guarda: se cambió la pregunta**. Las **catorce** relaciones entrantes de `Color` quedan repartidas en dos tratos: se **REPUNTAN** al canónico las cuatro de catálogo/amarre derivado (`TelaColor`, `TelaProveedorColor`, **`Modelo.idColor`** y `OrdenTelaColor`) y se quedan **CON RASTRO**, sin tocar una fila, las diez de documentos y movimientos asentados (matriz, corte, kardex PT, faltantes saldados, líneas de OC, snapshot, lotes, cíclico). En colisión de llave única **gana el destino** y lo descartado va a la **bitácora**. La lista se deriva de `prisma/schema.prisma` en una prueba (se enumeró mal tres veces; ahora un olvido es rojo de CI). **(2) SU PAREJA OBLIGADA, sin la cual esto sería un DAÑO:** `sincronizarMatriz` **acepta un color apagado que la orden YA TIENE** — si no, fusionar dejaba **ineditables** las órdenes que lo usaban, que es exactamente lo que §Post-F9.129 evitaba. Capturar uno NUEVO apagado se sigue rechazando, y si lo apagó una fusión el mensaje **dice a cuál ir**. **(3) TODO LO QUE COMPARA COLORES ENTRE DOCUMENTOS RESUELVE POR EL CANÓNICO** (`colores-canonicos.ts`, resolución **por lotes**: una consulta por nivel de cadena, nunca una por renglón): la explosión (`mrp.ts::cargarOrden` ⇒ los renglones duplicados **colapsan solos** y la orden pide **un** renglón con la suma), el **neteo contra las OC** (`comprometidoEnOc`), lo **dado por cubierto**, el snapshot previo, el plan de compra, el tablero R7, la pantalla «de qué color se compra la tela» y la **llave del linaje de modelos**. ⭐ **El hallazgo que la fila no traía, medido por el lead:** el paso 1 de `casar-color-de-tela.ts` compara `TelaColor.idColor` **en crudo**, así que con la fusión repuntando el catálogo habría quedado comparando el id viejo contra el nuevo y **no casaría nunca** — resuelto en el mismo diff. 🔴 **Y el defecto que HABRÍA COSTADO DINERO:** sin resolver el neteo, una OC escrita ANTES de la fusión no casaría con el requerimiento canónico y la explosión **volvería a pedir lo ya comprado** (§Post-F9.85 resucitado por una limpieza de catálogo); hay prueba de integración que lo fija (300 requeridos − 120 en OC = 180 pendientes). **Dónde NO se resuelve, dicho:** dentro de UNA orden todo cuelga de su propia matriz (corte, recibo, entrega, kardex, auto-avance RC) y ya es coherente consigo mismo; y los **impresos** dicen el color con el que se hizo el papel (D3). 📌 **Consecuencia declarada:** las **existencias de PT** siguen partidas por el color con el que entraron —son movimientos asentados— y se agotan solas (REGLA 0-B). ⚠️ **SIN migración** (el rastro `Color.idFusionadoEn` ya existía desde V1-E8s) — comprobado con `prisma migrate diff`: *«No difference detected»* — **sin permisos y sin seed**. **Medición:** 3559 unit + 5 archivos de integración medidos contra Postgres local (fusión 29 · MRP 4 · color-de-tela 37 · órdenes 3 · linaje 1) · **5 mutaciones, 5 muertas**. **V1** |
...
> **8 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.121, 0.123, 0.131, **0.156**) — la **0.159 se CERRÓ el
> 8-sep** (v0.130)
> — de ésas **dos son repasos de Daniel** y **una la ejecuta Gabriel**, así que **son 5 de construcción**
> (0.117, 0.120, 0.121, 0.131, 0.156), y la **0.131 no puede correr** hasta que Daniel entregue su
> corte de SINUBE ·
...
> ⚠️ **Tres avisos que no se pierden:** **(1)** la **0.159 empeoraba sola** —cada día más órdenes usaban
> los colores duplicados y más difícil era limpiarlo—, y por eso Daniel la subió a V1: **cerrada el 8-sep**;
> **(2)** la **0.161** vuelve
> a V1 **en cuanto a Daniel le salga un modelo que la necesite** (él avisa); **(3)** 🔑 **lo que es barato
> HOY no lo será después**: `0.155`, `0.156` y `0.161` cambian el modelo de datos, y mientras la
...
> **📊 Dónde va el programa — al 8-sep-2026, versión en `prueba` `0.130`.** De las **104 filas** numeradas: **66 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **25 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.121, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.152, 0.154, 0.155, 0.156, 0.157, 0.160, 0.161, 0.162). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
