...
> | **0.150** ⬜ | ⭐⭐ **BORRAR DE VERDAD LOS ERRORES DEL DÍA (folios incluidos) · y LA GUARDA que hoy cancela OP ya producidas** (Daniel 7-sep) | §Post-F9.211 🔑 **La frontera la puso Daniel y es limpia: TRABAJO HECHO vs. ERROR DE CAPTURA.** *«Para las ordenes que acabo de hacer y me equivoque, me gustaria poder **borrar por completo. Absolutamente todo. No quisiera ni perder folios de OP… me gusta tener todos los folios ocupados**… Hay errores que me gustaria **solo yo** poder eliminar… **Otro tema es cuando ya hay trabajo hecho…. ahi si estoy de acuerdo en dejar rastro.»* **D3 NO se relaja**: sigue gobernando todo lo que tiene trabajo detrás; lo que se reconoce es que un fantasma de hace diez minutos **no tiene historia que preservar**. 🔴 **DOS PIEZAS, y la primera es un DEFECTO, no una mejora.** **(A) LA GUARDA:** hoy `cancelarPedido` (`dominio/pedidos/pedidos.ts:759`) tiene **sólo dos** guardas —ya cancelada y orden **cerrada**— así que **marcando la cascada se cancela una OP AUNQUE YA ESTÉ PRODUCIDA**, justo lo que Daniel no quiere. Y `cancelarOrden` (`produccion/ordenes.ts:1224`) tiene **las mismas dos y ninguna más**: el criterio **no existe en ningún camino**. Desenlace elegido: **(b)** se cancela el pedido y las OP SIN actividad, y **las que sí tienen se quedan vivas y NOMBRADAS** (el estilo ya está en `pedidos.ts:803`). **(B) EL DESCARTE:** borrado FÍSICO sólo si se cumple TODO «sin vida», con **permiso propio de Daniel**, que **devuelve los folios** (`comun/secuencias.ts:62-67` es `valor+1` sobre una fila ⇒ `UPDATE … SET valor = valor-1 WHERE … AND valor = <el descartado>`: **atómico, y si alguien tomó folio después no casa y el hueco queda** — la promesa es «sin huecos **cuando lo deshaces en el momento**», dicho así a Daniel) y **libera el número de 5 dígitos** (ahí no hay contador: `consecutivosUsados` lo calcula leyendo los modelos). ⚠️ **DOS COSAS NO OBVIAS que el borrado debe deshacer:** la **liga aprendida** (`aprenderLiga`, `importacion-pdf.ts:469`, hace upsert en `ClienteModeloLiga` ⇒ si importó con el modelo EQUIVOCADO y borra, **la liga mala sobrevive y se la vuelve a sugerir** — su punto 17 por la puerta de atrás; **medir si el upsert creó o actualizó**) y el **PDF en R2** (deuda conocida del §4: `comun/archivos.ts` no tiene `DeleteObject`; éste es el sitio donde por fin importa). 📌 **DEUDA A PAGAR EN EL MISMO MOVIMIENTO:** «¿esta orden tiene actividad?» está escrita **a mano tres veces** (`requisitos-orden.ts:194` —`tieneActividadProduccion`, **sin exportar**—, `ordenes.ts:572` inline idéntica, y `etapas.ts:1214/:1240`); escribir la guarda a pelo sería **la cuarta**. Y `tieneActividadProduccion` **sola no alcanza**: sólo mira `EtapaMovimiento`, no ve compras, kardex de tela, notas de salida, cierres de maquila ni EsMa. 🔴 **HALLAZGO LATERAL, defecto vivo hoy:** un pedido con una OP **CERRADA** **no se puede cancelar por ninguna vía** —los dos `throw` se tapan (`:803` y `:828`)— y el mensaje de `:828` ofrece como salida *«o cancela el pedido sin arrastrar las OPs»*, **puerta que no existe en el código**. **V1** |
> **📊 Dónde va el programa — al 7-sep-2026, versión en `prueba` `0.123`.** De las **92 filas** numeradas: **61 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **18 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.121, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.150). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
