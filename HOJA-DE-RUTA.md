...
> | **0.151** ⬜ | 🔴 **BLOQUEA V1** · ⭐ **PREVENIR EL ERROR EN ORIGEN: el número de producción se teclea al importar, y la OP enseña de qué desarrollo nació** (Daniel 7-sep) | §Post-F9.212 ✅ **Lo que se construye EN LUGAR del borrado físico** (ver §212: se descartó tras medir que cancelar ya resuelve pantallas, reimportación y paro de producción). Daniel: *«ok, adelante con la guarda y **lo de prevenir el error**»*. **(1) EL NÚMERO, AL IMPORTAR.** Su queja original: *«me generó el pedido y la OP **sin preguntar el numero de modelo interno**… **quedamos que ese lo ponía yo, con una sugerencia previa**»*. 🔑 **Es un hueco de CABLEADO, no de diseño:** `salidaAProduccion` **ya acepta** `numeroProduccion` (`esquemas/salida-produccion.ts:60`, cuyo `describe` dice *«el sistema lo precarga y el usuario lo puede cambiar»*) → `salida-produccion.ts:332` → `nomenclatura.ts:1085`; **el importador simplemente no se lo pasa** (`importacion-pdf.ts:1227-1236`) y su contrato **no tiene el campo**. El panel manual ya es la plantilla (`PanelGenerarOP.tsx:325`, con propuesta, `/^\d{5}$/` y avisos de congruencia). ⚠️ **El campo sólo manda en el caso `nacido`**: en `reusado`/`heredado` `nomenclatura.ts:1307-1322` lo IGNORA con aviso ⇒ hay que apagarlo ahí o promete algo falso. ⚠️ En `analizar` la propuesta sólo puede ser **informativa** (`proponerNumeroProduccion` exige `tx` y lock del par) y hay que decirlo en el `describe`. **(2) LA OP ENSEÑA SU DESARROLLO** (su punto 17: *«En la OP no veo el modelo de desarrollo…. y en todo caso, ¿cómo lo cambio si hubo un error?»*). El dato **existe** (`Modelo.idModeloDesarrollo`) y `salidaAProduccion` lo devuelve (`:249-250`) **sólo en la respuesta del alta, ese instante y nunca más**: `esquemas/orden.ts:487-489` **no lo transporta** ⇒ la pantalla no lo puede pintar. Y para una OP nacida del PDF el nodo «Desarrollo» sale apagado con un tooltip que **MIENTE** (*«modelo anterior al módulo de Desarrollo»*), porque `crearOrdenDesdePdf` crea el renglón **sin `idDesarrollo`** (`:1218-1226`). **SIN migración** (la columna existe). **V1** |
...
> | **0.156** ✅ | 🔴 **BLOQUEA V1** — **y su historia es la lección de la noche: el lead clasificó mal por tomar un RODEO por una PREFERENCIA.** Daniel dijo primero *«hoy es más común meter felpa por un lado y cárdigan como otra tela por otro»*, y el lead concluyó que ésa era la forma buena y bajó la fila a post-V1, proponiendo incluso **retirar** el mecanismo del complemento. **Daniel lo corrigió:** *«**sí es importante meterlo como complemento** porque **hay proveedores que así lo manejan** y **para el control de la tela siempre es mejor ponerla como un complemento de su tela**»* ⇒ lo hacen separado **porque la receta no sabe llevar el complemento**, no porque sea mejor. ⭐ **Y la evidencia le da la razón, y estaba a la vista:** `CLAUDE.md` §5, de la ingeniería inversa del sistema viejo — *«Telas: doble componente ExTela1/ExTela2 (ej. felpa + cardigan, **mismo lote**)»*. **El cárdigan comparte LOTE con su felpa**: darlo de alta como tela independiente **rompe ese vínculo en silencio**, justo cuando 0.142 acaba de hacer que el lote viaje y el aviso de riesgo de tono dependa de él. | §Post-F9.210 + §Post-F9.214 🔴 **EL HUECO, MEDIDO:** `ModeloTela` (`schema.prisma:2810`) tiene **UN SOLO** `consumoPorPrenda` y `bom-modelo.ts` no menciona «complemento» ni una vez — pero la tela **lo declara** (`Tela.nombreComplemento`) y la compra **lo exige** (`CompraLinea.cantidadComplemento`; `exigirComplementosCapturados` no deja autorizar sin él). ⭐ **El sistema ya confesó el hueco por escrito** (`mrp.ts:3628-3631`): *«la explosión **NO sabe cuánto COMPLEMENTO lleva** una tela que lo tiene —**el BOM guarda un solo consumo por tela**— así que estas OC nacen con el complemento PENDIENTE»*. ✅ **Decidido por Daniel: el consumo del complemento es NÚMERO PROPIO**, no proporción del cuerpo. **CON migración** (columna nueva) ⇒ 🔑 **más barata AHORA que después de arrancar** (REGLA 0-B: hoy los datos son desechables). ✅ **CERRADA — v0.129.** **ENTREGADO:** la receta del modelo gana el consumo del complemento (`ModeloTela.consumoComplementoPorPrenda`), **se captura en el propio renglón y sin desplegar nada** —que era la queja literal de Daniel, *«sólo se ve el campo de la tela principal»*— **rotulado con el nombre que le da el catálogo** («Cardigan»), no con la palabra genérica; viaja **congelado** a la orden (`OrdenTela.consumoComplementoPorPrenda`) por las **cuatro** puertas que copian del modelo (alta, agregar renglón, restaurar y «traer del modelo»); y la explosión del MRP **ya emite la OC con la cantidad puesta**, en vez de dejarla pendiente para teclear orden por orden. ⭐ **EL HALLAZGO QUE CAMBIÓ EL ALCANCE: hacían falta DOS columnas, no una.** La fila pedía «columna nueva en `ModeloTela`», pero la explosión lee **la receta CONGELADA de la orden** y jamás el BOM (V1-E3d): con una sola columna el número no habría llegado nunca a la compra — la fila habría *parecido* entregada y no habría arreglado nada. 🔑 **DECISIÓN DEL CODER, hecha visible a propósito (§Post-F9.219·a, ⏳ pendiente de que Daniel confirme):** la CANTIDAD de la OC se calcula como **razón sobre el cuerpo de esa línea** (`cárdigan = cuerpo × complemento÷cuerpo`), no como `piezas × consumo`. **Por qué:** el cárdigan viaja con su felpa —mismo renglón, mismo proveedor, **mismo lote**—, así que si el comprador ajusta a 480 kg lo que hace falta es el cárdigan de esos 480; un requerido calculado aparte se separaría de su cuerpo al primer ajuste (§Post-F9.99, *«compré 480 en lugar de 481»*). ⭐ **Y se calcula en el PLAN, no en la generación** — hallazgo de la propia revisión: en cuanto el cárdigan deja de nacer vacío **empieza a costar** (`aCompraSalida` suma `complemento × precio` al subtotal), así que calcularlo al generar habría dejado la **previa prometiendo un total MENOR** del que la OC guarda, que es exactamente lo que §Post-F9.85 prohíbe. Ahora el número nace una vez, en el plan; la generación **lo copia**, y hay prueba que compara previa contra guardado. ⚠️ **Y con eso apareció la SEGUNDA mitad del mismo problema:** la previa pinta literalmente `cantidad × precio = importe`, así que un importe que ya incluye el cárdigan **deja de cuadrar a la vista** — en la última pantalla antes de comprometer el dinero. La línea ahora dice *«incluye 4.5 kg de Cardigan»*, con el nombre del catálogo (`PlanRenglon.nombreComplemento`). ⚠️ **TRAMPA que costó una prueba en rojo:** si el catálogo le QUITA el complemento a una tela ya congelada, mandar el número igual haría que `validarLineas` **rechazara la OC entera** —incluidas sus otras líneas— por un renglón que el comprador nunca capturó. Manda el catálogo de hoy: **quién** lleva complemento lo dice el catálogo, **cuánto** lleva lo dice la receta (§Post-F9.219·b). **REGLA 0-B respetada:** columnas **nullable**, **cero backfill**; sin capturar, la OC nace pendiente y `autorizarOC` la sigue parando — exactamente el sistema de ayer. **La tela suelta NO se tocó.** **Medición:** 10 unit nuevas (`complemento-de-la-receta.test.ts`) + **11 de integración** contra Postgres (`consumo-complemento.int.test.ts`, la cadena entera: receta → congelado → previa → OC → autorizar) + 4 de componente en `EditorBom.test.tsx` + 2 en `ExplosionMaterialesPagina.test.tsx` (que la previa DIGA cuánto cárdigan incluye su importe, y que no lo diga cuando no lo hay). **12 mutaciones** (9 de backend + 3 de frontend), **11 rojas a la primera**. ⭐ La doceava —quitar la puerta del **consumo del cuerpo en cero**— **NO la cazó ninguna prueba** donde vivía, porque desde ahí no era alcanzable: la regla se **extrajo a una función pura** (`razonDeComplemento`), se le escribió su prueba, y entonces la mutación sí cayó (`expected Infinity to be null`). Las tres que valen más: **quitar la guarda del catálogo** deja la generación tronando con «no lleva complemento»; **quitar el complemento del diff** deja la pantalla diciendo «guardado» sin guardar nada; y **quitarlo del importe del plan** separa la previa de la OC (3,240 prometidos contra 3,645 guardados). **SIN permisos ni semillas** ⇒ no depende de `SEED_ON_START`. 🔴 **LO QUE ESTA FILA NO HIZO, y NO se archiva como menor (§Post-F9.219·f + §6 de este documento): el COSTO del complemento.** Medido: `dominio/costos/` **no menciona «complemento» ni una vez** ⇒ el precosto valúa **sólo el cuerpo**. Mientras el cárdigan iba como tela suelta entraba al costo por su cuenta; **al moverlo a complemento, su costo desaparece del precosto — y de ahí sale el precio que se le cotiza al cliente**. No se construyó porque exige decidir **con qué precio** (el único que existe es `TelaColor.precioComplemento`, **por color**, y la receta no tiene color) y **no hay cascada de precios para el complemento**: inventarla de paso habría metido una regla de dinero que nadie decidió. **Recomendación del coder: bloquea V1.** **V1** |
...
> esa misma noche**).** De las **26 ⬜ pendientes** *(la **0.156** ya se cerró en la v0.129)*:
> **9 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.121, 0.123, 0.131, 0.151, **0.159**)
> — de ésas **dos son repasos de Daniel** y **una la ejecuta Gabriel**, así que **son 6 de construcción** ·
...
> HOY no lo será después**: `0.155`, `0.159` y `0.161` cambian el modelo de datos (y la **0.156** ya lo
> hizo, en la v0.129, justo por esta razón), y mientras la
...
> **📊 Dónde va el programa — al 7-sep-2026, versión en `prueba` `0.129`.** De las **104 filas** numeradas: **65 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **26 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.121, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.151, 0.152, 0.154, 0.155, 0.157, 0.159, 0.160, 0.161, 0.162). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
...
| **🔴 ¿El COSTO de la prenda debe incluir el complemento de la tela?** (fila 0.156, §Post-F9.219) — **medido, no supuesto:** `dominio/costos/` **no menciona «complemento» ni una sola vez**, así que el precosto y el costo real valúan **sólo el cuerpo**. Mientras el cárdigan se daba de alta como una tela suelta eso no se notaba (entraba como su propio renglón); **desde la 0.156 el cárdigan puede vivir dentro de su felpa, y entonces su costo desaparece del precosto**. ⚠️ **De ahí sale el precio que se le cotiza al cliente**, así que el número queda BAJO. **No se construyó en la 0.156 a propósito:** valuar el complemento exige decidir **con qué precio** —hoy sólo existe `TelaColor.precioComplemento`, por COLOR, y la receta no tiene color— y **no hay ninguna cascada de precios para el complemento**; inventarla de paso habría metido una regla de dinero sin que nadie la decidiera. **Recomendación del coder: 🔴 bloquea V1** — es dinero, y es la mitad que falta para que llevar el cárdigan como complemento sea de verdad equivalente a llevarlo suelto. | **antes de que alguien empiece a mover cárdigans a complemento** (o sea: en cuanto se use la 0.156) |
