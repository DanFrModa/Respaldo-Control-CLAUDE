...
> | **0.095** ⬜ | ⭐ **EL PACK, LA MITAD DE ARRIBA: que se CAPTURE, se VEA y NAZCA DEL PAPEL** (segunda mitad de la **0.084**, separada el 2-sep al medir que no cabía en una entrega). 🔴 **Y lo primero: el IMPORTADOR DE PDF va DENTRO, no al lado.** `DECISIONES.md:790` lo dice literal —*«Alcance: OP + `EtapaMovimientoDet` (corte/envío/recibo) + **importador de PDF** + matrices de captura»*— y hoy `importacion-pdf.ts:1157` sigue llamando a `fusionarPacksEnUnaCorrida`; su `matriz` **ni siquiera tiene campo `pack`** ⇒ **ninguna orden nacida de un PDF de C&A tiene packs, y el campo de la 0.087 es inalcanzable desde el único sitio del que los packs vienen de verdad**. El reporte del coder lo tapó con *«las pantallas quedan para la pieza B»* — **y el importador no es una pantalla**; él mismo retiró esa cobertura. ⚠️ **Y no se quita la fusión sin cablear el importador EN LA MISMA JUGADA**: sin ella dos packs con la misma talla llegan como dos renglones del mismo color **y el mismo pack vacío**, duplicado que sigue prohibido (verificado por el reviewer, no supuesto). **El resto del alcance:** los dos componentes de matriz (llave de fila = color+pack, opt-in), `PanelMatriz` + `AgregarColorMatriz` + `DialogoCopiarMatriz`, `AvanceProduccion.tsx` (~15 sitios de `claveCelda`) y `TableroWipPagina`. 📌 **Hasta que esto entre, Gabriel NO puede verificar el pack en `prueba`** — no hay nada observable. ⚠️ **Riesgo declarado mientras tanto:** el API **sí** acepta packs; una orden creada por API antes de esta fila pintaría **dos filas indistinguibles** en las matrices. No se bloqueó con una bandera porque sería código muerto por construcción | §Post-F9.10 |
...
>
>
> **📊 Dónde va el programa — al 2-sep-2026, versión en `prueba` `0.090`.** De las **39 filas** numeradas: **29 ✅ cerradas** · **2 🔶 a medias** (0.082, 0.084) · **1 ⏸️ aparcada** (0.087) · **7 ⬜ por hacer** (0.061, 0.072, 0.086, 0.094, 0.095, 0.096, 0.097). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
