...
> | **0.081** ⬜ | 🧹 **LO QUE SE ACUMULA SIN QUE NADIE LO VEA.** **(a) Archivos huérfanos en R2:** el motor `comun/archivos.ts` ya tiene `eliminarObjeto` y los adjuntos de orden ya lo usan, pero **modelos, bordados y proveedores NO** ⇒ borrar el registro deja el archivo **pagándose para siempre** sin que nadie pueda verlo. **(b) La venda de la memoria en CI:** `tsc --noEmit` murió con **OOM** y se destrabó subiendo el heap a 6 GB — **la misma venda que el lint ya llevaba**, puesta un paso más arriba: la cicatriz se repitió. Es señal de que algo **crece sin control**, no un problema del runner. ⚠️ **Ninguna de las dos cambia nada que Daniel vea** | — |
...
>
>
> **📊 Dónde va el programa — al 1-sep-2026, versión en `prueba` `0.080`.** De las **32 filas** numeradas: **18 ✅ cerradas** · **1 🔶 a medias** (0.073) · **13 ⬜ por hacer** (0.061, 0.068, 0.072, 0.075, 0.079, 0.081, 0.082, 0.084, 0.086, 0.087, 0.088, 0.089, 0.090). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA, así que un número de plan puede salir con otro —o no salir— sin que nadie renumere nada. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
