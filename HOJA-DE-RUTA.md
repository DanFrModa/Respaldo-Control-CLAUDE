...
- 🔴 **El `eslint` del CI está al filo de su techo de memoria, y las dos salidas fáciles están descartadas** (medido el 4-sep en el PR #307). Murió con OOM a **6011 MB de 6144**; el mismo `eslint .` con ese mismo techo **pasa** en la caja de desarrollo, así que es margen, no tamaño. **(a) Partirlo NO sirve**: `eslint src/dominio` y `eslint .` menos ese directorio mueren **las dos** con 3 GB, porque el costo es construir el **programa de tipos entero**, idéntico en las dos mitades. **(b) Subir el número está vetado a propósito** por el guardián de la fila 0.092 (`comun/techo-de-memoria.test.ts`), y su veto es bueno: el techo vive en **cuatro sitios de dos archivos**, y uno de ellos gobierna la imagen que va a **producción** — un techo por encima de lo que el contenedor puede dar cambia un OOM limpio de V8 por el OOM-killer del kernel. ⇒ **La cura es la que ya estaba escrita**: adelgazar el programa de tipos (proyectos de TS separados, tipos generados más chicos). **Merece fila propia**, y hasta entonces el lint puede caerse de forma intermitente: si pasa, es esto y NO el código.
