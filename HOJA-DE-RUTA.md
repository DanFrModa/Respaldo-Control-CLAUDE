...
> | **0.082** ⬜ | **LOS CINCO CABOS DE LAS PRENDAS INCOMPLETAS** (V1-E8k, §Post-F9.136); detalle en `docs/hoja-de-ruta/V1-etapas.md` §V1-E8k → *«Lo que queda ABIERTO»*. ⚠️ **CUATRO NO NECESITAN A DANIEL** y aquí se decía lo contrario: **O1** (la incompleta de prenda ya terminada se queda viva en Tránsito) **YA ESTÁ DECIDIDO POR ÉL** — §Post-F9.61: *«las que no vuelvan se quedan en tránsito, vivas»*—, y el saldo se limpia con un movimiento manual de PT, que es la conducta correcta: darle salida automática **sí sería inventar una merma**. **O2** un renglón `0/0/0` en la conciliación EsMa (ruido visual, `faltantePorCargar = 0`, sin descuadre). **O4** choque de nombres con «Órdenes incompletas» del menú, que es **otro concepto**. **O5** sesgo del acumulado en 3 pruebas (un recibo, una celda) — el MISMO que dejó viva la mutación que EXCEDE. ✅ **O3 CERRADA por reencuadre (Daniel, 1-sep, §Post-F9.173(b)) ⇒ se va a la 0.086, con las calificaciones de maquileros.** Era: **el KPI de calidad del maquilero IGNORA las incompletas** ⇒ quien entrega **200 incompletas sigue con calidad perfecta**, porque el indicador mira primeras vs. segundas. **Default propuesto: medirlas en su PROPIO número al lado de la calidad, no dentro** —meterlas en el % de segundas ensucia una cifra que significa *«qué tan bien cose»*; dejarlas fuera del tablero esconde a quien devuelve prenda inservible en volumen— | §Post-F9.61 · **.136** |
