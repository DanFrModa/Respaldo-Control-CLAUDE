...
> | **0.156** ⬜ | ⏸️ **post-V1 — y su respuesta cambió el DIAGNÓSTICO, no sólo la prioridad.** Daniel 7-sep: *«la mayoría lleva cárdigan. Pero a veces el cárdigan está dado como **una tela independiente**… **hoy es más común meter felpa por un lado y cárdigan como otra tela** por otro»* ⇒ 🔑 **el rodeo YA ES la práctica normal, y hace exactamente lo que esta fila iba a construir**: dos telas, cada una con su consumo y su línea de compra. El mecanismo de `Tela.nombreComplemento` —el que obliga a teclear el kilaje a mano en cada OC— resulta ser **la excepción rara**, no el camino. ⏳ **Pregunta que abre para más adelante (NO se decide hoy): si la práctica normal es tela separada, ¿vale la pena conservar el complemento?** · ⭐ **EL CÁRDIGAN NO CABE EN LA RECETA — y Compras paga esa factura en cada orden (hallazgo 6)** | §Post-F9.210 **Daniel:** *«si la tela que seleccioné tiene cárdigan, **no me da la opción para meter el consumo del cardigan**… hoy **no se ve el campo de la segunda tela**, sólo el de la tela principal»*. 🔴 **NO es un campo faltante: es un hueco de CADENA.** `ModeloTela` (`schema.prisma:2810`) tiene **UN SOLO** `consumoPorPrenda` y `bom-modelo.ts` no menciona «complemento» ni una vez — pero el concepto **sí existe en los otros dos extremos**: la tela lo declara (`Tela.nombreComplemento`, el `ExTela1/ExTela2` del Access) y la compra **lo exige** (`CompraLinea.cantidadComplemento`; `exigirComplementosCapturados` **no deja autorizar** sin él). ⭐ **Y el sistema ya confesó el hueco por escrito** (`mrp.ts:3628-3631`): *«la explosión **NO sabe cuánto COMPLEMENTO lleva** una tela que lo tiene —**el BOM guarda un solo consumo por tela**— así que estas OC nacen con el complemento PENDIENTE»*. ⇒ **hoy alguien teclea el kilaje a mano, orden por orden, para siempre.** ✅ **Decidido por Daniel: NÚMERO PROPIO**, no proporción del cuerpo. **CON migración** (columna nueva). **V1** |
...
> **11 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.121, 0.123, 0.131, 0.151, 0.153, **0.158**, **0.159**)
...
> **9 ⏸️ post-V1** (0.102, 0.116, 0.119, 0.130, 0.144, 0.154, 0.156, 0.157, 0.161), y de ésas la **0.144 va
