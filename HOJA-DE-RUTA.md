...
> | **0.118** ⬜ | ⭐ **EL DOCUMENTO DE FACTURACIÓN — «yo te digo qué facturarme»** (Daniel: *«nadie me factura si no le mando yo un documento con los datos con los que me tiene que facturar… **no al revés**. Y eso debe salir del sistema»*) | §Post-F9.186(k) ✅ **MEDIDO — es un IMPRESO, no un módulo**: `Proveedor` ya tiene `razonSocial`·`rfc`·`regimenFiscalSat`·`usoCfdiHabitual`·`codigoPostalExpedicion`, justo lo que pide un CFDI 4.0; los importes salen de la corrida (0.113). **El IVA va EXPLÍCITO**, no escondido como en el Excel de producción. ⚠️ **Proveedor sin RFC ⇒ NO se emite: hay que AVISAR cuál falta**, nunca inventarlo (REGLA 0-B: lo migrado viene incompleto). 📌 *Es lo que C&A le hace a él.* Depende de **0.113** y **0.110** |
...
> | **0.126** ⬜ | **EL COTEJO CONTRA EL ESTADO DE CUENTA DEL BANCO — el lado CON factura** (nace de §Post-F9.186(a): *«prefiero que los pagos formales salgan del pago que esté en el estado de cuenta, no de lo que yo pongo acá»*) | §Post-F9.189(h) La corrida (0.113) produce **la lista esperada**; lo que sobre del cruce son exactamente los pagos ad-hoc («el martes urgente»), señalados para atribuir. **Proponer, nunca aplicar en silencio**: dos pagos del mismo monto son ambiguos. Hoy no existe importador de estado de cuenta bancario ni conciliación de pagos contra banco (`esma/conciliacion.ts` concilia cargos/recibos, no banco). Depende de **0.113** |
> **📊 Dónde va el programa — al 3-sep-2026, versión en `prueba` `0.100`.** De las **68 filas** numeradas: **38 ✅ cerradas** · **1 🔶 a medias** (0.082) · **1 ⏸️ aparcada** (0.087) · **28 ⬜ por hacer** (0.061, 0.072, 0.086, 0.096–0.097, 0.099–0.104, 0.106–0.107, 0.109, 0.111, 0.113–0.114, 0.116–0.126). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
