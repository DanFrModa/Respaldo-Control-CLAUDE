...
> | **0.153** ⬜ | 🔴 **BLOQUEA V1** · ⭐ **EL PRECIO QUE SE PACTÓ EN LA NEGOCIACIÓN NO SE VE DESDE FUERA (hallazgo 15)** | §Post-F9.210 **Daniel:** *«Después de haber cerrado la negociación… **debería de cambiar el precio que se ve afuera**… Dice precio aprobado, **pero dentro de la negociación quedó otro**. Debe de haber congruencia.»* 🔴 **MEDIDO, tiene razón literal:** `registrarAcuerdo` (`negociacion.ts:298`) y `guardarMesa` (`:869`) escriben **sólo** `NegociacionEvento.precioNuevo`, y sus propios docstrings lo declaran (*«NO cambia el `precioAprobado` del renglón»*); `cambiarEstadoRenglon` a `cerrado` escribe el evento con **precio `null`** (`:491-492`). Y `precioNuevo` **se lee en DOS sitios de todo el repo**, los dos dentro del propio diálogo de negociación. Todo lo demás —PDF, Excel, cotización al cliente y **el precio que viaja a la orden**— lee `precioAprobado`. ⚠️ **La separación negociador/aprobador es DELIBERADA** (`negociacion.ts:8-12`): escribir `precioAprobado` al cerrar la mesa la borraría. ✅ **Daniel ya dio la salida barata en su propia frase: *«o estaría bien poner los dos, mejor»*** ⇒ **sólo lectura**: `include` de eventos, exponerlo tras la reja de importes, columna al lado de «Precio aprobado». **SIN migración, sin tocar la escritura ni los permisos.** **V1** |
...
> | **0.158** ⬜ | 🔴 **BLOQUEA V1** (Daniel 7-sep) · ⭐ **LOS AVÍOS QUE NO SE COMPRAN POR COLOR — que la etiqueta de lavado sume todo junto** | §Post-F9.213·A **Daniel, viendo la explosión con datos suyos:** *«hay ciertos avíos que **no se compran por color**. Debería de sumar todos. Como la **etiqueta de lavado**… ¿cómo le puedo hacer para definirle que algunas cosas se compran juntas **sin tomar en cuenta el color**?»* 🔴 **MEDIDO: la explosión parte por color SIEMPRE, sin excepción posible** — el avío se abre en un renglón por cada color de la matriz de la OP (`compras/mrp.ts:1359-1376`) y `claveAgrupada` (`:1557-1577`) lleva **un segmento de color incondicional**. **No existe ninguna bandera** (leído `Avio` entero, `schema.prisma:2146-2229`). ⚠️ **`esGenerico` NO SIRVE y hay que decirlo:** es **otro eje** —*«¿contra stock o contra la orden?»*, `schema.prisma:2167`— y su único efecto es netear contra el kardex; `porColor` se construye **antes y sin mirarlo** ⇒ un genérico con 3 colores **sale igual en 3 renglones**, y encima cambiaría la valuación del costo. ✅ **EL ARREGLO ES UN SOLO SITIO** (`mrp.ts:1360-1374`) **y el camino «sin color» YA EXISTE y aguanta**, verificado eslabón por eslabón: `RequerimientoOrden.idColorPrenda` nullable sin unique · `claveAgrupada` con rama `'sin'` · en el neteo el renglón sin color es **el caso privilegiado** (`comprometido-en-oc.ts:388-393`) · `CompraLinea` nunca **exige** color · **la recepción ni se entera** (el cruce por color es exclusivo de telas) · y **el kardex de avíos NO TIENE COLOR EN ABSOLUTO** (`schema.prisma:5766-5767`). **SIN migración de datos** (el snapshot se regenera y el diff ya contempla el paso a «sin color», `mrp.ts:1954`). **V1** |
...
> **11 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.121, 0.123, 0.131, 0.153, **0.156**, **0.158**, **0.159**)
> — de ésas **dos son repasos de Daniel** y **una la ejecuta Gabriel**, así que **son 8 de construcción** ·
...
> **📊 Dónde va el programa — al 7-sep-2026, versión en `prueba` `0.127`.** De las **104 filas** numeradas: **63 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **28 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.121, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.152, 0.153, 0.154, 0.155, 0.156, 0.157, 0.158, 0.159, 0.160, 0.161, 0.162). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
