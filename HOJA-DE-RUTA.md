...
> | **0.094** ⬜ | ⏳ **LA «FICHA DE ARTE» QUE VE EL ESTAMPADOR VA SIN FOTO** (medido el 1-sep por el reviewer de la 0.093, **fuera del alcance de esa etapa**). `impresos/impreso-envio-maquila.ts` **no imprime ni una imagen** — ni antes ni después; verificado **enumerando**: `impreso-orden.ts` es el **ÚNICO** archivo de todo `dominio/` que usa `Image`, y los otros tres impresos de producción, **«Ficha de arte» incluida**, no llevan ninguna. 🔴 **Por qué importa:** Daniel decidió (§Post-F9.177) que **la OP manda sobre la foto del arte** — y **ése es justamente el papel que el proveedor de arte tiene en la mano cuando trabaja**: la foto que la OP eligió **no le llega a quien la ejecuta**. ⏳ **PENDIENTE DE DANIEL, con default: SÍ debe llevarla** (consecuencia natural de su propia decisión), pero **no se construye sin que lo confirme** — puede haber una razón de operación que sólo él sabe. ⚠️ **Y es BARATA**: la 0.083 ya resuelve **cuál** foto manda en esa OP (heredadas + propias − ocultas) ⇒ imprimirla es **leer lo ya resuelto**. 📌 Se numera aunque no esté decidida, porque **sin número se pierde** | §Post-F9.177 ✅ **CONTESTADA POR DANIEL (2-sep, §Post-F9.181(c)): «si lleva foto la ficha de estampado»** ⇒ **sí la lleva**. Sin default que discutir; falta construirlo |
...
>
>
> **📊 Dónde va el programa — al 2-sep-2026, versión en `prueba` `0.091`.** De las **39 filas** numeradas: **31 ✅ cerradas** · **1 🔶 a medias** (0.082) · **1 ⏸️ aparcada** (0.087) · **6 ⬜ por hacer** (0.061, 0.072, 0.086, 0.094, 0.096, 0.097). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
