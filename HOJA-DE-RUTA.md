...
>
> ## 🔴 REGLA DEL PROGRAMA — NADA PENDIENTE EXISTE SIN NÚMERO (DANIEL, 31-ago-2026)
>
> **Textual:** *«no podemos tener pendientes sin un número. **Justo para eso ideamos los números: para que
> no se pierdan cosas ni se olviden.**»*
>
> ⚠️ **Cómo nació:** el lead le listó los pendientes y puso dos *«sin número aún»* —los que **nacieron ese
> mismo día**, o sea justo los que más riesgo tenían de perderse— y una fila titulada «sinónimos» en vez de
> un número. Al ir a numerarlos aparecieron **diez**, no dos: había pendientes reales enterrados en §4 con
> la etiqueta *«sin fase asignada»* o *«retomar cuando se priorice»* — entre ellos **el respaldo doble, que
> es la mitigación nº1 de la tabla de riesgos y hoy nadie la tiene**. Llevaban meses ahí.
>
> **Las tres partes de la regla:**
> 1. **Si es trabajo pendiente, tiene número. Sin excepción.** Ni «sin fase asignada», ni «retomar cuando
>    se priorice», ni un nombre en vez de un número. Esas etiquetas **son el mecanismo por el que algo se
>    pierde**, no una forma de anotarlo.
> 2. ⭐ **Un número puede agrupar VARIAS acciones si son del mismo tema** (Daniel, el mismo día: *«no
>    necesariamente tiene que ser una corrección por cada número»*). Diez números para diez arreglos
>    sueltos es ruido; **lo que no se vale es que una acción se quede fuera de todos**.
> 3. **Y se numera EN CUANTO NACE**, no cuando se prioriza. Un pendiente sin número no está «esperando
>    turno»: está **fuera del programa**, y nadie lo va a volver a mirar.
>
> 📌 **Qué NO necesita número:** lo que ya tiene fase asignada (F6, F10…), las notas históricas y los
> riesgos observados que no son trabajo. Si hay que hacer algo, es trabajo.
...
> | **0.061** ✅ | **La incompleta sale de Tránsito como merma · el divisor pasa a `recibido` · y ⭐ EL COSTO SE CONGELA AL CERRAR.** Las tres decididas por Daniel el 30-ago (**§Post-F9.154**), y la tercera **la levantó él**: *«¿en qué momento se define que ya se cerró el costo? ¿O va cambiando?»* — **va cambiando, medido**: se persiste el dinero y la base, pero la CANTIDAD se re-suma en cada lectura ⇒ con `recibido` el costo queda **vivo hasta el último recibo**. Adoptar el divisor sin el congelado habría dejado el costo bailando para siempre. 🔴 **Lo que el plan viejo decía MAL, ya medido:** el default **NO vive en dos sitios sino en CINCO** (+1 en SQL crudo), y el que el plan no nombraba —el `.default` del Zod— **puede reescribir órdenes ya costeadas** en un PUT que omita el campo ⇒ el cambio aplica **sólo hacia adelante**; y la **división entre cero YA está guardada en los seis divisores** ⇒ ahí no hay defecto, sólo falta **redactar** «aún no hay piezas recibidas». ⭐ **La merma está construida al ~70 %**: transacción, auditoría, almacén de tránsito y **la reversión al cancelar (que sale gratis: el motor ya revierte por origen)**; falta un código de catálogo y una rama. **NO es retroactiva al histórico** y no hay que programarlo: lo migrado no tiene el dato. **Seed ⇒ `SEED_ON_START=true`** · migración sólo por el `@default` · **sin permisos** | §Post-F9.154 ✅ **LAS TRES DECIDIDAS POR DANIEL (2-sep, §Post-F9.181(a))** — falta construirlas. La tercera la contestó con **«el costo se congela al cerrar la orden»**. ⚠️🔴 **Y al medirlo apareció que «CERRAR LA ORDEN» NO EXISTE HOY**: `EstadoOrden` es `capturada | completa | cancelada`, **no hay «cerrada»**, y **`completa` NO significa «terminada»** —el propio esquema la define como completitud de **CAPTURA** (tallas + receta liberada + arte)— ⇒ **el nombre invita a colgarlo del sitio equivocado**. ⚠️ **Y el candidato automático NO sirve, desmentido por la propia decisión (a) de esta misma fila**: como los faltantes se le cobran al maquilero y las incompletas salen como merma, **no vuelven** ⇒ una orden que perdió piezas **nunca llega al 100 % entregado y su costo no se congelaría jamás**. ⇒ **La fila CRECE**: hay que construir **el acto de cerrar la orden** (explícito, auditado A7, con permiso, reversible sólo por reapertura auditada D3) ✅ **v0.114 (5-sep):** las TRES de §Post-F9.154 en una sola fila. **(a) Merma de tránsito**: tipo `merma-incompletas` + `darSalidaMermaIncompletas` sellada con el origen del recibo (cancelar el recibo la revierte sola: el motor revierte por origen); no es retroactiva (REGLA 0-B). **(b) Divisor `recibido`**: el default vivía en **cinco sitios + uno en SQL crudo**, y el del Zod **podía reescribir órdenes ya costeadas** en un PUT que omitiera el campo ⇒ omitir ahora **conserva lo guardado**; `baseProrrateoAGuardar()` extraída a `cantidades.ts` para poder medirla sin Postgres. **(c) ⭐ Cerrar la orden**, que **NO EXISTÍA** (`EstadoOrden` era `capturada | completa | cancelada` y `completa` significa completitud de CAPTURA, no «terminada»): acto explícito con permiso nuevo `ordenes.cerrar`, auditado A7, motivo opcional al cerrar y **obligatorio al reabrir**; al cerrar se persiste el dinero **y la base** (defensa en profundidad: aunque un camino sin guarda moviera cantidades, el costo cerrado no se mueve), y `estaCongelado()` sólo rige con la orden cerrada **y** el sello puesto —una orden reabierta NO arrastra el congelado viejo—. 🔴 **Lo que apareció al medir**: la guarda cubría las ETAPAS pero **no la ORDEN** — ocho escrituras seguían entrando sobre una orden cerrada (cancelar, encabezado, matriz, copiar-matriz, referencias, precio de maquila, receta congelada), y **cancelar dejaba `estado='cancelada'` con `cerradaEn` puesta: dos finales a la vez**; se cerraron las **17 puertas** que esta fila enumera —la 17ª la encontró la revisión: **cancelar el PEDIDO arrastraba en cascada sus órdenes CERRADAS**— y lo que queda libre (comentarios, adjuntos, consultar e imprimir) va con razón escrita. De paso: `DialogoOrden.test.tsx` estaba **roto de antes** (30/30 en rojo por un mock incompleto) y el censo de permisos dejó 22 unitarias en rojo hasta declarar el permiso nuevo por escrito; tres dobles de prueba mentían sobre el sello y **se arreglaron los dobles, no la guarda** (que trata «no viene» como CERRADA a propósito). **Dos migraciones aditivas** (el valor `cerrada` va SOLO en la suya: Postgres no deja usarlo en su misma transacción) validadas con `prisma migrate diff`; **ninguna fila se reescribe**. Defaults a confirmar por Daniel en §Post-F9.200. 🔴 **CUATRO rondas con el reviewer independiente hasta el APROBADO, y las cuatro encontraron algo real.** **R2 (seis):** la **puerta 17** —`cancelarPedido` pisaba órdenes CERRADAS—, el congelado puesto a gobernar a **todos** los publicadores del unitario, seis afirmaciones vivas de que el divisor por omisión seguía siendo `cortado`, `cerrarReceta` documentada como NO bloqueada, el cierre visible en el Centro de Órdenes y un LIBRE sin nombrar. **+1 de regalo, y lo cazó correr las `.int.test.ts` en local**: un **«403-tras-commit»** en `cerrarOrden`/`reabrirOrden` —el permiso de lectura se comprobaba DESPUÉS del commit ⇒ el usuario ve un error y la orden ya quedó cerrada—; arreglado antes de la transacción, con prueba de que tras el 403 `cerradaEn` sigue NULL. **R3:** apareció el **QUINTO publicador** (`costos/margenes.ts`, que divide en SQL agregado y por eso no puede llamar a la regla única: la **traduce** — es la única copia y por eso la única que podía separarse sin que el compilador chistara), y las frases de conteo pasaron a **cruce mecánico**. **R4 — las dos redes que faltaban en la VERIFICACIÓN misma:** el cruce **no vigilaba la línea que la ronda anterior acababa de escribir** (tercer asalto de la misma cicatriz dentro de la fila, y el más feo: el instrumento daba falsa seguridad sobre su propio objetivo ⇒ **una herramienta de verificación hay que probarla contra la prosa recién escrita**, y al ampliarla saltaron dos afirmaciones falsas escondidas tres rondas, una en el TSDoc de la función que ES la regla), y **«un congelado en CERO se respeta como cero» no tenía prueba** (la mutación `IS NOT NULL` → `> 0` dejaba 60/60 en verde). **24 mutaciones del reviewer, todas muertas.** |
...
> | **0.067** ✅ | **El candado de compra**: reabrir la receta de una OP **congela el gasto de esa orden** hasta cerrarla (§Post-F9.160(a) · .165). 🔴 **La medición evitó el atajo:** `recetaLiberadaEn` **no** es el candado —el código lo dice literal— así que apoyarse en él habría dado *«la pantalla dice receta no liberada y la OC sale igual»*. ⭐ **El guard vive DENTRO de las dos puertas** por las que ya pasan todas las bocas ⇒ cero consultas extra; y aparecieron **DOS bocas más** que nadie contaba (`duplicarOC` y **`autorizarOC`, donde sale el dinero**) ⇒ **siete, no cinco**. 🔴 Revisión **RECHAZADA con 2 bloqueantes**: el candado **heredó la exención** de editar cantidad/precio (con la receta congelada se subía una línea de 100 a **5,000 kg** sin que corriera), y **la salida en una orden cancelada existía en el dominio y no en la pantalla**. ✅ **§Post-F9.168 CERRADA** (Daniel, 31-ago: *«está bien como dices»*): se queda así — sólo se congela una receta liberada **completa**. El precio aceptado con conocimiento es el caso de **39 renglones firmados de 40**, donde no se podrá congelar; la alternativa era peor (una orden **imposible de cerrar**) | §Post-F9.160(a) · .165 · **.168** |
> | **0.068** ✅ *(la (b) entregada como **0.085**; la (a) como **0.090** ⇒ FILA COMPLETA)* | **El aviso del avío distinto** entre OP hermanas + que el aviso **lleve** a `desautorizarOC` (que **ya existe**) 🔶 **MITAD (b) CERRADA (2-sep, v0.085): el aviso ya LLEVA al botón, a quien puede usarlo.** 🔑 El «nunca» escrito en el código **era más ancho que su propia justificación**: correcto para quien no tiene `compras.desautorizar`, **falso para Administración y Dirección** — §Post-F9.68 tiene **dos mitades**, y la segunda (*enseñar lo que sí se puede usar*) aplicaba a quien Daniel pidió que lo tuviera. **No era decisión nueva: coherencia entre dos reglas suyas.** ⭐ **Sólo frontend**: la regla ya estaba completa y ya viajaba; *no había nada que decidir en el dominio, sólo dejar de esconder una puerta que ya existía*. ⭐ **LLEVAR NO ES HACER, por construcción**: el panel **no tiene la mutación**, sin la llave el diálogo **ni se monta**, y hay prueba que hace clic y **afirma que no se llamó nada**. 🔴 El coder **cazó un defecto que su propio cambio habría creado** (el chip no se refrescaba ⇒ el segundo clic chocaba con un 409: *«habría mandado a rebotar justo a la única persona que sí puede»*). 🔴🔴 **RECHAZADO por la RAMA GEMELA EN LA PROSA**: la razón derogada seguía viva **con la palabra «nunca» en otros dos sitios** —uno **a tres líneas de donde ahora sí se pinta el botón**—, y ninguna mentía sobre su archivo, **pero las dos documentaban como diseño la mitad derogada**. ⭐ Al corregirlas se vio que la premisa era falsa **por partida doble**: *«aquí sólo entra el comprador»* tampoco era cierto, **Dirección abre esa bandeja igual**. Segundo hallazgo: un **residuo callado** — la puerta ofrece **todas** las OC comprometidas de la OP, no sólo las del material tocado; **no se estrecha** (exigiría contrato) pero **ya no está callado**. Ficha `V1-E9q`. ⏳ **La mitad (a) —el aviso del avío distinto entre OP hermanas— NO existe en código**, verificado ⇒ la fila **no se cierra** ✅ **MITAD (a) CERRADA (2-sep, v0.090): el aviso de la OP que se desvía del grupo.** 🔴 **SEIS RONDAS de revisión**, y en cuatro de ellas **el arreglo era correcto y lo falso era la JUSTIFICACIÓN añadida** ⇒ *una excepción contada a medias es una afirmación nueva*. Lo que salvó la etapa: **la función que «servía» habría APAGADO el aviso justo en el caso de Daniel** (`desviadoAProposito` calla lo agregado a mano **y** lo excluido ⇒ el cierre café **y** la jareta). Y el defecto grande: como **no hay estado de «cerrada»**, una OP de 2019 **votaba para siempre** ⇒ **señalaba a la nueva y correcta**; se acotó, y el reviewer midió que el recorte **apagaba el caso estrella** sobre familias migradas ⇒ se tomó **declarado entero y fijado en pruebas**, con la mitigación exacta: **vuelve al grupo en cuanto una persona firma su receta** (§Post-F9.180, confirmada por Daniel). ⭐ **El método se midió funcionando**: buscar el contraejemplo por adelantado encontró **dos cuadrantes antes que el reviewer**, el coder **desmintió la corrección que le propuso el lead** (la cantidad sugerida **no discrimina nunca, por construcción**), y el barrido extendido a los archivos vecinos halló un tercero **sin que nadie lo señalara**. Ficha `V1-E9v` |
>
...
> | **0.070** ✅ *(pieza A; la B queda)* | 🔴 **La receta compartida.** ✅ **Ratificada por Daniel:** *«que los cuatro lean la del desarrollo, ésta es la correcta»* (§Post-F9.170). **Pieza A = las LECTURAS** (19 de 42): las 3 canónicas, **los 5 `include` anidados** que el plan no veía, y `copiarRecetaDelModelo`. 🔴 **Sin los `include`, el precosto de un hijo sale VACÍO y de ahí sale el precio al cliente.** ⭐⭐ **La revisión encontró una mutación que SOBREVIVÍA los 2,345 tests**: las medidas por talla eran **la única de las cinco tablas sin lectura canónica** ⇒ se arregló **por la vía estructural** (nació `leerMedidasAvioBom`, 4 copias de la regla → 1) y **la consulta ya no se puede escribir mal**. El guardián nuevo **cubre a quien todavía no ha escrito el código**: el reviewer lo probó haciendo que **otro archivo** leyera la tabla. ✅ **Desplegable sola**, verificado por tres puertas. 📌 Deudas nombradas para la pieza B: **la LECTURA del origen de `copiarBom`** (silenciosa y destructiva, y **no es un escritor** ⇒ puede caerse entre las piezas) y la excepción de `versiones.ts` | §Post-F9.167 · **.170** |
> | **0.071** ✅ *(entregada como **0.078**)* | E3 | **La salida a producción hace nacer un modelo POR RENGLÓN DE PEDIDO.** Hoy `promoverAProduccionNucleo` **transforma una fila** en vez de crear otra. 🔴 **Medido el 31-ago — el renglón viejo era falso en tres cosas:** (1) *«y 1 renglón de inventario PT»* es **FALSO**: la existencia PT lleva `idOrden` en su llave (`schema.prisma:4184`), así que 4 OC **ya dan 4 renglones**; §Post-F9.170 lo desmintió y esta fila no se actualizó. (2) *«hace nacer N modelos»* es **ENGAÑOSO**: cada salida hace nacer **UNO**; los N son **N llamadas** (una OP ya lleva **varios colores**, `OrdenLinea` es por color) ⇒ **el cuerpo del endpoint no cambia**. (3) *«sin contrato»*: **SÍ mueve el contrato** (salida de `salida-produccion` + `pedidos-mes`) y el frontend, o Pedidos enseñará el código de desarrollo **para siempre**. 🔴 **Dos guardas rompen la propia operación** (`liga-orden.ts:159` y `:290`: el desarrollo es del padre y la orden pasaría a apuntar al hijo). 🔴 **Hay que CONSERVAR la rama legado** (`origen:'produccion'` ⇒ heredar sin derivar) o **ninguno de los ~4,987 modelos migrados podría generar OP**. 🔴 **La idempotencia hay que construirla de cero**: hoy el freno es un **efecto de borde**; después, **doble clic = 2 modelos** y números quemados de una serie de **999 por par**. ✅ **DANIEL LO CERRÓ (31-ago, §Post-F9.172(b)): *«se reúsa cuando sea el mismo modelo»*** ⇒ el número pertenece **al MODELO, no a la OP**, y la llave del reuso **ES** la llave de idempotencia — elegirlo no cuesta trabajo extra, **lo ahorra**. ⏳ **Falta sólo el borde**, con default: el hijo **no guarda el color** (el color vive en `OrdenLinea`, por decisión suya) ⇒ *«el mismo modelo»* es (A) el **renglón de pedido** —y entonces una **OC nueva del mismo color estrena número**, partiendo la prenda en dos— o (B) **desarrollo + color**, guardando en el hijo el color del que nació como dato de **identidad** (la OP sigue mandando sobre lo que se produce). **Default (B).** ⛔ **PRERREQUISITO: la pieza B** (los escritores) ✅ **CERRADA (1-sep, v0.078): se tomó (B)** — el default se confirmó con su propia frase: con (A), una OC nueva del mismo color estrenaría otro número ⇒ **la misma prenda con dos números**, que es lo contrario de *«se reúsa cuando sea el mismo modelo»*. La llave `(desarrollo, color)` **es** la de idempotencia, tal como se anticipó. 🔴 **El residuo quedó DECLARADO, no cerrado:** promover a mano un desarrollo **sin hijos** sigue dando UN número a todos los colores —**tenga o no ficha**—; se probó una segunda guarda y **se retiró porque retiraba una capacidad, no un descuido** ⇒ lo decide Daniel (§Post-F9.175). Lo fija la prueba `RESIDUO MEDIDO` y lo avisa el diálogo en ámbar. ⭐ **Las dos correcciones del reviewer fueron de HONESTIDAD, no de lógica:** un docstring que sobrevivió al retiro de su guarda y seguía prometiéndola, y el aviso ámbar —única mitigación de cara al usuario— **sin una sola prueba**. *Un residuo declarado sólo vale lo que valga su declaración.* **37 mutaciones, 37 muertas.** Ficha `V1-E9j` |
> | **0.072** ⏸️ | E4 | **Corregir en bloque** las órdenes de la familia, **saltando la ya cortada y reportándola** (Daniel: *«Ok como propones»*). El precedente ya existe: `traerDelModelo` devuelve `traidos` + `respetados` ⏸️ **FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya quiero salir»* — hoy funciona en Excel o a mano y no bloquea operar. Ficha completa en la tabla *«Lo que espera a la fase 2»* |
>
> #### Bloque 4 · Callejones sin letrero y cabos
>
> | Versión | Qué trae |
> |---|---|
> | **0.073** ✅ *(CERRADA MIDIÉNDOLA, 2-sep: sus tres piezas ya estaban hechas)* *(su pieza GRAVE entregada como **0.074**; queda la tela sin color, que es la 0.078)* | 🔴 **Medido el 31-ago: NO son «del mismo tipo» — son tres cosas distintas en una bolsa**, y la que más pesa **no estaba en el renglón**. **(1) La fusión de colores no se ve.** *«Sólo vive en la bitácora»* es **FALSO**: existe `Color.idFusionadoEn` con índice y FK (`schema.prisma:1015`, migración `20260829120000`) — lo que falta es **exponerlo** (`esquemaColorSalida` y `aColorSalida` lo omiten). ⭐⭐ **Y su otra mitad es la grave, la que el plan callaba: la vista previa del PDF no avisa del desvío.** `catalogoColoresPorNombre` (`importacion-pdf.ts:723-733`) **no filtra por `activo`** ⇒ un color absorbido cuenta como «ya existe», la OP nace en otro color **en silencio**, y la cadena de precio **casa por NOMBRE** (`costos/resolucion-precios.ts:170`) ⇒ puede caer en otro `TelaColor` con **otro precio** ⇒ **precio equivocado hacia el cliente**, con el único rastro en la bitácora. ⚠️ Trampa anotada: si el color sembrado nace **activo**, la prueba pasa verde **sin tocar el defecto**. **(2) La tela sin color en el almacén** — ver la deuda de §4: el letrero **no dispara donde el plan cree**, y de la respuesta de Daniel sale **cerrar la vía sin OC**, que es 0.072. **(3) La búsqueda por referencia: ✅ DESBLOQUEADA (Daniel, 31-ago, §Post-F9.172(a): *«está bien la 3»*) ⇒ camino (iii), BÚSQUEDA POR SINÓNIMOS — el papel del cliente NO se toca. Pero SALE DE AQUÍ igual**: es **etapa propia y CON MIGRACIÓN** (`ClienteDepartamento` no tiene la columna de fusión que `Color` sí; su rastro vive sólo en la bitácora), mientras las otras dos no llevan ninguna. Y el defecto **no está en la búsqueda ni en la fusión** (las dos bien escritas): el importador escribe la División **dos veces** —al catálogo con FK y como **texto crudo** en `OrdenReferencia.valor` (`importacion-pdf.ts:1165-1174`)—, y fusionar mueve el catálogo pero **no el texto**. Los tres caminos tienen tamaños incomparables (0 · ~40 líneas · una etapa) ✅ **CERRADA EL 2-sep SIN CONSTRUIR NADA.** Medida contra el código: sus tres piezas ya existían, **repartidas en tres versiones distintas**. La **grave** salió como **0.074**; la **búsqueda** como **0.075**; y **la tela sin color se resolvió dentro de la 0.080** al reconstruir ese letrero — conserva el «a dónde ir» y añade el diagnóstico de §Post-F9.144(d) **sólo donde es verdad** (recibiendo contra una OC; *«en un ajuste o un traspaso no hay compra que verificar»*), que es exactamente el cuidado que la decisión pedía. La **exposición de la fusión** (`fusionadoEn` con el nombre del destino, `contrato/esquemas/color.ts:41-66`) también está. ⭐ **Una fila cerrada por MEDIRLA, no por construirla** |
> | **0.074** ✅ *(entregada como **0.073**)* | **LOS NOMBRES, EN VEZ DE LOS IDS** (+1 remate). 🔴 **El encargo estaba mal en TRES cosas y el coder las midió:** el bloque estaba duplicado **SEIS** veces, no dos (una copia **con el nombre exacto de la canónica**) · mandaba a copiar **el estándar equivocado** (`BitacoraPagina` **cae al id crudo**; el embudo del frontend **ya existía**) · y `Usuario` es de **borrado SUAVE**, así que un inactivo **sí se pinta**. 🔴 **La revisión encontró tres cableados sin fijar** cuyo fallo pintaría **«Usuario dado de baja» en todos los renglones**, y un `it()` que prometía una reja RBAC **que nunca ejercitaba** (escalada de privilegio sobre tech packs). ⭐⭐ **Lección más cara: verificar con `grep` una cadena que YA EXISTÍA antes del cambio no verifica nada** — pasó **dos veces**, y una corrección se reportó hecha **sin estar en el árbol**. 🔴🔴 **La medición del 31-ago tumbó 2 de los 3 cabos: YA ESTABAN CONSTRUIDOS.** (a) el **parpadeo de red** salió en la **0.005** — y su causa **nunca fue el `retry: false`** que el plan culpaba, sino la derivación que colapsaba *«no se pudo preguntar»* en *«no hay sesión»*; hoy son **cuatro** estados (`sesion/contexto.ts:68-76`) y sólo `indeterminado` retiene al usuario, con pantalla propia. (b) el **costo en la tarjeta móvil** se arregló en `6f5064dd` (28-ago) — ⚠️ **el plan citaba MEDIA LÍNEA** de la medición (`V1-etapas.md:4089`) y **se comía el «✅ muere tras el arreglo»**. ⇒ Queda **un solo tema**: **cinco** pantallas pintan el **id crudo de un usuario** donde va su nombre —`PanelComentarios:52` · `PanelHitosOrden:104` · `AdjuntosOrden:138` · `TechPackDesarrollo:138` (copia literal del anterior) · `ProyectosPagina:761`— y **tres de las cinco se montan en el mismo `DialogoOrden`** (`:494,511,516`): arreglar sólo comentarios deja **dos hermanos a cinco centímetros**. 🔴 **No es «una línea»**: el contrato **no manda el nombre** y `nombresDeAutores` es de servidor ⇒ 5 esquemas + 5 proyecciones + helper único en `comun/` (hoy ya son **2 copias**, serían 7) + regenerar contrato. ⚠️ `aOrdenSalida` (`produccion/ordenes.ts:496`) es **SÍNCRONA** ⇒ el mapa se pasa **resuelto** o sale **N+1** en el listado. D3: un usuario dado de baja sale `null` y **el comentario se sigue viendo**. **Remate:** la tarjeta móvil de Modelos **no pinta `codigoDesarrollo`** y el escritorio sí (`:653-655`) |
> | **0.077** ✅ *(entregada como **0.075**)* | ✅ **DESBLOQUEADA (Daniel, 31-ago, §Post-F9.172(a)).** **La búsqueda por referencia entiende los DOS nombres tras fusionar departamentos** — el texto capturado del documento del cliente **NO se reescribe nunca**. 🔑 El defecto **no estaba ni en la búsqueda ni en la fusión** (las dos bien escritas): el importador escribe la División **dos veces**, al catálogo con FK y como **texto crudo** en `OrdenReferencia.valor` (`importacion-pdf.ts:1165-1174`). ⭐ Se arregla en **un solo sitio** —`armarBusqueda` (`produccion/ordenes.ts:1237-1250`)— y quedan cubiertos sus **tres** consumidores (buscador global, consultas ligeras, Centro de Órdenes): patrón embudo. ⚠️ **LLEVA MIGRACIÓN**: `ClienteDepartamento` **no tiene** la columna de fusión que `Color` sí tiene (`schema.prisma:1015` vs `:7286-7310`) — su rastro vive sólo en la bitácora. 🔴 **Rama gemela a la vista:** el sinónimo resuelve en **LOS DOS SENTIDOS** (quien tiene el papel viejo busca por el nombre viejo) y la cadena puede tener **más de un salto** — copiar `colorCanonico` (`catalogos/colores.ts:481`), no reinventarlo | §Post-F9.172(a) |
> | **0.075** ✅ *(entregada como **0.084**)* | La **bandeja de cuadre** pregunta *«¿se logró lo prometido?»*, no *«¿ya capturaste?»* (§Post-F9.144(b): **los estimados son METAS**) ✅ **CERRADA (2-sep, v0.084).** El segundo final va en un **EJE APARTE**, no como cuarto estado: una versión sale **`aprobada` Y `no_lograda` a la vez**. **`rechazada` NO servía** —significa *«la receta está mal, corrígela»* y devuelve el renglón a la cola; aquí la receta está **sana** y falló el **costo**—. ⭐ **Abarató la etapa que la meta YA estuviera guardada** (`costoEstimado`, V1-E8w): el trabajo era comparar y **enseñar la brecha**, no capturar. **«NO FIRMA, LLEVA» intacto**: contestar es **opcional**. 🔴 **Destapó un defecto EN VIVO: la columna «Cliente» salía SIEMPRE vacía** —el ancla buscaba el expediente de la versión y versionar **no crea expediente**—, arreglado de paso porque es el mismo eslabón. 🔴🔴 **DOS RECHAZOS:** la columna «Prometido» **enseñaba dinero a Ventas, Logística, Asistente y Secretarial** *(mismo dato con dos rejas distintas; y el coder había razonado siete renglones sobre ese permiso para el menú **mientras lo mandaba por el endpoint de al lado**)*; y el join **subía un solo nivel** cuando el linaje es una CADENA ⇒ en la **segunda ronda de negociación** la fila salía con brecha `—` e impacto `—`: **silencio parcial en la pantalla que existe para romper el silencio**. ⭐⭐ **La lección: el coder descartó una mutación diciendo «da SQL inválido» y el reviewer construyó la variante VÁLIDA** que deja el unitario en verde con la recursión muerta ⇒ *lo que salva a la CTE es INTEGRACIÓN, no el texto* — reescrito en tres capas, con un «NO SE RETIRAN» en las pruebas que de verdad protegen. **34 mutaciones.** Ficha `V1-E9p` |
> | **0.076** ✅ *(entregada como **0.072**)* | ⭐ **LA RECETA COMPARTIDA, PIEZA B — los ESCRITORES.** 🔴 **Revisión: CUATRO rondas, TRES rechazos** — el guardián nuevo se dio por cerrado **tres veces** y las tres la forma de al lado salió verde (la última, `export abstract class`, **ya se escribe en este repo**: es `ErrorDominio`). ⭐⭐ **El arreglo estructural no fue el regex: fue que la lista de formas dejó de vivir en prosa** (`FORMAS_DE_DECLARAR`, un `it` generado por renglón, cada uno marcado con la ronda que lo dejó pasar) — *un comentario no ejecuta*. También cazó **dos pruebas VACÍAS** (el `beforeEach` truncaba entre `it`s ⇒ eran ciertas por construcción) y la **raíz** de una rama gemela del frontend (el mock devolvía «cargando» ⇒ el botón **no existía nunca**). La mitad que faltaba de la 0.070: editar la receta parado en un hijo **se BLOQUEA** y manda al modelo de desarrollo. 🔴 **«Resolver los escritores» era el defecto con otro nombre**: habría reescrito la receta del padre y de los hermanos **en silencio**. **13 puertas**, 12 bloquean y 3 excepciones **resuelven** (la marca de agua + el origen de las dos copias). 🔴 Incluye **la puerta que no estaba en ninguna lista**: copiar un hijo en la mesa daba **receta VACÍA** ⇒ precio bajo al cliente. Guardián nuevo **por función, que CUENTA** en vez de perseguir sintaxis. ⛔ **Prerrequisito de la 0.071** | §Post-F9.135 · .167 · .170 |
> | **0.078** ✅ *(entregada como **0.080**)* | **CERRAR LA VÍA DE RECIBIR TELA SIN OC.** Nace de la respuesta de Daniel (§Post-F9.159(a)): *«es imposible. ¿De quién recibiríamos sin OC?»* ⇒ **se IMPIDE, no se advierte**. 🔴 **La vía existe CONSTRUIDA** (`dominio/inventarios/entradas-tela.ts`, «entrada por FACTURA/REMISIÓN **SIN** orden de compra», `idOrdenCompraLinea` nullable) ⇒ **no es un letrero: es una guarda de dominio**. Va con el **letrero de diagnóstico** de la tela sin color (§Post-F9.144(d)), que hoy **no dispara donde el plan cree**: se enciende con `tela.colores.length === 0`, o sea la tela **sin ningún color**, donde no hay color que contrastar contra ninguna OC | §Post-F9.144(d) · **.159(a)** ✅ **CERRADA (1-sep, v0.080).** Se cierra el **RENGLÓN** sin OC, **no el documento** (la factura con su PDF se queda); los **ajustes** quedan fuera **medido**: exigen motivo, dejan bitácora y son `movimientoManual` ⇒ corrección auditada, no puerta trasera. La guarda va en el **embudo** `validarCabeceraYLineas` (crear/editar/**confirmar**) y falla **cerrada**; el contrato exige el campo y **TypeScript obligó a 41 sitios de prueba a declararlo**. 🔴 Al construirla apareció un bug real: `confirmarEnTransaccion` **tiraba** el campo ⇒ confirmar quedaba fuera del embudo. LEER no se rompe (D3), sin backfill. 🔴🔴 **DOS RECHAZOS, los dos por el defecto que la etapa venía a cerrar:** el letrero **afirmaba sin haber preguntado** (el XML sin cruce daba `[]` ⇒ *«este proveedor no tiene nada»*, y el propio servidor prueba que ese caso existe), y luego el **sexto camino** — por deep-link la consulta va **acotada a UNA orden**, así que `[]` no significa lo mismo. ⭐⭐ **Y la corrección del primer rechazo introdujo una regresión: al hacer la frase más específica se volvió MÁS falsable** — *una frase más fuerte necesita un disparador más ESTRECHO, no más ancho*. ⭐ El reviewer demostró que dos pruebas pasaban por la razón equivocada **rompiendo el productor hasta dejar la lógica muerta y viendo 25/25 en verde** (el nombre del sexto estado CONTENÍA al quinto y las aserciones eran por substring). 18 mutaciones muertas + 1 superviviente **declarada** y verificada leyendo la prueba de integración que la mata. Ficha `V1-E9l` |
> | **0.079** ✅ *(la PRENDA como **0.082**; el ARTE, vía la fila 0.093, como **0.083** ⇒ FILA COMPLETA)* | **LAS FOTOS DEL ARTE, POR ORDEN.** Daniel (§Post-F9.171(b)): *«la foto debería de ser **de la OP no del desarrollo**. Si el desarrollo tiene fotos está bien que podamos **heredarlas**, pero también la opción de **quitarlas de la OP** y meter fotos directo a la OP»*. La mitad existe —el arte de la OP ya es suyo, se puede quitar y agregar a mano— pero **las fotos siguen siendo del modelo** | §Post-F9.171(b) ✅ **PRENDA CERRADA (1-sep, v0.082).** 🔴 **Medir la fila desmintió la decisión que la sostenía:** §Post-F9.169(b) decía *«meter fotos directo a la OP → NO existe»* y era **FALSO** para la prenda — ya existía desde F2-E3, **ajustado a petición del propio Daniel**. ⭐ **Él dijo «eso me parece que ya existe» y TENÍA RAZÓN; la medición que lo desmintió era la equivocada** ⇒ la etapa pasó de «construir todo» a **la media frase que faltaba**: quitar de la OP una **heredada**. **No se copió `OrdenArte.excluido`** —aquélla congela una fila propia; las fotos **no se copian a propósito** (viven en R2)— sino una marca `OrdenFotoOculta`. ⭐ **Asimetría deliberada** entre ocultar/mostrar, con su primera justificación **medida y FALSA** («irrecuperable») y **corregida en tres sitios**: *decisión correcta, argumento falso, se corrige el argumento*. 🔴 **RECHAZADO por la TERCERA superficie**: la foto oculta **reaparecía un clic después** en «Modificar» de la misma orden — y la razón para no cablearlo **era medible y falsa**. ⭐ El doble de la prueba **respeta el `enabled` real**: el reviewer demostró, rompiendo el doble Y el cableado a la vez, que sin eso habría 23/23 en verde **con el defecto dentro**. 23 mutaciones, 23 muertas. Ficha `V1-E9n` ✅ **CERRADA.** ⚠️ **La fila se quedó marcada a medias por descuido de documentación**: sus dos mitades entraron —la prenda el 1-sep (v0.082) y el arte el mismo día por la fila 0.093 (v0.083)— y nadie volvió a cerrarla. **Corregido el 2-sep al repasar el mapa.** 📌 *Un recordatorio de que el estado de una fila hay que cerrarlo cuando se entrega, no cuando alguien se acuerda* |
> | **0.080** ✅ *(su única parte viva, entregada como **0.076**; las otras dos ya estaban construidas)* | 🔴🔴 **LAS DEFENSAS QUE CREEMOS TENER.** ⚠️ **Esta fila la escribí YO el 31-ago tomando por buenas tres notas de deuda de §4 SIN MEDIRLAS, y dos eran viejas.** ✅ **(a) El anti-lockout de usuarios YA EXISTE** — `dominio/admin/guard-administradores.ts`, aplicado en `usuarios.ts:317` como **invariante GLOBAL**. La nota de §4 decía que faltaba. **Falsa.** ✅ **(c) El respaldo doble YA EXISTE y está enganchado** — `comun/jobs/respaldo-bd.ts`: copia **mensual cifrada (AES-256-GCM)** a R2, que **verifica con HeadObject que el objeto llegó**, aplica retención y **deja rastro pase lo que pase**, con el requisito escrito de *«que NO falle en silencio»*. Construida en **V1-E6a (17-ago)**, cron configurable por `RESPALDO_CRON`. Mi fila decía *«hoy nadie la tiene»*. **Falsa, y era la afirmación más alarmista de las tres.** 🔴 **(b) LO ÚNICO VIVO, y es de la peor clase: `singletonKey` NO serializa nada, y el COMENTARIO AFIRMA QUE SÍ.** `comun/jobs/index.ts:9-13` dice que *«pg-boss garantiza que, para un `singletonKey` dado, a lo sumo UN job está en `created`/`active`; un segundo `send` se descarta»*. **Medido ejecutando contra pg-boss real: los dos se aceptaron.** Los índices únicos sólo existen para colas **con política**, y `index.ts` las crea **todas sin `policy`** (verificado: cero ocurrencias) ⇒ **la defensa anti-duplicado de la Ruta Crítica es decorativa, y su comentario la declara viva** | — |
> | **0.081** ✅ *(la (a) entregada como **0.081**; la (b) se separó a la **0.092** al medirla)* | 🧹 **LO QUE SE ACUMULA SIN QUE NADIE LO VEA.** **(a) Archivos huérfanos en R2:** el motor `comun/archivos.ts` ya tiene `eliminarObjeto` y los adjuntos de orden ya lo usan, pero **modelos, bordados y proveedores NO** ⇒ borrar el registro deja el archivo **pagándose para siempre** sin que nadie pueda verlo. **(b) La venda de la memoria en CI:** `tsc --noEmit` murió con **OOM** y se destrabó subiendo el heap a 6 GB — **la misma venda que el lint ya llevaba**, puesta un paso más arriba: la cicatriz se repitió. Es señal de que algo **crece sin control**, no un problema del runner. ⚠️ **Ninguna de las dos cambia nada que Daniel vea** | — ✅ **(a) CERRADA (1-sep, v0.081).** Embudo `eliminarObjetosBestEffort` en **7 puertas** — el lead sólo había medido 3; aparecieron el **logo de empresas** (también al reemplazarlo) y el **BOM**. Cuenta corregida por el reviewer: **9 sitios de borrado, 5 nuevos** (uno sirve a tres puertas), **11 puntos de borrado en R2**. 🔴 **RECHAZADO en la 1ª vuelta: la invariante estrella —borrar DESPUÉS del commit— no tenía NI UN guardián, y 34 pruebas verdes lo tapaban.** Mover la llamada dentro de la transacción daba **34/34 en verde**, y la causa era estructural: las 34 llamaban con `bd = { tx }`, con lo que **nunca hay commit** ⇒ *la suite no podía distinguir «antes» de «después» porque no existía el commit*. ⇒ **un guardián POR PUERTA** (medido: con uno solo, **6 de 7 mutaciones habrían pasado**), con tres aserciones de las que una **guarda al guardián** y otra impide el **paso vacuo**. ⭐ El reviewer le buscó **cuatro neutralizaciones más** y las cuatro fallan ruidosamente — incluida la sutil (pasar otro espía), que **sólo** mata la tercera aserción. ⚠️ **Acoplamiento medido y aceptado con regla escrita**: quitar una aserción del helper apaga las 7 (41/41 en verde con el bug) ⇒ *tocar `src/pruebas/commit-r2.ts` es tocar las 7 puertas y merece la ceremonia de producción*. Ficha `V1-E9m` |
> | **0.083** ✅ *(entregada como **0.079**)* | 🔴 **EL CONTRATO PROMETE UNA PÁGINA QUE EL SISTEMA RECHAZA.** Cuatro endpoints publican `porPagina` **máx 500** —`/telas`, `/avios`, `/proveedores`, `/ordenes/consulta`— y el dominio topa en **100** (`comun/paginacion.ts:17`, con su razón escrita: *«nadie lee más y protege la base»*) ⇒ **quien lea el OpenAPI y pida 500 recibe 400**. Lo destapó la **primera prueba** que se escribió contra `/api/colores`, que **no tenía ninguna** (0.074 lo arregló ahí y dejó los otros cuatro). ⭐⭐ **Y el barrido encontró lo que lo vuelve una decisión, no un arreglo: DOS PRUEBAS BLINDAN LA MENTIRA** —`contrato/esquemas/tela.test.ts:293` y `avio.test.ts:173` aseveran `'500' → success`— con este comentario: *«el tope se subió a 500 para que los dropdowns carguen todo el catálogo (fix dropdowns)»*. 🔴 **Ese arreglo NUNCA funcionó**: se subió el contrato y **no** el dominio, así que el desplegable que pidiera 500 lleva desde entonces recibiendo **400**. ⇒ la pregunta no es «¿bajo el contrato?» sino **¿sube el dominio o baja el contrato?** — y enfrente hay una razón escrita. Toca `comun/paginacion.ts` (**los 51 esquemas honestos**) y reescribir esas dos pruebas. **Medido: hoy nadie pide >100** (barrido de los 244 `porPagina` del frontend; máximo exacto 100) ⇒ **no hay urgencia, sí hay una promesa falsa publicada** | — ✅ **CERRADA (1-sep, v0.079). La fila TENÍA RAZÓN y el lead la «corrigió» mal:** dije que eran **tres** porque telas *«valida con su propio esquema»* — pero ese esquema propio (`catalogos/telas.ts:625`) **es `esquemaPaginacion.extend(...)`** ⇒ también topa en 100. **Leí la declaración sin seguir de qué estaba hecha.** Y también dije que la gravedad era menor *«porque nadie pide 500»*: falso — `ConcentradoPagina.tsx:45` dice *«el backend desplegado rechaza >100»* ⇒ **alguien ya se estrelló y lo rodeó**, o sea que la ausencia de llamadas era **la cicatriz, no la prueba**. ⭐ Las dos las zanjó **ejecutar una sonda**, no razonar. 🔑 Regla estructural que salió de aquí: **un listado es honesto ⟺ dominio y ruta validan con el MISMO esquema**. 🔴 **El entregable no eran los cuatro números sino el GUARDIÁN, y lo rechazaron por MUDO**: un override *dentro* del `extend` lo burlaba con 6/6 verde. Se cerró **la clase, no el caso** (barrido por comportamiento; un endpoint nuevo entra sin que nadie lo apunte) y aun así apareció **una tercera forma**: la red **se comparaba consigo misma** y no veía lo que su glob nunca miró. *Una red que se mide con la misma vara que vigila no es una red.* Ficha `V1-E9k` |
> | **0.082** 🔶 *(O1 BORRADO, O2 y O4 entregados como **0.088**; sólo queda O5, que espera al pack)* | **LOS CINCO CABOS DE LAS PRENDAS INCOMPLETAS** (V1-E8k, §Post-F9.136). 🔴 **LA FILA ESTABA MAL ESCRITA Y SE REESCRIBIÓ AL MEDIRLA (2-sep-2026)** — los cuatro cabos cambiaron de forma: **O1 · BORRADO, no construido.** Decía *«ya está decidido por él… darle salida automática SÍ SERÍA INVENTAR UNA MERMA»* citando §Post-F9.61. 🔴 **Falso desde el 30-ago**: §Post-F9.154(a) dice **«la incompleta sale de tránsito como MERMA, automática»**, auditada y reversible — y **§Post-F9.154 nació justo de descubrir que esa cita decía lo contrario** ⇒ **la fila repetía el error que ya se había corregido una vez**. El trabajo vive en la **0.061**; y *«el saldo se limpia con un movimiento manual»* está **RETIRADO** por REGLA 0-B (`DECISIONES.md:11095`). Deja prosa caducada en `produccion/transito.ts:47-48` y `HOJA-DE-RUTA.md:2913-2915` (*«una decisión que Daniel no ha tomado»*) → **la reescribe la 0.061**. **O2 · ✅ (v0.088), pero AL REVÉS de como estaba escrito.** No era «ruido visual»: `recibido === 0` **⟺ todos los recibos del grupo trajeron sólo incompletas** (`recibos.ts:227,257-262`) ⇒ **es la única huella de esa entrega**, justo lo que Daniel pidió ver. Esconderlo la habría borrado — y **el filtro que la fila quería construir YA EXISTÍA** (`ConciliacionCargosPagina.tsx:51`). Lo que sí faltaba: `dominio/esma/conciliacion.ts` **no tenía NI UNA prueba propia**. **O3 · ✅ cerrada por reencuadre** (§Post-F9.173(b)) ⇒ se fue a la **0.086**, con las calificaciones de maquileros. **O4 · ✅ (v0.088), y tampoco era lo que decía.** **NO hay choque de nombres** —enumerado: los dos conceptos **nunca comparten pantalla** y los textos ya van calificados—. Lo que estaba mal era **la descripción, y publicada en el contrato**. **O5 · ⬜ ESPERA.** Dos de sus tres pruebas viven en `recibos.int.test.ts`, que la 0.087 estaba tocando. ⭐ **Medido: el sesgo está SÓLO en las pruebas, NO en el código** — las tres funciones acumulan bien (`incompletas.ts:190,193` · `recibos.ts:1271-1293` · `cargos.ts:74-77`) ⇒ **es un remate de red, no un fallo de negocio**. Ficha `V1-E9t` | §Post-F9.61 · **.136** · .154(a) |
> | **0.084** ✅ *(la mitad de ABAJO como **0.087**; la de ARRIBA, vía la fila 0.095, como **0.091** ⇒ FILA COMPLETA)* | ⭐ **EL PACK, COMO CAMPO PROPIO** (§Post-F9.10, abierta desde el **6-ago-2026** y **sin número hasta el 1-sep** — la encontró el repaso que Daniel pidió). C&A manda **varios tendidos en una misma OP** (pack A con corrida 1-2-2-1, pack B con 1-1-1-2…) y hoy el pack **vive dentro del nombre del color**: «Negro A», «Negro B». ✅ **La primera mitad ya se construyó** (`V1-E8g` / §Post-F9.129): el importador de PDF **dejó de componer** el color con la letra y los packs se suman en **un solo renglón de color**. ⏳ **Falta la segunda:** el pack como **campo propio que viaja al CORTE y al ENVÍO a maquila** (opcional al recibir), y que **el ETL y la captura MANUAL de OP cumplan la misma regla que el importador** — hoy sólo la cumple el importador. ⚠️ **Lo que NO entra** (§Post-F9.132/.133): partir los colores ya capturados en `prueba` — *«lo viejo ahorita es irrelevante»*; ese trabajo **no desaparece, cambia de sitio** y se vuelve requisito del **ETL de Access del arranque** | §Post-F9.10 · .129 · .132 · .133 🔶 **MITAD DE ABAJO CERRADA (2-sep, v0.087): el pack ya es campo propio y VIAJA al corte y al envío.** ⭐ **Tres cosas que el encargo no nombraba, todas medidas:** (1) un `pack` **nullable habría DESTRUIDO en silencio** la unicidad «un renglón por color» —en Postgres dos `(orden, color, NULL)` son distintos— y **Prisma 7.8 no sabe expresar `NULLS NOT DISTINCT`** (verificado con `prisma validate`) ⇒ `text NOT NULL DEFAULT ''`; (2) 🔴 **un defecto latente de la RUTA CRÍTICA** que los packs disparaban: comparaba renglón por renglón contra un total ya agregado ⇒ **«corte terminado» con 100 de 150**; (3) el **kardex** habría recibido renglones duplicados ⇒ se pliega en la frontera, con las **dos** puertas cubiertas. ⭐ **La propiedad que hace segura la etapa está MEDIDA, no prometida:** una orden sin packs evalúa sólo la guarda vieja — se copió literal del commit anterior y se corrió contra la nueva en **576 escenarios, 0 diferencias**, con control negativo de 98. 🔴 **RECHAZADO** porque la regla «el pack de un renglón con producción viva no se cambia» **fallaba en SILENCIO por DOS puertas**: sólo miraba renglones con `id`, así que **borrar y recrear la esquivaba** y `copiarDetalleOrden` —que arma su lista sin ids— **no la ejecutaba nunca**. Arreglada **por color** (independiente de la identidad de fila) ⇒ **una sola corrección cierra las dos**; la aritmética se extrajo a **función pura** porque las pruebas de las dos puertas son de integración y **sin extraerla no se podía mutar nada localmente**. El reviewer midió que **no quedó demasiado ancha** (15 casos, 0 desacuerdos; control negativo de 7). ⚠️ **El coder retiró una declaración suya propia**: aquello **no era un hueco preexistente**, *«el daño lo introduce esta etapa»*. Ficha `V1-E9s` |
> | **0.085** ✅ *(entregada como **0.077**)* | ⭐ **SI YA SE COMPRÓ, AVISA — Y EL AVISO LLEGA AL COMPRADOR** (§Post-F9.173(a), Daniel 1-sep). 🔴 **Corrige y completa la 0.067**: reabrir una receta con compra detrás **hoy no avisa a nadie**, y el comprador *«se entera cuando se entera»* — para entonces la tela puede venir en camino. **El aviso es el que permite alcanzar a negociar.** Debe decir **qué se compró y en qué OC**, para ver si la **OC interna** se puede cancelar. ⛔ **NUNCA cancelar la OC en automático** — textual: *«eso hay que negociarlo con el proveedor»*—; si acaso, **llevar** a `desautorizarOC`, que ya existe y ya es del dueño (patrón §Post-F9.145: el aviso que pide un acto lleva a hacerlo). ✅ **Lo ya construido NO se toca**: reabrir sigue **sin revertir nada**, y él ratificó la razón de fondo | §Post-F9.165 · **.173(a)** |
> | **0.086** ⏸️ | **LAS CALIFICACIONES DE MAQUILEROS, con sus PARÁMETROS DE ACEPTACIÓN** (§Post-F9.173(b), Daniel 1-sep): *«hay que fijar un parámetro de aceptación de **faltantes y segundas**… lo haremos cuando hagamos las calificaciones de maquileros»*. ⇒ **Cierra O3 de V1-E8k por reencuadre, no por default**: se preguntaba si el KPI de calidad debe contar las **incompletas** (hoy quien entrega 200 sigue con calidad perfecta, porque el indicador mira primeras vs. segundas). **Esa pregunta no se decide sola** — decidir sólo lo de las incompletas sería **fijar medio criterio**. ⛔ **NO se le vuelve a preguntar** hasta que esta pieza arranque, y entonces se le pregunta **entera** | §Post-F9.136 · **.173(b)** ✅ **CONTESTADA POR DANIEL (2-sep, §Post-F9.181(b))**: *«podríamos dejar **2 %** como máximo entre segundas y faltantes. Pero creo que debería de ser una tela variable puesta en algún lugar de la configuración… **modificable en el tiempo**»* ⇒ **parámetro CONFIGURABLE, default 2 %**, no una constante. ⏳ Dos precisiones menores **con default**: ¿2 % **sumando** segundas + faltantes o **cada uno**? *(default: sumados)* · ¿**global** o **por maquilero**? *(default: global)*. 📌 **Medir antes de construir** si hoy se capturan las dos cifras por separado ⏸️ **FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya quiero salir»* — hoy funciona en Excel o a mano y no bloquea operar. Ficha completa en la tabla *«Lo que espera a la fase 2»* |
> | **0.087** ⏸️ *(APARCADA por Daniel el 2-sep)* | ⏳ **LA RECETA SE COMPARTE, PERO LA FICHA SE COPIA — Y DIVERGE** (§Post-F9.174, medido el 1-sep por el reviewer de E3). Padre editado a *«95% algodón 5% elastano»* ⇒ el hijo **nacido antes** se queda en *«algodón 100%»*, el nacido después lleva la nueva, **y la OP del primero se lleva la vieja al papel del cliente**. 🔑 Daniel preguntó *«¿cómo controlas que los cuatro lleven lo mismo?»* y la respuesta construida —**una sola receta**— **cubre el BOM y NO la ficha**: `CAMPOS_FICHA_HEREDADOS` **copia** composición, maquila/corte base, curva, temporada, género, tipo, nº de operaciones, secuencia de estampado y `llevaArte`. ⚠️ **Preexistente de V1-E9a; E3 lo pone en el camino principal** (hasta hoy casi no había hijos). ⭐ **Default propuesto: (c) AVISAR de la divergencia** —no compartir, que reescribiría lo que dicen OP ya impresas, ni callar—: es el patrón que él ya aprobó dos veces (§Post-F9.135 p.4 *«advirtiendo de la diferencia»* y §Post-F9.64 *«avisar no es bloquear»*). **No bloquea E3** | §Post-F9.170 · **.174** ⏸️ **APARCADA POR DANIEL (2-sep, §Post-F9.181(d))**: *«es algo poco relevante. **No quiero perder tiempo en esto. Ya quiero sacar la primer versión.**»* **No se construye.** ⭐ **Pero dejó un REENCUADRE que vale más que las dos opciones propuestas**: *«la composición de la tela real se va a definir con la compra… entonces **la OP debe definir llevar 50-50. Como se hizo la compra**»* ⇒ para la composición **la verdad no está ni en el padre ni en el hijo: está en LA COMPRA**, y **no debería heredarse y congelarse sino DERIVARSE de la tela comprada**. ⚠️ **La fila enumeraba MAL los campos: decía 9 y son 12** (`modelos.ts:139`; faltaban `descripcion` e `idMaquileroCotizado`). **Coste de aparcarla, dicho:** un hijo nacido antes de editar el padre **se lleva el texto viejo al papel del cliente** |
> | **0.088** ✅ *(CERRADA EN CERO por Daniel el 2-sep)* | ⏳ **EL BOTÓN «PASAR A PRODUCCIÓN»: ¿SE RETIRA O SE BLINDA?** (§Post-F9.175, planteada el 1-sep por E3 — **no es un riesgo teórico, es alcanzable hoy**). Desde la **0.078** el camino bueno es **generar la OP** (un modelo por color); el botón hace lo contrario: **transforma** el desarrollo, con **UN número para todos los colores** y **sin vuelta atrás**. La única guarda rechaza el desarrollo **con hijos** ⇒ **queda abierto cualquiera SIN HIJOS todavía, tenga o no ficha** = *el bug original de Daniel por otra puerta*. 🔴 **Se probó una segunda guarda y se retiró**: rompía `crearDesarrolloConModeloNuevo` → promover ⇒ **no era una valla contra un descuido, era retirar una capacidad** — y eso no se hace desde el código. **(a) retirar el botón** · **(b) dejarlo** con lo que hoy tiene. **Default (b)**, y sólo por no retirar capacidades sin que el dueño lo pida. ⚠️ **Entre el usuario y el bug hay UNA sola cosa: el aviso ámbar** ⇒ si (b), el trabajo es **hacerlo difícil de perder**; si (a), es retirarlo. **Puede cerrar en cero**, y aun así lleva número: sin él se pierde | §Post-F9.175 ✅ **CERRADA EN CERO (Daniel, 2-sep, §Post-F9.181(e)): «Ok. Déjalo.»** ⇒ opción **(b)**, el botón **se queda con lo que hoy tiene** y **no se construye nada** — justo lo que la propia fila anticipaba (*«puede cerrar en cero, y aun así lleva número»*). ⚠️ **Lo que queda siendo verdad, dicho y no escondido:** el botón **sigue pudiendo transformar un desarrollo SIN HIJOS**, con un número para todos los colores y **sin vuelta atrás**; **entre el usuario y eso hay UNA sola cosa: el aviso ámbar, que ya existe**. No queda desprotegido: queda **protegido por un aviso y por nada más**, y así lo aprobó el dueño |
> | **0.089** ✅ *(entregada como **0.086**)* | **EL DETALLE DEL PEDIDO SE QUEDÓ SIN EL NÚMERO DE PRODUCCIÓN.** Nace del e2e rojo de E3. Hasta la 0.078, `/pedidos/administrar` lo enseñaba **por accidente**: el renglón pinta `codigoModelo` (`PedidosPagina.tsx:308`) y ese código **era** el de producción porque la promoción transformaba el modelo; hoy el desarrollo no se transforma ⇒ enseña el de desarrollo. **No es un defecto de E3** —lo que enseña es cierto y buscable (D3)— pero la vista del MES sí trae los dos (`codigoDesarrollo` + `prod. #N`) y el detalle no. ⚠️ **Medido ejecutando una sonda de tipos, no leyendo:** la línea del detalle **SÍ tiene `numeroProduccion`**, pero vale **`null` SIEMPRE** para un renglón de desarrollo ⇒ enseñarlo daría un hueco, no el número. Los que hacen falta son los de los **hijos por color**, que piden la misma agregación en servidor de la vista del mes ⇒ contrato + dominio + frontend, **no un retoque** | — ✅ **CERRADA (2-sep, v0.086).** ⚠️ **La trampa era que el campo SÍ existe pero vale `null` SIEMPRE** para un renglón de desarrollo ⇒ enseñarlo daría **un hueco, no el número**; hacían falta los de los **hijos por color**, agregados en servidor. ⭐ **Tres hallazgos del coder que el encargo no nombraba:** el detalle tiene **DOS lectores** y la pantalla pinta **desde el payload de la LISTA** ⇒ el agregado va **por página**, lo que abre un fallo nuevo —**que el lote le dé a un pedido los números de OTRO**, con prueba propia—; el `.describe()` **estaba a punto de volverse mentira**; y la regla **iba a existir dos veces en el frontend** ⇒ extraída, y cambiar el separador pone rojas **las dos páginas**. Ficha `V1-E9r` |
> | **0.090** ✅ *(entregada como **0.086**)* | **`POST /api/ordenes` SE SALTA LA ENTRADA A PRODUCCIÓN ENTERA.** Deuda previa (§Post-F9.34) que **E3 no cerró y cuya FORMA cambió**, destapada por su barrido de cierre. Medido enumerando: `crearOrden` (`produccion/ordenes.ts:774`) tiene **exactamente dos** llamadores — `salidaAProduccion` (`salida-produccion.ts:182`), que **sí** le pasa `idModeloDeLaOrden`, y la ruta `POST /api/ordenes` (`ordenes.rutas.ts:145`), que **no**. ⇒ por esa puerta nace una OP de un modelo que sigue en `origen = "desarrollo"` y **sin `numeroProduccion`**, ahora además **sin modelo por color**. 📌 Se numera porque la nota de `revision-modelo.ts:213` decía *«es alcance de V1-E3»* y **se leería como cerrada sin estarlo** — la corrigió el barrido, pero la deuda sigue viva y sin número **no existe** | §Post-F9.34 ✅ **CERRADA (2-sep, v0.086): falla CERRADO.** 🔴 Medido: **nadie la llamaba** (cero usos en frontend, e2e y migración) **pero seguía publicada en el OpenAPI**. ⭐ **El predicado es más fino que el del encargo, y la medición es la razón**: rechaza cuando el modelo que queda **SELLADO en la OP** es de desarrollo, **no** cuando lo es el del renglón — escribirlo así **habría roto `salidaAProduccion`**, que pasa `idModeloDeLaOrden` mientras el renglón sigue apuntando a su desarrollo. La puerta buena **no puede dispararla por construcción**; la mala **se cierra antes de cualquier escritura** (ni persiste ni consume folio). **(b) descartado midiendo** (`lineas` es opcional ⇒ acuñaría un hijo con nº que nadie confirmó: *una segunda puerta al acto más delicado*) y **(c) retirar: prohibido**. Ficha `V1-E9r` |
> | **0.091** ✅ *(entregada como **0.089**)* | 🔴 **EL HUÉRFANO NO ERA INVISIBLE NI LATENTE: ESTABA VIVO.** La fila decía **«✅ Hoy NO ocurre»** y **«los siete puentes»** — **las dos falsas**. `receta-orden.ts:2641` (`quitarRenglonReceta`, rama del **arte agregado a mano**) hacía `tx.ordenArte.delete(...)` ⇒ cascada a `OrdenArteFoto` ⇒ **la fila `Archivo` viva y su objeto de R2 pagándose para siempre**; y era el peor caso, porque un arte a mano **sólo puede llevar fotos propias**. ⭐ **Por qué la fila no lo vio: su lista era CORRECTA Y NO SERVÍA** — el borrado es de `ordenArte`, **un padre que nació con la v0.083**, dos semanas después de escribirse la fila. **Números reales** (sonda sobre el esquema, confirmados por el reviewer con sonda propia): **11 FKs hacia `Archivo` = 8 puentes + 3 campos del dueño** que corren el mismo riesgo y la fila ni mencionaba; **19 modelos vigilables** por cierre de ancestros. ⇒ El guardián **NO lleva lista a mano: calcula la superficie del esquema**. 🔴 **RECHAZADO dos veces**, las dos por el defecto que venía a matar: la correspondencia «puerta ↔ prueba de commit» **era una lista a mano que ya había envejecido una vez** (probado: **con la fuga puesta el guardián de hard-delete sigue 8/8 VERDE**), y el guardián resultó ser **por MÓDULO, no por puerta** (15 sitios en 12 módulos) sin decirlo. Ficha `V1-E9u` |
> | **0.092** ✅ *(entregada como **0.089**)* | 🧹 **LA VENDA DE LA MEMORIA — ahora la impide una prueba, no una nota.** ⚠️ **Y el techo NO estaba en un sitio: está en CUATRO** (`ci.yml` ×3 + `backend/Dockerfile` inline) — **`ci.yml:313-320` ya avisaba** de que el techo del workflow no cruza al `docker build` (*«hubo cuarta vez el mismo día»*) ⇒ **un guardián de un solo sitio habría sido la rama gemela literal**. El guardián registra los cuatro, cubre **las dos grafías** que Node acepta y **sólo prohíbe SUBIR** (bajar sí: el Dockerfile lo pide para builders chicos), con el mensaje nombrando las tres curas. ⭐ **Y el daño de PROCESO —que un `Killed` se lea como pase— quedó cerrado con el pre-vuelo `npm run gates`**, que serializa los 8, traduce `137`/`134` y **sólo imprime la frase de verde si pasó todo**. **Se validó SOLO, en vivo**: la caja se quedó sin memoria de verdad, el kernel mató un gate y el script imprimió *«MUERTO POR EL OOM-KILLER — la corrida NO VALE y NO es un pase»* **sin frase de verde**. Ficha `V1-E9u` |
> | **0.093** ✅ *(entregada como **0.083**)* | ⭐⭐ **LAS FOTOS DEL ARTE, POR OP** (§Post-F9.177, Daniel 1-sep: *«aplica para fotos de la prenda **pero también del arte**»* — cierra §Post-F9.176 en el lado grande: **no era sólo la prenda**). 🔴 **Medido: no existe NADA** — `OrdenArte` **no tiene campo de fotos**, ningún endpoint las expone, ninguna pantalla las pinta; cuelgan de `ModeloArteFoto → ModeloArte`, o sea **del modelo**, que es justo lo que él dice que no debe ser. 🔴 **Agravante:** un arte **agregado a mano** en la OP (`idModeloArte = NULL`) **hoy no puede tener foto en absoluto** — no hereda una que no quiere: **no hay dónde ponerla**. ⚠️ **Decisión de diseño a tomar MIDIENDO, no por gusto:** `OrdenArte` **congela** una copia del modelo, pero las fotos de la prenda **no** se congelan ⇒ (A) congelar `OrdenArteFoto` al crear la orden —coherente con `OrdenArte` y resuelve gratis el arte a mano— o (B) heredar+ocultar+agregar como la prenda —ya construido y probado—. **No imponerlo desde el encargo.** 📌 **Lo que NO se negocia:** hereda por defecto y **la OP manda**; **ocultar ≠ borrar** (D3, galería y R2 intactos, y lo que una OP oculta las demás lo siguen viendo); y **el IMPRESO es la segunda superficie** — la 0.082 ya tuvo que filtrarlo ahí. ⚠️ **Copiar el patrón de la 0.082, no reinventarlo**, incluido el guard que respeta las fotos heredadas del **padre por color** (0.078), que es donde un guard ingenuo daba un 404 | §Post-F9.169(b) · **.177** ✅ **CERRADA (1-sep, v0.083).** Se tomó **(B) heredar**, contra la corazonada del lead, y las tres razones son del código: congelar **deshacía la 0.081** (las filas compartiendo `idArchivo` se irían por CASCADE con su objeto de R2) · sin backfill **todas las órdenes existentes perderían las fotos de arte de su impreso, que hoy sí las llevan** · y `calcularDesalineacion` **nunca compara fotos**. ⭐ Heredar sale **más barato que en la prenda**: `idModeloArte` ya viene resuelto por linaje ⇒ el 404 de la 0.078 **no puede ocurrir**. 🔴 **El coder CORRIGIÓ EL ENCARGO en permisos y tenía razón**: la receta de la OP la gobierna `desarrollo.administrar`, no `ordenes.*` — con el mío, la pantalla pintaba el botón y el servidor daba **403** a quien sí puede cambiar precio y proveedor de ese renglón. 🔴 **RECHAZADO porque el cableado a su ÚNICA pantalla no tenía NI UNA prueba** (borrar el componente entero dejaba **1 967/1 967 en verde**) — y eso volvía aire el residuo de *«la pantalla esconde el botón»*: **un control compensatorio sin prueba no es un control**. ⭐ El reviewer demostró en **dos pasos** que **la honestidad del doble es lo que hace reales las aserciones**. 🔴 Una justificación citaba **una función fantasma** escrita hasta en el SQL de la migración ⇒ ahora **el guardián exige que los cuatro escritores existan con ese nombre**, para que un renombre aterrice ahí. **80 mutaciones entre los dos.** Ficha `V1-E9o` |
> | **0.094** ✅ *(entregada como **0.092**)* | ⏳ **LA «FICHA DE ARTE» QUE VE EL ESTAMPADOR VA SIN FOTO** (medido el 1-sep por el reviewer de la 0.093, **fuera del alcance de esa etapa**). `impresos/impreso-envio-maquila.ts` **no imprime ni una imagen** — ni antes ni después; verificado **enumerando**: `impreso-orden.ts` es el **ÚNICO** archivo de todo `dominio/` que usa `Image`, y los otros tres impresos de producción, **«Ficha de arte» incluida**, no llevan ninguna. 🔴 **Por qué importa:** Daniel decidió (§Post-F9.177) que **la OP manda sobre la foto del arte** — y **ése es justamente el papel que el proveedor de arte tiene en la mano cuando trabaja**: la foto que la OP eligió **no le llega a quien la ejecuta**. ⏳ **PENDIENTE DE DANIEL, con default: SÍ debe llevarla** (consecuencia natural de su propia decisión), pero **no se construye sin que lo confirme** — puede haber una razón de operación que sólo él sabe. ⚠️ **Y es BARATA**: la 0.083 ya resuelve **cuál** foto manda en esa OP (heredadas + propias − ocultas) ⇒ imprimirla es **leer lo ya resuelto**. 📌 Se numera aunque no esté decidida, porque **sin número se pierde** | §Post-F9.177 ✅ **CONTESTADA POR DANIEL (2-sep, §Post-F9.181(c)): «si lleva foto la ficha de estampado»** ⇒ **sí la lleva**. ✅ **CERRADA (3-sep, v0.092).** La ficha imprime las fotos del arte **de la OP** (la principal al frente, tope **4**, 12 MB por foto), leyendo lo que la 0.083 ya resuelve. La regla de imagen se **extrajo a `impresos/imagenes-impreso.ts`** y se comparte con el impreso de la orden. 🔑 **El tope se aplica ANTES de presignar** y el título cuenta **lo PEDIDO, no lo bajado**: con 6 fotos, tope 4 y 2 caídas en la descarga dice **«4 de 6»** y deja los huecos visibles — **medido por el reviewer**. ⚠️ **Ahí se destapó que `impreso-orden.ts` hace lo contrario** (descarta callado y su conteo miente) ⇒ fila **0.106**; y que el **recibo de maquila de arte** tiene el hueco simétrico ⇒ fila **0.107**. **Dos rondas de reviewer:** la 1ª cazó una **aserción tautológica** (`x − x === 0`) que llevaba el 🔑 y que una mutación de imagen espuria **no mataba**, un `servicioArchivos()` fuera del `try` que podía **tumbar el papel de una OP SIN arte**, y **cero cobertura** del cableado contra BD. Todo cerrado y re-verificado **con mutaciones propias del reviewer**. Se añadió además una aserción que **truena si el entorno trae `R2_*`**, para que la prueba no se degrade en silencio el día que esa ausencia se llene. **Sin migración, sin permisos, sin seed** |
> | **0.095** ✅ *(entregada como **0.091**)* | ⭐⭐ **EL PACK, LA MITAD DE ARRIBA: que se CAPTURE, se VEA y NAZCA DEL PAPEL.** ✅ **CERRADA (2-sep, v0.091).** La fusión salió **y el cableado entró en la misma jugada**: `fusionarPacksEnUnaCorrida` → **`agruparPacksEnRenglones`**, que agrupa **por pack** usando **`normalizarPack` del dominio** — cerrando de paso la petición de §Post-F9.129 de *«escribir la regla UNA vez y compartirla»* ⇒ **el ETL de Access ya no tiene que inventarla**. El reviewer **enumeró las puertas de escritura** y confirmó que **el hueco no existe por construcción**. ⚠️ **ETAPA ENTREGADA SIN REPORTE**: la sesión del coder se perdió a mitad; el trabajo quedó intacto y con gates verdes, y se **reconstruyó el reporte leyendo el diff** antes de revisarlo — *los gates dicen que no está roto, **no dicen qué falta***. 🔴 **RECHAZADO** por tres cosas: la regla «con packs o sin packs» **no tenía guarda en pantalla y el diff creaba el estado inválido** —incoherente consigo mismo, porque **a 200 líneas** sí se pre-chequeaba el exceso *«para no mandar al usuario a comerse un 400»*—; **seis textos falsos en código vivo**, dos en archivos que se contradecían solos; y 🔴🔴 **la medición que lo prueba todo**: el mutante `Math.max`→`+` del exceso **sobrevivió las 2 033 pruebas del frontend** ⇒ **la etapa cambió la aritmética del recibo de las órdenes SIN packs y nada la pinzaba**. ⭐ **Tres hallazgos del coder de correcciones**: el botón del pie **no era la única puerta** (*«Guardar y salir»* la esquivaba), la guarda **tenía un hueco sin recortar espacios** (y recortar obligó a recortar en `contenido()`, o **un renglón viaja sin su `id`** ⇒ borrar-y-recrear en silencio), y eran **TRES** caminos al 400. Ficha `V1-E9w` | §Post-F9.10 |
> | **0.096** ⬜ | 🔴 **BLOQUEA V1** · 🔍 **EL REPASO DE DANIEL SOBRE INVENTARIOS** (nace el 2-sep, de sus palabras: *«Recuerda que aún falta inventarios y finanzas. **Ya quiero entrarle a eso**»*). ⚠️ **No es una función a construir: es una VERIFICACIÓN del dueño que nunca ha ocurrido** — el módulo se construyó en junio-julio y **él no lo ha mirado**. Cubre PT (existencias, movimientos, traspasos, kardex), telas y avíos (entradas, notas de salida) e inventario cíclico. ⭐ **Precedente que fija el método**: el repaso del 13-ago (`DIAGNOSTICO-FLUJO-COMPLETO.md`) **dio origen al track V1 entero** ⇒ lo que salga **nace con su propio número**. 📌 **Antes de que él entre**, una pasada medida de qué hay construido, qué está a medias y **qué hacía el Access viejo que aquí no esté** (`Documentacion_MJD/04-Inventarios.md`), para que él **JUZGUE y no DESCUBRA** | §Post-F9.182 📌 **CONTESTADO por Daniel el 4-sep (§Post-F9.193):** *«todo está correcto»* a las doce decisiones, con dos matices (11: sí ubicación; 12: salida de avíos y telas que no es por OP, autorizada sólo por él). Sus hijas: 0.099–0.104 y 0.136–0.139. **Inventarios entra completo en la V1** («a fuerzas», §Post-F9.190) y arranca cuando finanzas cierre |
> | **0.097** ⬜ | 🔴 **BLOQUEA V1** · 🔍 **EL REPASO DE DANIEL SOBRE FINANZAS** (nace el 2-sep: *«Recuerda que aún falta inventarios y finanzas. **Ya quiero entrarle a eso**»*). ⚠️ **F9 se construyó completa el 10-jul-2026 (6/6) y DANIEL NUNCA LA HA REVISADO** — cuenta corriente de terceros, CxP con fold de EsMa, CxC, importación de CFDI 4.0 de proveedores y de ventas, reportes fiscales y aging configurable. **Igual que la 0.096**, lo que salga del repaso **nace con su propio número**. ⚠️ **Dos pendientes operativos ya conocidos que hay que poner delante de él:** el **ETL de saldos de apertura está LISTO y NO CORRIDO** (espera su corte de SINUBE, D15c) y ~~falta capturar el RFC de FR Moda~~ ✅ **EL RFC YA ESTÁ CAPTURADO** (Daniel, 7-sep: *«Teníamos pendiente subir el RFC. **Ya lo subí**»*, §Post-F9.218) ⇒ **de los dos requisitos operativos sólo queda el corte de SINUBE**. 📌 Y el menú de Finanzas **exige `SEED_ON_START=true`** por los permisos de F9 | §Post-F9.182 |
> | **0.106** ✅ | 🔴 **EL IMPRESO DE LA ORDEN TIRA FOTOS EN SILENCIO — Y ADEMÁS SU CONTEO MIENTE** (nace el 3-sep de la fila 0.094; lo anotó su coder y lo **confirmó el reviewer midiendo**). `impresos/impreso-orden.ts` descarga primero y **recorta después** (`:508-529` y `recortarArtes` `:774-776`): (a) descarta artes **sin decirlo**, y (b) el título cuenta **sobre lo ya descargado**, no sobre lo que la orden pide. 🔴 **Por qué importa: es el papel que va a PISO.** Si dice «3 artes» y la prenda lleva 5, se produce mal. ⭐ **La cura ya está escrita**: la 0.094 lo hizo al revés y está medido — con 6 fotos, tope 4 y 2 caídas al bajar, la ficha dice **«4 de 6»** y deja los huecos visibles; el mismo caso en el impreso de la orden diría *«2 de 2»* y las otras dos **desaparecerían sin rastro**. ⇒ **contar sobre lo PEDIDO y aplicar el tope ANTES de presignar**, reusando `impresos/imagenes-impreso.ts` | 0.094 · reviewer 3-sep | ⬜ ✅ **v0.103 (4-sep):** el coder **midió el defecto antes de tocar** (prueba roja: con 3 artes / 6 fotos pedidas, tope 4 y 2 caídas, el arte «Espalda» DESAPARECÍA y la 3ª y 4ª foto de otro arte ocupaban su lugar; `artesOcultas` `undefined`; las fotos del modelo tenían el mismo defecto). Cura: `ImagenPedida` (lo que la orden PIDE, sin tocar R2) → **tope ANTES de presignar/bajar** (`recortarAlTope`, compartido con la ficha de arte en `imagenes-impreso.ts`) → `presignarKeys` best-effort → lo caído queda `dataUrl: null` **conservando posición y rótulo** (casado por recorrido, no filtrar-y-mapear); título «se muestran N de M» sobre lo pedido; hueco con marco del mismo tamaño; la principal sigue blindada (si cae, hueco en la posición 0). `recortarFotos`/`recortarArtes` quedan como cinturón idempotente. 15 pruebas nuevas + 8 de los helpers; 6 que fijaban el descarte silencioso reescritas; **5 mutantes muertos**; PDF real renderizado y su texto extraído («se muestran 3 de 8», 1 página). Sin migración, permisos ni contrato. Reviewer independiente **APROBADO** (repitió 2 mutaciones —la del «hueco filtrado» ES el código viejo y matarla reproduce la prueba roja—; leyó las 6 reescrituras: todas MÁS estrictas; md5 del contrato idéntico). Tres remates antes del commit: prueba del cinturón de `bloqueFotos` (mutante superviviente cazado), el motivo del presign en el `warn` de la ficha, y el presupuesto horizontal del aviso comentado. El tope de peso por imagen y la amplificación por lote quedan como fila propia **0.140** (el reviewer midió que el tope solo no cierra el riesgo) |
> | **0.107** ✅ | 🔴 **EL RECIBO DE MAQUILA DE PROCESOS DE ARTE TIENE EL HUECO SIMÉTRICO** (nace el 3-sep de la fila 0.094). Se le puso la foto a la **ficha**; el **recibo** que el proveedor de arte tiene en la mano sigue sin llevarla. 🔑 **Es LA RAMA GEMELA con nombre y apellido**: se arregló un lado y el verde del lado arreglado **no dice nada** del otro. ⚠️ **Antes de codear: ENUMERAR** los impresos que un proveedor de arte recibe (envío, ficha, recibo) y **leerlos uno por uno** — no buscar por redacción. La regla de imagen ya vive en `imagenes-impreso.ts` ⇒ esto debería ser **reusar, no reescribir** | 0.094 · reviewer 3-sep | ⬜ ✅ **v0.112 (4-sep):** enumerados primero los papeles que recibe un proveedor de arte (orden ✅ con foto desde 0.083/0.106 · hoja de envío ❌ **por decisión escrita de la 0.094**, es el remito · ficha ✅ desde 0.094 · **recibo 🔴 el hueco** · entrega a cliente no aplica) ⇒ **dos de tres la llevaban**. El bloque de fotos se **extrajo** del envío a `impresos/bloque-fotos-arte.ts` (topes, resolución en tres capas, alto de la tarjeta, los dos textos del hueco) y lo usan la ficha y el recibo: cero copias, y una mutación en el módulo compartido pone rojas pruebas de **los dos** papeles. `esReciboDeArte` usa el criterio que ya existía (`generaEntradaPt === false`), cubre el arte post-costura y el recibo sin proceso; el de costura sigue sin fotos; orden sin fotos o descarga fallida imprime igual. El recibo era uno de los **dos impresos de producción sin pruebas propias**: ahora las tiene (el otro, la **entrega a cliente**, sigue sin ellas). El **aviso del hueco** de cada papel quedó fijado por prueba en los **dos** lados —el de la ficha viajaba por el valor por omisión del bloque compartido y voltearlo dejaba las 180 en verde (hallazgo del reviewer, cerrado en la misma ronda: ahora esa mutación pone roja la prueba de la ficha, y sólo ésa)—. Y una cicatriz que vale por sí sola: a un coder lo interrumpieron **con una mutación aplicada y sin revertir**, `esReciboDeArte` invertido, o sea el recibo de arte sin foto y el de costura con ella; se descubrió al reanudar midiendo el árbol, no leyéndolo. Sin migración, sin permisos, sin contrato, sin frontend. Reviewer independiente en dos rondas: la primera rechazó con tres hallazgos y la segunda los verificó cerrados **midiendo**, con nueve mutaciones propias, dejando uno más —el aviso por omisión— que se cerró con la prueba que él mismo propuso, y dijo expresamente que no pedía nada más. |
>
>
> | **0.098** ✅ *(entregada como **0.093**)* | 🔴 **LOS TRES GRAVES DE INVENTARIOS** (nacen del repaso 0.096, medidos el 2-sep). (1) El **botón de impreso** del inventario de telas colgaba de la pantalla/consulta **MUERTA** (la legada por lote), no de la vigente por color. (2) **«Kardex de materiales»** y **«Traspaso de materiales»** abrían en la pestaña de tela legada. (3) **«Ajuste de telas por color»** pedía **la DIFERENCIA en vez del CONTEO** y no enseñaba el saldo — **la pantalla con la que se va a cargar el inventario entero de telas al arrancar**, y con **cero pruebas** | 0.096 ✅ **CERRADA (3-sep, v0.093).** ⭐ **El punto 2 no era un caso, eran DOS** — la medición corrigió el encargo: la pata de tela del **traspaso** tiene **reemplazo vigente** dictado por Daniel (§Post-F9.32) ⇒ se retira y se muda a Avíos; la del **kardex NO lo tiene** — es la única ventana a los ~34 mil movimientos por lote que migró el ETL de F4-E6 ⇒ **se conserva** y sólo se le quita la mentira. *No se retira lo viejo por viejo: se retira lo que tiene a dónde mandar al usuario.* El conteo captura **lo contado**, lee por **Σ directa bajo lock** (nunca la vista, D3) y aplica la diferencia como dos movimientos; **un conteo que cuadra no escribe nada** ⏳*(pregunta abierta a Daniel: ¿debe quedar rastro de que se contó?)*. **Dos rondas de reviewer:** la 1ª probó **por mutación** que la **rama gemela seguía viva** —14/14 pruebas pasaban con `difComplemento < 0` roto— y que el componente nuevo tenía **cero pruebas**; la 2ª cazó un **desbordamiento de entero** (`2147483648` llegaba hasta Postgres) y **prosa contradictoria en dos archivos**. Todo cerrado. **Sin migración, sin permisos, sin seed** |
> | **0.099** ✅ | **EL CONTEO CÍCLICO, EXTENDIDO A TELAS Y AVÍOS** (nace del repaso 0.096). Hoy el cíclico completo —con su cabecera, su conteo ciego y sus estados— **sólo existe para producto terminado**. Telas y avíos se ajustan a mano | 0.096 · Daniel 2-sep ⚠️ **NO se solapa con 0.098**: aquélla copió el *patrón* sobre el kardex de tela-color; esto es extender **el motor entero**. Su motor de PT **no es reusable** tal cual (`idModelo/idColor/idTalla`, `bloquearArticuloPt`, sus tablas `InventarioCiclico*`) ✅ **Daniel 4-sep (§Post-F9.193, decisiones 4·5·6):** se captura **lo contado** con el saldo del sistema a la vista y el sistema calcula y aplica la diferencia; se extiende a telas y avíos; si el almacén se movió entre el alta y el cierre del conteo, **avisar y dejar decidir**, no bloquear. ⚠️ **Es la pantalla del ARRANQUE** (con ella se carga el inventario de telas el día uno). 🔴 **CORREGIDO el 5-sep-2026 midiendo: esta fila estaba escrita con dos hechos falsos.** Decía que «Ajuste de telas por color» *pide la resta, no enseña el saldo y no tiene ninguna prueba automática*, y **las tres afirmaciones ya no son ciertas**: la **0.098** las curó para tela×color — `CapturaConteoTelaColor.tsx` captura **lo contado** (*«Un renglón CONTADO: lo que se vio en el anaquel. Nunca una diferencia»*, citando la decisión 4), el saldo se pinta desde `GET .../telas/color/saldo`, y `AjusteTelaColorPagina.test.tsx` son 236 líneas de pruebas. ⇒ **En telas lo que falta es el MOTOR, no la captura**: folio, teórico **congelado al alta** (hoy se lee el saldo **vivo**, que para un conteo de almacén entero no basta), estados, cancelación, hoja impresa y exactitud. **Donde la queja sigue entera es en AVÍOS** (`AjusteMaterialesPagina.tsx`: Entrada/Salida sobre lo existente, se captura el movimiento y no se enseña el saldo). ⭐ **Y un hallazgo que esta fila no tenía: la decisión 6 falta TAMBIÉN EN PT** — `generarAjusteCiclico` aplica `delta = contado − teórico congelado` y sólo llama a `existenciaPtBloqueada` para el no-negativo, **nunca contra la teórica**: con 100 congelado, 95 contado y 20 que entraron de verdad en medio, escribe −5 y deja 115 cuando el anaquel dice 95, **sin avisar y sin ninguna prueba del caso** ⇒ el aviso se construye para las **tres** dimensiones. La puerta a abrir es `exigirAlmacenDelTipo(..., 'PT', ...)` de la **0.137** (cambiar el tipo exigido por el del almacén, **nunca quitar la guarda**). Además: poder anotar mercancía con existencia cero (hoy el conteo no incluye lo que el sistema cree que no tiene) ✅ **CERRADA (6-sep-2026, v0.120).** El cíclico completo —folio, teórico **congelado al alta**, estados, cancelación suave, hoja impresa y exactitud— pasa de contar **sólo producto terminado** a contar **las tres dimensiones**, y lo que decide cuál es el **TIPO DEL ALMACÉN**, no un campo que teclee nadie: un almacén guarda una sola clase de mercancía (0.137), así que un conteo suyo sólo puede ser de ésa. La dimensión se deriva al dar de alta, se **persiste** en el encabezado y de ahí en adelante manda ella; `exigirAlmacenDelTipo` sigue siendo la única puerta —se le cambió el tipo exigido, **nunca se le quitó la guarda**—. Un **detalle por dimensión** (espejo del kardex), no columnas anulables de las tres en una sola tabla: PT cuenta **entero**, telas y avíos `Decimal(14,4)`, y la tela lleva además su **segundo componente** (D5). Lo que depende de la llave del artículo —enumerar, congelar bajo lock, aplicar al kardex, describir— vive detrás de un **adaptador** de cuatro operaciones (`dominio/indicadores/ciclico/`); lo que no, vive una sola vez. ⭐ **El aviso de la decisión 6, construido para las TRES:** el defecto **también estaba en PT** —el delta se aplicaba contra el teórico congelado sin mirar nunca la existencia de ahora—, así que el caso del dueño (100 congelado, 95 contado, 20 que entraron de verdad) escribía −5 y dejaba 115 donde el anaquel decía 95, **sin una palabra**. Ahora el primer intento vuelve con el aviso —artículo por artículo, con lo congelado, lo que hay AHORA, lo contado y **en cuánto va a quedar**— y **sin escribir nada**; el segundo, con `confirmarMovimiento`, aplica. Es un **dato de la respuesta, no una excepción**: avisar no es bloquear. También: **renglones a mano** (se anota mercancía con existencia CERO y el alta ya **no rechaza la hoja vacía** — es el arranque, no un error), el ajuste de telas/avíos exige **ADEMÁS** el `.mover` de su dimensión (abrir el cíclico no debía regalar la llave del inventario de telas), y su movimiento **no se cancela desde Inventarios** —la hoja quedaría `cerrado` contando otra historia que el kardex—: el texto vive una sola vez en `inventarios/cancelacion-comun.ts` y lo usan las tres puertas de material (misma puerta trasera que cerró la 0.104). 🔴 **Lo que apareció al medir la segunda vuelta, y no era menor:** quién lleva segundo componente se decidía por el **catálogo de HOY** en la captura y por el **teórico CONGELADO** en el ajuste ⇒ ponerle `nombreComplemento` a una tela con la hoja abierta obligaba a capturar un número que el ajuste **ignoraba en silencio**; y la mitad espejo —quitárselo— leía la existencia del complemento como 0 y dejaba la hoja **imposible de cerrar** (el no-negativo chocaba contra un cero falso). Ahora **manda lo congelado** en las dos, y la forma de la hoja viaja al adaptador al re-leer la existencia. 🔴 **Y un agujero de seed, hermano del que la 0.137 tapó en avíos:** el seed sembraba tres almacenes de PT y uno de AVÍO y **ninguno de tipo TELA** (sólo el ETL los creaba) ⇒ en una base sin ETL, **la pantalla del arranque de telas no tenía almacén que ofrecer**. Se siembra «Almacén de telas», idempotente. **Pruebas:** las de integración del cíclico pasan de **11 a 27** (las tres dimensiones end-to-end, con el efecto verificado **en el kardex**) y el unitario suma **16 casos** sobre `planearAjuste`, que no tenía ni uno: el caso exacto del dueño en las tres dimensiones, los **cuatro cuadrantes** de la tela (incluida la entrada con cuerpo 0), el redondeo por **escala** y el caso sin movimiento. **Migración aditiva** con `DEFAULT 'PT'` para lo existente, **sin backfill** (REGLA 0-B). El seed agrega el almacén de telas ⇒ aparece solo en el próximo despliegue. 🔴 **RONDA DE CORRECCIÓN — el reviewer independiente RECHAZÓ con 10 hallazgos, y NINGUNO era funcional: todos eran GUARDAS Y PRECONDICIONES SIN UNA SOLA PRUEBA** (el patrón que ya hundió la 0.098 y la 1ª ronda de la 0.137). Quedaron cubiertas, con su mutante muerto cada una: la guarda del almacén **al agregar renglón y al cerrar** —borrarla de cualquiera de las dos dejaba las 27 pruebas en verde, y **no hay segunda barrera** porque el motor de kardex no mira el almacén—; el **seed de telas** (romperle la idempotencia o borrar la llamada entera no rompía nada, con su gemelo de avíos probado cinco líneas más arriba); la **escala decimal de telas** (mutarla a piezas no rompía nada: contar 12.5 kg de felpa habría escrito **13**); el **rechazo de capturar un complemento que la hoja no congeló** —en telas es la ÚNICA defensa, porque PT y avíos llevan un segundo cinturón en su `guardarConteo` y `tela.ts` no—; y que **`idsTela`/`idsAvio` filtren de verdad**. 🔴 **Y un defecto real que salió de ahí: la idempotencia del seed colgaba del NOMBRE** mientras el historial invita a renombrar y `SEED_ON_START` es permanente ⇒ el siguiente despliegue tras un renombre habría creado un **segundo** almacén global de telas, partiendo el inventario en dos (el daño exacto que la 0.137 vino a evitar). La llave pasó a ser **el TIPO**, en telas **y en avíos**, con un solo helper compartido. Más tres frases falsas: la guarda se pasa **tres** veces y no dos, la **migración** decía «SIN semillas nuevas» (y quien despliega lee la migración, no el historial), y el TSDoc de `existenciasTelaColorPorColor` describía sólo su uso viejo sin lock cuando esta fila le dio un llamador que la usa como lectura **autoritativa bajo lock**. Cierre: **35 pruebas de integración del cíclico** (eran 11) + **17 del seed** (eran 15) + **27 unitarias** (eran 11); **16 mutantes lanzados, 15 muertos** y **1 superviviente declarado** —revertir sólo la mitad del motor de la coherencia del complemento, redundante con la del adaptador—, y la suite de integración completa en verde |
> | **0.100** ✅ | **EL TRASPASO DE PT DEJA RASTRO**: hoja de traspaso + **motivo obligatorio** (nace del repaso 0.096) | 0.096 · Daniel 2-sep ✅ **Daniel 4-sep (§Post-F9.193, decisiones 2·3):** hoja impresa con el **folio del traspaso que ya existe** (sin folio nuevo) y **motivo obligatorio** al meter o sacar PT a mano, igual que telas y avíos (hoy se mueven mil piezas sin una palabra, `movimiento-pt.ts:111`). El repaso midió que PT **no tiene ni un solo papel** (ni existencias, ni kardex, ni traspaso): la hoja de traspaso es lo decidido; existencias y kardex a Excel se agregan si Daniel los pide ✅ **v0.115 (5-sep):** **motivo obligatorio** al mover producto terminado a mano —movimiento de entrada/salida y traspaso—, con el **mismo texto y los mismos límites que materiales** (`inventario-material.ts`) y persistido en **`Movimiento.observaciones`**, la columna que telas ya usaba ⇒ **sin migración**; la validación vive en el dominio (A1), no sólo en el Zod de la ruta. **Hoja de traspaso de PT en PDF** calcada de `impreso-traspaso-tela.ts` (folio existente —**no se crea secuencia nueva**, A3—, fecha, usuario, ambos almacenes, motivo y detalle color×talla), servida por una ruta binaria con `inventario-pt.ver` ⇒ **sin permisos nuevos ni seed**. El impreso se mide sobre el **texto renderizado** con `extraerTextoPdf`, no sobre el buffer. **REGLA 0-B**: aplica sólo hacia adelante — un movimiento viejo sin motivo se lee e imprime sin tronar y **no se rellena**. Fuera de alcance por decisión de Daniel (§Post-F9.193, decisiones 2·3): existencias y kardex de PT a Excel. ⚠️ Construida **en paralelo** con la 0.061 y la 0.111 bajo la regla nueva de Gabriel —lista de archivos prohibidos medida y puesta en el encargo, con orden de parar y avisar antes que tocarlos— y con su **propio puerto de Postgres** para las pruebas de integración. ✅ **Reviewer independiente APROBÓ** — y lo juzgó **en frío**: el primer reviewer alcanzó a medir su segunda vuelta (10 puertas limpias, integración completa **164 archivos / 3 113 pruebas EXIT=0**, 9 mutaciones muertas, generados reproducidos byte a byte) pero **murió sin escribir veredicto**, así que el segundo revisó el cambio como si nadie lo hubiera visto y sólo se apoyó en esa evidencia para no repetirla. Verificó lo que la fila prometía: la hoja imprime **el folio que ya existía** (ninguna secuencia nueva, A3 intacto), **sale igual pidiéndola desde cualquiera de las dos patas** —su mutación de usar el folio de la pata en vez del de la salida muere— y **rechaza los seis casos** que debe rechazar (no-traspaso, no-PT, gemela ausente, pata de entrada huérfana con la trampa de `Number('') === 0` separada antes de convertir, y traspaso cancelado), todo con A9 por empresa; y que el **motivo obligatorio lo es en el DOMINIO** y no sólo en el Zod de la ruta —las pruebas llaman con la base falsa y revienta antes de tocar nada, y la de integración afirma que `movimiento.count()` **no cambia**—. Comprobó además por grep que **el ETL no se rompe** (la migración importa el motor de kardex, no el dominio). 🔴 **Sus tres observaciones, arregladas en esta misma ronda porque las tres eran «código bueno que nada vigila»**: (N1) quitar el `setMotivo('')` tras guardar dejaba **15/15 en verde** —si se pierde, el motivo del movimiento anterior queda pegado y se adjunta **en silencio** al siguiente: una palabra equivocada en el rastro es peor que ninguna, y el rastro es lo que esta fila vino a construir—; (N2) `urlImpresoTraspasoPt` **no la ejecutaba ninguna prueba** porque los dos archivos **re-implementan el helper dentro de su propio `vi.mock`** y afirmaban contra sí mismos (apuntarla a una ruta de TELA dejaba 13/13 en verde; no había defecto vivo y la hoja de tela arrastra la misma forma, pero el helper lo introdujo esta fila); (N3) el fixture del «orden del catálogo» tenía `orden` e **`id` coincidiendo**, así que ordenar sólo por `id` dejaba **20/20 en verde** — y en producción una talla añadida después trae id alto y orden bajo, o sea columnas barajadas. **N4 se midió y se dejó como está** con razón escrita: quitar el `idEmpresa` de la búsqueda de la pata gemela también deja 20/20, pero **no es explotable** (el id sale del `origenId` que el motor escribió en la misma transacción sobre una pata ya filtrada por empresa) — es defensa en profundidad, no un hueco. 📌 Y dejó dicho, para que nadie se sorprenda, que la hoja queda **también disponible para el traspaso de reclasificación de auditoría** (`calidad/auditorias.ts` llama al motor sin `origenTipo`): es coherente —es un traspaso real entre almacenes de PT— y la guarda de cancelado lo protege. |
> | **0.101** ✅ | **LOS DOS AVISOS DE LA SALIDA DE TELA** (nace del repaso 0.096) | 0.096 · Daniel 2-sep ✅ **Daniel 4-sep (§Post-F9.193, decisiones 8·9):** (a) al sacar tela para una orden, **avisar sin bloquear** si se saca más de lo que la orden pide (hoy no compara contra nada); (b) el aviso de «riesgo de tono» **sólo cuando hay más de una partida** de ese color, **con la lista de partidas a la vista** para escoger a conciencia (hoy sale siempre, `SalidaTelaColorOrdenPagina.tsx:252`) ✅ **v0.117 (5-sep):** las dos decisiones de §Post-F9.193 (8 y 9). **(a) Aviso de sobre-salida**, que no existía: módulo de dominio nuevo `inventarios/previa-salida-tela-orden.ts` + `POST /api/inventarios/telas/color/salidas-orden/previa` con permiso **REUSADO** `inventario-telas.mover` (sin permisos nuevos, sin seed, sin migración); **avisa y deja pasar**, y suma **lo ya salido antes** contra esa orden (si no, sacar de a poquito lo evade). ⭐ **«Lo que la orden pide» sale de `RequerimientoOrden`, el MISMO snapshot que lee el tablero del comprador** — no se recalcula nada, para no crear el segundo publicador de la misma cifra (la cicatriz de la 0.061, donde eran cinco); **si no hay snapshot, el aviso CALLA**, y la pantalla lo dice en vez de fingir que todo sobra. Se compara **por TELA y no por color** (decisión (c) de Daniel, misma regla que `estatusMaterialesOrden`) y **sólo el cuerpo**, porque la explosión no conoce el complemento — el aviso lo nombra («sacas 400 **de cuerpo**») para no afirmar algo falso. Se le puso también a la **pantalla legada**, que ya no está en el riel pero **seguía registrando salidas** y era la puerta trasera. **(b) El aviso de tono**, que salía SIEMPRE: pasa a **tres estados** — `varias-partidas` (alarma ámbar, con la lista de folios y fechas) · `origen-desconocido` (**línea NEUTRA, no alarma**) · `sin-riesgo` (calla). 🔴 **Lo que apareció al medirlo, y es el techo del dato de hoy**: las patas del traspaso van con `idPartida = NULL`, así que en un almacén alimentado sólo por traspasos —**el del cortador, que es donde arranca la pantalla**— el sistema **nunca** sabe de qué partidas es la tela ⇒ la línea neutra sale en **todas** las capturas. Se eligió **decirlo** antes que callar (mentir por omisión) o encender la alarma (devolverle a Daniel su queja original, *«un aviso que sale siempre no lo lee nadie»*), y el ámbar queda **fijado por pruebas** sólo para el caso accionable. Cuando conviven ambos, **gana la alarma** —precedencia clavada con prueba en los dos niveles— y la lista añade *«y hay N más cuyo origen no se puede nombrar»*, para **no presentar como completo lo que no lo es**. La cura de verdad nace como fila **0.142** (que la partida viaje en el traspaso), **decisión de Daniel**. 🔴 **CUATRO vueltas con el reviewer independiente, y las TRES primeras cayeron por lo mismo: el módulo AFIRMABA UNA PROPIEDAD QUE NO TENÍA.** El código nunca estuvo mal; lo que fallaba era lo que el módulo decía de sí mismo. **R1 (5 hallazgos):** el aviso de tono **podía FALTAR** pese a que la cabecera prometía *«puede sobrar; nunca faltar»* —medido: 800 kg físicos en el almacén de corte, cero partidas vistas, y se callaba—; la rama que cerraba la puerta trasera de la pantalla legada **se podía borrar entera y todo seguía verde**; `tieneExplosion` se calculaba, se publicaba y **no lo leía ninguna pantalla** (⇒ el silencio significaba a la vez «vas bien» y «no tengo con qué compararte»); el aviso decía «ahora sacas 400» cuando del anaquel salían 600; y una tolerancia **inerte** hacía que su prueba pasara por el redondeo y no por lo que decía vigilar. **R2:** la regla generalizada **no cubría el caso mixto que su propia cabecera decía cubrir** —causa estructural: compara un **acumulado histórico que nunca baja** contra un **neto de hoy**, así que en cuanto un almacén consume algo queda ciego—, y el **complemento**, que es la razón de ser del aviso (§Post-F9.11 punto 2), **no lo vigilaba nadie**: se podía quitar de cualquiera de los dos lados con todo en verde. **R3:** la frase nueva decía que homogeneizar *«no se puede»* y **para uno de los dos fallos era falso** (existe una formulación local, medida: `800 > 500` la vería) — importaba porque **la fila 0.142 se iba a acotar leyendo ese párrafo**; y cuando las dos condiciones son ciertas a la vez, **nada fijaba cuál gana**, así que un refactor inocente habría escondido la lista de partidas justo en el caso dañino. **R4 APROBADO**: *«cada afirmación se sostiene contra el código o contra una medición»*. ⭐ Y el reviewer midió lo que no se le pidió: que la alternativa descartada es **aún más pegajosa** de lo que el párrafo cuenta —basta **cancelar una salida** para encenderla, porque el inverso copia el `idPartida` de la salida y las salidas van con NULL—, lo que **apuntala** la decisión de descartarla y le ahorra el descubrimiento a quien escriba la 0.142. ⏩ **SUPERADA EN PARTE POR LA FILA 0.142 (6-sep-2026) — no leas de aquí el estado de hoy:** el traspaso ya **nombra el lote**, así que la frase de arriba («la línea neutra sale en **todas** las capturas del almacén del cortador») **ya no es cierta**; ese estado queda para la tela que nadie puede nombrar (la traspasada ANTES de la 0.142, la del ajuste de entrada del conteo cíclico y la devuelta al cancelar una salida que tampoco llevaba lote). Y la comparación **acumulado contra neto** que el reviewer diagnosticó aquí **se cerró**: hoy es neto contra neto. Lo único que sobrevive intacto de este diagnóstico es que **las salidas a orden siguen sin nombrar lote**, y por eso el saldo por lote no cuadra del todo en un almacén que consume. |
> | **0.102** ⬜ | ⏸️ post-V1 · **CONSUMO DE TELA POR CORTE Y POR MODELO** (nace del repaso 0.096) | 0.096 · Daniel 2-sep ✅ **Daniel 4-sep (§Post-F9.193, decisión 10):** se construye — es la base para saber si la merma real cuadra con el consumo teórico; hoy hay que ir color por color |
> | **0.103** ⬜ | 🔶 duele · **UBICACIÓN DEL MATERIAL EN EL ALMACÉN** (Daniel 2-sep: *«sí quiero que haya un lugar donde está ubicado. Principalmente para telas y avíos, pero podríamos dejarlo también para producto terminado»*) | 0.096 ✅ **CERRADA (3-sep):** *«de texto libre está bien. **Por ahora no un catálogo de posiciones** — si en algún momento se requiere lo hacemos»*. Texto libre por artículo × almacén; sin catálogo, sin validación, sin pantalla de administración |
> | **0.104** ✅ | **LA SALIDA QUE NO ES POR OP** — venta o devolución de material, autorizada sólo por Daniel (2-sep: *«el 99 % sale por medio de una OP pero deberíamos tener la opción de sacar alguna venta o cualquier otra cosa»*) | 0.096 ✅ **CERRADA (3-sep):** *«por ahora que toque **sólo inventarios**… pero **sí debe existir una salida por otro medio que sólo ajuste de inventario**… **siempre autorizada sólo por mí. Nadie más**»*. 🔴 **MEDIDA y REESCRITA (3-sep):** en **PT la capacidad YA EXISTE** (`MovimientosPtPagina` ofrece todos los tipos; el seed ya trae `venta-mostrador`/`otras-salidas`, heredados de Access) ⇒ **no se construye**. En **telas y avíos NO existe nada** (sólo salida por orden, por nota y ajuste) ⇒ ahí sí se construye. 🔴 **Y destapó un hueco VIVO**: hoy `inventario-pt.mover` lo tiene **hasta Secretarial** ⇒ **cualquiera puede registrar una «Venta de Mostrador» y sacar PT**. Lo cierra la **0.105**, que va ANTES ➕ **Daniel 4-sep (§Post-F9.193, respuesta 12):** *«debería de haber manera de sacar por ejemplo una devolución, o una venta de avíos que ya no se usen… que no sea mediante la descarga o aplicación a una OP. Esto autorizado siempre por mí. Lo mismo en telas»* — confirma lo cerrado el 3-sep y nombra los dos casos: **devolución al proveedor** y **venta de material que ya no se usa** ✅ **v0.119 (5-sep):** salida de **telas y avíos** que **no pasa por ninguna orden** — devolución al proveedor y venta de material que ya no se usa (§Post-F9.193 punto 12), sólo ajuste de inventario, **motivo obligatorio** (ahí se anota a quién) y **la misma llave para registrar y para cancelar**. Permiso nuevo **`salida-material.registrar`** (Administrador + Administración/Dirección, los dos únicos perfiles) y **dos tipos de movimiento dedicados** por seed (`venta-material`, `devolucion-proveedor`), para que el kardex distinga una devolución de una venta y de un ajuste de conteo. 🔴 **RECHAZADA en la primera vuelta por un hallazgo que derribaba la premisa de la propia fila: un tipo dedicado NO RESERVA NADA por sí solo.** Los dos rótulos entraban al catálogo **global y activos**, y los escritores genéricos aceptan cualquier tipo que no sea `traspaso` ⇒ medido con una sesión que sólo tenía `.mover` (permiso que llevan **8 perfiles de 9**, contra **2** que llevan el nuevo): **escribir «Venta de Material» estaba PERMITIDO**, y los rótulos ya salían en el desplegable de producto terminado. Textual del reviewer: *«un tipo dedicado es, implícitamente, una afirmación sobre quién lo escribió; si cualquiera puede escribirlo, no clasifica mejor: clasifica igual de mal y con más confianza»*. ⇒ Cura: `CODIGOS_TIPO_RESERVADOS` + `rechazarTipoReservado` exportados desde `salida-sin-orden.ts` y llamados en **todos** los escritores, más una bandera **`capturaManual`** decidida **en el dominio** que saca los rótulos del desplegable (la pantalla no repite códigos). ⭐ **Y el coder encontró una CUARTA puerta que el censo del reviewer NO enumeraba**: la vista **legada por lote** (`telas.ts` `ajustarInventarioTela`, `POST /inventarios/telas/ajustes`), con el mismo `inventario-telas.mover` y aceptando cualquier tipo — *cerrar tres de cuatro deja el rótulo igual de falsificable* ⇒ lección: **un censo de puertas se queda corto; se enumera mecánicamente, no de memoria**. ⭐ **Y así se cerró: la re-revisión NO encontró una quinta puerta, y lo demostró por tres vías independientes** — barrido de las **659 operaciones del contrato** (exactamente **4** aceptan `idTipoMov`, las cuatro cerradas), las **15 constantes `COD_*`** cruzadas contra los reservados (**cero coincidencias**: todos los demás movimientos llevan tipo fijo, no elegido) y **el catálogo mismo**, que resultó cerrado *por ausencia* (**no existe ninguna ruta que cree, renombre ni borre un tipo de movimiento**: el único escritor en todo el repo es el seed). El ETL acepta cualquier tipo pero **no está expuesto en ninguna ruta**. **H2** (el **inverso** de una salida sin orden se cancelaba con `.mover`) se cerró **más allá de lo pedido**: no mira sólo el `origenTipo`, **sigue la cadena `idMovimientoInverso` hacia atrás** con tope anti-ciclo, así que cancelar el inverso-del-inverso también pide la llave. Y se reforzaron **dos pruebas que pasaban por el motivo equivocado** (la de PT no sembraba existencia ⇒ sin la guarda fallaba por *«no hay existencia»*, no por la guarda; y un `not.toBeNull()` habría pasado con `undefined`). **Sin migración**; sí siembra permiso y tipos ⇒ el menú y los rótulos aparecen solos en el siguiente despliegue. 🔴 **TRES rondas con el reviewer independiente, y las dos primeras cayeron por lo mismo: la fila afirmaba una propiedad que no tenía.** **R1**: el rótulo dedicado no reservaba nada (medido con sesión de sólo `.mover`) y la doc decía «en PT no se construyó nada», **falso de hecho**. **R2**: la guarda de cancelación **fallaba ABIERTA** —recorría la cadena de inversos con tope de 20 y al agotarse dejaba pasar; medido: `.mover` podía cancelar a profundidad **20**—, y **tres mutaciones sobrevivían con 5/5 en verde** en la pantalla nueva porque los mocks de captura la apagaban (`hayRenglones` siempre falso ⇒ `guardar()` sin **una sola línea** de cobertura: se podía mandar **toda salida de tela al endpoint de avíos** sin que nada se pusiera rojo). **R3 APROBADO.** El fail-open se cerró con un **conjunto de ids visitados** —medido en las **dos** direcciones: reinstalar el tope reproduce el fallo en el 20 exacto, y la alternativa «fail-closed al agotar» **rompe una cadena legítima**— y el reviewer le buscó borde propio con **6 casos que el barrido del repo no cubre** (auto-ciclo, ciclo que vuelve al movimiento de partida, forma «rho», cadenas de 200 por ambos lados): **los 6 pasan**. Sus tres mutaciones de la pantalla **mueren**, más **tres nuevas suyas**. ⭐ **Y verificó la FUSIÓN con `prueba` con su propio diff**, que era donde algo podía romperse sin verse: `movimientos-pt.ts` quedó **exactamente** la versión de `prueba` + 7 líneas (1295+7=1302), con lo de la 0.138 —tope de renglones, `refine desde ≤ hasta`, ventana de 12 meses, `saldosAntesDelPeriodo`— **vivo y entero** en la función de al lado; el `numstat` de los 33 **no tiene ni una eliminación ajena**; y el contrato pasó de **657 a 659** operaciones con **cero eliminadas** y la del kardex **byte-idéntica**. Cierre: unit backend **3460/3460**, frontend **2294/2294**, integración de inventarios **235/235**, regresiones **426/426** y **677/677**, contrato regenerado byte-idéntico |
> | **0.105** ✅ *(entregada como **0.095**)* | 🔴 **LOS PERFILES DEJAN DE HEREDAR POR CASCADA** (Daniel 3-sep: *«los permisos por cascada no son funcionales. Así lo hice en la primera versión de Access y luego lo modifiqué por permisos concretos… puede haber alguien que tenga el permiso A pero no el B, y otra persona el B pero no el A»*) | Daniel 3-sep ✅ **MEDIDO:** el **motor NO es cascada** —`RolPermiso` N:M explícito, `UsuarioRol` N:M con UNIÓN, guardia = `permisos.has(clave)`, **cero** comparaciones de nivel— ⇒ **su escenario A-sin-B ya funciona hoy y no se toca el motor**. 🔴 **La cascada vive SÓLO en el seed** (`prisma/seed.ts:111`, `sin(base,…)`): corriendo `definirRoles()`, **76 de 122 permisos no se le restan a NADIE** ⇒ los tiene todo perfil salvo `Basico`. **Secretarial puede HOY**: `compras.autorizar`, `esma.cargo-validar`, `pedidos.modificar`, `notas.cancelar`, `inventario-pt.mover`. **De los 76, 22 pueden hacer daño; 54 son de sólo ver.** 🔑 **El mecanismo es un guardián al revés: el silencio OTORGA.** ⇒ voltear el seed de **RESTA a SUMA** + prueba que truene si un permiso queda **sin atribuir**. 📌 Daniel (3-sep): **los perfiles concretos se arman AL FINAL**, con los puestos reales; esta fila construye **el mecanismo**, no el contenido. ⭐ **Y su petición de «un permiso suelto además de su perfil» YA SE PUEDE**: un usuario lleva **varios perfiles** y sus permisos son la **unión**. **Habilita 0.104 y 0.109**  ✅ **CERRADA (3-sep, v0.095).** El seed pasó de **RESTA a SUMA**: los 9 perfiles declaran **lo que tienen**, no lo que les falta ⇒ **un permiso nuevo nace en NADIE** hasta que se le nombre dueño **con su razón escrita** (`SOLO_ADMINISTRADOR` y `NUEVOS_DESDE_LA_FOTO`, ambos con `razon` obligatoria **por el tipo**). **El reparto NO cambia**: los 9 conjuntos quedan idénticos —`122·122·100·94·85·76·76·76·0`, verificado por dos fuentes independientes— porque es un cambio de **FORMA**. ⭐ La atribución **muere en los DOS sentidos** (clave nueva sin dueño · clave fantasma) y **`Basico` en cero es incondicional**. ⭐⭐ **El coder desmintió el encargo dos veces, con razón**: la prueba de huérfanas pedida **no podía fallar nunca** (Administrador lleva las 122) y **`SEED_ON_START` NO hace falta**. 🔴 **CUATRO rondas, y el hallazgo mayor no fue el código sino la PROSA**: 13 comentarios describían la cascada muerta y **TRES eran falsos de dato** — uno llevaba **ocho días** mintiendo en el catálogo de permisos (§Post-F9.123, `862cdb41`). *La misma enfermedad de la fila, en la documentación.* 📌 Deja viva la **0.120**. **Sin migración, sin permisos nuevos, sin `SEED_ON_START`** |
> | **0.108** ✅ | 🔴 **`npm run gates` PUEDE REPORTAR VERDE UN OOM** (lo detectó de paso el reviewer de la 0.094) | 3-sep ⚠️ **PARCIALMENTE DESMENTIDA AL MEDIRLA**: el coder forzó un OOM real (`--max-old-space-size=40` → **EXIT 134**) y `gates.ts` **sí lo traduce y retiene la frase de verde** (probado en `gates-locales.test.ts` para 134 y 137). **Lo que SÍ es real**: si el proceso muere **antes de imprimir el resumen**, no hay `✗` ni frase — y ahí *«si no ves la frase, no pasó»* vive **sólo en la prosa**. Además el verde del reviewer corrió con **8 GB heredados del contenedor**, no elegidos. ✅ **v0.100 (3-sep):** `gates.ts` ya no puede reportar verde un paso que murió sin resumen: cada gate declara su **huella de fin** (`huellaDeFin`, el texto que prettier/vitest imprimen sólo al terminar) o dice por escrito por qué no la tiene (`sinHuellaPorque`, y el tipo lo obliga); un `exit 0` sin huella es **«TERMINÓ SIN RESUMEN… NO es un pase»** con exit real 1 (`esPase = codigo===0 && evidencia`); la salida se canaliza en vivo con cola acotada (medido: 200 000 líneas / 195 MB íntegras a 88 MB de RSS); imprime el heap con el que corre; `process.exitCode` en vez de `process.exit`. Reviewer independiente APROBÓ (OOM real 134 y SIGKILL 137 reproducidos; 200 000 líneas / 195 MB íntegras a 88 MB de RSS). Ronda de corrección: (R1) el motivo del gate mudo se imprime en su línea; (R2) la huella se exige por el COMANDO real del `package.json` (lista blanca `prettier|vitest`, hay que alimentarla), no por el nombre del script; (R3) cola por flujo, stdout y stderr separadas — 🔴 **el reviewer demostró que R3 era un ROJO FALSO reproducible**, no teórico: con las escrituras separadas 60 ms, la cola compartida daba `TERMINÓ SIN RESUMEN` sobre una corrida perfecta, y su mutación fiel pasaba 30/30 ⇒ se extrajo `correrComando` y una prueba con **proceso real** (`gates-locales.proceso.test.ts`, spawn de `node -e`) la mata. **32 pruebas, 7 mutaciones muertas.** **El lint completo del backend no se reprodujo localmente por memoria (fila 0.092) — lo juzgó el CI.** |
> | **0.109** ✅ | ⭐ **COBRAR EL FALTANTE AL MAQUILERO — Y QUE DEJE DE APARECER COMO PENDIENTE** (Daniel 3-sep). Definió los términos: **faltante** = *«de 1000 entrego 995 y faltan 5»*, **nunca volvió**, y **se le cobra**; **incompleta** = *«regreso las 5 sin confeccionar, pero ahí están»*, y **no se cobra** | Daniel 3-sep 🔴 **MEDIDO — la regla existe en la prosa y NO en el código**: `incompletas.ts` documenta las **cuatro cubetas** (`enviado = primeras + segundas + faltantes + incompletas`) y dice que el faltante *«se le cobra»*, pero **`esma/cargos.ts` no menciona `faltante` ni `pendiente`: cero**. 🔑 **Y hoy es imposible**: `pendiente = enviado − (buenas + incompletas)` ⇒ **faltante ≡ pendiente, el mismo número**, así que cobrar no baja nada y la lista crece para siempre. ⇒ **tercera cubeta de saldado**, con **columna propia** como `cantidadIncompletas` (queda fuera de producción/inventario/pago **por construcción**). **Diseño cerrado con Daniel:** un botón **«cerrar la orden»** (*«se cierra por orden»*), que **lo aprieta quien recibe, no él**; salda **siempre** el pendiente y **PROPONE** el cargo esperando su visto bueno — **nunca cobra solo**. Dos desenlaces: **cerrado y cobrado** o **cerrado y perdonado**; los dos limpian la lista. **Reversible** (D3). ⚠️ Una orden puede tener **varios maquileros vivos** ⇒ un cargo por cada uno. ⭐ **Es el MISMO acto que congela el costo (0.061)**. ⛔ **CERO pantallas nuevas**: el estado de cuenta ya es una *«línea de tiempo unificada de los 4 conceptos con la marca de pendiente de revisión»* ✅ **v0.105 (3-sep):** `CierreMaquilaOrden`+`Det` por orden × maquilero × proceso (fecha, desenlace cobrado/perdonado, precio congelado, motivo, deshacer suave); la cubeta `cantidadFaltantes` en tabla APARTE de `EtapaMovimientoDet` para que nada que produzca, inventaríe o cobre la sume; **`pendiente = enviado − devuelto − saldado`** con el tercer parámetro OBLIGATORIO y todos los consumidores actualizados (WIP, tope del recibo, existencias del maquilero, `kpi_wip`, Resumen). 🔴 **El cobro es un DESCUENTO, no un cargo** (el coder lo midió contra la fórmula del saldo: un cargo le SUBE lo que se le debe; y es la palabra de Daniel en §Post-F9.147) — nace `capturado` y se aprueba con el flujo de revisión de la 0.115, cero pantallas nuevas. Botón en la captura del recibo; piezas/precio/importe los deriva el servidor; el maquilero saldado desaparece del selector y de Existencias. Migradas sin precio: saldan sin proponer cobro (REGLA 0-B). Sin permisos nuevos. Migración aditiva + DROP/CREATE de `kpi_wip` con `faltantes_saldados`. Gancho 0.061: evento `cierre-maquila-resuelto`. Decisiones tomadas solas en **§Post-F9.190**. Reviewer independiente APROBADO 🔴 **RECHAZADA en 1ª re-verificación** (4 bloqueantes, dos de dinero): el diálogo usaba la suma PLANA con celdas negativas y el servidor Σ máx(0, celda) ⇒ el número aprobado ≠ el escrito, y las órdenes migradas con total ≤ 0 no se podían cerrar (la MISMA trampa ya cazada en `AvanceProduccion.tsx:1146`) → un solo helper `celdasSaldables`/`totalSaldable` para las dos caras; cancelar un envío no contaba los CIERRES vivos (enviado 0 / saldado 100 / pendiente −100, y el descuento cobrando un envío inexistente) → guard bajo el mismo lock; carrera revisar↔deshacer (`updateMany` condicional en los dos lados, y `revisarMovimiento` filtra `canceladoEn`); `CLAVE_WIP` sin invalidar. 🔴 **RECHAZADA en 2ª**: el barrido «mecánico» dejó viva la fórmula vieja en el WHERE de Indicadores (`kpis.ts:391`, tres copias en un archivo + gemela en `resumen.ts`) y los impresos Excel/PDF del WIP dejaban de cuadrar → columna «Saldados» y prueba sobre la TABLA (un PDF con una celda de menos sigue siendo `%PDF-` válido). 3 rondas, 16 mutaciones medidas, 3 describes de integración nuevos que el CI juzga con Postgres. Decisiones tomadas solas en **§Post-F9.191** |
> | **0.110** ✅ *(entregada como **0.096**)* | 🔴 **LA FACTURACIÓN NO PUEDE QUEDAR SIN DEFINIR** (Daniel 3-sep: *«es un campo **obligatorio** de llenar. A fuerzas hay que definir si es con, sin o ambas»*) | Daniel 3-sep ✅ **MEDIDO — sus tres valores YA EXISTEN con esos nombres**: `Proveedor.modalidadFacturacion` = `solo_con` · `solo_sin` · `ambos`. 🔴 **Pero es NULLABLE**, y `conFactura` en `AbonoMaquilero`/`DescuentoMaquilero`/`PagoMaquilero` **también** ⇒ un movimiento se puede guardar **sin decir** si lleva factura, justo donde él dijo *«no deben mezclarse»*. El *«tercero»* que no entendía es el **filtro de la vista**, legítimo **sólo si el dato de abajo siempre está clasificado**. ⇒ obligatorio en el proveedor **y** por movimiento; **derivado** cuando el proveedor es de un solo tipo. ⚠️ **REGLA 0-B**: obligatorio para lo NUEVO; lo migrado en null **se tolera**  ✅ **CERRADA (3-sep, v0.096).** ⭐ **El mecanismo ya existía casi entero** —`resolverConFactura` ya derivaba en `solo_con`/`solo_sin` y ya LANZABA en `ambos`— y lo único abierto era su rama `default`: sin modalidad, el movimiento se guardaba **sin clasificar**, y ese blanco **ya contaba como «sin factura»** (`convivencia-esma.ts:96`). **Un guardián con la salida de emergencia abierta.** 🔴 **Y el campo NO EXISTÍA en la pantalla de alta** ⇒ **todo proveedor creado por la UI nacía sin clasificar**: no era olvido, es que no se podía poner. ⭐⭐ **La rama gemela real estaba donde nadie la buscaba: el MOTOR de terceros**, cuyo `esquemaMovimientoTerceroCrear.esFiscal` traía **`.default(false)`** ⇒ la ruta REST paría «sin factura» en silencio — y **el propio proyecto ya lo había denunciado** en `esquemas/cxp.ts:68-74`; el motor se había quedado atrás. **Cerrar sólo EsMa habría pasado en verde.** 🔑 **Y el reviewer probó que el arreglo del motor no lo sostenía nada: DOS mutaciones sobrevivían las 2,421 pruebas** —devolver el `.default` y desconectar el resolvedor— *«la puerta cerró bien, pero nada sujetaba el cable entre la puerta y el marco»*. Cerradas con una unit (que asevera `'esFiscal' in datos === false`, no sólo `undefined`) y dos de integración con **fixture que DISCRIMINA** (`solo_con`, para que el valor derivado y el sin derivar difieran). ⭐ **Y el coder halló una gemela dentro de su propia función**: `esFiscal` se escribe en **dos** sitios —la fila y **la bitácora**— así que se podía dejar el dato bien y **la auditoría mintiendo** (A7). **13 mutaciones, 13 muertas.** 📌 Destapa la **0.124**. **Sin migración, sin permisos nuevos** |
> | **0.124** ✅ | 🔴 **DOS CAMPOS CONTESTAN LA MISMA PREGUNTA — y nada impide que se contradigan** (nace el 3-sep de la 0.110; el coder lo declaró fuera de alcance **con razón** y el reviewer lo confirmó). `Proveedor.factura` (booleano, §Post-F9.22 — lo usan **entrada de tela y el CFDI**) vs `Proveedor.modalidadFacturacion` (enum `solo_con`·`solo_sin`·`ambos` — lo usan **EsMa y CxP**) | 3-sep 🔴 **CONSECUENCIA EN DINERO, medida por el reviewer:** un proveedor con `factura=false` + `modalidadFacturacion='solo_con'` manda **una entrada de tela** a la relación de pagos (`esFiscal:false`) y **una captura manual de CxP** al estado de cuenta del banco (`true`) ⇒ **sus pagos se parten por los dos caminos según por qué puerta entraron**, que es justo el reparto que §Post-F9.186(a) usa para decidir de dónde sale el dinero. ⚠️ **La 0.110 AGRAVA su visibilidad**: el alta ahora pregunta las dos cosas en la misma pantalla. Mitigado con un **aviso NO bloqueante en los DOS sentidos** —bloquear chocaría con la REGLA 0-B: los migrados llegan sin modalidad y quedarían encerrados; *avisar informa, bloquear ata*—. 🔑 **Pero el arreglo de fondo NECESITA DECISIÓN DE DANIEL**: si la modalidad ya dice «con, sin o ambas», **¿para qué sigue existiendo el booleano?** La lectura del coder y del reviewer es que **sobra y debería derivarse** (`solo_sin ⇒ no emite`). ⚠️ **NO se arregla como parte de otra fila**: tocar `Proveedor.factura` mueve `admiteCfdi`, `exigirProveedorQueFactura` y todo el camino del CFDI de entrada de tela — radio de explosión mucho mayor. **Medir enumerando los consumidores de cada campo, leyéndolos** | ✅ **DANIEL DECIDIÓ (3-sep, §Post-F9.188(d)): SE CORRIGE** — *«es un error que existan»*. 📌 **No nació de las dos sesiones en paralelo**: los dos campos ya existían al arrancar el día (`schema.prisma` 666 y 727), de dos fases distintas (F1-E1B/R15 puso `factura`; **EsMa F6-E4** —no Finanzas— puso `modalidadFacturacion`); el #290 los **encontró**, no los creó. **Diseño:** una sola verdad, `modalidadFacturacion`; `factura` deja de capturarse y se **deriva** (`solo_sin`→false, `solo_con`/`ambos`→true) donde aún se lea (entrada de tela, CFDI). ⚠️ REGLA 0-B: sin backfill; lo migrado que sólo traiga `factura` se tolera ✅ **v0.106 (4-sep):** una sola verdad = `modalidadFacturacion`; `factura` **se deriva** (`solo_sin`→false, `solo_con`/`ambos`→true, `null`→sin definir) en una función única del dominio que usan TODOS los consumidores que antes leían la casilla (entrada de tela, CFDI, `admiteCfdi`/`exigirProveedorQueFactura`, CxP, EsMa, corrida); `factura` sale del contrato de escritura y del formulario; la columna se conserva como histórico (REGLA 0-B, sin backfill; lo migrado sin modalidad se tolera como sin definir). El aviso de contradicción de la 0.110 ya no tiene qué comparar. ⚠️ **La regla R15 `factura ⇒ RFC + régimen` (un `.refine`) desaparece sin remapear** —casi todas las altas de `prueba` y los ~155 migrados (todo lo fiscal al 0 %, §Post-F9.188) la habrían chocado (REGLA 0-B), y `regimenFiscalSat` hoy no decide ningún cálculo (su único destino es el CFDI propio, R14); el RFC se sigue exigiendo al capturar un CFDI, y RFC/régimen llegarán por la constancia (0.119). Prueba roja inicial: el mismo proveedor con los dos campos EN CONTRA por las dos puertas. Contrato regenerado (dos `factura?: boolean` menos). Sin migración, sin permisos 🔍 **Reviewer independiente**: ingeniería APROBADA a la primera (todas las afirmaciones del coder reproducidas, dos mutaciones con rojas exactas, generados byte a byte; la caída de la regla R15 dictaminada CORRECTA y sin aviso interino porque el RFC sigue exigido en los tres embudos donde nace un cargo fiscal); RECHAZO sólo por remates, arreglados en la misma ronda: una fixture con forma de persona real en un test del frontend (repo público, cicatriz 0.123) y cinco afirmaciones que el cambio dejaba falsas en `DECISIONES.md`, `HOJA-DE-RUTA.md` y `docs/cambios-frontend-daniel.md` —una de ellas mandaba a Daniel a revisar una casilla que ya no existe. `factura` sigue en la SALIDA del API, derivada: deuda anotada en §4. |
> | **0.111** ✅ | **LO PENDIENTE DE LA DECISIÓN DE DANIEL SE VE JUNTO AL SALDO** (Daniel 3-sep: *«cada semana me pueda meter a algún lugar donde estén todos los maquileros que tengan algo pendiente por pagar o por descontar»* · *«**no quiero otra pantalla** para ver los pendientes»*) | Daniel 3-sep 🔴 **MEDIDO — el tablero YA EXISTE** (`saldos-todos.ts`: saldo de cada maquilero activo, drill-down al estado de cuenta, segmentable por facturación). **Pero su fórmula es `Σcargos VALIDADOS + abonos − pagos − descuentos`** ⇒ **todo lo que espera la decisión de Daniel es invisible**, tanto en el tablero como en la pantalla donde va a pagar. Contablemente correcto, pero **su trabajo pendiente es justo lo que no se ve**. ⇒ **no es una pantalla, es una columna**: *«pendiente de tu decisión»* junto al saldo. **Depende de 0.109** ⏳ **Daniel va a explicar el proceso semanal completo** — *«es una de las pantallas más importantes del sistema, debe estar muy bien hecha»*; incluye **anticipos**, que **no existen** hoy (hay `abonos`, sin amortización contra trabajo futuro) 📌 **El CORTE del tablero («saldo ≠ 0 o pendiente ≠ 0») llega con 0.115**; a esta fila le queda la **columna** con el importe pendiente junto al saldo ✅ **v0.113 (4-sep):** el bloque «por revisar» del tablero, la bandeja de CxP y la corrida cuentan también los **cargos `propuesto`** (recibos de maquila y, desde la 0.114, cortes y empaques): definición única en `formula-saldo.ts` (`DEFINICION.cargo.pendiente` = `estado='propuesto' AND sin_costo=FALSE`, consumida por `WHERE_PENDIENTE_CARGO`, `sqlPendiente('cargo')` y `pendienteDeRevisionCargo`) y **valuación única** en `esma/cargo-propuesto.ts` (precio de referencia por proceso —costura `maquilaOrd`, resto `aplicacionOrd`, caída al `precioPactado`—, y para un **servicio** sólo su `precioPactado`; la forma TypeScript y la SQL salen del mismo sitio); `armarPendiente` suma él mismo `partidas = planas + cargos` para que ningún camino se olvide de los recibos; `pendienteParaSalida` oculta importes y nunca los conteos. 🔴 **Hallazgo del camino**: al lote de la bandeja le faltaba `OR cp."id_maquilero" IS NOT NULL` — sin eso, el maquilero con SÓLO recibos por validar jamás habría llegado a CxP. Un cargo propuesto tiene `conFactura` NULL ⇒ cae en la relación **sin factura** (criterio único). Sin migración, sin permisos, sin seed. Reviewer independiente **APROBADO**: *«las tres puertas del dinero leen del mismo agregado y valúan igual: ocho mutaciones murieron y la integración completa, a solas, dio 164/164 y 3097/3097»*. Rechazó una ronda por las cuatro descripciones viejas que sobrevivían en el contrato publicado, cerrada con cruce mecánico (cero ocurrencias de la frase vieja en todo el repo). |
> | **0.112** ✅ | **LOS DATOS DE PAGO DEL PROVEEDOR: BENEFICIARIO Y VARIAS CUENTAS** (nace el 3-sep de leer el Excel real de Daniel, `08_28_26.xlsx`). 🔴 **Hallazgo que él no había mencionado: el BENEFICIARIO casi nunca es el proveedor** — «CESAR VICTORIA 1», «LEOPOLDO DIAZ» y «ARTURO LOPEZ» **se depositan cada uno a OTRA persona** (nombres reales omitidos: son datos personales). Y **«CESAR VICTORIA 1/2/3» NO son tres proveedores: es UNO con tres cuentas** (Daniel lo corrigió), partido en tres renglones **porque Excel no sabe modelar otra cosa** | Daniel 3-sep 🔴 **MEDIDO:** `Proveedor` sólo tiene `formaPago`·`metodoPago`·`banco`·`clabe`·`obsPago` ⇒ **UNA cuenta, SIN campo de beneficiario, sin distinguir CLABE de tarjeta**. La relación de Daniel **no cabe**. ⇒ entidad propia de **destinos de pago** 1—N: `beneficiario` · `banco` · `tipoCuenta` (CLABE/tarjeta) · `cuenta` · **alias** (su «1», «2», «3») · **una default** · las demás como **historial reutilizable** (textual suyo). ⚠️ **REGLA 0-B**: lo migrado sin datos bancarios **se tolera**. 📌 **VA ANTES de 0.113**: sin beneficiario y sin varias cuentas, la corrida **no puede generar la lista del banco** 📌 **Daniel decidió (3-sep, §Post-F9.188(b)): la cuenta fiscal queda LIBRE**, no se fuerza a una sola por proveedor; la guarda de 0.113 será «el pago con factura sale a *una* cuenta fiscal» ✅ **CERRADA (3-sep, v0.098):** entidad `ProveedorCuentaPago` 1—N (beneficiario · banco · CLABE/tarjeta · cuenta · alias · esFiscal · esDefault · notas · activo) con CRUD dominio→API→UI en la pantalla del proveedor. **Una sola default por proveedor la garantiza la BASE** (`esDefault` true/NULL + índice único; probado tirando el índice). Retirar = historial reutilizable (D3). Modo edición de cuentas. Los campos viejos `banco`/`clabe` dejan de leerse sin borrarse (REGLA 0-B). **Tres rondas de reviewer**: la 1ª cazó ~50 nombres reales sembrados en código/esquema/migración (→ «TALLER NORTE 1/2/3» con notas 🔒), la falta de edición, la lista que no se refrescaba y una grieta de A7; la 2ª cazó que la corrección de la lista **reseteaba el formulario del proveedor** (N1); la 3ª rompió a mano las correcciones y aprobó. Una migración, sin permisos, **sin `SEED_ON_START`**. Fusionada con #290 conservando las dos cosas en el formulario |
> | **0.113** ✅ | ⭐ **LA CORRIDA SEMANAL DE PAGOS Y EL CONCENTRADO — mata los dos Excel** (Daniel 3-sep, explicando su proceso real: producción le pasa un Excel de maquilas, él revisa recibo por recibo, arma **otro** Excel y se lo manda a finanzas; **una relación con factura y otra sin**). *«Quiero automatizar todo ese proceso y **quitar por completo todos los reportes de Excel**… al final debo tener una relación con todo lo que se paga en efectivo y transferencia de todo lo que yo mismo definí»* | Daniel 3-sep ⭐ **YA CAMBIÓ SOLO, medido**: Daniel retecleaba cantidad y precio *«para evitar algún error… en lugar de poner 100 ponen 1000»*. **En v2 eso es IMPOSIBLE**: `registrarReciboMaquila` **topa** el recibo contra lo pendiente bajo lock (`recibos.ts:373,746-747`) ⇒ **su reteclear era compensación de un defecto de Access**; su paso pasa a ser **revisar y ajustar**. ⚠️ Lo que el tope NO caza y sí necesita sus ojos: **capturar de menos** y **el precio**. 🔑 **La frontera NO es «maquila vs. lo demás», es: el monto nace de una ORDEN (costura, arte, corte, empaque) vs. de un ESTADO DE CUENTA (transportistas y demás)** — corrección de Daniel. **El concentrado lo PRODUCE el sistema** (sólo los que llevan monto, ordenados por monto, **totales separados de efectivo y transferencia**); las columnas de fórmula desaparecen. **Un pago = una cuenta = UN renglón**; partir un pago son **dos pagos y dos renglones** (*«así debe salir en la relación para poder hacer las dos transferencias»*) ⚠️ **no colapsar por proveedor al armar el concentrado: rompería las transferencias**. **Finanzas (Lupita) lo CONSULTA en el sistema.** 📌 Depende de **0.110** (sin facturación obligatoria un pago caería fuera de las dos relaciones) y **0.112** ✅ **v0.102 (4-sep-2026):** `CorridaPago`+`RenglonCorridaPago` (ciclo borrador→cerrada→ejecutada; dos por semana con/sin; el renglón **congela el destino**: beneficiario, banco, cuenta, alias, `cuentaEsFiscal`); el monto lo teclea Daniel; el pago nace **al ejecutar** en UNA transacción (maquilero → `PagoMaquilero` a cuenta sin aplicaciones, `revisado`; proveedor → `MovimientoTercero` vía CxP; concepto → la corrida es su registro); forma de pago default por proveedor (`formaPagoPreferida`) y cambiable por renglón; guarda fiscal bloquea; **una pantalla por rubros** = tablero de saldos + columna «a pagar esta semana» + predeterminados en cero + agregar concepto + **relación ejecutable** con cuenta completa + confirmación con totales al ejecutar; concentrado Excel/PDF; `concepto` y `referencia` por renglón (estructura del archivo real de finanzas). **Asimetría con/sin unificada** en `formula-saldo.ts` (`whereSegmentoFactura`/`sqlSegmentoFactura`, «sin» = false o sin definir) con guardia de recorrido del árbol. 4 permisos ⇒ **`SEED_ON_START`**. Migración `20260903190000_la_corrida_semanal_de_pagos` (4 enums, 4 tablas, 1 columna, CHECKs, índices y UNIQUE de idempotencia). 🔴 **RECHAZADA en 1ª ronda** (6 bloqueantes): el servidor **se creía el `origen` que mandaba el cliente y con eso elegía el libro** (EsMa/CxP) → lo deriva el dominio; un nombre de pila real en el esquema; guardia del segmento de **lista fija** (reincidencia de 0.115) → recorrido del árbol, que de estreno cazó un caso legítimo (`con_factura` NOT NULL); exclusividad del renglón y los 5 CHECK **sin prueba** (el «22/22» no pudo medirse sin Postgres); la relación ejecutable existía en el API **sin botón**; TSDoc que afirmaba un corte que el código no aplicaba (decisión: TODA la cartera, ordenada por lo que pide decisión). ⭐ **2ª ronda APROBADA**: el reviewer verificó los seis rompiendo cosas (dos intrusos propios en otras carpetas cazados por la guardia nueva; el origen cruzado afirma el LIBRO, no el campo; 4 mutaciones más), y confirmó que romper el derivado del origen deja el suite unitario en verde ⇒ **el CI con Postgres es condición del merge, no trámite**. Seis remates de esa ronda cerrados antes del commit (`tieneSaldo` importado y no reescrito; el CHECK de idempotencia probado con un movimiento real; `cuenta_check` ejercitado; excepción de la guardia por línea, no por archivo; relación Prisma singular sobre las FKs únicas; el botón de la relación ejecutable oculto sin `consultas.ver-importes`). 34 mutaciones medidas + 40 `it` de integración por lectura. Decisiones tomadas solas con default en §Post-F9.189 (g-bis); deudas en §4 |
> | **0.114** ✅ | **CORTE Y EMPAQUE: SERVICIOS SOBRE LA ORDEN, NO MAQUILAS DE IDA Y VUELTA** (Daniel 3-sep: *«en corte **no necesitas mandar y recibir mercancía**… sólo hay que poner su **cantidad y precio** para meterlo en la OP, pero **no va y viene**. Lo mismo el empaque… **el empaque no toca el inventario**. Y el corte es donde **nace la cantidad**, pero no sale ni entra mercancía. **Simplemente sucede y ya**»*) | Daniel 3-sep ⭐ **MEDIDO — el modelo YA distingue las dos formas y nadie la había usado para el pago**: `registrarCorte` crea `EtapaMovimiento(tipo=corte, **idTipoProceso=NULL**, idTercero=cortador)`, valida el rol y **no mueve inventario**. Ese `NULL` **es** la señal de «no es maquila de ida y vuelta». 🔴 **Falta UNA cosa: corte NO escribe `precioPactado`** (la columna existe; la usa el envío a maquila) ⇒ **sin precio no nace el cargo** y el cortador no se puede pagar desde la orden. 🔴 **Empaque NO EXISTE** como acto: se construye calcado de corte. 🔑 **Regla de C&A**: la cantidad del empaque **es suya, NO se deriva del recibo** — 1000 fabricadas / 990 entregadas, se paga lo empacado y **las 10 restantes no se mueven, se quedan quietas en inventario**. ⛔ **NO convertirlos en `TipoProceso`**: los metería al flujo de envío/recibo que Daniel dice que NO son. `idTipoProceso = NULL` se queda. 📌 Alimenta **0.113** ✅ **v0.107 (4-sep):** `EsMaCargo` aprende «servicio sobre la orden» (`idTipoProceso` nullable + `servicio ∈ {corte, empaque}` con CHECK «exactamente uno»; etiqueta del cargo en UNA función para los seis consumidores); el corte persiste `precioPactado` y crea su cargo `propuesto` en la misma transacción; **`registrarEmpaque`** calcado del corte (rol nuevo `empaque`, permiso nuevo `produccion.empaque`, sin kardex, sin tope contra lo recibido —regla de C&A—, aviso ámbar no bloqueante); `corte`/`empaque` entran a los roles de maquila ⇒ el cortador cae en el rubro **maquila** de la corrida y en «Saldos de todos»; cancelar arrastra el cargo como el recibo; el WIP publica `empacado` y el panel de Avance gana la 7ª etapa. Migración `20260904080000_corte_y_empaque_servicios_sobre_la_orden` (sin tocar filas). **SIN evento RC** (el hito de empaque ya es dueño del proceso: un empaque parcial des-completaría un hito; pregunta abierta a Daniel, §Post-F9.195). ⚠️ **`SEED_ON_START` OBLIGATORIO** + paso manual: marcar «Empaque» a los talleres que empacan. Decisiones en §Post-F9.195. 🔍 **Reviewer independiente: APROBADA a la primera** (5 mutaciones propias cazadas por las pruebas, migración idéntica al diff de Prisma, paridad de permisos corte/empaque cruzada mecánicamente en los seis perfiles, D3 demostrado: el camino del empaque toca 4 tablas y ningún kardex); pidió dos frases de doc de módulo y acotar el selector de la pantalla semanal, hechas en la misma ronda. Fusionada con prueba v0.106. |
> | **0.115** ✅ | 🔴 **EL ESTADO «REVISADO» SÓLO MANDA EN LOS CARGOS — y el defecto está DUPLICADO** (nace el 3-sep de una pregunta de Daniel sobre anticipos: **él no lo buscaba, cayó midiendo su petición**). Su anticipo: *«es simplemente **un abono** y queda con **saldo negativo** el maquilero»*, y *«el encargado de producción debe poder **solicitármelo**… que lo ponga y al ver yo los pendientes me aparezca»* | §Post-F9.186(h)(i) 🔴 **MEDIDO en DOS implementaciones**: `esma/saldos.ts` y `esma/saldos-todos.ts` (SQL) filtran los **cargos** por `estado='validado'` ✅ pero **abonos, pagos y descuentos NO** 🔴 ⇒ **8 agregaciones, 6 mal**, y como cada pantalla usa la suya **arreglar un archivo pasaría en VERDE**. 🔑 El campo existe en los cuatro y **sólo manda en uno**: un guardián que parece que cuida. **Consecuencias VIVAS**: el anticipo **movería el saldo antes de que Daniel apruebe**, y el descuento por faltante (0.109) **bajaría el saldo al proponerlo**. ⭐ Cerrada ésta, **el anticipo sale gratis**: el mecanismo (`estadoRevision: capturado`) ya existe. ➕ **También el RESPALDO a la vista**: `consultarExistenciaMaquilero` ya existe (pantalla «En poder del maquilero», hoy `subVista` — **por eso Daniel no la conocía**) pero da **piezas, no importe**; falta `Σ enPoder × precioPactado` **y enlazarla desde la pantalla de PAGOS** (decisión suya). ⚠️ Dicho: ese respaldo **es referencia, no garantía** 📌 **Criterio ampliado por Daniel (3-sep, §Post-F9.188(a)):** en la **bandeja de CxP** el maquilero con todo sin revisar **NO desaparece** — sale con saldo 0 y su «pendiente de tu decisión» (corte `saldo ≠ 0 o pendiente ≠ 0`, el mismo que esta fila le pone al tablero de EsMa) ✅ **CERRADA (3-sep, v0.099):** la fórmula del saldo vive **UNA sola vez** (`dominio/esma/formula-saldo.ts`, objeto `DEFINICION` del que se GENERA el SQL; agregar una condición sin su columna truena) y **los cuatro conceptos respetan su estado de revisión** en las tres implementaciones (Prisma, tablero SQL, lote de CxP). Al medir aparecieron **cuatro** copias, no tres (`migracion/cuadre-f6.ts`, que se deja a propósito sin el filtro: compara contra Access, que no conocía la revisión). El dinero excluido **se ve** como `pendienteRevision {abonos, pagos, descuentos, neto, partidas}` en las siete superficies donde se ve el saldo (tarjeta, tablero, estado de cuenta, PDF, Excel, bandeja de CxP y estado de cuenta del proveedor), y el corte del tablero y de la bandeja de CxP pasa a `tieneSaldo ∨ hayPendiente` — por **partidas**, no por neto (§Post-F9.188(a): el maquilero con todo sin revisar NO desaparece). Una **guardia** pone rojo el CI si alguien vuelve a escribir una suma con criterio propio (detector dirigido: agregación sobre las cuatro tablas + literal del criterio; probada con una quinta copia por `FROM`, `JOIN`, sin comillas y con esquema calificado). **Cuatro rondas de reviewer**: la 1ª cazó el PDF cortando por el neto, dos ramas sin prueba y una guardia de lista fija; la 2ª, **una prueba de integración que aseveraba el contrato viejo** (el CI la habría puesto roja: ningún coder pudo correrla sin Postgres) y la guardia **evadible por `JOIN`**; la 3ª, dos textos visibles con el corte viejo. Los coders reportaron «20/20» y «38/38» mutaciones cazadas y ambas cifras resultaron **falsas al muestrear** — de ahí la regla de §8 sobre el cruce mecánico. ⚠️ **Todos los saldos de `prueba` cambian**: lo capturado sin revisar deja de sumar (REGLA 0-B, no se compensa). Sin migración, sin permisos, **sin `SEED_ON_START`**. Deuda explícita: el pie del PDF (ver §4) |
> | **0.116** ⬜ | ⏸️ post-V1 · **EL MENÚ ENCIENDE DOS OPCIONES A LA VEZ** (Daniel, con captura: *«le pico a un submenú y en lugar de sombrearme esa opción, me sombrea dos»*; y corrigiendo al lead: *«pasa en **cualquier** submenú de Desarrollo»*) | 3-sep 🔴 **CAUSA, una línea**: `NavegacionModulos.tsx:163` `pathname === ruta \|\| pathname.startsWith(`${ruta}/`)` ⇒ `/modelos` también da verdadero en `/modelos/recetas-por-revisar`. ⚠️ **El `startsWith` está a propósito** (ilumina «Modelos» en el detalle `/modelos/123`) ⇒ **la regla es: gana la MÁS ESPECÍFICA**, no igualdad exacta. **Alcance: 42 de 111 rutas (37 %)** · Desarrollo **4 de 7** (las 3 que no fallan son los padres) · peores `/esma` 10, `/indicadores` 9, `/calidad` 6. **La prueba debe fijar los 42**, aseverando **en negativo** (el padre NO se enciende) y cuidando la gemela (el detalle SÍ debe encenderlo). 🔽 **DESPRIORIZADA por Daniel**: *«es lo que **menos me importa**… **es sólo estético**, hazlo cuando ya no tenga pendientes»* — 📌 lección: **el lead la iba a hacer sólo porque era fácil y estaba medida. Lo barato de hacer no es lo valioso de tener** |
> | **0.117** ⬜ | 🔴 **BLOQUEA V1** · **LA FACTURA DEL MAQUILERO NO SE COTEJA CONTRA LO RECIBIDO** (Daniel: *«los maquileros que facturan, **reviso su factura contra lo que recibí**… todo eso lo hacemos más manual»*) | §Post-F9.186 🔴 **MEDIDO**: los CFDI **sí se importan** (F9) pero viven para los **reportes fiscales** y CxP — **cero** coincidencias de CFDI ligado a orden/recibo/conciliación. **El sistema tiene las dos mitades y no las une.** ⭐ **0.118 la SIMPLIFICA**: deja de comparar dos documentos nacidos por separado y pasa a cotejar **el que Daniel emitió** contra **su copia** |
> | **0.118** ✅ | ⭐ **EL DOCUMENTO DE FACTURACIÓN — «yo te digo qué facturarme»** (Daniel: *«nadie me factura si no le mando yo un documento con los datos con los que me tiene que facturar… **no al revés**. Y eso debe salir del sistema»*) | §Post-F9.186(k) ✅ **MEDIDO — es un IMPRESO, no un módulo**: `Proveedor` ya tiene `razonSocial`·`rfc`·`regimenFiscalSat`·`usoCfdiHabitual`·`codigoPostalExpedicion`, justo lo que pide un CFDI 4.0; los importes salen de la corrida (0.113). **El IVA va EXPLÍCITO**, no escondido como en el Excel de producción. ⚠️ **Proveedor sin RFC ⇒ NO se emite: hay que AVISAR cuál falta**, nunca inventarlo (REGLA 0-B: lo migrado viene incompleto). 📌 *Es lo que C&A le hace a él.* Depende de **0.113** y **0.110** ⏸️ **FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya quiero salir»* — hoy funciona en Excel o a mano y no bloquea operar. Ficha completa en la tabla *«Lo que espera a la fase 2»* 🔁 **VUELVE A LA V1 (Daniel, 4-sep-2026, §Post-F9.191):** al precisar que el maquilero debe facturar el total ya neto del faltante *«por eso quedamos que nosotros le vamos a dar un documento con el que va a facturar»*, este documento pasó a ser el mecanismo normal de la 0.109 para los que facturan; Daniel: *«Está bien en la fase 1»*. Orden: tras 0.114, antes de 0.117 ✅ **v0.108 (4-sep):** `dominio/pagos/documento-facturacion.ts` — un documento por RENGLÓN de la corrida (sólo `conFactura` + transferencia + maquila/proveedor + corrida cerrada/ejecutada + sin faltantes fiscales de ninguno de los dos lados; cualquier otro caso ⇒ `facturable:false` con motivo y lista de faltantes, nunca se inventa); IVA **explícito** con el monto del renglón como TOTAL (`comun/iva.ts`, tasa en un solo sitio, `subtotal + iva === total` al centavo por propiedad sobre 101 importes); `Empresa` gana `regimenFiscalSat` y `codigoPostalFiscal` (migración `20260904093000_la_ficha_fiscal_de_la_empresa`, aditiva, sin backfill; se capturan en Administración › Empresas); 3 endpoints con los permisos del concentrado (sin permisos nuevos, **sin SEED_ON_START**); PDF por pago y PDF de corrida con hoja de «no se emitieron»; el concentrado proyecta `facturacion` por renglón en 2 consultas; botón por renglón deshabilitado con los faltantes en el tooltip; **nunca** el número de cuenta en el documento; `acceso-corrida.ts` = movimiento puro de la reja para romper un ciclo de imports. Defaults a confirmar por Daniel en §Post-F9.196. ⚠️ Para que salga el primer documento, Gabriel captura régimen y CP fiscal de FR Moda en Administración › Empresas. 🔍 **Reviewer independiente: RECHAZADA en 1ª ronda** por dos aserciones del impreso que no vigilaban nada (las etiquetas se imprimen en mayúsculas; una CLABE impresa dejó la suite verde) y una columna mal rotulada en la hoja de «no se emitieron»; **APROBADA en 2ª** tras volver a meter cada defecto (5 mutaciones, todas cazadas); reja, migración e IVA verificados ejecutando; barrida mecánica de 1 732 expectativas sin hallazgos. Fusionada con prueba v0.107. |
> | **0.119** ⬜ | ⏸️ post-V1 · **LEER LA CONSTANCIA DE SITUACIÓN FISCAL** (Daniel: *«que el sistema lea toda la información… el sistema no se equivocará porque es un PDF y siempre es el mismo formato»* · *«les pediré siempre el **PDF original**, no escaneada. Si cambia el formato, te pido actualizarlo»*) | §Post-F9.186(l) ✅ **El lugar YA EXISTE**: `TipoArchivoProveedor` incluye **`CONSTANCIA`** ⇒ **Daniel puede cargarlas desde hoy**, no depende de ninguna fila. ⚠️ **Su premisa es correcta a medias, dicho y aceptado**: con el PDF original el texto se extrae exacto, pero **el riesgo no es leer mal una letra: es leer el CAMPO EQUIVOCADO** (escaneada = imagen; y si el SAT cambia el formato, un lector por posición **devuelve el campo de al lado sin trunar**). **Dos condiciones aceptadas**: mostrar lo leído antes de guardar la 1ª vez, y **decir «no pude leerla»** en vez de llenar a medias. 🔑 **Su plan de «si cambia me avisas» DEPENDE de la segunda.** ⭐ Y su regla del PDF original **se hace cumplir sola**: un escaneado no tiene texto ⇒ se rechaza ⭐ **CRECE (Daniel 4-sep, §Post-F9.192(8)):** *«para dar de alta un proveedor o cliente que factura, debemos de subir la constancia de situación fiscal»* ⇒ no es «leer un PDF»: es **la puerta de alta** de todo proveedor **y cliente** que factura (el RFC, la razón social, el régimen y el CP salen del documento oficial, no se teclean). **V1, antes del kick-off** |
> | **0.120** ⬜ | 🔴 **BLOQUEA V1** · 🔴 **`roles.administrar` SE USA COMO INTERRUPTOR DE «ES ADMIN» EN 5 MÓDULOS** (lo encontró el reviewer de la 0.105 midiendo). **Es literalmente la queja de Daniel, con nombre** | 3-sep 🔴 **MEDIDO**: `ruta-critica/rutaOrden.ts:216` · `bandeja.ts:191` · `cumplimiento.ts:104` · `compras/ordenes-compra.ts:279` · `produccion/tipos-proceso.ts`. Dar «administrar roles» concede **de pasada**: **capturar cualquier proceso de RC** (sin filtro por responsabilidad), **ver la bandeja completa** y **editar una OC ya autorizada** — y **no se ve desde ninguna pantalla**. 🔑 Es *«tener A implica B»*, justo lo que él quiere evitar. **Hoy no cambia nada** (está en `SOLO_ADMINISTRADOR`), **pero es una mina para su recorte final**. ⚠️ **Antes de codear: leer los 5 sitios** — puede que cada uno necesite un permiso **distinto**; meterlos bajo uno solo **repetiría el defecto con otro nombre**. ✅ **DANIEL DECIDIÓ (3-sep, §Post-F9.187(c)):** *«déjalo así por ahora. **Lo arreglamos cuando vayamos a producción**»* ⇒ la fila **se queda ⬜ y entra al checklist de F10**, que es cuando se reparten los permisos de verdad entre los 23 usuarios reales — el momento exacto en que el defecto deja de ser teórico. **No se toca antes.** ⭐ **7-sep, §Post-F9.218(c): lo REPITIÓ sin que hiciera falta** —*«hay que definir los permisos **una vez que vayamos a salir**»*— porque el lead volvió a preguntárselo **sin mirar que la fila ya traía la respuesta**. Sin cambio de fondo, **pero con una consecuencia de calendario que sí es nueva: «cuando vayamos a salir» ES AHORA**, con V1 a días ⇒ deja de ser un pendiente lejano y **entra en el trabajo del arranque**. 🔑 **Y dentro de él, el orden:** el arreglo de código va **antes** del reparto — mientras los cinco poderes cuelguen de `roles.administrar` **no habrá cinco interruptores que mover**, y su reparto sería una ilusión. **No se adelanta nada: se ordena.** |
> | **0.121** ✅ | 🔴 **BLOQUEA V1** · **LA ANTIGÜEDAD DE SALDOS DE LOS JUEVES** (Daniel: *«este archivo lo necesito **todos los jueves**… ahí voy decidiendo qué vamos a pagar y lo voy capturando. Me gustaría que **todo eso lo haga directo en el sistema**»*) | §Post-F9.186(j) ✅ **CASI TODO EXISTE**: `Proveedor.diasCredito` · `calcularVencimiento()` ya se llama al registrar · aging con **cubetas configurables** (las suyas, 150/200/250, **se configuran**) · los maquileros **entran a la cuenta** por `convivencia-esma.ts` y hay **vista fiscal** que filtra `conFactura`. 🔴 **EL HUECO, y CORRIGE al lead**: había dicho *«los maquileros sin antigüedad está bien, les pagas cada semana»* — **su archivo lo desmiente**: **BORDA PRINT lleva 8 días** y **BORDADOS COMPUTARIZADOS 30**. **Él SÍ los envejece**; el sistema les pone `fechaVencimiento: null` y los manda a una cubeta sin edad. 🔒 **El plazo es SIEMPRE del proveedor** (*«las inconsistencias son errores de Lupita»*) ⇒ **la fecha se calcula sola**: otro error que muere por construcción. 📌 **Lupita no debería capturar nada**: facturas por CFDI (ya existe), pagos por 0.113. ⭐ **7-sep, §Post-F9.218(a) — SE CAE LA MITAD DEL DISEÑO, y la tira él:** se le preguntó por los tramos de antigüedad (7/14/21+) y contestó *«**es irrelevante. Ni siquiera veo eso.** Solo con que pongas **los días vencidos** es suficiente»* ⇒ la pantalla de los jueves enseña **el número de días vencidos por renglón**, no cubetas (el aging por cubetas **no se retira**: sigue donde está para quien lo use). ⚠️ **Simplifica la PRESENTACIÓN, no el arreglo:** sin `fechaVencimiento` no hay días vencidos que enseñar, así que calcularla a los maquileros con plazo **sigue siendo el trabajo de la fila** | ✅ **CERRADA — v0.128 (7-sep-2026).** ⭐ **LA CAUSA NO ERA LA QUE LA FILA SUPONÍA, y se midió:** no es que «el plazo no llegue» ni que el fold se salte `calcularVencimiento` — es que **la deuda de maquila nunca pasa por el motor**. Los cargos viven en las tablas de EsMa (F6), que la convivencia de F9 re-expresa **por LECTURA sin migrarlas** (opción (b) de F9-E1), y **esas tablas no tienen columna de vencimiento** (`EsMaCargo` ni siquiera tiene columna de fecha: su fecha es su `creadoEn`) ⇒ `convivencia-esma.ts` escribía `fechaVencimiento: null` **a mano, para los cuatro conceptos**. ⇒ **El arreglo es DERIVARLA al leer** (fecha del cargo + `Proveedor.diasCredito`), con la misma aritmética del motor: `sumarPlazo`, extraída de `calcularVencimiento` a `aging-comun.ts` para que la fórmula siga siendo UNA (A1). **SIN migración, SIN columna nueva, SIN permisos, SIN seed y SIN backfill** — REGLA 0-B se cumple sola, porque un número derivado no tiene pasado que reparar; y de regalo el histórico ya migrado **también** envejece, sin tocarlo. 🔒 **Muere por construcción el error de Lupita**: la fecha no se teclea en ningún lado, y cambiar los días de crédito del proveedor re-envejece toda su deuda. ⚠️⚠️ **LA TRAMPA QUE CASI SE LLEVA EL ARREGLO, y está medida con mutación: en EsMa el `abono` es un CARGO** (suma, «cargo extra al maquilero», `SIGNO_SALDO` desde F6) mientras que el `abono` del **motor** RESTA — y la proyección etiqueta los renglones de EsMa con los orígenes del motor. Pedirle el vencimiento a `calcularVencimiento`, que decide por el **ORIGEN**, habría devuelto `null` para un renglón que **sí** envejece, **en silencio y sólo para ese concepto** ⇒ quién envejece se decide por el **SIGNO**, leído de la definición única. **Lo entregado:** (1) el vencimiento derivado en el estado de cuenta (donde había «—»); (2) `diasVencidos` por fila en **la corrida semanal, que YA ES la pantalla de los jueves** (fila 0.113, §Post-F9.189(f): *«eso puede vivir en la pantalla y de ahí ir llenando la información de pagos»*) — columna propia pegada al saldo, y **la única de referencia que vale igual para un maquilero y para un proveedor**; (3) el agregado que la calcula, en **DOS consultas, nunca N+1**, sobre las **tres** fuentes de cargo (motor · cargo EsMa · **abono** EsMa) y las **dos** de crédito. **El número es el del cargo MÁS VIEJO que sobrevive a los pagos**, aplicándolos **de más viejo a más nuevo** — la MISMA convención de `netearCubetas`, para que esta columna y la bandeja no cuenten historias distintas del mismo proveedor. **Tres estados que NO se colapsan:** `n d` · `al día` (debe, dentro de plazo) · `—` (nada que envejecer); un `0` donde va `—` se lee «no debe nada», que es lo contrario. 🔻 **Lo que NO se hizo, dicho:** la cubeta «Maquila» **sigue sin repartirse** en las cuatro cubetas del aging (haría falta que EsMa registrara por el motor) y en la bandeja `vencido` **sigue siendo sólo del motor**; §Post-F9.218(a) pide justamente eso, no retirar las cubetas. Y **un pago sigue sin amarrarse a una factura**, así que la edad es la mejor suposición posible (el saldo total nunca se ve afectado) — límite heredado y declarado en `aging-comun.ts`. **Medición: 9 mutantes lanzados, 9 muertos** (invertir el FIFO → 5 unitarias · no aplicar créditos → 5 · quitar el tope en 0 → 1 · decidir por la etiqueta en vez del signo → 1 unitaria + 1 de integración · ignorar `dias_credito` en el SQL → 6 de integración · borrar la rama del abono del agregado → 1 · quitar el segmento del SQL de días → 1 unitaria + 1 de integración · colapsar el `0` en «—» en la pantalla → 2 del frontend · devolver `null` en la corrida → 1 de integración). 🔴 **Y una mutación SE ESCAPÓ y descubrió un hueco real:** quitar el filtro `es_fiscal` del **motor** dejaba la prueba de segmento en VERDE, porque toda la deuda del fixture era de maquila —que filtra por su propia columna—; **se añadió el caso que faltaba** (factura fiscal vieja + cargo de maquila sin factura) y ahora sí muerde. ⏳ **Tres decisiones del lead con default, pendientes de ratificación de Daniel:** `DECISIONES.md` **§Post-F9.220** (a: el más viejo *que sobrevive a los pagos* · b: los tres estados · c: la columna va en la corrida que ya existe, no en pantalla nueva). **V1** |
> | **0.122** ⏸️ | 🔴 **EL PRECIO DE LISTA COMPONE MAL LOS FACTORES — cotizaría de más** (nace de medir F8 contra los Excel reales de Daniel; **F8 se construyó sin haber visto una cotización suya**) | §Post-F9.186(m) 🔴 **MEDIDO en los DOS clientes**: él divide **tres veces seguidas**, el sistema **suma y divide una vez**; con el redondeo al alza **no da lo mismo** — Milano 92.20 → él **167** / sistema **168**; C&A 83.11 → él **136** / sistema **137**. **1 de cada 3-4 modelos, un peso arriba**, en un precio ya negociado. ⭐ **El código PREDIJO esto** (`costos/precio-lista.ts:16-20`: *«si la fórmula difiere, se cambia SOLO aquí, una línea»*) ⇒ **el arreglo es UNA LÍNEA**. 🔽 Daniel lo relativizó (*«no pasa nada que tenga un peso de diferencia»*) ⇒ **NO se toca sin su visto bueno explícito**. ✅ Lo que sí está bien: los 4 factores existen y **son por cliente** (C&A 35/5 vs Milano 40/7), redondeo al alza, tela 1 + tela 2. ➕ **LA FORMA DE LA PANTALLA** (descarta el diseño obvio): **nunca salir de la cuadrícula** —*«me preguntan de un modelo mientras negocio otro»*—, llegar tecleando el código, editar en celda con recálculo al instante, **guardado continuo**, y el **desglose de avíos en ventana emergente** (sus datos **ya existen**: `PrecostoLinea` con `consumo` a 4 decimales). ⭐ **Y lo que Excel NO puede darle**: el **margen del CONJUNTO ponderado por piezas** mientras negocia — medido en su archivo: calculado **39.1 %** vs cerrado **33.3 %** sobre 39 modelos. 🔽 **DANIEL DECIDIÓ (3-sep, §Post-F9.187(b)):** *«es irrelevante que haya un peso de diferencia. Déjalo así. **Me da igual**»* ⇒ **la aritmética NO se toca**. ⚠️ **Eso cierra sólo la MITAD de la fila**: lo que sigue vivo —y es lo que él sí pidió— es **la FORMA de la pantalla** y **el margen del conjunto ponderado**. Confundir *«me da igual el peso»* con *«me da igual la fila»* perdería justo lo que pidió. ⏸️ **FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya quiero salir»* — hoy funciona en Excel o a mano y no bloquea operar. Ficha completa en la tabla *«Lo que espera a la fase 2»* |
> | **0.123** ⬜ | 🔴 **BLOQUEA V1** · 🔴 **LOS DATOS PERSONALES QUE YA SE PUBLICARON — remediar y poner la regla** (nace el 3-sep de revisar el PR #287 antes de mergearlo). El repositorio es **PÚBLICO**. 🔴 **Y el #287 SE MERGEÓ a `prueba` el 3-sep a las 07:20 UTC** —lo hizo la sesión que lo abrió, mientras esta revisión estaba en curso y con la recomendación de no mergearlo ya escrita— **así que los cinco Excel están hoy en la rama compartida**, no sólo en una rama de trabajo | 3-sep 🔴 **LO MEDIDO, abriendo el XML de los cinco:** cuentas, CLABE, tarjetas y RFC **sí** se habían quitado (cero rastros, verificado), pero quedaron **77 nombres completos de beneficiarios pegados a su monto** en `relacion-pagos-sin-factura.xlsx` (hoja «General», columna O) y **nombres de autor reales en los metadatos** (`docProps/core.xml`). ⚖️ **Lo que el #287 SÍ arregló al entrar**: retiró de la prosa tres de los cuatro nombres publicados —dos de `DECISIONES.md` §Post-F9.185 y uno más de `HOJA-DE-RUTA.md` fila 0.112—. **El cuarto lo dejó vivo** (§Post-F9.186(f)) y lo retira esta versión. ⛔ **Los archivos NO se retiraron**, por instrucción de Daniel (*«esperamos a mañana a Gabriel… ahorita déjalo así»*, §Post-F9.187(a)) — y porque quitarlos del tip **no arregla nada**: el historial y `refs/pull/287/head` quedan igual. 🔴 **Lo que sigue pendiente y NO es código:** **(i) de Gabriel** — **pedir a GitHub Support el purgado** de los objetos, que ahora hay que pedirlo por **tres** sitios y no dos: los cinco `.xlsx` **en el historial de `prueba`** (merge `209348a`), **la rama del #287** (`refs/pull/287/head` sobrevive aunque se borre la rama) y los **tres nombres de la v0.093** (commit `651f847`); y decidir si los archivos se retiran del tip. **(ii) de Daniel** — ✅ **DECIDIDO (3-sep, §Post-F9.188(c)): el repositorio se pone PRIVADO** mientras se pide el purgado; lo ejecuta Gabriel. ⏳ **Tres decisiones abiertas de Daniel sobre qué dato personal cabe en un repo público**, las tres levantadas por el reviewer y NINGUNA archivada: **(a)** los alias de proveedor **persona física** («CESAR VICTORIA 1», «ARTURO LOPEZ», «LEOPOLDO DIAZ»), que quedan pegados a información de pago — el reviewer **vota seudonimizarlos**; **(b)** los **nombres de pila de empleadas con una valoración de desempeño** («las inconsistencias son errores de Lupita»), que son cita textual de Daniel pero identifican a una persona real — alternativa: «el capturista» / «finanzas»; **(c)** si al citar un archivo se puede nombrar a quien lo manda. 🔑 **La regla que deja:** *un dato personal se quita del **archivo**, no del texto; un `.xlsx` es un ZIP de XML y hay que mirar **celdas y metadatos**; y una limpieza que no se puede **volver a correr** no es una limpieza, es una afirmación* |
> | **0.125** ✅ | **EL CATÁLOGO DE CONCEPTOS DE PAGO QUE NO SON PROVEEDORES, CON PREDETERMINADOS** (nace el 3-sep de la respuesta (c) de Daniel a las preguntas de 0.113: *«quiero dejar pagos para cosas que no necesariamente están dadas de alta como proveedores — nóminas por fuera, gratificaciones, pago de algún servicio como agua… debería de poder tener como un catálogo de otras cosas que no son proveedores»*) | §Post-F9.189(c) 🔴 **No existía en ninguna fila.** Hoy el único destinatario de un pago es `Proveedor`. ⇒ **catálogo aparte** (✅ Daniel: *«que sean un catálogo aparte, no proveedores»*): nombre · rubro · forma de pago default · **cuentas de pago con la misma forma que las del proveedor** · ⭐ **marca `predeterminado`**: los marcados **se cargan en cero en cada corrida nueva** para que Daniel les ponga la cantidad (*«caja chica, nómina por fuera… no quiero que se me vaya a olvidar»*); los demás se agregan desde el catálogo. **Prerequisito de la relación SIN factura de 0.113** (sus rubros de nómina y servicios salen de aquí). Sin ETL: arranca en cero ✅ **v0.102 (4-sep-2026):** `ConceptoPago` (nombre · rubro · forma de pago default · `predeterminado` · cuentas `ConceptoPagoCuenta` con la misma forma que `ProveedorCuentaPago`); los predeterminados **se cargan en cero en cada corrida nueva**; pantalla de catálogo con prueba de componente; permisos `conceptos-pago.administrar`/`.ver` ⇒ `SEED_ON_START`. Entregada junto con 0.113 |
> | **0.126** ⏸️ | **EL COTEJO CONTRA EL ESTADO DE CUENTA DEL BANCO — el lado CON factura** (nace de §Post-F9.186(a): *«prefiero que los pagos formales salgan del pago que esté en el estado de cuenta, no de lo que yo pongo acá»*) | §Post-F9.189(h) La corrida (0.113) produce **la lista esperada**; lo que sobre del cruce son exactamente los pagos ad-hoc («el martes urgente»), señalados para atribuir. **Proponer, nunca aplicar en silencio**: dos pagos del mismo monto son ambiguos. Hoy no existe importador de estado de cuenta bancario ni conciliación de pagos contra banco (`esma/conciliacion.ts` concilia cargos/recibos, no banco). Depende de **0.113** ⏸️ **FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya quiero salir»* — hoy funciona en Excel o a mano y no bloquea operar. Ficha completa en la tabla *«Lo que espera a la fase 2»* |
> | **0.127** ⏸️ | **EL LIBRO DE CAJA CHICA CON REPOSICIÓN** (nace el 3-sep de leer el archivo real semanal de finanzas: dos de sus tres hojas son **libros de caja** —«Caja admon» con importe fijo de 12,000 y «Caja Comercial» de 3,000— con gastos por fecha y concepto, **saldo corriente** y renglones de **reposición** que devuelven la caja a su importe) | 3-sep 📌 **«Caja chica» en la corrida (0.113) NO es un gasto: es la REPOSICIÓN** = importe de la caja − saldo corriente. Hoy entra como concepto predeterminado del catálogo (0.125) con **monto manual**; esta fila lo vuelve calculado: libro por caja (una o varias), gastos con concepto libre, saldo corriente por suma (D3), y el renglón de la corrida que **propone la reposición** de la semana. Sin ETL del histórico de Excel (REGLA 0-B). Depende de **0.113** y **0.125** ⏸️ **NACE APARCADA A FASE 2 (Daniel, 4-sep-2026, §Post-F9.190):** *«ya no quiero entorpecer más la primera versión de salida… por ahora funciona con el archivo de la caja chica, que al final no va ligado a nada de producción»*. En la V1 la caja chica es un concepto predeterminado de la corrida con monto a mano. Ficha completa en *«Lo que espera a la fase 2»* |
> | **0.128** ✅ | **VALIDAR ES DE DANIEL; CAPTURAR LO RECIBIDO ES DE QUIEN RECIBE — dos permisos, no uno** (Daniel 4-sep, §Post-F9.192(1): *«la entrada la da la persona responsable de recibos o de producción. Pero la validación sólo la doy yo. O sea, es un permiso para meter lo recibido y otro para validarlo»*) | §Post-F9.192 🔴 **MEDIDO en el repaso 0.097**: el permiso para validar cargos de maquila (`esma.modificar`, el que usa `revisarMovimiento`) **nunca se le quita a nadie** — Directivo, Gerencial, Ventas, Logística, Asistente y Secretarial pueden convertir un recibo en deuda fijando el precio; y a esos mismos perfiles sí se les quitó VER la cuenta por pagar. 0.095 sólo cambió cómo se declaran los permisos; 0.115 hizo que sólo lo revisado sume. ⇒ **separar** «capturar lo recibido» (quien recibe) de «validar / revisar» (Daniel y su círculo), en EsMa **y** en la cola de cargos; mirar de paso la **puerta lateral** («entrada sin factura» en CxP a un maquilero, hoy restringida al círculo). Sin migración; permisos ⇒ `SEED_ON_START`. **V1** (dinero) ✅ **v0.111 (4-sep):** permiso nuevo `esma.revisar` (catálogo + seed); `revisarMovimiento` y su ruta pasan de `esma.modificar` a `esma.revisar`; `esma.modificar` sigue para capturar abonos/descuentos y el override «pagada»; `esma.cargo-validar` queda como permiso propio de validar; **seed**: `esma.revisar` a Directivo (+ admin y Administración/Dirección) y `esma.cargo-validar` fuera de Gerencial/Ventas/Logística/Asistente/Secretarial — **sin migración de datos porque `sembrarRoles` sincroniza** (`deleteMany notIn` + `createMany`; medido con una int que le devuelve a mano la liga a Gerencial, re-siembra y exige que desaparezca; los roles personalizados no se tocan) ⇒ **`SEED_ON_START`**; todos los caminos que ponen `revisado`/`validado` recorridos: corrida (`pagos.corrida-armar`, más estrecho, TSDoc), cierre de maquila (descuento nace capturado), ETL (modo migración, fuera del API, TSDoc); puerta lateral de CxP medida: `cxp.administrar` sólo en administrador; frontend: «Autorizar» por `esma.revisar`, sin permiso no se dibuja; prueba de equivalencia catálogo↔seed con `RETIRADOS_DESDE_LA_FOTO`. El **CI cazó lo que las pruebas locales no podían**: cuatro archivos de integración armaban su usuario «con todos los permisos del flujo» y esa lista se había quedado sin el permiso nuevo, así que revisaban una partida como paso de arreglo y el sistema —con razón— se lo negaba; se les añadió el permiso **sin tocar ninguna aserción** (la regla nueva se mide aparte: sin el permiso rechaza, y ese permiso no alcanza para capturar). Defaults a confirmar por Daniel en §Post-F9.198. Reviewer independiente **APROBADO** tras corregir en la misma ronda su hallazgo principal: crear un pago que nacía ya «revisado» no exigía el permiso de revisar, así que la puerta nueva tenía una ventana abierta al lado. |
> | **0.129** ✅ | ⭐ **LA RECEPCIÓN CONTRA LA OC HACE NACER LA CUENTA POR PAGAR — telas Y avíos, una sola entrada para inventario y estado de cuenta** (Daniel 4-sep, §Post-F9.192(2)(3): *«la persona que recibe (a partir de una OC) mete las cantidades y precios… el precio debería de ser el de la OC, la cantidad puede variar un poco, por eso se mete a mano… es la misma entrada que se ocupa tanto para inventario como para su estado de cuenta»* · *«Lo ideal es recibir con la factura. Pero si no fuera el caso, está bien dejarla como pendiente. Todo se recibe a partir de la OC. Tanto telas como avíos»*) | §Post-F9.192 🔴 **MEDIDO en el repaso 0.097**: la recepción de material contra OC (el camino de los AVÍOS) mueve inventario y actualiza la OC **pero no toca CxP**; existe una función para hacerlo y **nadie la llama** (*«hoy es capturable a mano»*). En **telas** sí está resuelto (cuatro casos: CFDI · factura pendiente · sin factura → cargo no fiscal · sin definir) y la deuda nace al confirmar la entrada. ⇒ **igualar avíos a telas**: precio default de la OC editable, cantidad a mano; con factura → cargo provisional que la factura sustituye; sin factura → cargo no fiscal según la modalidad del proveedor (0.110). **V1** (finanzas + inventarios) ✅ **v0.110 (4-sep):** helper ÚNICO `terceros/cargo-de-entrada.ts` (los cuatro casos + cerrojo del RFC, extraídos tal cual de la entrada de tela; decide con `emiteFactura(modalidadFacturacion)` de la 0.124: `solo_sin` ⇒ cargo no fiscal inmediato, `solo_con`/`ambos`/sin definir ⇒ pendiente del CFDI); la entrada de tela lo usa sin cambiar su comportamiento; `recibirCompra` gana `precioUnit` por renglón (default el de la OC, distinto se acepta y la bitácora guarda los dos; va al kardex como costo), calcula el importe de avíos **y** libres, crea el cargo en la MISMA transacción (`refTipo = recepcion-compra`, a lo sumo uno por recepción) y `reversarRecepcion` lo cancela como la entrada de tela; salida con `precioOc`/`precioDistintoOc`, `importe`, `idMovimientoTercero` y `deuda` (`cargo-no-fiscal` · `factura-pendiente` · `sin-importe` · `en-entrada-de-tela` · `cancelada`, decidida en el servidor: una recepción reversada no enseña una deuda inexistente); la OC expone la modalidad del proveedor para el resumen previo; pantalla con columna Precio, importes y aviso de deuda. Sin migración (sólo un comentario del schema), sin permisos nuevos. `exigirRfcDelProveedor` se mudó a `terceros/facturacion-proveedor.ts` (evita un ciclo de imports). Construida primero sobre 0.105 sin la 0.124 (cicatriz: revivió la casilla `factura`); fusionada con la rama de la 0.124 y reescrita sobre `emiteFactura` antes de la revisión. Defaults a confirmar por Daniel en §Post-F9.197. Reviewer independiente **APROBADO** en la segunda ronda (los hallazgos de la primera se arreglaron en la misma); avisó que su medición fue sobre la base previa al rebase, y el cruce con la 0.114 se comprobó archivo por archivo (no se tocan) y el CI lo confirmó verde sobre la base nueva. |
> | **0.130** ⬜ | ⏸️ **post-V1.** Daniel 7-sep: *«de vez en cuando. Podría ir en V2 **sin problema**»* · **LA VENTA DE SOBRANTES Y SEGUNDAS SIN FACTURA — una salida de PT a nombre del cliente, no un proceso** (Daniel 4-sep, §Post-F9.192(6): *«clientes que les llego a vender sin factura son sólo calderos que les vendo lo que sobra (normalmente segundas). Eso ahorita lo hacemos como una salida aparte… no sería necesario llevar todo el proceso sin factura. Sólo registrar lo que sale del inventario para determinado cliente»*) | §Post-F9.192 ✅ **DISEÑO CONFIRMADO por Daniel (4-sep: *«Está bien tu propuesta para la venta de sobrantes»*):** salida de inventario PT por **«venta directa»** — cliente (marcable como *ocasional*), piezas por color/talla, precio unitario y cómo se cobró (efectivo / transferencia / queda a deber); kardex inverso auditado (D3), nota de salida sencilla, sin pedido ni orden; «queda a deber» ⇒ cargo CxC del segmento sin factura; cobrado ⇒ no toca CxC. Gemela de **0.104** para PT, **autoriza sólo Daniel**. ⭐ **HEREDA EL MECANISMO DE LA 0.104, no lo reinventes** (nota del 5-sep-2026): allí el revisor midió que un tipo de movimiento dedicado **no reserva nada por sí solo** —los tres escritores genéricos aceptan cualquier tipo que no sea `traspaso`, así que con sólo `inventario-pt.mover` (que tienen 8 perfiles de 9) cualquiera podía estampar el rótulo reservado—; la 0.104 dejó el conjunto de **códigos reservados** exportado y rechazado en los tres escritores, y filtrado del desplegable de PT. El código de «venta directa» de esta fila **entra a ese mismo conjunto**, o repite el defecto con otro nombre. CxC se queda **sin** separación con/sin (sus clientes de producción todos facturan). **V1** (inventarios «a fuerzas») |
> | **0.131** ⬜ | 🔴 **BLOQUEA V1** · **LA CARGA DE APERTURA DESDE LOS ARCHIVOS DE DANIEL — Excel de estados de cuenta con/sin factura, el corte de SINUBE y los XML** (Daniel 4-sep, §Post-F9.192(4)(7): *«Vamos a generar Exceles con estados de cuenta de los que tienen factura y de los que no… Los saldos y estado de cuenta los mandaremos aparte»* · *«Te voy a mandar un archivo de SINUBE para que veas cómo va a salir la información… los datos son irrelevantes porque se van a borrar… pero para que veas la estructura. Te mando también los XML»*) | §Post-F9.192 · D15c El ETL de apertura de F9 está **listo y nunca corrido**. Con esta respuesta cambia la **fuente**: no un corte neto, sino **sus Excel por segmento** + el archivo de SINUBE + los CFDI. ⇒ (1) recibir los tres archivos de muestra y **medir su estructura** contra el ETL (columnas, fechas por factura, RFC); (2) ajustar el ETL para que lea ESA forma, cargando **cada factura pendiente con su fecha** (la antigüedad funciona desde el día uno; idempotente); (3) correrlo con los reales **antes de producción**. ⚠️ **Lo viejo es basura** (REGLA 0-B): nada de compensar el histórico de inventarios (*«no te metas en eso»*). Los archivos con datos reales **NO entran al repo** (0.123). ✅ **DANIEL CONTESTÓ (5-sep-2026, §Post-F9.201 puntos 2 y 5):** las **muestras van ya** (un Excel con factura, otro sin, el archivo de SINUBE y unos XML) y **el corte real se hace EL DÍA DEL ARRANQUE**, no antes —cualquier corte anterior se desactualiza y habría que rehacerlo—. ⭐ **Y de los XML históricos se importan SÓLO LOS VIVOS**, textual: *«Solo los que están vivos. Tanto de clientes como de proveedores»* ⇒ únicamente los CFDI **pendientes de pago o de cobro**, que son los que forman la cartera y hacen que la antigüedad sirva desde el día uno; los ya saldados no mueven ningún saldo. **No cierra la puerta**: el cargador masivo admite el resto después si el contador los pide. 🔴 **Los archivos con datos reales NO entran al repo —ni las muestras— y el lead los ABRE y los revisa antes de usarlos**, aunque vengan presentados como irrelevantes (cicatriz del 3-sep, fila 0.123). **V1 — antes del arranque** |
> | **0.132** ✅ | **LA BANDEJA DE CUENTAS POR PAGAR EN DOS LISTADOS: Todos / Con factura / Sin factura, con totales y antigüedad partidos** (Daniel 4-sep, §Post-F9.192(5): *«Sí, está bien tu recomendación»* al default del repaso) | §Post-F9.192 🔴 **MEDIDO en el repaso**: la bandeja (la lista del viernes de «a quién le debo») sólo filtra «con saldo» / «todos»; cada renglón trae **un saldo revuelto** y la antigüedad sale mezclada; el estado de cuenta individual sí se parte en el servidor. La corrida (0.113) ya vive por segmento y el criterio único ya existe (`whereSegmentoFactura`) ⇒ **reusar**: selector de segmento en la bandeja que reparta totales, cubetas y KPIs en servidor (nunca pivote en cliente). Sin migración ni permisos. **V1** (finanzas) ✅ **v0.109 (4-sep):** `esquemaBandejaCxpQuery` gana `segmento ∈ {todos, con, sin}` (default `todos`, inválido ⇒ 400) y la salida lo devuelve; `bandejaPorPagar` sólo traduce y **pasa** el segmento a `carteraCombinadaPorProveedor` (que ya sabía partir con el criterio único: `es_fiscal` en el motor y `con_factura` de EsMa vía `formula-saldo.ts`, «sin» incluye lo sin definir), y el `resumen` es del segmento; nada más cambia (mismos cortes con-saldo/visibles, búsqueda, paginación). Frontend: segunda fila de chips con testids propios, segmento en la URL, títulos de KPIs y tabla, el clic al proveedor lleva el segmento y el estado de cuenta arranca en él (su PDF ya lo aceptaba). La bandeja no tiene impreso propio. Int (m): `con + sin = todos` medido contra Postgres **en cartera y maquila** (con un cargo EsMa `con_factura = null` en «sin»); el **vencido se netea por relación** (un pago sin factura no rejuvenece una deuda facturada), fijado con su propia prueba. Sin migración, sin permisos, sin seed. 🔍 **Reviewer independiente: RECHAZADA en 1ª ronda** por dos afirmaciones que no se sostenían medidas —un «byte por byte» falso y la invariante «con + sin = todos», que **no vale para el vencido**: el neteo de créditos ocurre dentro de cada relación, así que un pago sin factura no rejuvenece una deuda facturada—; **APROBADA en 2ª** tras verificar contra `netearCubetas` el `it` nuevo que fija ese comportamiento y comprobar que la corrección llegó a los cuatro sitios donde vivía la afirmación; 4 mutaciones propias cazadas por las pruebas. |
> | **0.133** ⏸️ | **EL PDF DEL REPORTE FISCAL SE TRUNCA EN PERIODOS LARGOS** (repaso 0.097: *«el Excel sí trae todo. Para el contador, manda siempre el Excel»*) | §Post-F9.192 ⏸️ **FASE 2 (§Post-F9.190):** hay con qué operar (el Excel). Cuando se retome: paginar el PDF o limitar el periodo con aviso |
> | **0.134** ⏸️ | **LOS PAGOS NO SE LIGAN A LA ORDEN DE COMPRA** (repaso 0.097: *«el sistema no puede decirte si una compra concreta ya se pagó»*) | §Post-F9.192 ⏸️ **FASE 2 (§Post-F9.190):** el pago vive en la cuenta corriente del proveedor (0.113) y la OC en compras; ligarlos es una aplicación pago→OC que hoy no bloquea operar |
> | **0.135** ⏸️ | **LA FUSIÓN DE ESMA DENTRO DE FINANZAS NUNCA OCURRIÓ** (repaso 0.097: *«el menú la marca como interina desde antes de F9; hoy conviven las dos vistas del mismo maquilero»*) | §Post-F9.192 ⏸️ **FASE 2 (§Post-F9.190):** con 0.113/0.115 las dos vistas ya cuentan igual (criterio único del saldo y del segmento); lo que queda es de menú y de duplicidad, no de dinero |
> | **0.136** ✅ *(CERRADA MIDIÉNDOLA, 4-sep)* | 🔴 **EL MENÚ DE TELAS Y SU IMPRESO HABLAN CON EL INVENTARIO MUERTO** (repaso 0.096, tres cosas GRAVES; Daniel 4-sep: *«todo está correcto»* al default 1) | §Post-F9.193(1) 🔴 **MEDIDO en el repaso**: el inventario de telas se rehízo de **por lote** a **por color** y las cuentas se partieron en dos; el botón «Imprimir PDF» quedó en la pantalla vieja y **lee la cuenta vieja** (`ExistenciasTelasPagina.tsx:118` · `impreso-inventario-telas.ts:33`) ⇒ hoy imprime una hoja **prácticamente en blanco sin avisar**, y la pantalla nueva no tiene botón. Y **«Kardex de materiales» y «Traspaso de materiales»** cuelgan del menú de Telas y abren sobre el motor viejo por lote (`KardexMaterialesPagina.tsx:44` · `telas.ts:748` · `catalogo.ts:1616`): quien busque los movimientos de una tela ve vacío, y un traspaso desde ahí no cambia «Existencias de telas». En agosto se movió «Ajuste de materiales» por esta misma razón y se dejaron las otras dos. ⇒ mover el impreso a la pantalla nueva (tela · color · cuerpo y complemento · por almacén) y **quitárselo a la vieja**; las dos pantallas apuntan al motor nuevo o salen del menú de Telas. Sin migración. **V1 — es lo primero de inventarios** (se descubre el peor día) ✅ **YA ESTABA CONSTRUIDA — la entregó la v0.093 (fila 0.098) el 3-sep, un día DESPUÉS del repaso que la midió rota.** El coder de esta fila midió antes de tocar y no escribió código: el impreso lee `consultarExistenciasTelaColor` (la MISMA consulta de la pantalla, mismo esquema Zod ⇒ imposible que filtre distinto), el botón vive en «Existencias por color» y la pantalla vieja tiene prueba que blinda su ausencia; «Traspaso de materiales» es hoy «Traspaso de avíos» bajo Avíos con puntero al traspaso por color; «Kardex de materiales» sigue bajo Telas A PROPÓSITO como única ventana al legado, con pestaña «Telas (lote · legado)», aviso permanente y vacío que manda al kardex por color. Mutaciones medidas 5/5 (typecheck rojo al volver a la consulta legada; pruebas rojas al mover el botón o la entrada del menú). 📌 Dos matices sin fila: el impreso NO subtotaliza por almacén (Daniel dijo «por almacén»: es una tabla plana tela → color → almacén con total general; si quiere subtotal por bodega, fila chica nueva) y el kardex legado arranca en la pestaña de tela (etiquetado, no cambiado). ⚠️ Lección: el repaso del 2-sep se contestó el 4-sep y una de sus tres cosas graves ya no existía — **antes de numerar lo que un repaso dice roto, medirlo contra `prueba` del día** |
> | **0.137** ✅ | **NADIE VERIFICA QUE EL ALMACÉN SEA DEL TIPO CORRECTO** (repaso 0.096, «otras cosas») | §Post-F9.193 El desplegable acota, pero **nada impide meter producto terminado en la bodega de telas**: la validación vive sólo en pantalla. ⇒ regla en el dominio (A1): cada movimiento exige que el almacén admita ese tipo de artículo; prueba de 400 con almacén ajeno; sin migración. **V1** (chica) ✅ **v0.104 (4-sep):** guard único `exigirAlmacenDelTipo(tx, idAlmacen, tipo, idEmpresa)` en `comun/almacenes.ts` (existe → activo → empresa → tipo; una lectura; mensaje con el nombre), **22 llamadas dentro de la transacción** en inventarios (PT, telas, partidas por color, avíos, entradas de tela), compras (recepción), notas de salida y cíclico — **ninguno de los once escritores validaba el tipo, ni uno**. NO en cancelaciones (inverso, D3) ni en el ETL. Cambiar el tipo de un almacén con movimientos se rechaza (`admin/almacenes.ts`). Cruce mecánico tipos-creados-vs-escritores en las `.int.test.ts`: 3 archivos incoherentes antes (recibían avíos en almacén TELA) → 0. 13 unit + 13 int nuevas; 3 mutantes muertos; contrato intacto. 🔴 **RECHAZADA en 1ª ronda por el despliegue, no por el código**: ni seed ni ETL creaban un almacén AVIO ⇒ recepción de compra, notas de salida y ajuste/traspaso de avíos habrían rechazado SIEMPRE en `prueba` («no lo alcanza el seed de hoy» es justo lo que §7.3 prohíbe archivar) → el seed siembra «Almacén de avíos» global e idempotente (**SEED_ON_START**), los 10 desplegables filtran por tipo, `calidad/auditorias.ts` exige `activo` (§Post-F9.194). ✅ **2ª PASADA DE PRODUCCIÓN CERRADA (v0.116, 5-sep):** los **4 sitios** a `PT` — `recibos.ts` (almacén de primeras y de segundas), `entregas-cliente.ts` (origen de la entrega) y `etapas.ts` (origen del envío). 🔴 **En `etapas.ts` no había laguna, había DUPLICACIÓN**: ya exigía PT a mano con un **segundo `findUnique` del mismo renglón** que acababa de leerse… y ese chequeo **no tenía ni una prueba** (medido por el reviewer: su mensaje no aparecía en ninguna). Pasó de no probado a probado. `rechazarAlmacenDeTransito` **se queda** y está medido por qué: el tránsito **es de tipo PT**, o sea que **pasa** la guarda nueva y aun así no vale para recibir ni entregar. ⚠️ **La trampa que hundió la 1ª ronda NO se repitió**: se corrió el seed real sobre base virgen y los tres almacenes PT existen (Primeras · Segundas · Tránsito). ⭐ **`exigirAlmacen` (la versión SIN tipo) YA NO EXISTE**: esta conversión la dejó en **cero llamadores** y se retiró — su TSDoc decía *«se conserva para los pocos casos donde el tipo lo garantiza otra cosa»*, una política con cero casos, y una puerta más débil al lado de la buena es exactamente cómo el próximo escritor elige mal (el modo de fallo que originó esta fila: once escritores, ninguno validando). Las tres propiedades que comprobaba —existe, activo, de la empresa— siguen vigiladas, **medido con tres mutaciones**. ⚠️ Las menciones a esa función en el relato de **F4-E5** (aquí y en `docs/hoja-de-ruta/F4-etapas.md`) se dejan como están: son historia, y reescribir historia para que cuadre con el presente es peor. **8 mutaciones lanzadas, 8 muertas**, incluida la del **falso positivo** (79 de 103 pruebas rojas al hacer que la guarda rechace un tipo que sí cuadra). Sin migración, sin permisos, sin seed, contrato intacto |
> | **0.138** ✅ | **EL KARDEX DE PRODUCTO TERMINADO NO SE PUEDE FILTRAR POR FECHAS — y nadie midió el volumen** (repaso 0.096, «otras cosas»: *«con diez años cargados, pedirlo trae todo»*; *«el propio código recomienda medir la consulta de existencias con los diez años cargados»*) | §Post-F9.193 ⇒ rango de fechas en el kardex (servidor, con índice si hace falta) y **medir** existencias y kardex con el histórico cargado en `prueba` antes del arranque. Sin migración salvo índice. **V1** (chica) ✅ **v0.118 (5-sep):** rango de fechas **en el servidor** (`desde`/`hasta`, bordes inclusivos, ancla en el día de México) + **ventana por omisión de 12 meses** y **tope de 1 000 renglones** (máx. 5 000) — **los dos**, porque sólo el rango deja que la pantalla siga pidiendo diez años y sólo el tope daría el pedazo más viejo. Defecto medido con el código viejo: **25 000 renglones / 8,3 MB** en una consulta ⇒ ahora **1 000 / 335 KB**. ⭐ **Lo que el rango OBLIGABA a arreglar y nadie había visto: cortar por fecha ROMPÍA la columna «Saldo»** —el primer renglón arrancaría en cero y todos los de abajo mentirían— ⇒ **`saldosAntesDelPeriodo`**, agregación en SQL crudo que siembra el saldo corrido. 🔴 **Y el reviewer cazó que el tope enseñaba EL PEDAZO MÁS VIEJO del periodo**: la base sintética tenía los folios **revueltos** (correlación **−0.0007**) mientras que en producción el folio es la secuencia atómica, o sea **cronológico** — con una base de folios reales (**0.99999997**) la pantalla decía *«2025-09-05 en adelante»* y el usuario veía **hasta 2026-02-07**, siete meses recientes escondidos. Es el propio argumento del coder («cortar 1 000 de 25 000 daría el pedazo más viejo») aplicado a su diseño. ⇒ **corte por la COLA** (folio DESC), con el saldo anterior anclado al **primer renglón visible** en dos ramas excluyentes y comparando con **la misma llave con la que la lista se ordena y se corta** — `(folio, id)`, **no la fecha**, que es la trampa de los movimientos del mismo día a ambos lados del límite. **Invariante medido**: con existencia real 13 125, el último saldo visible es 13 125 con tope 5000/1000/100/10/**1**; el reviewer lo re-midió en su propia base con **48 combinaciones** (el corte cayendo en cada posición de matrices de 3 y 4 renglones) — **intacto**. **NO se puso índice, y está medido**: cambia el plan pero **no el tiempo**, y en la consulta del saldo anterior **lo empeora** (68 → 109 ms); la selectividad ya la da `id_modelo`. **Sin migración, sin permisos, contrato regenerado.** ⏳ Queda para **Gabriel** medir el volumen en `prueba` (SQL copiable y probado en `docs/modulos/inventario-pt.md`), y **apareció fila nueva**: **existencias no tiene tope ni paginación** (56 860 filas de una vez). 🔴 **TRES vueltas con el reviewer independiente, y la clave de esta fila es que él NO se fió de ninguna medición ajena: rehízo las dos que decidían.** **R1 (7 hallazgos):** rehizo el **juicio del índice** capturando la consulta que Prisma emite de verdad (`$on('query')`) y no sólo confirmó la conclusión — la mejoró: en la consulta del saldo anterior **el índice EMPEORA** (68 → 109 ms), dato que el coder no tenía; encontró **6 mutaciones vivas en `saldosAntesDelPeriodo`**, la función de la que cuelga toda la columna «Saldo»; y midió que **el tope enseñaba el pedazo MÁS VIEJO del periodo** porque la base sintética tenía los folios **revueltos** (−0.0007) mientras el folio real es **cronológico** — con la pantalla diciendo *«en adelante»* y siete meses recientes escondidos. Además: la línea del periodo decía *«a hoy»* cuando **no hay techo** y enseñaba renglones **posteriores a hoy**; el tope `5000` vivía **duplicado** entre contrato y dominio (⇒ el OpenAPI publicado quedaría falso en silencio); y a Gabriel no le quedaba **nada ejecutable**. **R2:** ⭐ **midió el punto en disputa y SE RETRACTÓ POR ESCRITO** — construyó una base de escenarios para romper al coder (movimientos de otra empresa y otro modelo cayendo en el mismo grupo) y las cuatro guardas discutidas dieron **huella idéntica**: *«mi ejemplo era falso, y lo retiro»*; sólo `id_empresa`/`id_modelo` eran reales. Y encontró lo de fondo: **el desempate `d."id"`, la línea de la que depende TODO el invariante, era la única sin prueba** — quitarlo dejaba **56/56 en verde**, porque **todas las fixtures creaban movimientos de UN SOLO renglón** y la llave degeneraba; con una matriz color×talla —la forma **normal** de un movimiento en este sistema— el saldo decía **7 en vez de 107**: cien piezas perdidas en silencio. **R3 APROBADO**, tras barrer **48 combinaciones** con el corte cayendo en cada posición de matrices de 3 y 4 renglones (invariante intacto), comprobar que la frase vieja del contrato **no sobrevive en ningún generado** (0 apariciones) y **correr él mismo** los SQL que el coder dejó para Gabriel (`ON_ERROR_STOP=1`, exit 0). ⭐ Y descartó un mutante que sobrevivía (`take: limite + 2`) **midiéndolo antes de reportarlo**: respuesta idéntica en 52 combinaciones ⇒ **mutante equivalente, no hueco de pruebas**. |
> | **0.139** ⏸️ | **«COMPROMETIDO» Y «DISPONIBLE» EN PRODUCTO TERMINADO** (repaso 0.096, «otras cosas»: no existen) | §Post-F9.193 ⏸️ **FASE 2 (§Post-F9.190):** no bloquea operar — el pedido y la orden ya dicen qué está prometido; el disponible es una resta que hoy se hace mirando los dos. Cuando se retome: comprometido = pedido no entregado por modelo/color/talla, disponible = existencia − comprometido, ambos en servidor |
> | **0.140** ⏸️ | **EL IMPRESO DE LA ORDEN BAJA IMÁGENES SIN TOPE DE PESO — y la impresión por lote lo multiplica por cien** (nace el 4-sep del reviewer de la 0.106) | 0.106 · reviewer 4-sep 🔴 **MEDIDO**: `impreso-orden.ts` descarga sin `maxBytes` (la ficha de arte sí pasa `MAX_BYTES_FOTO_ARTE` = 12 MB), cada foto puede pesar hasta el límite de subida (50 MB) y `POST /ordenes/impresos` acepta **1..100 órdenes**: `impresoOrdenes` acumula los cien `DatosImpresoOrden` en memoria y los manda ENTEROS al worker por `postMessage` (clon estructurado ⇒ ×2). Peor caso con tope de 12 MB: 100 × 7 × 12 MB ≈ 8 GB ⇒ **el tope por imagen lo baja, no lo cierra**. La 0.106 ya redujo el riesgo (de «todas las imágenes» a ≤ 7 por orden). ⏳ **Decisión con default para Daniel:** (a) tope de **12 MB por imagen** como la ficha —una foto más pesada sale como HUECO con aviso, no se baja— y (b) el lote se arma **orden por orden** (o con un tope de órdenes por PDF, p. ej. 20), sin acumular los cien en memoria. Sin migración ni permisos. **V1** (chica, y es memoria del servidor de `prueba`) ⏸️ **SUSPENDIDA POR DANIEL (5-sep-2026)**, con el trabajo **casi terminado**: *«no es tan necesario, **casi no uso esa función**, luego la retomamos. No quiero perder tokens en eso»*. ⇒ La rama **`trabajo/el-impreso-por-lote-sin-reventar` está comiteada y empujada** con el estado de la **ronda 4 sin verificar** (el commit enumera qué quedó cerrado y qué abierto). Lo ya verificado por el reviewer al cerrar la ronda 3: tope de **12 MB** por imagen con una sola definición · el lote **se parte en varios PDFs** entregados como ZIP, sin cortar ninguna orden a la mitad · **piso por hoja** de al menos un arte y una foto (§Post-F9.202) · `application/zip` en el anti-caché · 6 pruebas de la costura PDF-vs-ZIP · **pico de 100 órdenes de ~13 GB proyectados a ~2,4 GB, sin crecer con el lote**. Abierto: tres frases falsas en comentarios, la suelta de memoria que es un **no-op medido**, el «barrido mecánico» que no barre, y la **mitad frontend de la costura sin prueba**. ⚠️ **Sale de v0.118: al retomarla, fusionar `prueba` primero.** |
> | **0.141** ⏸️ | **LA NOTA DE CRÉDITO DEL MAQUILERO QUE YA FACTURÓ COMPLETO — el caso remoto** (Daniel 4-sep, §Post-F9.191: *«los descuentos son mediante una nota de crédito si es que ya está la factura completa… es en caso remoto que ya haya hecho la factura y no pueda cancelarla»*) | §Post-F9.191 ⏸️ **FASE 2 (§Post-F9.190):** el camino normal NO es éste — el maquilero factura **el total ya neto del faltante** porque el sistema le da el documento con qué facturar (0.118). Sólo si ya facturó completo y no puede cancelar, el descuento de la 0.109 se respalda con su **nota de crédito** (CFDI de egreso): medir si la importación de CFDI acepta egresos y ligarla al descuento. Mientras: el descuento queda «por revisar» y Daniel lo aprueba con motivo teniendo la nota en la mano. Depende de 0.117/0.118 |
> | **0.142** ✅ | 🔴 **LA PARTIDA NO VIAJA EN EL TRASPASO — y por eso el aviso de tono no puede funcionar donde hace falta** (nace de la 0.101, medido por su reviewer el 5-sep-2026) | 0.101 · 5-sep 🔴 **MEDIDO, con la aritmética cerrada**: las patas del traspaso se escriben con `idPartida = NULL` (`partidas-telas.ts`, *«las patas del traspaso no son entradas de compra»*), así que en un almacén alimentado **sólo por traspasos** —el caso del **almacén del cortador**, que es justo donde arranca la pantalla de salida— `Σ entrado = 0` **siempre** ⇒ el sistema **nunca sabe de qué partidas es la tela que hay en el anaquel**. La 0.101 hizo lo único que se podía hacer sin este cambio: **decirlo** (línea neutra «no se sabe de qué partidas es esta tela»), en vez de callar —que era mentir por omisión— o de encender una alarma permanente —que era devolverle a Daniel su queja original—. ⚠️ **Pero las DOS mitades de lo que Daniel pidió siguen sin entregarse en ese almacén**: *«sólo cuando hay más de una partida»* y *«con la lista de partidas a la vista»* — donde nunca hay partidas, nunca hay lista. ⭐ **Y hay un segundo defecto que sólo esto cura**: la regla actual compara un **acumulado histórico que nunca baja** (lo que entró con partida) contra un **neto de hoy** (la existencia), así que en cuanto un almacén consume algo se queda **ciego de forma sistemática** al caso mixto — medido: 500 con partida consumidos + 300 llegados por traspaso ⇒ dice «sin riesgo» con tela de origen desconocido en el anaquel. ⇒ **que el traspaso lleve la partida** (o el reparto de partidas, si la pata mueve varias), y entonces el aviso de tono funciona **en el almacén donde se escoge el rollo**, que es el único sitio donde sirve. ✅ **DANIEL DECIDIÓ QUE SÍ (5-sep-2026, §Post-F9.201 punto 1): se construye.** Era la única de las cuatro cosas que dependían de él que **bloqueaba trabajo**. El traspaso conserva el lote de origen (y su reparto, si la pata mueve varios); **aditivo y SIN backfill** (REGLA 0-B): cambia lo que se escribe de aquí en adelante, no toca ni un movimiento ya registrado, y la tela vieja se queda sin lote —la línea neutra sigue saliendo para ella—. Se preguntó, en vez de hacerse, porque toca el **motor del kardex** y **D3** | ✅ **CERRADA — v0.122 (6-sep-2026).** ⚠️ **El número es TENTATIVO hasta el merge**: la 0.121 se la lleva la fila 0.145, que va por delante; si ésa se atasca en revisión y ésta entra antes, baja a 0.121 y hay que moverlo en los CUATRO sitios que lo llevan (esta fila, la línea 📊 de abajo, `HISTORIAL-DE-VERSIONES.md` y `frontend/src/version.ts`) — **`node herramientas/verificar-documentos.mjs` dice si quedó alguno sin mover**, y también recalcula el contador de esta tabla contra la línea 📊. ⭐ **El traspaso ahora NOMBRA el lote en sus DOS patas**: se captura igual (color + cantidad, **sin pantalla nueva**) y el dominio reparte **FIFO por folio de partida** contra el **saldo por lote del origen**, expandiendo la captura a un renglón de kardex por lote; el motor ya pasaba el mismo arreglo a las dos patas, así que la salida descuenta del lote en el origen y la entrada lo nombra en el destino. 🔴 **UNA SUPOSICIÓN DEL ENCARGO RESULTÓ FALSA Y SE MIDIÓ: esto NO tocó el motor del kardex.** La fila nació diciendo que había que tocar `comun/kardex.ts` («se preguntó porque toca el motor y D3»), y al medirlo `registrarMovimientoTela` **ya escribía** `idPartida` y `registrarTraspasoTela` **ya pasaba el mismo arreglo de líneas a las dos patas**: la fuga era **una OMISIÓN de un argumento** en `traspasarTelaColor` — llamaba `aLineasMotor(...)` sin el tercer parámetro. **`kardex.ts` no se tocó ni una línea**, y por eso la fila salió aditiva de verdad: **sin migración** (la columna `movimiento_det_tela.id_partida` ya existía, nullable, con FK e índice), **sin permisos**, **sin seed**, **sin ETL**. ⭐⭐ **Y hubo un SEGUNDO defecto que sólo esto curaba, con su propia decisión: la cuenta de lotes del aviso de tono pasó a NETO** (`Σ entradas − Σ salidas` que nombran el lote, decisión P1 del lead). Sin eso, hacer que el traspaso nombre el lote habría cambiado *«callar»* por **«avisar de más»**: la salida del origen también nombra el lote, así que sumando sólo entradas una bodega vacía gritaría «hay dos lotes, escoge». Con el neto, los dos lados de la comparación (`existencia` contra `Σ saldos`) son por fin **la misma clase de número**, que era exactamente el fallo medido por la 0.101 (500 con lote consumidos + 300 traspasados ⇒ decía «sin riesgo»). El campo del contrato se **renombró** `entrado`→`saldo` y `entradoConocido`→`saldoConocido`: dejarle el nombre viejo a un número que ahora resta era la mentira que esta casa persigue. **Efecto lateral aprovechado:** una partida cancelada ya no necesita filtro especial —su inverso copia el `idPartida` y el par se neutraliza solo en la Σ—; el `anuladoPor` que tenía la versión vieja, con el neto, **restaría dos veces**. 🔻 **LO QUE SIGUE SIN CUADRAR, dicho en el código, en el doc del módulo y aquí: las salidas a ORDEN no nombran lote** (decisión de Daniel §Post-F9.9, no se tocó) ⇒ **el saldo por lote nunca cuadra del todo en un almacén que consume**: puede listar un lote que la producción ya se llevó. Está **medido** en la integración, no sólo razonado. Y la tela traspasada **antes** de esta fila se queda sin nombre —**REGLA 0-B: aditivo, sin backfill**— y sigue saliendo por la línea neutra. **Además:** la **hoja del traspaso** sale **desglosada por lote** (número del proveedor + folio de partida, «—» cuando no se sabe), que es lo que deja casar el rollo físico con la nota. **Cuatro decisiones tomadas por el lead con default y ⏳ PENDIENTES DE RATIFICACIÓN DE DANIEL** (P1 neto · P2 FIFO automático sin pantalla · P3 la salida a orden no se toca · P4 desglose en la hoja): `DECISIONES.md` §Post-F9.206. **Pruebas:** 13 unitarias nuevas de la regla pura del reparto (+1 del formateo del lote en la hoja) —**cinco mutantes cazados** (quitar el FIFO, atar el complemento al cuerpo, no emitir el renglón sin lote, quitar el redondeo del remanente, quitar el tope del saldo negativo)— y **dos mutantes descartados por EQUIVALENTES tras medirlos**, uno de los cuales era una guarda muerta que **se borró en vez de dejarla como rama que nadie vigila**; más 5 de integración del traspaso (el lote en las dos patas, cuerpo y complemento por separado, el segundo traspaso que ya no puede repartir el lote que se fue, el remanente sin lote, y la prohibición de cancelar una sola pata) y **las 4 pruebas de la 0.101 que esta fila pone rojas a propósito, REESCRITAS** —ninguna borrada— más 3 nuevas. ⚠️ Para que el tercer estado (`origen-desconocido`) siguiera medido hubo que escribir a mano un movimiento **sin partida** en la prueba: **ninguna puerta del dominio produce ya tela sin lote por traspaso**, y sin ese helper ese estado se quedaba sin un solo caso contra la base. 🔴 **RONDA DE CORRECCIÓN — el reviewer RECHAZÓ y tenía razón; los 6 hallazgos se arreglaron, ninguno se archivó como «menor».** ⭐⭐ **El grave, medido por él contra la base: el traspaso podía escribir el LOTE EQUIVOCADO y decir `sin-riesgo`.** Como las salidas a orden no nombran lote (P3), el saldo por lote del origen queda **inflado**, y el FIFO se llevaba el folio más viejo **aunque ya estuviera consumido**: entran 500 de `L-VIEJO` → se surten 500 a una orden → entran 300 de `L-NUEVO` (lo único que hay) → se traspasan ⇒ el destino recibía **`L-VIEJO`**, el aviso callaba y **la hoja impresa salía con un lote falso**. 🔑 **Peor que antes de la fila**: cambiaba un *«no sé»* honesto —que mandaba a revisar el rollo— por una afirmación falsa con confianza total, justo lo contrario de lo que el propio impreso predica (*«decir un lote que no se sabe sería peor que callar»*). **Arreglado:** el reparto va **acotado a la existencia real** leída bajo el mismo lock (por eso `validarNoNegativoTelaColor` ahora la devuelve en vez de tirarla), quitando el déficit `Σ saldos − existencia` **del folio más viejo al más nuevo** y **por componente**. **Lo que el tope aporta, medido, y es sobre la IDENTIDAD del lote: que el lote escogido sea uno que la existencia pueda respaldar** cuando el desajuste viene sólo de consumo no apuntado; lo que no alcanza viaja sin lote. 🔴 **Y en la 3ª vuelta el reviewer cazó que la otra propiedad que yo presumía «medida» —*«nunca se nombra más tela de la que hay»*— es VACUA: la da `validarNoNegativoTelaColor`, no el tope, y su prueba pasaba con el tope BORRADO.** Reproducido por mí: 4 rojas al borrar el tope y la prueba `INVARIANTE` **verde**. ⇒ **la prueba vacua se sustituyó** por una que fija la propiedad real (*el tope cambia CUÁL lote, no CUÁNTO*), y la frase se corrigió en los **7 archivos** que la llevaban — **incluido el recuadro que Daniel lee para decidir**, donde vendía corto el propio freno y podía llevarle a contestar «mándalo sin lote» creyendo que no hace casi nada. 🔴 **Y aquí me pasé de prometer, y lo cazó una prueba mía, no el reviewer:** escribí también *«nunca se nombra un lote sin saldo»* y **es FALSO cuando además ha entrado tela sin lote** — el desajuste `Σ saldos − existencia` mezcla **dos causas de signo contrario** (consumo no nombrado, que infla; entrada sin lote, que desinfla) y **se tapan entre sí**: 500 consumidos sin nombrar + 200 entrados sin lote ⇒ el tope sólo ve 300 y le deja 200 al fantasma. La prueba que lo desmintió **se quedó, con ese nombre** (`🔻 LÍMITE:`) para que nadie vuelva a prometerlo. 🔻 **No cura la raíz** (P3 sigue): con varios lotes vivos y consumo parcial el nombre puede ser el del lote de al lado ⇒ **declarado en el código, en el doc de módulo, en el historial y en un recuadro propio de `DECISIONES.md §Post-F9.206` que Daniel tiene que leer ANTES de ratificar P2 y P3**. ⚠️ **Y el mismo tope NO se aplica al aviso de tono, a propósito**: ahí listar de más es inofensivo y esconder sería callar un aviso real — misma cifra, dos usos, dos criterios. **Los otros cinco:** (2) había **duplicado a mano `SUMAS_TELA_COLOR`**, la aritmética que `kardex.ts` centraliza con un TSDoc que promete lo contrario —quedaban **tres** copias y ese comentario se volvió falso— ⇒ **se exporta y se reusa**, con la lista de consumidores mantenida en el propio TSDoc *(la guía «no toques `kardex.ts`» cedió ante la invariante «no dupliques la aritmética»)*; (3) el TSDoc que **justificaba** el helper `entrarSinLote` era **falso por partida doble** —el ajuste de entrada del **conteo cíclico** (v0.120) sí produce tela sin lote, y cancelar una salida también, y eso lo decía **otro archivo del mismo PR**— ⇒ el helper se queda, con la razón buena (no meter dos módulos ajenos en la prueba); (4) **un hecho falso copiado a ONCE sitios** —el reviewer contó siete y al barrerlo con `grep` aparecieron **cuatro más**, incluidos el contrato y una nota que yo mismo había escrito la ronda anterior—: *«el sobrante de un conteo entra sin lote»* es falso **en las dos lecturas** (sobrante = *contado < sistema* = **salida**, y baja la existencia; el faltante entra por una entrada que **sí** crea partida) ⇒ corregidos los once y **nombrada la causa real**, que es el ajuste de entrada del cíclico; (5) comentarios que quedaron mintiendo, incluido *«sin partida, como toda salida»* —ya no: la pata del traspaso sí la lleva— y la prosa que decía «las salidas **a orden**» como si fueran las únicas ⇒ **mapa completo** de las cinco que no nombran lote; (6) un lote con **cuerpo +100 y complemento −100** se caía de la lista porque el filtro miraba la **suma** ⇒ ahora *vivo* se decide **por componente**. **Mutantes de esta ronda: 5 lanzados, 4 muertos y 1 descartado por equivalente tras medirlo** (quitar el tope → 4 rojas · déficit del más nuevo al más viejo → 3 rojas · un solo déficit sobre la suma de componentes → 1 roja). 📌 **Y una afirmación MÍA que resultó FALSA, dicha aquí porque el reviewer la cazó:** dije haber *«extendido `contar-filas.py`»* para cruzar los cuatro sitios de la versión. **Ese archivo nunca estuvo en el repo** —era un script efímero de mi scratchpad—; el resultado que reporté era correcto, pero la herramienta no existía. **Ahora existe de verdad**: `herramientas/verificar-documentos.mjs` (Node, sin dependencias) cruza los cuatro sitios de la versión, recalcula el tablero contra la línea 📊 y exige el orden descendente del historial; sale 1 con el detalle si algo no cuadra, y está probado mutando los documentos. 🔴 **3ª VUELTA — RECHAZADA otra vez, y nada de lo que bloqueaba era de conducta: eran afirmaciones de VERIFICACIÓN falsas, una de ellas en el papel que Daniel lee para decidir.** Los 7 hallazgos, arreglados: (1) ⭐⭐ **la propiedad que yo presumía «medida» era VACUA** —*«nunca se nombra más tela de la que hay»* la da `validarNoNegativoTelaColor`, **no el tope**, y su prueba pasaba con el tope BORRADO (reproducido: 4 rojas y ésa verde)— ⇒ **prueba sustituida** por *«el tope cambia CUÁL lote se nombra, no CUÁNTO»*, que ahora **sí se pone roja** (5 rojas al borrar el tope), frase corregida en los **7 archivos** que la llevaban (código, su prueba, el módulo de al lado, el doc de módulo, `DECISIONES.md`, el historial y esta fila), y **reescrito el recuadro de Daniel**, que con la frase vieja **vendía corto el propio freno** y podía llevarle a contestar «mándalo sin lote» creyendo que no hace casi nada; (2) la afirmación falsa del hallazgo 3 de la vuelta anterior **sobrevivía en la cabecera del archivo que la corrige** —el archivo se contradecía a sí mismo 150 líneas más abajo—; (3) el comentario del `🔻 LÍMITE:` describía **un mecanismo que no ocurre** (*«con tope el saldo del fantasma queda en 0»*: queda en 300, el tope acota un pool **en memoria** y no escribe kardex) y su aserción era **vacua** ⇒ las dos arregladas, y la nueva **fija que el kardex NO se toca**; (4) dos documentos daban **cifras distintas del mismo hecho** (siete vs once sitios) — la cicatriz que mi herramienta vino a impedir, **en la fila que la estrena**; (5) el mapa que se declaraba **COMPLETO** no listaba la **cuarta puerta**: un traspaso desde un almacén con tela sin lote —y, desde esta ronda, **el remanente que deja el propio tope**— deposita tela sin nombre en el destino. ⚠️ **Y al barrerlo con `grep` —en vez de leer el mapa y darlo por bueno— la lista corta aparecía en SIETE archivos más**, cada uno enumerando tres causas como si fueran todas: el **contrato**, la **pantalla** (sus comentarios **y el texto que lee el almacenista**), la **prueba de la pantalla**, la **prueba unitaria del aviso**, la cabecera del **reparto** (`partidas-telas.ts`), el **doc de módulo** y el **historial**. Un octavo —el **impreso**— ya decía «cuatro», pero nombraba **otra** cuarta: se alineó. **Los ocho dicen ahora las mismas cuatro** y apuntan al mapa. Es la misma cicatriz de *«un dato repetido en N sitios necesita un cruce mecánico»* con la de *«el aviso que importa es el que está pegado a la cosa»*: arreglar el mapa donde VIVE no sirve de nada si la lista corta sigue en el párrafo que alguien va a leer — y una de esas copias era el texto que ve el almacenista en pantalla. El cruce, para que no dependa del ojo, se hizo con un script que exige las CUATRO marcas en los diez archivos que enumeran las puertas; (6) restos de prosa vieja, incluido **el impreso, que se me quedó fuera del barrido**, y el docblock de `validarNoNegativoTelaColor`, **cuyo contrato cambié sin tocarle la documentación**; (7) 🔑 **la herramienta funcionaba pero NADA la ejecutaba** —el CI corre todo con `working-directory: backend|frontend`— ⇒ **cableada al CI** desde `frontend/src/documentos.test.ts` (job bloqueante), con el diagnóstico del script como mensaje de fallo y **probada desincronizando un documento**; y su localizador de la versión **se ancló a la fila que la entrega** —la del número MÁS ALTO del tablero, aceptando las DOS redacciones que el tablero usa (`CERRADA (2-sep, v0.085)` y `CERRADA — v0.122`), y diciendo de qué fila la leyó—. 🔴 **La primera versión de ese candado era una trampa, y se midió antes de dejarla:** exigía UNA sola línea con el formato `CERRADA — v`, que **la fila 0.142 estrenó** y que en `prueba` **no aparece ni una vez** (0 contra 20 con el formato de siempre) ⇒ corrido contra los documentos de `origin/prueba` salía en **rojo**, y habría tumbado el CI de la siguiente fila culpando a ésta. Con el criterio nuevo, corrido contra esos mismos documentos, sale **verde** y acierta la fila (0.099 → v0.120). **Y de paso el archivo dejó de estar en tierra de nadie:** el `format:check` del frontend lo incluye por glob (`../herramientas/*.mjs`), comprobado desformateándolo a propósito; ESLint sigue sin alcanzarlo, a propósito. ✅ **CIERRE DE LA 3ª VUELTA, MEDIDO DE NUEVO POR OTRO CODER** (el contenedor se reinició a mitad de la corrección y el sucesor midió punto por punto en vez de creerle a la prosa): **los 7 hallazgos estaban aplicados**. Re-medido aquí: **8 gates en verde** · **integración completa en verde** contra un Postgres local (`initdb`+`pg_ctl`, locale `C.utf8`, sin Docker) · **mutante del tope relanzado ⇒ 5 unitarias rojas —la sustituta entre ellas— y 1 de integración**, o sea la prueba nueva **sí muerde**; y el candado de documentos **probado desincronizando `version.ts`**, que lo pone rojo con el diagnóstico del script. 🔴 **Y aparecieron TRES cosas más, arregladas:** (a) el candado de la versión, tal como quedó, **habría tumbado el CI de la fila siguiente** —ver el hallazgo (7)—; (b) el mapa incompleto del hallazgo (5) **vivía en siete archivos más**, texto de pantalla incluido; y (c) ⭐ **la palabra «medidas» del doc de módulo era falsa para una de las puertas**: que *«el ajuste de ENTRADA del conteo cíclico no crea partida»* estaba escrito en el TSDoc de `ciclico/tela.ts` y repetido en varios documentos, pero **ninguna prueba lo sostenía** —y de ese hecho cuelga el tercer estado del aviso de tono—. Y con la segunda puerta pasaba lo mismo a medias: lo medido era cancelar una **ENTRADA** (que sí lleva partida), y de ahí se **deducía** el caso que importa —cancelar una **salida** sin lote—. ⇒ **se midieron las dos, con pruebas nuevas de integración**: una en `indicadores/inventario-ciclico.int.test.ts` (cuenta de más, exige `ajuste-ciclico-entrada` y `idPartida = NULL`) y otra en `inventarios/partidas-telas.int.test.ts` (surte a una orden, cancela y exige que el inverso vuelva **sin lote**). Las otras dos puertas ya tenían la suya. Las tres se midieron antes de tocarlas. 📌 **Para el merge:** esta fila sale **detrás de la 0.145**; si esa fila usa `§Post-F9.206`, la mía renumera a **§Post-F9.204** y sus referencias son **exactamente cinco**: `Documentacion_MJD/DECISIONES.md` (la sección), `HOJA-DE-RUTA.md` (esta fila), `docs/modulos/inventario-telas-avios.md`, `backend/src/dominio/inventarios/partidas-telas.ts` y `backend/src/dominio/inventarios/previa-salida-tela-orden.int.test.ts` — `grep -rn "Post-F9.206"` las saca todas.
> | **0.143** ⬜ | 🔶 duele · 🔴 **LA PANTALLA DE EXISTENCIAS DE PT PIDE EL INVENTARIO ENTERO, SIN TOPE NI PAGINACIÓN** (nace de la 0.138, medido por su reviewer el 5-sep-2026) | 0.138 · 5-sep 🔴 **MEDIDO**: `consultarExistenciasPt` (`dominio/inventarios/movimientos-pt.ts`) lee la vista `existencia_pt` con SQL crudo y **todos sus filtros son opcionales** — sin modelo, sin almacén y sin `LIMIT`, así que `GET /api/inventarios/pt/existencias` devuelve **la vista entera en una sola respuesta**, y la pantalla la pinta completa. Contra una base sintética de 525 000 renglones dio **56 860 filas en 614 ms**; el número de `prueba` falta, y el SQL para sacarlo ya está copiable en `docs/modulos/inventario-pt.md` §«Cómo medir esto en `prueba`», con su umbral: **por encima de ~5 000 filas (o ~1 MB / ~2 s) esta fila se justifica sola**. 🔑 **Es el hermano de la 0.138 en el mismo módulo, pero NO se cura igual**: el kardex se acotó por **fechas**, y una existencia **no tiene fecha** — no hay periodo que recortar. ⏳ **Decisión con default para Daniel:** (a) **exigir al menos un filtro** (modelo o almacén) antes de consultar —lo que hoy hace el operador de todas formas—, o (b) **paginar** con tope duro y aviso honesto de que hay más, como la 0.138. Default propuesto: **(b)**, porque (a) le quita la vista de «todo el almacén» a quien hoy la usa. Es **cambio de contrato** (la respuesta gana el aviso de recorte) ⇒ toca frontend. Sin migración ni permisos. **V1** (chica, y es memoria del servidor de `prueba`) |
> | **0.144** ⬜ | ⏸️ **post-V1, PERO PRIMERA DE LA FILA.** Daniel 7-sep: *«podría ir en V2. **Pero con prioridad**»* · ⭐ **LA REPARACIÓN COMO TERCER SERVICIO SOBRE LA ORDEN — y que el COSTO recoja lo real** (Daniel 6-sep: *«a veces hay que hacer reparaciones a un modelo… funciona similar a empaque o corte. Es sólo una cantidad a un precio que se le paga y **se le debe de cargar al costo**»*) | §Post-F9.204 ✅ **DECIDIDA ENTERA POR DANIEL (6-sep):** se paga **por pieza reparada** · la registra **cualquier proveedor con un rol de maquila** (`ROLES_MAQUILA_ESMA`) — **SIN rol nuevo y SIN casilla que marcar**, porque *«el reparador puede ser un proveedor de empaque o de maquila»* y crear un rol obligaría a repetir el paseo manual de la 0.114 · **avisa, NO bloquea** contra lo recibido (como el empaque) · y **el costo recoge lo REAL pagado**. ⭐ **MEDIDO — el pago SÍ está preparado y el COSTO NO:** `crearCargoDeServicio` recibe el servicio por parámetro y el CHECK de la migración no nombra valores, así que un tercer servicio no lo rompe; pero `costo-orden.ts:162-165` calcula `procesos = (maquilaOrd ?? maquilaBase) + aplicacion + Σ artes` y **ningún archivo de `costos/` ni `edr/` lee `EsMaCargo`** ⇒ 🔴 **hoy se paga el corte y el empaque y ese dinero NO llega al costo de la prenda** — falta para los tres, no sólo para la reparación. ⚠️ No es un catálogo de pantalla: es un enum de BD (`ServicioOrden`), así que meter la reparación es **migración + los puntos que marque el compilador** (la etiqueta vive en un `Record` exhaustivo que obliga a nombrarla). ⭐ **Considerar partirla en dos entregas**: (a) la reparación se paga; (b) el costo recoge los tres — (b) toca el motor de costeo. **V1** |
> | **0.146** ⬜ | 🔶 duele · **ELEGIR EL LOTE A MANO CUANDO EL CORTADOR ESCOGE UNO CONCRETO** (Daniel 6-sep, §Post-F9.205·1: *«está bien que decida el sistema **pero que haya posibilidad de seleccionar otro si es que el cortador decide un lote específico**»*) | §Post-F9.205 ✅ **DECIDIDA POR DANIEL.** Nace **encima de la 0.142**, que entregó el reparto **FIFO automático**: el automático **se queda tal cual** y esto se le añade. 🔑 **Por qué va aparte y no dentro de la 0.142:** aquélla llevaba **tres rondas de revisión** y sólo cubre el reparto automático; meterle una pantalla de selección habría reabierto el ciclo entero y retrasado lo que ya sirve. Decisión de alcance del lead, comunicada a Daniel. ⇒ Al capturar el traspaso, poder **ver los lotes disponibles del origen con su saldo** y **elegir uno**, en vez de aceptar el que propone el FIFO. 📌 La pieza de servidor **ya existe**: `saldosPorPartidaTela` (0.142) es justo la lista que la pantalla necesita. ⚠️ **Respetar el tope de la 0.142**: lo elegido a mano tampoco puede nombrar más tela de la que la existencia real respalda. **V1** |
> | **0.147** ⬜ | 🔶 duele · ⭐ **SEGUIR AÑADIENDO PDF A UN PEDIDO YA HECHO — aunque ya tenga OP generadas** (Daniel 7-sep, probando el flujo completo con datos reales) | §Post-F9.207 ✅ **PEDIDO POR DANIEL, y su RAZÓN es de diseño, no comodidad:** *«prefiero siempre hacer una OP de la misma manera **para tener toda la info completa de la OC del cliente**»* ⇒ capturar el modelo a mano **NO es equivalente**: se pierden los SKUs, los packs, la referencia y el pantone que el importador extrae del PDF. 🔴 **MEDIDO: hoy es imposible.** `confirmarImportacionPdf` (`dominio/pedidos/importacion-pdf.ts:810`) hace `tx.pedido.create` (`:987`) — **el importador sólo sabe CREAR, nunca AÑADIR** — y el contrato **ni siquiera acepta un pedido de destino**. ⇒ cada PDF nace en su propio pedido y la explosión no los junta. ⚠️ **El matiz que lo hace difícil, y es suyo: «aunque ya estén hechas algunas OP»** — no basta con permitir añadir renglones a un pedido abierto; hay que decidir qué pasa cuando el pedido ya parió órdenes (¿se pueden mezclar renglones nuevos con OP vivas? ¿la explosión los junta sin más?). 🔑 **Es la otra cara del punto 16 de sus hallazgos** (el importador crea el pedido y la OP de golpe, sin dejar verificar ni deshacer): un flujo que **sólo sabe crear** — ni añadir, ni verificar antes, ni corregir después. **Se miden juntos.** 📌 Rodeo que usó para no frenarse: meter la OP nueva desde su propio PDF y juntarlas en la explosión con «agregar una OP de otro pedido». **V1** |
> | **0.148** ✅ | ⭐ **EL SEMÁFORO DEL PRECIO TARGET ESTABA INVERTIDO** (Daniel 6-sep, punto 14 de sus hallazgos) | §Post-F9.208 ✅ **CERRADA — v0.123.** 🔴 **Era un DEFECTO, no una mejora**, y el propio esquema lo delataba: `ListaPreciosLinea.precioTarget` está documentado como *«TARGET PRICE del CLIENTE: el precio objetivo que ÉL nos da»* —o sea, lo que el cliente quiere PAGAR—, pero `simularMesa` (`dominio/desarrollo/negociacion.ts:741`) calculaba `precioObjetivo >= precioTarget`. ⇒ target $200 y cotizando **$190** pintaba **«no llega» en ROJO** (justo cuando sí se llega), y cotizando **$210** pintaba **«llega» en VERDE** estando caros: **el semáforo estaba al revés en los DOS sentidos**, no sólo la palabra. Daniel: *«Esta al revés. Si el cliente pide 200 y le doy 190, claro que llega. Y si se pasa, entonces no llega.»* **Arreglo quirúrgico:** `>=` → `<=` (la igualdad cumple: darle su número al centavo es dárselo). **Los colores NO se tocaron** —con el booleano corregido `default`/`destructive` ya quedan bien—, ni la otra etiqueta: el `Cumple`/`Debajo` del MARGEN (`MesaNegociacion.tsx:467`) es otra comparación y **está bien**, como él mismo dijo (*«La misma etiqueta en precio sugerido si esta bien»*). ⚠️ **Las pruebas DEFENDÍAN el defecto** (`negociacion.int.test.ts` :1171/:1181/:1263) y se voltearon; se añadieron el caso de la **igualdad** —sin él, un futuro `<` pasaría todo— y uno que **clava la dirección con el ejemplo literal de Daniel**. **SIN migración, SIN permisos, SIN semilla.** **V1** |
> | **0.149** ⬜ | 🔶 duele · ⭐ **EL LINAJE DEL MODELO, VISIBLE EN LAS DOS DIRECCIONES — y retirar el botón que no puede funcionar** (Daniel 7-sep) | §Post-F9.209 🔑 **Nació de una pregunta suya, y la pregunta era buena:** *«me generó una OP y le puso por default el modelo de producción 54002. Si me voy a ese modelo de desarrollo, hay un botón que dice "pasar a producción" … y me ofrece poner el 54003. ¿Qué pasa si lo pongo?»* ✅ **MEDIDO: no pasa nada malo — se lo rechaza.** La GUARDA A de `promoverAProduccionNucleo` (`dominio/modelos/nomenclatura.ts:775-787`) impide transformar un padre que ya tiene hijos, nombrándolos. 🔴 **Pero la incongruencia que él percibe es REAL, y son TRES sitios que no miran si hay hijos:** (a) el botón se pinta con sólo `origen === 'desarrollo'` (`ModelosPagina.tsx:906`); (b) `consultarPropuestaProduccion` (`nomenclatura.ts:1391-1408`) sólo comprueba `yaEnProduccion` y **le ofrece el 54003 sin saber que hay hijos** —un número que el sistema va a rechazar—; (c) el aviso ámbar del diálogo explica bien el peligro general pero **no dice lo único que aplica a su caso**. ⚠️ **Y el peligro era real antes de la primera OP:** pulsarlo entonces **sí** funcionaba y **no tiene vuelta atrás** —el modelo se queda con UN número para todos sus colores y sus OP dejan de hacer nacer modelos por color para siempre—. 🔑 **Los dos botones hacen lo OPUESTO** y el código lo dice en una línea (`nomenclatura.ts:900-906`): promover **transforma la fila** del desarrollo; derivar **crea una fila nueva** y lo deja intacto. 📌 **Va junto con el punto 17 de sus hallazgos** (*«En la OP no veo el modelo de desarrollo»*): el linaje existe en la base (`Modelo.idModeloDesarrollo`) y **ninguna pantalla lo enseña, ni hacia arriba ni hacia abajo** —`esquemas/orden.ts:487-489` ni siquiera lo transporta—. Y el propio código dejó la pregunta escrita sin contestar (`nomenclatura.ts:735-737`: *«¿se retira el botón del catálogo?»*): **el recorrido de Daniel la contestó**. **V1** |
> | **0.150** ✅ | ⭐ **LA GUARDA: la cascada YA NO cancela OP producidas** (Daniel 7-sep) | §Post-F9.211 + §Post-F9.212 + §Post-F9.215 ✅ **CERRADA — v0.124.** 🔴 **ERA UN DEFECTO, no una mejora:** `cancelarPedido` (`dominio/pedidos/pedidos.ts`) tenía **sólo dos** guardas —ya cancelada y orden **cerrada**— así que marcando la cascada **se cancelaba una OP aunque ya estuviera producida**, justo lo que Daniel dijo que no quería (*«No quiero que se borren las OP en ese caso»*). Y `cancelarOrden` tenía **las mismas dos**: el criterio **no existía en ningún camino de cancelación**. ✅ **ENTREGADO:** `dominio/produccion/actividad-orden.ts`, el criterio ÚNICO con **15 señales** (3 gratis —vienen en la fila que la cancelación ya leía— y 12 conteos). Desenlace **(b)**: se cancela el pedido y las OP **sin** actividad; **las que tienen trabajo se CONSERVAN y se NOMBRAN** con su motivo en lenguaje de negocio. **Deuda pagada:** `tieneActividadProduccion` se **exporta** y el conteo inline de `ordenes.ts` se colapsa sobre ella (las de `etapas.ts` **no**, y se dice por qué: filtran por tipo de proceso, no son el mismo predicado). 🔴 **Las dos trampas, tapadas y verificadas por el reviewer:** al kardex se llega por **tres caminos** (PT por columna · **tela** por `origenTipo`+`origenId` de TEXTO · **avíos** por la nota de salida), y `cancelarMovimientoPt` **copia `idOrden` al inverso** ⇒ se descartan los anulados por el encabezado, o la guarda habría bloqueado **para siempre** una orden cuya actividad se deshizo entera. ✅ **«Comprado» = `ESTATUS_OC_COMPROMETIDA`** (el borrador **NO** cuenta): **decidido por Daniel**, §Post-F9.215. **A2:** `bloquearEtapasDeOrden` por orden, antes de contar. **Y murió un atasco sin salida:** un pedido con una OP **CERRADA** no se podía cancelar por ninguna vía —los dos rechazos se tapaban— y el mensaje ofrecía *«o cancela el pedido sin arrastrar las OPs»*, **puerta que no existía**. ⚠️ **Dos rondas:** el reviewer RECHAZÓ la primera con dos falsos positivos medidos —programar la RC conservaba la orden (los procesos de duración 0 nacen auto-completados) y una nota de salida **en borrador** también—, los dos disparando en el caso de uso real. **SIN migración, SIN permisos, SIN semillas.** **V1** |
> | **0.151** ✅ | 🔴 BLOQUEA V1 · ⭐ **EL NÚMERO DE PRODUCCIÓN SE TECLEA AL IMPORTAR, Y LA OP DICE DE QUÉ DESARROLLO NACIÓ** | §Post-F9.212 + §Post-F9.217 ✅ **CERRADA — v0.127.** **Daniel:** *«me generó el pedido y la OP **sin preguntar el número de modelo interno**… **quedamos que ese lo ponía yo, con una sugerencia previa**»* + *«**en la OP no veo el modelo de desarrollo**»*. 🔑 **Era un hueco de CABLEADO, no de diseño:** `salidaAProduccion` **ya aceptaba** el número (su `describe` decía *«el sistema lo precarga y el usuario lo puede cambiar»*) y el importador **nunca se lo pasaba**. ✅ **ENTREGADO:** el número por archivo en el contrato del confirm + la propuesta **informativa** en `analizar` (se dice que **NO aparta** el número) + el **linaje** (`idModeloDesarrollo` + código) transportado en el contrato de la orden y pintado. **SIN migración.** ⭐ **EL AGUJERO QUE ENCONTRÓ QUIEN LA RETOMÓ, y era EL CASO DE ESTRENO:** el servidor numera **una sola vez, al analizar, con la liga APRENDIDA** — pero **la primera vez de cada modelo del cliente se liga A MANO en la vista previa** (lo dice el propio importador). ⇒ ese renglón **no ofrecía casilla** y la OP nacía con el número que eligiera el sistema: **la queja de Daniel, sin cubrir, justo cuando nace el modelo**. Y re-ligar a otro modelo **dejaba precargado el número del anterior**, que puede ni ser de su serie. Arreglado **sólo en frontend**: si la numeración no está vigente, la casilla se ofrece **vacía y opcional** con el motivo a la vista, y cambiar la liga **hace caducar** el número precargado. 📌 **NO se hizo la propuesta en vivo para el modelo elegido a mano, y la razón es buena:** exige que el servidor **renumere la TANDA** (cuatro colores = cuatro números); pedirlo fila por fila **reintroduciría** el defecto que `numero-produccion-pdf.ts` vino a cerrar —las cuatro recibirían el mismo—. 📌 **Y NO se liga el expediente de Desarrollo automáticamente, medido:** `Desarrollo` es `@@unique([idProyecto, idModelo])` ⇒ **un modelo puede tener N expedientes, uno por proyecto**, así que ligarlo solo sería adivinar. Se contesta con el **linaje**, que es un hecho. 🔴 **Y muere un aviso que MENTÍA:** *«modelo anterior al módulo de Desarrollo»* en una OP nacida del PDF — estaba en **CUATRO sitios de TRES pantallas**, hoy en cero. **Medición (ya con `prueba` mezclada, que es el árbol que sale):** **3545** unit + **2349** front · 63 archivos de integración en cinco tandas · **7 mutaciones**, tres de ellas sobre los arreglos de la 2ª ronda. *(Antes del merge eran 3525 y 2341: v0.125 y v0.126 traían 20 y 8 pruebas más. Las mutaciones se corrieron antes del merge y no se repitieron; es una afirmación de proceso, no medible desde el repo.)* **V1** |
> | **0.152** ⬜ | 🔶 duele · ⭐ **EL PRECOSTEO TOMA DEL CATÁLOGO — se acaba la tela «suelta» que se duplica (hallazgos 4 + 12 + 5 + 3 de Daniel)** | §Post-F9.210 🔑 **UNA raíz, no cuatro quejas.** *«No entiendo por qué hay doble información… **toda la información debe de venir desde la receta**… no sé por qué en el precosteo hay espacio para meter otra tela que no viene de un catálogo»* + *«no me deja meter telas… **solo avíos**»*. **MEDIDO:** `PrecostoLinea` guarda **FK opcional Y copia propia**, y `agregarLineaManual` (`desarrollo/precostos.ts:912`) acepta **`idAvio` y nada más** — `idTela` **no existe en el esquema de alta** (`esquemas/precosto.ts:70-108`) y sólo se escribe en `lineasBomDesdeModelo`. ⇒ **para la tela la ÚNICA vía es la libre**, y por eso se ven dos telas. **SIN migración: `idTela`/`idTelaProveedor` YA existen en la tabla.** 3 archivos + cablear `SelectorTela` (**ya existe**). ✅ **Decidido por Daniel:** el texto libre **vive sólo en la mesa de negociación**, nunca en la receta ⇒ **no se rompe** su decisión de la jareta estimada (§Post-F9.139); los **conceptos de costo siguen libres**. ✅ **+hallazgo 3:** corte/maquila/empaque **sólo precio, no cantidad** (*«Solo debe de llevar el precio. no la cantidad»*) ⇒ quitar el input de Consumo de `DialogoPrecosto.tsx:552`. 📌 **Buena noticia medida:** lo suelto **NO llega a la OP** (`receta-orden.ts:302` copia del BOM del MODELO) — su temor no ocurre. **V1** |
> | **0.153** ✅ | 🔴 BLOQUEA V1 · ⭐ **EL PRECIO QUE SE PACTÓ EN LA NEGOCIACIÓN YA SE VE DESDE FUERA** | §Post-F9.210 + §Post-F9.216 ✅ **CERRADA — v0.126.** **Daniel:** *«dice precio aprobado, **pero dentro de la negociación quedó otro**. Debe de haber congruencia»* + su propia salida: *«**o estaría bien poner los dos, mejor**»*. ✅ **ENTREGADO, y es SÓLO LECTURA:** tres campos nuevos en la proyección del renglón (`precioNegociado` tras la reja de importes, `tienePrecioNegociado` y `precioNegociadoEn` sin ella — el mismo reparto que el archivo ya usa para `precioTarget` y `precioAprobado`), el `include` trae **el último evento con precio, filtrado y cortado EN SQL** (no N+1), y una columna **pegada a la derecha de «Precio aprobado»** sin mover ninguna existente. **Ni una ruta de escritura, ni un permiso, ni migración.** ⭐ **La separación negociador/aprobador NO se tocó** (es deliberada, `negociacion.ts:8-12`): el aprobado sigue siendo el que manda en PDF, Excel, cotización y **el precio que viaja a la ORDEN**, y hay pruebas que lo fijan con fixtures donde negociado ≠ aprobado. 🔴 **CORRIGE UNA MEDICIÓN DEL LEAD:** se dijo que `precioNuevo` se leía en **dos** sitios, ambos dentro del diálogo. Son **TRES**, y el tercero está **fuera**: `SeccionDesarrolloOrden.tsx:191-193`, en la pantalla de la ORDEN, vía `expedienteOrden`. El precio pactado **sí** se veía fuera — en la orden, no en la lista, que es donde Daniel lo buscaba. ⚠️ **DECISIÓN DEL CODER, hecha visible a propósito:** el criterio es «el último evento con precio», y eso **incluye el recálculo por mover los FACTORES** del cliente (`listas-precios.ts:1056-1069`), que escribe un evento **estructuralmente idéntico a un acuerdo** porque `NegociacionEvento` **no tiene columna de tipo**. Se eligió incluirlo para que lo de fuera **coincida con la última fila del historial** —que es justo lo que se pedía—, el rótulo dice *«último precio registrado»* y **no** «pactado», y queda **fijado con prueba**. ⏳ **Pregunta abierta para Daniel (default: como está):** ¿sólo los acuerdos? Distinguirlos exige **columna nueva** en la tabla de eventos. **Medición:** 3534 unit · 722 de integración en `desarrollo`+`pedidos`+`modelos` · **4 mutaciones** (criterio, `orderBy` del SQL, la reja de importes, y el PDF leyendo el campo nuevo), las dos primeras **complementarias a propósito**: como el `include` corta a 1, la unit no vigila el orden del SQL y la integración no vigila la función. **V1** |
> | **0.154** ⬜ | ⏸️ post-V1 · ⭐ **LA MESA Y LA LISTA: el total de avíos en un renglón, y el % de margen junto a cada precio (hallazgos 13 + 10)** | §Post-F9.210 **(13)** *«en la pestaña de negociación **no debe de venir los avíos desglosados**… solo dejar **un solo registro de avíos con el total** de lo que está en la ventana emergente»*. ✅ **El popup Y el subtotal YA EXISTEN** (`MesaNegociacion.tsx:283-291` → `PanelAvios:591-710`; el total lo calcula el servidor, `desgloseCostoLinea`, `listas-precios.ts:1720-1820`) ⇒ hoy **los avíos salen DOS veces**; falta filtrar `renglones` en `:313`. **Frontend puro, el más chico de todos.** **(10)** *«que en los precosteos pongas el % de margen a un lado de cada precio (target, precio calculado, precio aprobado)»*. **MEDIDO:** el renglón (`listas-precios.ts:165-200`) devuelve los cuatro importes y **ningún margen**; la aritmética ya existe (`proyectarMargen`/`calcularPrecioLista`) ⇒ **se reusa, no se copia** (A1). ⚠️ **Los tres márgenes van tras la MISMA reja que `precioSugerido`** (`puedeVerFactoresDePrecio`), NO la de importes: publicarlos sin ella **destaparía los factores por división** — el riesgo ya razonado en `esquemas/negociacion.ts:425-431`. **V1** |
> | **0.155** ⬜ | 🔶 duele · ⭐ **EL PROYECTO RECUERDA A SU COMPRADOR, SU GÉNERO Y SU AÑO — y los modelos lo heredan (hallazgos 1 + 2)** | §Post-F9.210 **(1)** *«poder poner el nombre de la compradora… ese catálogo debe de tener la opción de **seleccionar al comprador cuando se hace un precosteo**. Normalmente **un proyecto va dirigido a un solo comprador**»*. ✅ **El catálogo YA ESTÁ COMPLETO** (`ClienteContacto`, §Post-F9.152): puesto libre («compradora»), `activo` para alta/baja, y departamento **opcional y editable** ⇒ se cambian de Niños a Caballeros. **Falta SÓLO la liga:** FK opcional `idClienteContacto` en `Proyecto` + selector. **(2)** *«un proyecto nace con un género o departamento definido… todos los modelos nuevos… **debería jalar el género desde ahí. Que no vuelva preguntar**… lo mismo el año de entrega»* ⇒ ✅ **decidido: van EN EL PROYECTO y cada modelo los hereda CON OPCIÓN A CAMBIARLOS**. **MEDIDO: hoy no se puede heredar** — `ClienteDepartamento` no tiene FK a `Genero` (se llama «NIÑOS» pero el sistema no sabe que eso ES un género) y `Temporada` no tiene año. 🔑 **VALE MÁS QUE LA COMODIDAD:** un modelo sin género/tipo **NO SE PUEDE NUMERAR**, y el error (`nomenclatura.ts:222`) **salta hasta «Generar OP», después de teclear la matriz completa**. Precargarlos al nacer cierra por arriba ese agujero. **V1** |
> | **0.156** ⬜ | 🔴 **BLOQUEA V1** — **y su historia es la lección de la noche: el lead clasificó mal por tomar un RODEO por una PREFERENCIA.** Daniel dijo primero *«hoy es más común meter felpa por un lado y cárdigan como otra tela por otro»*, y el lead concluyó que ésa era la forma buena y bajó la fila a post-V1, proponiendo incluso **retirar** el mecanismo del complemento. **Daniel lo corrigió:** *«**sí es importante meterlo como complemento** porque **hay proveedores que así lo manejan** y **para el control de la tela siempre es mejor ponerla como un complemento de su tela**»* ⇒ lo hacen separado **porque la receta no sabe llevar el complemento**, no porque sea mejor. ⭐ **Y la evidencia le da la razón, y estaba a la vista:** `CLAUDE.md` §5, de la ingeniería inversa del sistema viejo — *«Telas: doble componente ExTela1/ExTela2 (ej. felpa + cardigan, **mismo lote**)»*. **El cárdigan comparte LOTE con su felpa**: darlo de alta como tela independiente **rompe ese vínculo en silencio**, justo cuando 0.142 acaba de hacer que el lote viaje y el aviso de riesgo de tono dependa de él. | §Post-F9.210 + §Post-F9.214 🔴 **EL HUECO, MEDIDO:** `ModeloTela` (`schema.prisma:2810`) tiene **UN SOLO** `consumoPorPrenda` y `bom-modelo.ts` no menciona «complemento» ni una vez — pero la tela **lo declara** (`Tela.nombreComplemento`) y la compra **lo exige** (`CompraLinea.cantidadComplemento`; `exigirComplementosCapturados` no deja autorizar sin él). ⭐ **El sistema ya confesó el hueco por escrito** (`mrp.ts:3628-3631`): *«la explosión **NO sabe cuánto COMPLEMENTO lleva** una tela que lo tiene —**el BOM guarda un solo consumo por tela**— así que estas OC nacen con el complemento PENDIENTE»*. ✅ **Decidido por Daniel: el consumo del complemento es NÚMERO PROPIO**, no proporción del cuerpo. **CON migración** (columna nueva) ⇒ 🔑 **más barata AHORA que después de arrancar** (REGLA 0-B: hoy los datos son desechables). **V1** |
> | **0.157** ⬜ | ⏸️ post-V1 · ⭐ **EL PAPEL QUE VE EL CLIENTE: el PDF de la cotización y la fecha de la cita (hallazgos 11 + 9)** | §Post-F9.210 **(11)** *«El PDF de la cotización no me gusta. Está muy simple. Le quisiera poner **una foto chica**… una **descripción ampliada**… **el modelo del cliente va en la primer columna**, es más importante para él su modelo que el nuestro»*. **MEDIDO** (`impresos/impreso-cotizacion.ts`): hoy va **1º nuestro código** (`:172`) · 2º descripción · **3º el suyo** (`:174`), y **sin fotos**. **(a)** reordenar = trivial, y tiene toda la razón: el papel es para el cliente. **(b)** ⭐ **la foto es MÁS BARATA de lo que parece: la maquinaria de meter fotos de R2 en un PDF YA EXISTE y está probada** (`produccion/impresos/imagenes-impreso.ts` + `bloque-fotos-arte.ts`, usadas por el impreso de orden, el de envío a maquila y el de recibo) ⇒ **se reusa**. **(c)** la descripción ampliada es **campo NUEVO** (`Modelo` sólo tiene `descripcion` corta y `composicion`, que es otra cosa) ⇒ ✅ **decidido: vive EN EL MODELO** (se escribe una vez, sale en todas sus cotizaciones). **(9)** *«la fecha de la lista debería ser fija de la fecha que se creó. No entiendo qué otra cosa sería la fecha opcional»* ⇒ **MEDIDO: hay DOS fechas** (`creadoEn` automática y `fecha` capturable con default hoy) y `fecha` **no decide nada**, sólo se pinta; es **el único campo del modelo sin comentario** mientras su vecino `lugar` tiene tres líneas. ✅ **Decidido: se queda, rotulada «FECHA DE CITA»** (*«Ok, dejala… solo ponle fecha de cita»*) ⚠️ **arrastra a `Cotizacion`**, que tiene la misma forma. **V1** |
> | **0.158** ✅ | 🔴 BLOQUEA V1 (Daniel 7-sep) · ⭐ **LOS AVÍOS QUE NO SE COMPRAN POR COLOR — la etiqueta de lavado ya suma todo junto** | §Post-F9.213·A ✅ **CERRADA — v0.125.** **Daniel, viendo la explosión con datos suyos:** *«hay ciertos avíos que **no se compran por color**… como la **etiqueta de lavado**… ¿cómo le puedo hacer para definirle que algunas cosas se compran juntas **sin tomar en cuenta el color**?»* (le salía dos veces: 11,771 pz y 1,387 pz). ✅ **ENTREGADO:** columna `se_compra_sin_color` en `Avio` (aditiva, default `false`) + el colapso en la función pura `gruposDeCompraDelAvio` + CRUD en el catálogo. ⚠️ **`esGenerico` NO servía, y queda dicho en el esquema:** es otro eje (*«¿contra stock o contra la orden?»*), se construye antes y sin mirarlo, y habría cambiado la valuación del costo. 🔴 **Y destapó un defecto que HABRÍA COSTADO DINERO, arreglado aquí:** marcar un avío cuyas OP **ya tenían OC** hacía que la explosión **volviera a ofrecer todo lo ya comprado** (`enOc: 0 / pendiente: 100`, medido con sonda por el camino real) ⇒ **se compraba dos veces**. Causa: en `repartirComprometidoPorColor` las cubetas CON color que ningún renglón reclama **se caían al piso**, y un `return` temprano salía antes de mirarlas. Restaura la invariante **Σ(enOc) = comprometido.enOc**. 📌 **Decidido y fijado con prueba:** sin renglón sin color las huérfanas **NO se reparten** —darle a Azul lo que una OC pidió para Rojo sería inventar un hecho (§Post-F9.86)—. **Medición:** 3535 unit · **suite de integración COMPLETA, 167 archivos / 3344 pruebas** (se corrió entera porque el arreglo vive en lógica compartida con TELAS: no cayó ninguna) · 2 mutaciones (el colapso: 4 unit + 34 integración en rojo; la absorción: 4 de 5 unit + la de integración). 🔶 **Queda la mitad gemela → fila 0.162.** **V1** |
> | **0.159** ⬜ | 🔴 **BLOQUEA V1** (Daniel 7-sep) · ⭐ **LOS COLORES DUPLICADOS: que se puedan fusionar aunque ya estén en órdenes, y que el MRP respete la fusión** | §Post-F9.213·A 🔑 **Nace de la pantalla de Daniel:** sus dos renglones de etiqueta decían «Blanco Hueso Pantone 14-0002 Tcx Pumice Stone» y «Blanco Hueso» — `Color.nombre` es `@unique` (`schema.prisma:1152`) ⇒ **necesariamente dos filas distintas para el mismo color real**. 🔴 **DOS defectos, y el primero lo deja atrapado:** **(1)** `fusionarColores` **SE NIEGA** si el color origen se usó fuera de las telas (`catalogos/colores.ts:400-403`) y **la primera referencia bloqueante de la lista es `OrdenLinea.idColor`** (`colores-fusion-referencias.ts:54-59`) ⇒ como los suyos están en las OP 5565/5566 y 5567, **hoy no se pueden unificar, ni ahora ni nunca**. **(2)** Y aunque se pudieran, **el MRP los seguiría partiendo**: `colorCanonico` (`colores.ts:507`) **no aparece en NINGÚN archivo de `dominio/compras/`**; `colorDelRenglon` devuelve el **id crudo** (`comprometido-en-oc.ts:123-129`). ⚠️ La negativa de la fusión **es deliberada y está razonada** (mover `OrdenLinea` reescribiría lo que el cliente pidió, D7): la solución **no** es levantar la guarda sin más, sino que la fusión sepa **repuntar sin reescribir el texto original** —el mecanismo ya existe para los departamentos, `ClienteDepartamento.idFusionadoEn`, §Post-F9.172(a)—. 📌 **No medido:** qué hace `casar-color-de-tela.ts` con nombres casi iguales (si casa por nombre exacto, «Blanco Hueso» y «Blanco Hueso Pantone…» no casarían nunca). **V1** |
> | **0.160** ⬜ | 🔶 duele · ⭐ **EL COLOR EN LA LÍNEA DE COMPRA AÑADIDA A MANO — hoy nace ciega y la recepción no la cruza** | §Post-F9.213·B 🔴 **Tapa el agujero del único camino manual que hoy resuelve el caso de las mangas.** Se puede agregar a mano un segundo renglón de la misma tela a una OC (`EditorLineasOc.tsx:66`, sin restricción de unicidad) — pero **nace SIN COLOR y no hay control para ponérselo**: `EditorLineasOc.tsx:300-325` dice textual *«aquí no se ELIGE (eso vive en «De qué color se compra la tela»); aquí se VE, y se puede quitar»*, y el único botón pone `idTelaColor: null`. ⇒ **En la recepción el cruce de color SÓLO se hace si el renglón lo tiene** (`recepciones.ts:1079-1090`), así que esos kilos **se pueden recibir de cualquier tono sin que nadie chiste**, y como el kardex de telas **sí** exige color al recibir (`MovimientoDetTela.idTelaColor` obligatorio), **quien recibe vuelve a inventar la correspondencia** — exactamente el mal que V1-E3u vino a cerrar. 📌 Con esto, el camino manual de las mangas queda usable **mientras llega la 0.161**. **V1** |
> | **0.161** ⬜ | ⏸️ **post-V1** — Daniel 7-sep: *«es raro, son pocos. **Si me llegara a salir alguno que necesite meter, te aviso para pasarlo a V1**»* · ⭐ **⭐⭐ EL CONSUMO POR PARTES EN LA RECETA — «150 de cuerpo y 50 de mangas» (la de fondo)** | §Post-F9.213·B **Daniel:** *«a veces hay modelos que llevan **dos colores en la misma prenda**. Ejemplo: **mangas de otro color**… En la receta pongo que lleva 200 gramos. Pero la realidad es que lleva **150 de un color y 50 de otro**»*. ⭐⭐ **Y la decisión de diseño la puso ÉL, sin que se la preguntaran:** *«el concepto de **dividirlo desde la receta es lo ideal**. Porque **la gente de desarrollo son los que realmente tienen esa información**… sí debe de venir desde allá»*. 🔑 **Y acotó el alcance en el mensaje siguiente, que es lo que hay que construir:** *«**no sé si con su color, pero sí con el consumo por partes** cuando el modelo lleva más de un color»* ⇒ **la receta lleva LAS PARTES Y SU CONSUMO, NO EL COLOR.** La receta es ficha técnica **estable** (igual para todos los colores del modelo); el color es **comercial y variable** (dos pedidos con combinaciones distintas **no cambian la receta**). 🔴 **MEDIDO: el techo de hoy es estructural, no un campo que falte.** El color viene de la **matriz de la ORDEN** (`piezasPorColorOrden`, `mrp.ts:499-514` sobre `OrdenLinea.idColor`), traducido por `OrdenTelaColor` (`schema.prisma:4014-4038`); la receta aporta **una** cifra y **cero** color. Ni `ModeloTela` (`:2808-2838`) ni `OrdenTela` (`:3910-3989`) tienen `idColor` ni campo de parte. Y `@@unique([idOrden, idTela])` (`:3982`) **impide meter la misma tela dos veces**, ni a mano (`receta-orden.ts:531-545`). 📌 **POR ESO ES BARATO:** `OrdenTelaColor` ya traduce color de prenda → color de tela con `@@unique([idOrdenTela, idColor])`; con partes pasa a **(color de prenda, parte) → color de tela** — **una columna más en una tabla que ya existe**, y el cerrojo `@@unique([idOrden, idTela])` **se deja intacto** porque el reparto vive DENTRO del renglón de la tela. Sin partes, la suma es el consumo de siempre y **nada cambia** (REGLA 0-B: sin retrocompatibilidad que inventar). ✅ **DECIDIDO por Daniel, y CORRIGE al lead: son «combinación 1», «combinación 2»… con etiqueta de TEXTO LIBRE, SIN catálogo.** *«un catálogo de partes podría ser muy extenso… a veces es una franja lateral, a veces una pretina, mangas, capucha, parte interna de la capucha… **No importa que a veces diga manga y a veces Mangas. Al final NUNCA vas a agrupar esos dos conceptos. Son cosas que viven en esa orden y no afecta a nada más**»*. 🔑 **El argumento del lead era una MALA ANALOGÍA:** por **color** el sistema **AGRUPA** —es la clave de compra de toda la explosión— y por eso dos «Blanco Hueso» parten un renglón en dos; **por parte no se agrupa NUNCA**. Un catálogo existe para hacer cosas comparables: si nada se compara, sólo añade una puerta que alguien debe abrir antes de que Desarrollo pueda capturar. ⚠️ **La orden referencia la combinación POR SU ID, nunca por el texto** — así la etiqueta se reescribe sin romper nada, que es lo que la vuelve segura siendo libre. **V1** |
> | **0.162** ⬜ | 🔶 duele · ⭐ **EL NETEO PIERDE LO YA COMPRADO CUANDO EL RENGLÓN NO TIENE COLOR — la mitad gemela** | §Post-F9.213·A 🔑 **Lo encontró el coder de la 0.158 midiendo por el camino real, y la mitad de la 0.158 SÍ se arregló ahí; ésta es la que quedó.** El hueco: en `repartirComprometidoPorColor` (`compras/comprometido-en-oc.ts:373`) el renglón **sin color** se lleva sólo la cubeta `porColor[null]`, y **las cubetas CON color que ningún renglón reclama se caen al piso** (y encima hay un `if (acervo <= 0) return propio` que sale antes de mirarlas). ⇒ un material sin color **vuelve a ofrecerse aunque ya esté comprado**. **Medido con sonda:** `enOc: 0 / pendiente: 100` donde debía decir `100 / 0`. ✅ **La 0.158 lo cerró para el neteo contra la ORDEN DE COMPRA** (era su primer uso: marcar un avío cuyas OP ya tienen OC). 🔴 **LO QUE QUEDA: `RequerimientoCubierto` tiene el MISMO hueco** —está indexado por *(orden, material, color)*—, así que las marcas de **«dado por cubierto»** de un material sin color no se encuentran y el faltante se vuelve a perseguir. ⚠️ **No se arregló junto** a propósito: toca la semántica de **«dar por cubierto»**, que es un acto de negocio de Daniel (§Post-F9.99, nacido de su *«compré 480 en lugar de 481»*), y merece su propia decisión y sus pruebas. 📌 **Y afecta también a TELAS sin color**, no sólo a avíos. **V1** |
>
> **🎯 QUÉ BLOQUEA DE VERDAD EL ARRANQUE (clasificado el 7-sep-2026 a petición de Daniel: *«ha de haber
> muchas cosas que no son indispensables y **están gastando tiempo**»*; **las cuatro dudosas las cerró él
> esa misma noche**).** De las **25 ⬜ pendientes**:
> **8 🔴 BLOQUEAN** (0.096, 0.097, 0.117, 0.120, 0.123, 0.131, **0.156**, **0.159**)
> — de ésas **dos son repasos de Daniel** y **una la ejecuta Gabriel**, así que **son 5 de construcción**
> (0.117, 0.120, 0.131, 0.156, 0.159), y la **0.131 no puede correr** hasta que Daniel entregue su corte
> de SINUBE ·
> **9 🔶 duelen pero tienen rodeo** (0.103, 0.143, 0.146, 0.147, 0.149, 0.152, 0.155, 0.160, **0.162**) ·
> **8 ⏸️ post-V1** (0.102, 0.116, 0.119, 0.130, 0.144, 0.154, 0.157, 0.161), y de ésas la **0.144 va
> PRIMERA** (*«con prioridad»*).
> ⚠️ **Tres avisos que no se pierden:** **(1)** la **0.159 empeora sola** —cada día más órdenes usan los
> colores duplicados y más difícil es limpiarlo—, y por eso Daniel la subió a V1; **(2)** la **0.161** vuelve
> a V1 **en cuanto a Daniel le salga un modelo que la necesite** (él avisa); **(3)** 🔑 **lo que es barato
> HOY no lo será después**: `0.155`, `0.156`, `0.159` y `0.161` cambian el modelo de datos, y mientras la
> versión empiece por `0.` los datos son desechables (REGLA 0-B) — después de arrancar, cada columna nueva
> se topa con órdenes y precios reales. Lo de pantalla y papel (`0.102`, `0.116`, `0.119`, `0.154`, `0.157`)
> cuesta lo mismo hoy que en seis meses.
> 📌 **De aquí en adelante toda fila nueva nace con esta etiqueta** (regla de Daniel, `CLAUDE.md` §7.5).
> **📊 Dónde va el programa — al 7-sep-2026, versión en `prueba` `0.128`.** De las **104 filas** numeradas: **66 ✅ cerradas** · **1 🔶 a medias** (0.082) · **12 ⏸️ aparcadas a fase 2** (0.072, 0.086, 0.087, 0.122, 0.126, 0.127, 0.133, 0.134, 0.135, 0.139, 0.140, 0.141) · **25 ⬜ por hacer en la V1** (0.096, 0.097, 0.102, 0.103, 0.116, 0.117, 0.119, 0.120, 0.123, 0.130, 0.131, 0.143, 0.144, 0.146, 0.147, 0.149, 0.152, 0.154, 0.155, 0.156, 0.157, 0.159, 0.160, 0.161, 0.162). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
> | **0.145** ✅ *(entregada como **0.121**)* | ⭐⭐ **CORREGIR UN MOVIMIENTO SIN FACTURA DEL ESTADO DE CUENTA DE UN PROVEEDOR — «sólo yo, ni con permiso»** (DANIEL, 6-sep-2026, §Post-F9.203: *«Quiero tener manera de modificar cualquier registro que se meta en cualquier estado de cuenta de los proveedores **sin factura**. **Sólo yo. Nadie más ni con permiso. Sólo yo.**»* — y sobre la forma, tras plantearle el costo: *«Sí, está bien **con rastro**.»*) | §Post-F9.203 · Daniel 6-sep 🔴 **DE DÓNDE NACE, medido**: de los cuatro conceptos de EsMa **sólo el descuento** se podía cancelar (`esma/formula-saldo.ts`, fila 0.109, y sólo para el deshacer de un cierre) ⇒ **un abono o un pago a un maquilero capturado por error NO se podía anular NUNCA**, mientras que en Cuentas por pagar se cancela todo desde F9-E1. Esta fila cierra esa asimetría. ✅ **CERRADA (6-sep, v0.121).** **(1) LA FORMA — un gesto, dos hechos:** en pantalla es **editar** (se abre el renglón con sus valores, se cambia lo que haga falta, se guarda); por dentro, en UNA transacción, el viejo queda **cancelado** —con su **inverso auditado** en el motor de terceros— y nace el bueno **ligado** a él (`idMovimientoCorregido` / `idAbonoCorregido` / `idDescuentoCorregido` / `idPagoCorregido`, todos `@unique`). ⇒ **D3 intacto**: nada se edita ni se borra y el saldo sigue siendo Σ de movimientos. **(2) «SÓLO YO» = BANDERA EN LA PERSONA, NO UN PERMISO:** `Usuario.puedeCorregirSinFactura` (patrón `esAuditor`), **que no se asigna desde ninguna pantalla ni endpoint** —se prende sólo por base de datos— y que **el DOMINIO exige** (A1), no la ruta. NO se reusó `roles.administrar` (el defecto de la 0.120 que Daniel señaló) ni se creó un `cxp.corregir` asignable: él dijo *«ni con permiso»*. El permiso del módulo **sigue haciendo falta además** — la bandera abre una puerta, no exime. **(3) «SIN FACTURA» ES DEL MOVIMIENTO, NO DEL PROVEEDOR:** un proveedor `ambos` tiene de los dos, y el renglón con CFDI queda **intocable aunque sea del mismo proveedor**. **(4) NO se cambia de proveedor ni de concepto:** no son campos del cuerpo (`z.strictObject` ⇒ 400 explícito) y el servidor los toma del corregido. ⭐⭐ **EL CASO DIFÍCIL, resuelto: el PAGO ya aplicado a cargos.** Cancelarlo cambiando sólo su renglón habría dejado los cargos marcados como pagados con dinero que ya no existe. Ahora las «prendas por pagar» se cuentan por la suma **VIVA** de `PagoAplicacion` (`pago.canceladoEn IS NULL`) y corregirlo **deshace la aplicación y la vuelve a hacer** bajo el lock por maquilero, con `cantidadPagada` y `Orden.pagada` recalculados: las prendas **no se duplican ni se pierden** (probado). Las `PagoAplicacion` del pago cancelado **no se borran** (D3): son el rastro. 🔴 **Dos límites DECLARADOS, no callados:** (a) el **importe** de un pago aplicado **no** se corrige —sale de las prendas por el precio del cargo, y el modelo promete `monto = Σ aplicaciones`—; se corrigen su fecha y sus observaciones. (b) El **descuento que propuso un CIERRE de orden** no se corrige suelto (su liga al cierre es `@unique` e intransferible): se deshace el cierre. 🔴 **Tercera ronda — los dos GEMELOS del mismo defecto, medidos con sonda:** (i) el `importeGuardado` llegaba **en negativo** sobre los movimientos MIGRADOS (`monto: -1234.5`, `conFactura: null` ⇒ corregibles), porque la ronda anterior sustituyó el `Math.abs` del cajón por el dato del servidor **sin llevárselo**: el motor lo aplicaba y las **seis ramas de EsMa** no. Se curó con un helper compartido, `dominio/finanzas/correccion-comun.ts::importeGuardadoDe`, y las nueve afirmaciones de «POSITIVO» que eran falsas quedaron corregidas. (ii) El **importe 0**, que ningún `Math.abs` endereza: el ETL carga `AbonoEsMa` vacío como cero, y el cajón validaba el importe en **todos** los envíos ⇒ corregir sólo la fecha era imposible (y el `min="0.01"` del campo lo bloqueaba antes, en el navegador). La regla que queda: **el mensaje del importe habla del importe NUEVO, así que sólo sale cuando hay uno nuevo**. Además se probó la **compuerta de privacidad del MOTOR** (borrar su `puedeVerImportes` dejaba 230/230 en verde) y `SQL_VIVO_PLANO` pasó a **derivar** su columna de `PLANO.columnas.canceladoEn` en vez de escribirla a mano, con guardia de texto sobre las tres sumas planas de `cuadre-f6.ts`. **Efecto lateral de fondo:** la condición de estar VIVO sube de sólo el descuento a los **tres** movimientos planos en la definición única de `formula-saldo.ts`, así que viaja sola a las **cinco sumas** del saldo y a las listas. ⚠️ **Migración aditiva** (bandera + cancelación suave en abono/pago + las cuatro ligas); **sin permisos nuevos, sin seed**. 🔧 **Paso manual de Gabriel:** prender la bandera en la base para Daniel (`UPDATE usuarios SET puede_corregir_sin_factura = TRUE WHERE username = ...`) — nadie más la tiene y **ninguna pantalla la reparte** |
>
> #### ⏸️ LO QUE ESPERA A LA FASE 2 — la lista que no se pierde (Daniel, 4-sep-2026, §Post-F9.190)
>
> **Daniel cortó la primera versión el 4-sep:** *«ya no quiero entorpecer más la primera versión de salida. Necesito ya empezar a operar con el sistema… Para la fase dos hay varias cosas que quiero integrar que hoy funcionan en Exceles. No es lo único. Pero ya quiero salir.»* Y pidió, textual: *«documenta muy bien lo que va en la V2 para no perderlo»*. **Esta lista es ese lugar.** Cada pieza conserva su número (nada pendiente existe sin número), su fila de arriba sigue diciendo qué se midió, y aquí queda **en lenguaje del negocio**: qué es, por qué espera, con qué se opera mientras tanto y de qué depende.
>
> **El criterio del corte:** lo que hoy funciona en un Excel o a mano **y no toca producción** no entra en la V1. **Lo que sí entra a fuerzas:** todo inventarios (0.096 y sus hijas, más lo que Daniel agregue al cerrar finanzas) y leer la constancia fiscal (0.119, *«nos ahorra mucho tiempo en el kick off»*).
>
> | # | Qué es | Por qué espera (con sus palabras) | Con qué se opera mientras | De qué depende |
> |---|---|---|---|---|
> | **0.127** | **El libro de caja chica con reposición.** Una o varias cajas con importe fijo (hoy dos: administración y comercial), gastos por fecha y concepto, saldo corriente por suma, y el renglón de **reposición** que devuelve la caja a su importe y que la corrida semanal **propone solo** | *«Está bien hacer el desarrollo de la caja chica. Pero ya no quiero entorpecer más la primera versión… Por ahora funciona con el archivo de la caja chica, que al final no va ligado a nada de producción»* | El archivo de Excel de finanzas sigue vivo (**no se importa**). En la corrida, «caja chica» es un **concepto predeterminado** (0.125) que se carga en cero cada semana y Daniel le pone el monto de la reposición **a mano**, mirando el archivo | 0.113 y 0.125 (ya en la V1) |
> | **0.126** | **El cotejo contra el estado de cuenta del banco** (lado con factura): importar el estado de cuenta y cruzarlo contra la relación que la corrida produjo; lo que sobre son los pagos ad-hoc («el martes urgente»), señalados para atribuir. **Proponer, nunca aplicar en silencio** (dos pagos del mismo monto son ambiguos) | Nació de *«prefiero que los pagos formales salgan del pago que esté en el estado de cuenta, no de lo que yo pongo acá»* (§Post-F9.186(a)); se corta porque no bloquea operar | La corrida produce la relación ejecutable y **los pagos nacen al ejecutarla**; el cotejo con el banco se hace a ojo, como hoy | 0.113 (ya en la V1) y un **importador bancario que no existe** |
> | **0.118** | **El documento de facturación — «yo te digo qué facturarme»:** un impreso por proveedor con los datos fiscales (razón social, RFC, régimen, uso de CFDI, CP de expedición) y los importes de la corrida, **con el IVA explícito**; sin RFC no se emite y se avisa cuál falta | *«nadie me factura si no le mando yo un documento con los datos con los que me tiene que facturar… no al revés. Y eso debe salir del sistema»* (§Post-F9.186(k)). Se corta porque hoy se hace a mano y no toca producción | Se sigue mandando a mano; los datos fiscales del proveedor **ya viven en el sistema** (R15) y la corrida ya da los importes | 0.113 y 0.110 (ya en la V1). Es un **impreso**, no un módulo: medido chico ⚠️ **Volvió a la V1 el 4-sep** (§Post-F9.191): es el mecanismo normal para que el maquilero facture el total neto del faltante |
> | **0.122** | **El precio de lista compone mal los factores:** Daniel divide tres veces seguidas y el sistema suma y divide una vez; con el redondeo al alza, **1 de cada 3–4 modelos sale un peso arriba** (medido en dos clientes). El arreglo es **una línea** (`costos/precio-lista.ts`) | *«Es irrelevante que haya un peso de diferencia. Déjalo así. Me da igual»* (3-sep). ⚠️ No se toca sin su visto bueno explícito | Se cotiza con la fórmula del sistema; la diferencia es de un peso por prenda en una minoría de modelos | Nada. Lleva además el rediseño de la pantalla de cotización que él describió (cuadrícula, editar en celda, desglose de avíos, margen), que sí es trabajo |
> | **0.086** | **Las calificaciones de maquileros con parámetro de aceptación:** un tope **configurable** (default **2 %**, sumando segundas y faltantes, global) que diga cuándo una entrega ya no es aceptable, y con eso el KPI de calidad que hoy ignora las incompletas | *«hay que fijar un parámetro de aceptación de faltantes y segundas… lo haremos cuando hagamos las calificaciones de maquileros»* (§Post-F9.173(b)); *«podríamos dejar 2 % como máximo… modificable en el tiempo»* (§Post-F9.181(b)). Se corta porque es calidad, no operación | El indicador de calidad existente (primeras vs segundas) y el ojo de Daniel; el faltante ya se cobra con la 0.109 | Nada. Dos precisiones quedan con default (sumados · global) y se le preguntan **enteras** cuando arranque |
> | **0.072** | **Corregir la receta en toda la familia de órdenes de una vez:** cuando un desarrollo sale a producción, cada renglón del pedido hace nacer su propia orden con su modelo (la «familia»); si después se descubre un error en la receta, hoy se corrige **orden por orden**. Esto es el botón de «corrígelo en todas», saltándose la ya cortada y avisando cuál fue | Daniel preguntó qué era y, al saber que el camino de a una ya existe: *«Ok. La 72 en la V2»* | Se corrige orden por orden (`traerDelModelo`, que ya devuelve traídos y respetados) | Nada |
> | **0.087** | **La ficha del modelo hijo diverge de la del padre** (composición, curva, temporada… se **copian** al nacer el hijo; si el padre cambia después, el hijo nacido antes se queda con lo viejo y la OP se lo lleva al papel). Default propuesto: **avisar** de la divergencia | Aparcada por Daniel el 2-sep: *«es algo poco relevante. No quiero perder tiempo en esto. Ya quiero sacar la primer versión»* (§Post-F9.181(d)) | La receta (BOM) **sí** es una sola para toda la familia (0.070); sólo la ficha descriptiva se copia. Se revisa a ojo | Nada. Daniel dejó un reencuadre en §Post-F9.181(d) que vale más que las opciones propuestas |
>
> **Las «varias cosas» que hoy viven en Excel** y que Daniel quiere integrar en la fase 2 **todavía no las nombró**: *«No es lo único»*. **Regla para cuando las nombre:** se numeran en el momento (⏸️, con sus palabras y la fecha), se agregan a esta tabla y **no se construyen** hasta que él abra la fase 2. No se le preguntan ahora: se recogen cuando salgan.
>
> **Lo que NO está aquí a propósito:** 0.116 (el menú que enciende dos opciones) sigue en la V1 pero **al final de todo** (*«es sólo estético, hazlo cuando ya no tenga pendientes»*), y 0.120 (`roles.administrar`) también va al final (*«lo arreglamos cuando vayamos a producción»*).
...
> 🔴 **La etapa se construyó en una sesión y se VERIFICÓ en otra** (30-ago): el coder fue detenido
...
> `desarrollo → precosteo → lista → cotización` y se inventariaron **18 avisos** (tabla completa en la
> ficha, vale oro): **cinco** estaban sin salida —y los tres de Pre-costeos **tenían botón pero sin
> medir el permiso**, que es la otra mitad del defecto—. 🔴 **Dos correcciones del reviewer, que valen
> más que el inventario:** (1) el aviso del **tipo de prenda sin dígito** se había dejado sin puerta con
> una razón **FALSA** —*"quien lo ve no administra el catálogo de Calidad"*—: medido, **`Administrador`,
> el rol de Daniel, sí lo administra** (2 de 9 roles) ⇒ **la puerta se construyó**, porque el recíproco
> de la regla es que *a quien PUEDE cruzarla, se le pinta*; (2) faltaba un **eco gemelo** (el **género**
> sin dígito, la misma idea con otras palabras — la cicatriz de barrer por frase), que **no lleva puerta
> porque su rama es inalcanzable hoy** (medido: `/api/generos` es sólo GET, sin pantalla de alta, y el
> seed re-siembra el dígito) y queda como **deuda con nombre** en §4; y (3) al barrer **la tabla**
> —no sólo la fila señalada— apareció un **#18** que el propio grep del barrido había devuelto y nunca
> se pasó al inventario (el vacío de Proyectos), también deuda con nombre. ⚠️ **Y la corrección del
> punto (1) se había escrito donde se EXPLICA el error y no en la fila que lo declaraba como hecho** —
> la misma forma del defecto que se persiguió toda la jornada: *corregir el relato y dejar el veredicto
> viejo en pie.*
...
> el 26-jul-2026 ese estado es **automático** (tallas + receta liberada, y arte si aplica) y "Órdenes
...
> ### 📌 Deuda con nombre, anotada el 4-sep-2026 (fila 0.124 / 0.106) — `factura` sigue saliendo del API, derivada
>
> `Proveedor.factura` dejó de capturarse y se **deriva** de `modalidadFacturacion`, pero **sigue en la SALIDA** del API porque `frontend/src/modulos/inventarios/CapturaEntradaTelaPagina.tsx` la lee (`proveedorResuelto?.factura === false` fuerza el documento a remisión): hoy obedece a la modalidad sin saberlo, y quedó arreglada gratis por la derivación. Dos nombres para la misma pregunta en el mismo payload es, en pequeño, lo que Daniel llamó *«un error que existan»*. **Se retira de la salida cuando esa pantalla lea `modalidadFacturacion` directamente**; mientras, es una proyección que no puede contradecir a la modalidad (el lado de escritura ya no existe). Lo señaló el reviewer de la 0.124 (R4).
>
> ### 📌 Deuda con nombre, anotada el 4-sep-2026 (fila 0.113 / 0.102) — tres cosas que la corrida deja a la vista
>
> 1. **`Proveedor.formaPago` (texto libre con la clave del SAT) quedó superado por `formaPagoPreferida`** (efectivo | transferencia): sigue en base y en el contrato (REGLA 0-B) pero ya no se captura ni se muestra. Si el timbrado vía PAC (R14) necesita la clave SAT, ese día se vuelve a mostrar el campo — hoy nadie la lee.
> 2. **No existe cancelación de `PagoMaquilero`** en EsMa (CxP sí tiene `cancelarMovimientoCxp`). Preexistente, pero la corrida lo vuelve alcanzable en masa: «marcar como pagada» crea N pagos de maquila de un clic. Mitigado con la confirmación de conteo y totales; la marcha atrás real (inverso auditado, D3) es una fila nueva.
> 3. **`crearPagoMaquilero` y `crearPagoACuentaMaquilero` exigen `esma.ver-pagos`** —un permiso de *ver*— para *crear* un pago (heredado de F6). Preexistente; se anota para que no se dé por bendecido al copiarlo.
>
> ### 📌 Deuda con nombre, anotada el 3-sep-2026 (fila 0.115 / 0.099) — el pie del PDF del estado de cuenta
>
> **El pie del PDF del estado de cuenta muestra el «Por revisar» sólo con el neto** (`impreso-estado-cuenta.ts::columnasBloqueSaldo`): con partidas que netean cero imprime `$0.00`. **No se arregló** en 0.115 porque el conteo ya se recupera del papel —la columna sólo aparece si hay partidas (`hayPendiente`), y cada renglón pendiente va marcado en los tres conceptos— y del Excel, que lleva el conteo en su propia fila; y porque tocar `estilos.saldoItem` cambia la maqueta de **un documento firmable** que sólo Gabriel re-verifica en `prueba`. **Se cierra la próxima vez que se toque el layout del impreso**, metiendo el conteo junto al importe (`textoPorRevisar`, que ya existe en `esma/comun.ts`). *(Reviewer de 0.115, 3-sep-2026.)*
>
> ### 📌 Deuda con nombre, anotada el 1-sep-2026 (V1-E9f / 0.074) — el hueco de `'SIN COLOR'`
>
> Si el papel del cliente viene **sin color**, `importacion-pdf.ts:252` convierte `''` → **`'SIN COLOR'`** y
> lo pasa por `resolverOCrearColor` **con redirect por fusión incluido** — pero la previa **excluye ese
> renglón** del lote (`:575`) y la UI ni pinta el bloque «Color».
>
> ⇒ **No es sólo que la previa no diga «(nuevo)»:** si existiera un `'SIN COLOR'` **fusionado**, el confirm
> **lo desviaría en silencio** — exactamente el defecto que la 0.074 vino a matar, en el único renglón que
> la etapa no cubre.
>
> ⚖️ **Por qué NO bloqueó la etapa, y por qué estuvo bien no tocarlo:** es angostísimo —exige un
> `'SIN COLOR'` **duplicado y fusionado** *antes* de que ninguna orden lo use, y
> `colores-fusion-referencias.ts` **bloquea la fusión** en cuanto hay `ordenLineas`— y taparlo obliga a
> decidir **qué pinta la UI en un renglón sin color**, que es **diseño, no corrección**.
>
> 📌 Se anota por *«un defecto conocido no es menor»*: queda **escrito con su razón**, no mencionado de paso.

> ### 📌 Deuda con nombre, anotada el 1-sep-2026 (V1-E9d / 0.072)
>
> **El guardián de escritores de receta NO puede cazar un `export default` ANÓNIMO** (`export default
> class { … }`, `export default { … }` con un método que escriba). No es un hueco del regex: **no hay
> binding que declarar**, así que el mecanismo entero del guardián —*«nombre propio en el conjunto»*— no
> les aplica, y el conteo tampoco las ve porque quedan absorbidas por el vecino.
>
> ⚖️ **Por qué NO bloqueó la etapa:** hay **cero `export default` anónimos** en todo el ámbito barrido, y a
> diferencia de `export abstract class` **no se pudo señalar un solo sitio donde este repo lo escriba** (el
> único `export default` vive en `src/pruebas/`, que está excluido). **La salida limpia es prohibirlo por
> lint**, no perseguirlo con más regex — perseguir formas es el regreso infinito que la cabecera del
> guardián acota al decir *«no es un parser»*.
>
> 📌 Quien toque `FORMAS_DE_DECLARAR`: **añade estas dos con esa nota**, para que se sepa que están fuera a
...
- 🟡 **ABIERTO POR LA 0.100 — LA FLECHA DEL PIE DE LA HOJA DE TRASPASO DE TELA SALE COMO UN
  ...
  dice `de X a Y`); la de **TELA** lo arrastra en `backend/src/dominio/inventarios/impresos/
  impreso-traspaso-tela.ts`, en su `` `${almacenOrigen} → ${almacenDestino}` ``.
  📌 **Por qué no lo arregló la 0.100**: es de otra fila y tocarlo era arrastre. Es **cosmético y sin
  riesgo de datos** —el papel dice los dos almacenes igual, sólo que el separador sale feo—, pero
  está **medido**, así que no se calla: arreglo de una línea (`de X a Y`, o el guion largo) el día
  que alguien toque ese impreso.
- 🟡 **ABIERTO POR LA 0.061 — EL «403-TRAS-COMMIT» DE LAS SIETE ESCRITURAS DE `ordenes.ts`.**
  El patrón *escribe → comitea → y sólo al final `obtenerOrden` exige `ordenes.ver` para poder
  devolver la orden* está en **siete** funciones de `backend/src/dominio/produccion/ordenes.ts`:
  `crearOrden` · `actualizarOrden` · `guardarMatrizOrden` · `copiarDetalleOrden` · `cancelarOrden` ·
  `guardarReferenciasOrden` · `agregarComentarioOrden`. Las siete exigen `ordenes.administrar` o
  `ordenes.cancelar`, **comitean**, y después piden `ordenes.ver` ⇒ una sesión que escriba pero no
  pueda ver recibe un **403 sobre un cambio que YA se guardó**: el usuario cree que no pasó nada y
  el cambio está hecho. Es la misma lección que `cancelarPedido` documenta desde F8-E3.
  ⚠️ **Hoy es LATENTE en las siete, medido**: ningún rol del seed escribe órdenes sin poder verlas,
  así que sólo lo dispararía un rol a medida — y repartir permisos a medida entre los 23 usuarios
  reales es justo lo que hará **F10**.
  📌 **Por qué no lo arregló la 0.061**: es un defecto de **F2**, no de esa fila. La 0.061 arregló
  **las dos que ella misma creó** (`cerrarOrden` y `reabrirOrden`, con el `verificarPermiso` antes
  de abrir la transacción y una prueba de integración que comprueba que tras el 403 `cerradaEn`
  sigue NULL); meterle mano a las otras siete en su ronda de cierre era el arrastre que convierte
  una corrección en otra fila.
- 🔴 **El `eslint` del CI YA NO CABÍA en su techo: subió a 8 GB SÓLO ahí, y esa venda fue la última** (medido en el PR #307; decisión de **Gabriel**, 5-sep-2026). Murió con **exit 134** contra 6144, **pico 6011 MiB**, reproducido dos veces. ⭐ **El guardián de la fila 0.092 hizo exactamente su trabajo:** el lead subió el techo en los tres sitios de `ci.yml` con el CI en rojo y prisa, la prueba se puso **roja**, el cambio se **revirtió** y la decisión subió a Gabriel en vez de quedarse en un `git push`. **Las dos salidas baratas se midieron y están MUERTAS: (a) partirlo NO sirve** — `eslint src/dominio` **a solas** revienta con **4096 y con 5120** (⚠️ la medición del 4-sep que decía «mueren las dos con 3 GB» **no probaba nada**: 3 GB es demasiado estrecho para cualquiera de las dos mitades; re-bisecada, la conclusión se sostiene pero ahora sí está medida) porque el costo es el **grafo de tipos**, que cada mitad carga entero; **(b) las palancas de configuración YA estaban puestas** — `eslint.config.js` usa `parserOptions.projectService` y **ya ignora** el cliente generado de Prisma (`src/datos/generated/`, 22 MB / 181 `.ts`). ⇒ Se subió a **8192 el paso del lint y NADA más**: `typecheck`, `build` y **`backend/Dockerfile` siguen en 6144**, porque el Dockerfile **no corre eslint** (no queda ningún hermano atrás, que es la rama gemela del guardián) y porque la memoria del runner de GitHub **es medible** (16 GB) mientras que la del contenedor que construye en Railway **no lo es** — y un techo por encima de lo que el contenedor da cambia un OOM limpio de V8 (134) por el OOM-killer del kernel (137). Medido a 8192: **pasa, pico 7034 MiB (~14 % de margen)**. ⛔ **La próxima vez NO se toca el número:** con ese margen y sin salidas baratas, toca **la cura** —adelgazar el programa de tipos (proyectos de TS separados, tipos generados más chicos)—, y **merece fila propia**.
...
- **🔴 ABIERTO POR V1-E8p (fusión de departamentos, §Post-F9.122(a)) — LA QUINTA PIEZA: la búsqueda por
  ...
  **además** el texto CRUDO de la División (`"2-HOMBRE"`) en `OrdenReferencia.valor` (D7), **indexado
  ...
  ✅ **CONTESTADA (Daniel, 31-ago-2026, §Post-F9.172(a)): *«está bien la 3»* ⇒ camino (iii), búsqueda por
  SINÓNIMOS — el texto capturado NO se reescribe nunca.** Lo que decía antes, y por qué era correcto
  esperar: reescribir un valor capturado de un documento
...
- **🔴🔴 DECIDIDO Y SIN CONSTRUIR (29-ago-2026, §Post-F9.144(c)) — LA COMPUERTA DE LA OP SE MUEVE A LA
  COMPRA.** Va aquí, y no colgado de una etapa, porque **atraviesa tres
  módulos** (Desarrollo, Producción y Compras) y **detiene trabajo real**: mientras no se resuelva, un
  modelo cerrado en la mesa con la OC del cliente en la mano **no puede tener OP** hasta que Desarrollo
  termine de buscar maquila.

  **Lo que Daniel pidió:** *"deberia de poder meter la OP y dejar pendiente la reconfiguracion de la
  receta para que se hagan las modificaciones antes de aprobar y comprar todo"*.

  **Lo que el sistema hace hoy — MEDIDO el 29-ago-2026, por nombre de símbolo:** `salidaAProduccion`
  (`dominio/produccion/salida-produccion.ts`) → `promoverAProduccionNucleo`
  (`dominio/modelos/nomenclatura.ts`) → `exigirRevisionAprobadaParaProducir`
  (`dominio/modelos/revision-modelo.ts`, **único sitio de llamada en todo el repo**), que **lanza** cuando
  `revisionBloqueaProduccion` es cierto (*es versión* **y** *estado ≠ `aprobada`*). Y la versión nacida de
  la negociación se crea con `origen: 'desarrollo'` + `revisionEstado: 'pendiente'`
  (`dominio/modelos/versiones.ts`) ⇒ **la OP se rechaza**. El sistema obliga a cuadrar **antes** de la OP;
  Daniel necesita cuadrar **después de la OP y antes de comprar**. **Es el orden invertido, no un matiz.**

  ✅ **DANIEL CONTESTÓ el 29-ago-2026 — DECIDIDO:** *"si, mueve la compuerta al comprar. Por que de hecho
  ya habiamos visto quer podria haber elementos de la receta ya aprobados y otros no. Asi podemos ir
  comprando la tela en lo que se terinan de aprobar los demas elementos"*.

  🔴 **PERO NO SE MUEVE TAL CUAL — SE DISUELVE. Esto es lo que hay que leer antes de codear**, porque
  «mover la compuerta a la compra» al pie de la letra **recrea el problema un paso después**. Son dos
  mecanismos con **granularidad distinta**: `Modelo.revisionEstado` es **TODO O NADA** (el modelo entero
  aprobado o no) y `OrdenTela/OrdenAvio.liberadoEn` es **POR RENGLÓN** (los dos en `prisma/schema.prisma`).
  Mudar el candado grueso a la puerta fina **bloquearía comprar TODO** hasta que el modelo entero estuviera
  aprobado — o sea, **impediría justo lo que Daniel acaba de pedir**. **El plan son dos piezas:**
  ...
  `copiarRecetaDelModelo` (`dominio/produccion/receta-orden.ts`), por donde pasa **el 100 % de las
  órdenes**, **no menciona `liberadoEn` ni una vez** en su cuerpo, y el esquema **no le pone `@default`**
  ⇒ **todo renglón nace `liberadoEn = NULL` = sin firmar, hoy, por construcción.** La firma sólo la
  escriben el acto de **liberar** y el **ETL de migración**. **La pieza 2 no es código: es una prueba** que
  fije ese comportamiento para que nadie le ponga un `@default` después. **El trabajo real es la pieza 1.**

  ⚠️ **LA CONSECUENCIA QUE CAMBIA UNA EXPECTATIVA, y por eso va escrita:** al quitar esa exigencia,
  **aprobar el modelo deja de frenar la producción por sí solo**. Su efecto pasa a ser que los renglones
  nazcan sin firma ⇒ **frena la COMPRA, no la captura del pedido ni la OP.** Si alguien esperaba lo
  primero, **ya no ocurrirá**. Está dicho a Daniel y **él lo aceptó en el mismo mensaje**.

  ⬜ **Lo que la etapa todavía tiene que resolver** (enumerado en la decisión): 🔴 **qué pasa con la COPIA
  de la receta que la OP se lleva** (`copiarRecetaDelModelo` congela el BOM al crear la orden, §Post-F9.34
  — si la OP nace antes de reconfigurar, se copia la receta VIEJA; **esto no lo contestó Daniel** y es lo
  que más mueve el alcance) · **si cortar también puede** sin receta firmada (sin default) · y que la
  **tercera puerta** de `crearOrden` **deja de ser un hueco** ⇒ hay que borrar esa advertencia del código
  y de la deuda de `V1-etapas.md` §V1-E7d en vez de dejarla contando un peligro que ya no existe.

  ⚠️ **Toca de rebote la bandeja de `V1-E8r`:** su criterio de orden se midió sobre *"por los caminos de la
  UI una versión frenada no llega a tener OP"*, **que deja de ser cierto** ⇒ se revisa en la misma etapa.

  **El porqué completo, las citas textuales de Daniel y la cadena medida están en
  `Documentacion_MJD/DECISIONES.md` §Post-F9.144 (c).** *No se copia aquí a propósito: una copia deriva.*

  ⬜ **Estado: DECIDIDO, SIN ETAPA ASIGNADA. Sólo documentación — no se tocó una línea de código y no sube
  la versión.**

- **⭐⭐ §Post-F9.144 (29-ago-2026) — CÓMO SE NEGOCIA DE VERDAD, y qué AFINA.** Daniel explicó el proceso
  completo de su negociación. **Sólo documentación.** Detalle y citas en `DECISIONES.md` §Post-F9.144; en
  una línea cada pieza:

  - **Son TRES momentos, no dos** (§Post-F9.139 decía dos): **vender** (receta y lista ya hechas) →
    **negociar** (rápido, con estimados, **sin tocar receta ni catálogos**) → **oficina**, donde
    ⭐ **DESARROLLO —no Daniel—** hace una **versión nueva** de la receta y **sale a buscar** avíos, artes
    y maquilas que cumplan. Separa dos roles que la documentación venía mezclando.
  - ⭐⭐ **Los estimados NO son datos: son METAS.** *"me quitan un cierre y yo le pongo que estimos que la
    mquila costara 5 pesos menos"* no registra un hecho — **fija el precio que hay que conseguir**, y él
    mismo advierte *"no es seguro que se consiga"*. ⇒ **Afina §Post-F9.140:** la bandeja (ya construida en
    `V1-E8r`) **no debe preguntar «¿ya lo capturaste?» sino «¿se logró lo que se prometió, sí o no?»** —
    porque la respuesta puede ser **que no**, y eso tiene que verse. ⬜ **Afinación pendiente, no
    construida.**
  - ✅ **El choque de la compuerta — CONTESTADO por Daniel:** *"si, mueve la compuerta al comprar"*, con
    su razón (*"podria haber elementos de la receta ya aprobados y otros no"*). ⚠️ **No se mueve tal cual:
    se DISUELVE** en la firma por renglón que ya existe — ver el bullet de arriba. ⬜ **Sin construir.**
  - ⭐ **Inventarios: el lead estaba equivocado.** Daniel **rechaza** que el almacén dé de alta colores
    (*"el color se da de alta en la OC… no puede recibir nada que no se haya comprado con una OC"*). ⇒ **la
    pregunta abierta de §Post-F9.106 queda CERRADA EN CONTRA** y el alta **no se construye**; lo que queda
    es **afinar el letrero** de `V1-E8o` para que diga **qué significa**, no sólo a dónde ir. Ver la deuda
    de `V1-E8o` más abajo en esta misma sección.
  - ✅ **Los factores: RATIFICACIÓN del dueño** (*"Nadie mas que yo ve los factores por favor…"*). Ya está
    así en **tres capas** (`puedeVerFactoresDePrecio` en el servidor · `EditorFactoresCliente` en la
    pantalla · `roles-reparto.test.ts`, que afirma que **Gerencial NO lo tiene** y se pone roja si alguien
    se lo da). **Sin trabajo** — se registra para que no se afloje "por comodidad" más adelante.
...
- **⭐⭐ DECISIONES DEL 29-ago-2026 (§Post-F9.138–.142) — QUEDA ESCRITO EL DISEÑO DE
  ...
  🔁 **AFINADAS el 29-ago-2026 por §Post-F9.144** (el proceso real de la negociación, explicado por
  Daniel): `.138` y `.139` (los estimados son **METAS**, y los momentos son **TRES** — el tercero lo opera
  **Desarrollo**), `.140` (la bandeja pregunta *«¿se logró?»*, con **dos** desenlaces) y `.141` (el hilo es
  la **materia prima** con la que Desarrollo reconfigura). **Ver el bullet de §Post-F9.144 arriba en esta
  misma sección** — y el **bloqueante de la compuerta de la OP**, que nació de la misma conversación.
...
> ⚠️ **CÓMO SE LEE ESTA SECCIÓN.** Una nota de deuda se escribe cuando la deuda se **descubre** y casi nunca se vuelve a tocar cuando se **paga** ⇒ **es el documento menos medido del repo**. Antes de convertir una nota en trabajo, en una fila del programa o en una frase para Daniel, **se re-mide contra el código**.
>
> **No están podridas** —eso también se midió, con muestra del 1-sep—: la de `confirmarLogo` sigue siendo **cierta**, y la del borrado físico en R2 también (cuatro módulos de adjuntos llaman `eliminarObjeto`; **tres no**: `fotos-modelo.ts:404`, `arte-modelo.ts:846`, `proveedores.ts:~841` — es la fila **0.081**). **La falsa era una**, y está tachada justo debajo.

- **Deuda técnica — los 4 `adjuntos-*` no tienen guardián del commit (1-sep-2026, señalada por el reviewer de `V1-E9m`, explícitamente FUERA del alcance de 0.081(a)):** `adjuntos-desarrollo`, `adjuntos-entrada-tela`, `adjuntos-pedido` y `adjuntos-orden` llevan su `try/catch` **en línea** con la MISMA invariante (borrar en R2 **después** del commit) y la misma exposición ⇒ **hoy la mutación que la viola sobreviviría ahí**. **No es regresión** —este diff no los toca— pero ahora que `src/pruebas/commit-r2.ts` existe, extenderlo cuesta **~5 líneas por puerta**.
- **Deuda técnica — el guardián de huérfanos es por MÓDULO, no por sitio de borrado (2-sep-2026, v0.089, declarada DENTRO del código):** hay **15 sitios en 12 módulos** (`admin/empresas.ts`, `modelos/arte-modelo.ts` y `comun/jobs/respaldo-bd.ts` llevan **dos cada uno**). El guardián exige que **algún** `*-r2.test.ts` importe el módulo **e invoque** `exigirBorradoTrasElCommit`, no que pruebe cada función. **Hoy no hay hueco** —los dos módulos con dos puertas las tienen las dos cubiertas, medido— **pero una TERCERA puerta en uno de ellos pasaría inadvertida**. Y el guardián es **de texto**: no ve `$executeRaw` con DELETE (hoy 0), el despacho por variable, los borrados anidados (hoy 0) ni el SQL de `prisma/migrations/**`; tampoco escanea `scripts/` (3 hard-deletes de demo) ni `migracion/` (0), cubiertos por REGLA 0-B — ⚠️ **si mañana un ETL borra una `Orden` con adjuntos, no lo ve**. ⭐ **Lo que lo salva de envejecer es que la SUPERFICIE se calcula del esquema**, no se lista a mano.
- **Deuda técnica — CUATRO puertas a R2 siguen sin guardián de commit, y ahora se sabe por MEDICIÓN que su suite tampoco las ve (2-sep-2026, v0.089):** los cuatro `adjuntos-*` sueltan su objeto con un `try/catch` **en línea** fuera de la transacción —correcto hoy— pero **sin `*-r2.test.ts`**, así que la mutación «mover el borrado DENTRO de la transacción» **sobreviviría**. Precedente medido en esta misma etapa: con esa mutación puesta en otra puerta, **la suite que ya la cubría se quedó 44/44 EN VERDE**. ⭐ **Ya no está callada**: el guardián de la 0.091 las lleva en `PUERTAS_R2_EXENTAS` **nombradas como deuda declarada**, así que la lista no puede pudrirse en silencio. Cuesta ~5 líneas por puerta. *(Viene del reviewer de la 0.081(a), que la dejó fuera de alcance a propósito.)*
- **Deuda técnica — «qué orden cuenta como VIVA» está escrito TRES veces, y sólo dos quedaron unificadas (2-sep-2026, decidida en `V1-E9r` con razón medida):** `backend/src/dominio/pedidos/ordenes-vivas-del-renglon.ts` unificó las dos de Prisma (los renglones del mes y el detalle del pedido); **la tercera sigue suelta en SQL crudo** — `consulta-mes.ts:245`, `JOIN ordenes o ON … AND o.estado <> 'cancelada'`, que alimenta **los TOTALES de la página**. ⚠️ **Consecuencia visible si divergieran: los totales discreparían de los renglones DE LA MISMA PANTALLA.** **No se absorbió a propósito**, con tres razones medidas: (1) parametrizar un enum en SQL crudo **no tiene precedente en el repo** —los 7 sitios usan literal— y un fallo de inferencia **reventaría sólo en CI**; (2) exportar un `Prisma.Sql` **arrastra el alias `o` como contrato oculto**, y la variante con `Prisma.raw` *«tiene forma de inyección»*; (3) **no son la misma cosa** (dos `where` de Prisma vs. una condición de `JOIN` dentro de un agregado). ⭐ **Mitigación construida**: el comentario de `:245` **NOMBRA la función**, así que `grep filtroOrdenesVivasDeLineas src/` **saca las tres de un golpe**, y el docstring del módulo estrena una sección **«LO QUE ESTE MÓDULO NO UNIFICA»**.
- **Deuda técnica — la puerta de des-autorizar es MÁS ANCHA que el aviso (2-sep-2026, declarada en `V1-E9q`):** `AvisoCambioSobreLoComprado` recibe `d.ocsComprometidas`, que el servidor arma como **lo comprometido de TODA la orden** (`receta-orden.ts:1321-1323`), mientras el texto del aviso nombra **sólo las OC del material que se acaba de tocar** ⇒ con una OC de tela y otra de avíos ambas autorizadas, cambiar la tela **ofrece des-autorizar las dos**. **No se estrechó a propósito**: los botones acompañan a los chips, y **los chips ya pintan toda la orden desde la 0.077**, así que estrecharlos sólo aquí haría que **botón y chip contaran cosas distintas en el mismo bloque**; hacerlo de verdad exige que el servidor devuelva las OC **del cambio**, o sea **contrato**. ⚠️ Riesgo acotado: **nada se des-autoriza sin que una persona elija el folio, escriba el motivo y confirme**.
- **Deuda técnica — higiene de dos pruebas de la entrada de tela (1-sep-2026, señalada por el reviewer de `V1-E9l`):** en el `describe` viejo de `CapturaEntradaTelaPagina.test.tsx` los espías `espiaLineasOc`/`espiaProveedorTelas` **no se limpian** entre pruebas, y `:675` usa `toHaveTextContent('hay')` **sin anclar**. ⚠️ **Medido: hoy NO mienten** — se les puso `mockClear()` y las 25 siguieron pasando, y ningún estado contiene «hay» como subcadena. El riesgo es **latente**, no vivo. Se anota porque en esta misma etapa la forma sin anclar **sí** dejó pasar la lógica de alcance completamente muerta con 25/25 en verde.
- **~~Deuda técnica — anti-lockout en el dominio de USUARIOS (descubierta 2026-07-06)~~ → 🔴 FALSA, MEDIDA EL 1-sep-2026. NO ES DEUDA: está cubierto, y por partida doble.** El guard vive **en el EMBUDO**: `usuarios.ts` tiene **una sola** llamada a `exigirQuedaAdministrador` (:339), dentro de `actualizarUsuario` (:276) — y **las dos puertas que la nota nombraba delegan ahí**: `desactivarUsuario` (:435) y `asignarRoles` (:540) hacen `return actualizarUsuario(...)`. Quedan cubiertas **por omisión**, que es el patrón *embudo*. ⭐ **Y hay una QUINTA puerta que la nota ni vio, también cubierta**: `auth/login.ts:218` escribe `bloqueado` sin pasar por el embudo (los 5 intentos fallidos), y está resuelto a conciencia con `claveQueQuedariaHuerfana` **bajo el lock del guard**, con el compromiso de seguridad escrito: *«al último administrador vivo no se le bloquea la cuenta por intentos fallidos… la alternativa es un ERP capaz de auto-inutilizarse con cinco tecleos mal dados»*. 🔴 **Esta nota ya causó daño**: el 31-ago originó una fila del programa (la 0.080) y una frase alarmista a Daniel, las dos falsas. Se corrige **y no se borra**, para que el siguiente no la re-descubra. *(Nota original abajo, tachada:)* ~~el guard anti-lockout se cerró para roles, pero `usuarios.ts` NO tiene guard análogo~~ — el guard anti-lockout se cerró para **roles** (`asignarPermisos`/`eliminarRol`), pero `backend/src/dominio/admin/usuarios.ts` (`desactivarUsuario` / `asignarRoles`) **NO** tiene guard análogo: desactivar al último usuario administrador, o quitarle sus roles, puede provocar el mismo lockout del RBAC por la otra puerta. Fix futuro: aplicar la MISMA invariante ("≥1 usuario activo con `roles.administrar`") bajo el mismo `pg_advisory_xact_lock` de clave constante. Sin fase asignada — retomar cuando se priorice.
...
- **Deuda técnica — el vacío de PROYECTOS no distingue «no hay ninguno» de «no coincide con la búsqueda» (detectada por reviewer el 29-ago-2026 en `V1-E8t`, NO arreglada):** `frontend/src/modulos/desarrollo/ProyectosPagina.tsx` dice **siempre** *"No hay proyectos que coincidan con la búsqueda"*, también cuando el universo está en cero y nadie ha buscado nada. **Es el MISMO defecto que `V1-E8t` acabó de arreglar un piso más abajo** (el vacío de «Listas de precios», con `vacioDeUniverso`) y §Post-F9.128 ya había arreglado en el diálogo de candidatos — **en la cabeza del mismo flujo que esa etapa barrió**. **Por qué NO se arregló ahí:** cae **fuera del estado prohibido** que la etapa perseguía (no deja a nadie sin salida: la tarjeta «Nuevo proyecto» está al lado y visible para quien puede crearlos), pero **dentro de la idea** que dijo haber buscado, así que callarlo sería exactamente la clase de nota al pie que este proyecto prohíbe. Fix, barato y ya con molde: derivar `vacioDeUniverso` como en `ListasPreciosPagina` y decir *"todavía no hay ningún proyecto: crea el primero"* en esa rama. Sin fase asignada.
...
- **🔴 DEUDA NUEVA (29-ago-2026, `V1-E8o`) — «la TERCERA puerta»: en el ALMACÉN, la tela sin el color
  ...
  **cuatro** pantallas: entrada de tela, traspaso, ajuste y salida por orden — y desde §Post-F9.14 la
  ...
  ⚠️ **Por qué el alta esperaba:** la pantalla vive bajo `inventario-telas.mover` y el servidor exige
  **`compras.administrar`** para `agregarColorATela` → un almacenista pulsaría el botón y se comería un
  **403**. **No existía decisión de Daniel** sobre cuál debe ser el permiso, e inventarlo está prohibido.

  🔁 **ACTUALIZACIÓN (29-ago-2026, §Post-F9.144(d)) — el ALTA queda CERRADA EN CONTRA: no se construye.**
  Daniel no eligió permiso: **rechazó la premisa.** *"no se puede dar de alta un color al recibir. El color
  se da de alta en la OC. Por que no puede recibir nada que no se haya comprado con una OC… Por que
  recibiria algo que no este dado de alta? quiere decir que no hubo una OC previa?"* ⇒ **no hay permiso que
  elegir, porque la función no debe existir.**
  ⭐ **Y el reencuadre es lo que vale:** esto se venía tratando como **una función que falta**; es un
  **SÍNTOMA**. Si el almacén se topa con un color inexistente, lo que falla está **antes** — se está
  recibiendo algo **sin OC**. Montar el alta ahí habría **borrado la única señal** de que faltó la orden de
  compra.
  ⬜ **Lo que SÍ queda pendiente (y es lo único):** **afinar el LETRERO** para que además de decir *a dónde
  ir* diga **qué significa** — *«este color no viene de ninguna OC; verifica que la compra exista»*—,
  convirtiendo un callejón sin salida en un **diagnóstico**. **No se construyó nada el 29-ago: sólo quedó
  escrito.**
  ✅ **DECIDIDO (Daniel, 30-ago-2026, §Post-F9.159(a)): se IMPIDE — bloqueo, no aviso.** *«es imposible.
  Porque sin OC no podemos recibir tela. **¿De quién recibiríamos sin OC?** No puede suceder.»* ⇒ aquí
  decía *«sin decidir… pregunta para Daniel»* sobre algo **que él ya había contestado**, y así se la
  habría vuelto a preguntar. *(Queda superada la línea 736 de `DECISIONES.md`, que permitía las dos vías.)*
  🔴 **Y de esa respuesta sale trabajo que el plan no tenía:** la vía sin OC **existe construida**
  (`dominio/inventarios/entradas-tela.ts`, con `idOrdenCompraLinea` nullable) y **hay que cerrarla**. No es
  un letrero: es una **guarda de dominio**. §Post-F9.159 la asignó a la 0.072, no al paquete del letrero.
  ⚠️ **Y el letrero, medido el 31-ago, no dispara donde el plan cree:** se enciende con
  `tela.colores.length === 0` (`CapturaRenglonesTelaColor.tsx:362`) — la tela **sin ningún color en el
  catálogo**, donde no hay ningún color que contrastar contra ninguna OC. La frase *«este color no viene de
  ninguna OC»* habla de un color que ahí **no existe**. El caso hermano —renglón de OC **con** tela y **sin**
  color, `idTelaColor: null`, las ~7,978 OC migradas— se pinta **sin decir nada** (`:272`), y **ése** es el
  silencio real.
  ...
  **cubría sólo la mitad** — se usó para no hacer NADA, cuando el permiso bloquea *construir el alta*,
  ...
  backend (`agregarColorATela`) ya existe.~~ 🔁 **Ya contestado el 29-ago-2026 (§Post-F9.144(d)): NO se
  monta.** Quien lo retome, lo único que hay que hacer es **el letrero**.
...
> ⏸️ **Lo que Daniel mandó a la FASE 2 el 4-sep-2026** (caja chica, cotejo bancario, documento de facturación, precio de lista, calificaciones de maquileros, corregir en bloque la familia) vive con ficha completa en §1, tabla *«Lo que espera a la fase 2»* (§Post-F9.190). No es hueco: es decisión suya, con sus palabras.
