...
> | **0.080** ⬜ | 🔴🔴 **LAS DEFENSAS QUE CREEMOS TENER Y NO TENEMOS.** Tres cosas del mismo tema: el sistema **se comporta como si estuviera protegido** y no lo está. **(a) El anti-lockout, por la otra puerta:** el guard que impide dejar el sistema sin administrador se cerró para **roles**, pero `dominio/admin/usuarios.ts` (`desactivarUsuario`/`asignarRoles`) **no lo tiene** ⇒ desactivar al último administrador provoca **el mismo lockout** por la puerta de al lado. Es **la rama gemela, en seguridad**. **(b) `singletonKey` NO serializa nada, y la Ruta Crítica cree que sí** — **verificado ejecutando** contra pg-boss real: dos `send` con la misma clave, **ambos aceptados**; los índices únicos sólo existen para colas con política, y `comun/jobs/index.ts` las crea **todas sin política** ⇒ **la defensa anti-duplicado de la RC es decorativa**. **(c) El respaldo doble** (`pg_dump` diario cifrado a R2, PLANMAESTRO §2.2): es **la mitigación nº1 de la tabla de riesgos** y **hoy nadie la tiene** — estaba planeada «al inicio de F1, en cuanto Gabriel monte R2», y **R2 lleva meses montado**. ⚠️ El cliente `pg_dump` va **atado a la major del servidor** | PLANMAESTRO §2.2 |
