...
> | **0.123** ⬜ | 🔴 **LOS DATOS PERSONALES QUE YA SE PUBLICARON — remediar y poner la regla** (nace el 3-sep de revisar el PR #287 antes de mergearlo). El repositorio es **PÚBLICO**. Dos exposiciones medidas: **(1)** el PR #287 subió cinco Excel de Daniel «limpios» que **no lo estaban** — cuentas, CLABE, tarjetas y RFC sí se quitaron (cero rastros, verificado), pero quedaron **77 nombres completos de beneficiarios pegados a su monto** en `relacion-pagos-sin-factura.xlsx` (columna O) y **nombres de autor en los metadatos** de los archivos (`docProps/core.xml`); **(2)** hay **TRES nombres reales de personas publicados en `prueba` desde la v0.093**, repartidos en **DOS archivos** (medido, no estimado): `DECISIONES.md` §Post-F9.185 (**dos**) y `HOJA-DE-RUTA.md` fila **0.112** (**tres**, uno de ellos que no aparece en el otro archivo) | 3-sep ⛔ **NO se mergeó** el #287; su documentación entró **sin los Excel** y retirando **cuatro** nombres distintos (los tres publicados + uno que el #287 dejaba vivo). 🔴 **Lo que sigue pendiente y NO es código:** **(i) de Gabriel** — cerrar el PR #287 y **borrar su rama remota** (los archivos siguen públicos ahí) y **pedir a GitHub Support el purgado** de los objetos (borrar la rama NO basta: `refs/pull/287/head` queda); ⚠️ el purgado hay que pedirlo por **las dos exposiciones**: los cinco `.xlsx` de la rama del #287 **y** los tres nombres del historial de `prueba` (v0.093, commit `651f847`). **(ii) de Daniel** — decidir si el repositorio **debe seguir siendo público**. ⏳ **Tres decisiones abiertas de Daniel sobre qué dato personal cabe en un repo público**, las tres levantadas por el reviewer y NINGUNA archivada: **(a)** los alias de proveedor **persona física** («CESAR VICTORIA 1», «ARTURO LOPEZ», «LEOPOLDO DIAZ»), que quedan pegados a información de pago — el reviewer **vota seudonimizarlos**; hoy se conservan porque sin ellos §Post-F9.186(b) no se entiende y ya estaban en `prueba`; **(b)** los **nombres de pila de empleadas con una valoración de desempeño** («las inconsistencias son errores de Lupita», «Lupita no debería capturar nada»), que son cita textual de Daniel pero identifican a una persona real — alternativa: «el capturista» / «finanzas»; **(c)** si al citar un archivo se puede nombrar a quien lo manda. 📌 Las tres esperan a que Gabriel y Daniel resuelvan **(i)** y **(ii)**, por instrucción de Daniel del 3-sep (*«esperamos a mañana a Gabriel… ahorita déjalo así»*, §Post-F9.187(a)). 🔑 **La regla que deja:** *un dato personal se quita del **archivo**, no del texto; un `.xlsx` es un ZIP de XML y hay que mirar **celdas y metadatos**; y una limpieza que no se puede **volver a correr** no es una limpieza, es una afirmación* |
> **📊 Dónde va el programa — al 3-sep-2026, versión en `prueba` `0.094`.** De las **65 filas** numeradas: **33 ✅ cerradas** · **1 🔶 a medias** (0.082) · **1 ⏸️ aparcada** (0.087) · **30 ⬜ por hacer** (0.061, 0.072, 0.086, 0.096, 0.097, 0.099–0.123 salvo la 0.098). ⚠️ **Los huecos NO se rellenan**: la fila numera el PLAN y la entrega numera la LLEGADA. Ver *«Cómo se numeran»* en `HISTORIAL-DE-VERSIONES.md`.
